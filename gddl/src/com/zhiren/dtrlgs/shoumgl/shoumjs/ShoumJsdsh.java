...
public class ShoumJsdsh extends BasePage {
	// 流程状态的类别
//	private static long WODRW = 1;// 刚刚提交未选择流程的任务
	...
	//设置默认流程
	private static String XiTongMoRenLiuCheng="收煤结算";
		...
		if (!(this.getChange().equals("") || this.getChange() == null)) {
			...
			for (int i = 0; i < change.length; i++) {
				...
				if (record.length > 2) {
					...
					// renyxxb_id, "");
					try {
						BalanceLiuc.TuiHuiDianChang_kuangfjsmkb(Long.parseLong(record[0]));
...
//						} else {
						 ...
						 if(yfb_id>0)
							BalanceLiuc.tij("kuangfjsyfb", yfb_id, renyxxb_id, false, "");
//						}
					} catch (NumberFormatException e) {
						// TODO 自动生成 catch 块
						e.printStackTrace();
...
//						} else {
						 ...
						 if (con.getUpdate("update  " + record[1]
						       							...
						       							ResultSetList rsl_fayslb = con
						       									.getResultSetList("select f.YUANSHDWB_ID,f.id  from fahb f  where  f.JIESB_ID="
						       											+ Long.parseLong(record[0]));
						       							while (rsl_fayslb.next()) {
						       								fahb_idList.add(new ShoumcbBean(rsl_fayslb
						       										.getLong("YUANSHDWB_ID"), rsl_fayslb
						       										.getLong("id")));
						       							}
						       							ShoumzgInfo.CountChengb(fahb_idList, true);//收煤暂估
							BalanceLiuc.tij(record[1], Long.parseLong(record[0]),
									renyxxb_id,true, "");
				  long yfb_id=this.getKuangfjsyfb_idByKuangfjsymkb_id( Long.parseLong(record[0]));
				  if(yfb_id>0)
				      BalanceLiuc.tij("kuangfjsyfb", yfb_id,
							renyxxb_id,true, "");
						}else{
							throw new Exception();
						}
...
//	 }
	...
	public void getSelectData() {
		...
		String ShenHeDeBiaoMing="kuangfjsmkb";
		long renyxxb_id = ((Visit) getPage().getVisit()).getRenyID();
		String strWeizhitj = "";
		String liucztb_id="";
...
//		} else if (getWeizSelectValue().getId() == LIUCZ) {// 得到未审核的任务
		...
		sql = "select * \n"
				...
				+ "','ShoumShowjsd','jiesdbh',js.bianm,js.bianm) as bianh,"
...
//			}
			...
			if(BalanceLiuc.isHadThis(renyxxb_id, "场地采购结算", "填写发票")){
				tishi="确认填写单子";
			 egu.addToolbarItem("{"
					 + new GridButton(
					 "填写发票",
...
//		}
		...
		if (!(this.getChange().equals("") || this.getChange() == null)) {
			...
			for (int i = 0; i < change.length; i++) {
				...
				if (record.length > 2) {
					...
					if(yfb_id>0)
					BalanceLiuc.huit("kuangfjsyfb",yfb_id,
							renyxxb_id, "");
	...
	//根据矿方结算信息表得到矿方结算煤款
	public long getKuangfjsyfb_idByKuangfjsymkb_id(long Kuangfjsymkb_id){
		JDBCcon con=new JDBCcon();
		ResultSetList rsl=con.getResultSetList("select * from Kuangfjsyfb where kuangfjsmkb_id="+Kuangfjsymkb_id+"");
		long Kuangfjsyfb_id=0;
		if(rsl.next()){
			Kuangfjsyfb_id=rsl.getLong("id");
		}
		con.Close();
		return  Kuangfjsyfb_id;
	}
