 ...
 * 时间：2011-4-20
 * 描述：
 *   修改月末余额库存金额=期初+来煤款+来运费-耗煤款-耗运费+水分差-盘点亏损
 */
...
//	 年份下拉框
	...
	public void getSelectData() {
		...
		String sql = 
			...
			"decode(shoufcyllbb_id,0,'入库煤量',1,'暂估煤量',2,'暂估冲回煤量')as	shoufcyllbb_id," +
			"		riq,\n" +
			"       qickc_shul,\n" + 
			"       qickc_danj,\n" + 
			"       qickc_jine,\n" + 
	...
	egu.getColumn("shoufcyllbb_id").setHeader("原料类别");
	egu.getColumn("shoufcyllbb_id").setWidth(90);
	
	List l = new ArrayList();
	l.add(new IDropDownBean(0, "入库煤量"));
	l.add(new IDropDownBean(1, "暂估煤量"));
	l.add(new IDropDownBean(2, "暂估冲回煤量"));
	...
	egu.getColumn("benyrk_danj").setHeader("本月入库<br>煤款单价");
	egu.getColumn("benyrk_danj").setWidth(70);
	
	egu.getColumn("benyrk_yunfdj").setHeader("本月入库<br>运费单价");
	egu.getColumn("benyrk_yunfdj").setWidth(70);
	
	egu.getColumn("benyrk_jine").setHeader("本月入库<br>煤款金额");
	egu.getColumn("benyrk_jine").setWidth(80);
	
	egu.getColumn("benyrk_yunfje").setHeader("本月入库<br>运费金额");
	egu.getColumn("benyrk_yunfje").setWidth(80);
	
	egu.getColumn("benyhy_shul").setHeader("本月耗用<br>数量");
	egu.getColumn("benyhy_shul").setWidth(70);
	
	egu.getColumn("benyhy_danj").setHeader("本月耗用<br>煤款单价");
	egu.getColumn("benyhy_danj").setWidth(70);
	
	egu.getColumn("benyhy_yunfdj").setHeader("本月耗用<br>运费单价");
	egu.getColumn("benyhy_yunfdj").setWidth(70);
	
	egu.getColumn("benyhy_jine").setHeader("本月耗用<br>煤款金额");
	egu.getColumn("benyhy_jine").setWidth(80);
	
	egu.getColumn("benyhy_yunfje").setHeader("本月耗用<br>运费金额");
	egu.getColumn("benyhy_yunfje").setWidth(80);
	...
	String script = "gridDiv_grid.on('afteredit',function(e){\n" + 
		...
		"   e.field == 'QICKC_DANJ' || e.field == 'BENYRK_DANJ' || e.field == 'BENYRK_YUNFDJ' || e.field == 'BENYHY_DANJ'|| e.field == 'BENYHY_YUNFDJ'){\n" +
		...
		"	var benyrk_yunfdj = e.record.get('BENYRK_YUNFDJ');\n" +
		"	var benyhy_danj = e.record.get('BENYHY_DANJ');\n" +
		"	var benyhy_yunfdj = e.record.get('BENYHY_YUNFDJ');\n" +
		...
		"	var benyrk_yunfje = eval(benyrk_shul||0)*eval(benyrk_yunfdj||0);\n" +
		"	e.record.set('BENYRK_YUNFJE',benyrk_yunfje.toFixed(2));\n" +
		
		"	var benyhy_jine = eval(benyhy_shul||0)*eval(benyhy_danj||0);\n" +
		"	e.record.set('BENYHY_JINE',benyhy_jine.toFixed(2));\n" +
		
		"	var benyhy_yunfje = eval(benyhy_shul||0)*eval(benyhy_yunfdj||0);\n" +
		"	e.record.set('BENYHY_YUNFJE',benyhy_yunfje.toFixed(2));\n" +
		...
		"	var yuemyekc_jine = eval(qickc_jine||0)+eval(benyrk_jine||0)+eval(benyrk_yunfje||0)-(eval(benyhy_jine||0)+eval(benyhy_yunfje||0)-eval(shuifctzl_jine||0))-eval(pandks_jine||0);\n" +
		...
		"}\n" + 
		"if(e.field == 'QICKC_JINE' || e.field == 'BENYRK_JINE'|| e.field == 'BENYRK_YUNFJE' || e.field == 'BENYHY_JINE'|| e.field == 'BENYHY_YUNFJE' || e.field == 'SHUIFCTZL_JINE' || e.field == 'PANDKS_JINE' ){" +
		"	var qickc_jine = e.record.get('QICKC_JINE');\n" +
		"	var benyrk_jine = e.record.get('BENYRK_JINE');\n" +
		"	var benyrk_yunfje = e.record.get('BENYRK_YUNFJE');\n" +
		"	var benyhy_jine = e.record.get('BENYHY_JINE');\n" +
		"	var benyhy_yunfje = e.record.get('BENYHY_YUNFJE');\n" +
