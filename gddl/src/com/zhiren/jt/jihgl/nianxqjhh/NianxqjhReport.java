...
public class NianxqjhReport extends BasePage {
...
//			" where id="+getTreeid()+") " ;
		...
		if(getShenhDropDownValue()!=null){
			shenhzt=" and n.zhuangt="+getShenhDropDownValue().getId();
		}
		 String ArrHeader[][]=null;
		 int ArrWidth[]=null;
		if(jib==3){//电厂
			
			sbsql.append("select decode(grouping(n.jizzt),1,'总计',decode(n.jizzt,0,'现存机','新增机')) as jizzt,\n");
			sbsql.append("sum(n.fadl) as fadl,sum(n.fadbhm) as fadbhm,sum(n.zonghcydl) as zonghcydl,\n");
			sbsql.append("sum(n.gongrl) as gongrl,sum(n.gongrbzmh) as gongrbzmh,sum(n.rulrz) as rulrz,\n");
			sbsql.append("sum(n.fadxybml) as fadxybml,sum(n.gongrxybml) as gongrxybml,sum(n.xuybml) as xuybml,\n");
			sbsql.append("sum(n.fadxyml) as fadxyml,sum(n.gongrxyml) as gongrxyml,\n");
			sbsql.append("sum(n.xuyyml) as xuyyml,sum(n.dianhzryl) as dianhzryl,sum(jisfdl) as jisfdl,sum(gongrbml) as gongrbml,sum(biaomh) as biaomh \n");
			sbsql.append("from nianxqjhh n where nianf="+intyear+" and n.shujzt="+intyear+" and n.diancxxb_id="+this.getTreeid()+"\n");
			sbsql.append(shenhzt);
			sbsql.append("group by rollup (n.jizzt) order by grouping(n.jizzt) desc,jizzt\n");
			
			ArrHeader=new String[1][17];
			ArrHeader[0]=new String[] {"单位","发电量","发电原煤耗","综合厂用电率","供热量",
					 "供热标准煤耗","入炉热值","发电需用标煤量","供热需用标煤量",
					 "需用标煤量","发电需原煤量","供热需原煤量","需用原煤量","点火助燃油量","计算发电量","供热标煤量","标煤耗"};

			ArrWidth=new int[] {150,80,80,80,80,80,80,80,60,60,60,60,60,60,60,60,60};
			
		}else{//集团，分公司
			
			sbsql.append("select decode(grouping(dc.fengs)+grouping(dc.mingc),2,'总计',1,dc.fengs,dc.mingc) as danw,\n");
			sbsql.append("decode(grouping(n.jizzt),1,'合计',decode(n.jizzt,1,'新机','现存机')) as jizzt,\n");
			sbsql.append("sum(n.fadl) as fadl,sum(n.fadbhm) as fadbhm,sum(n.zonghcydl) as zonghcydl,\n");
			sbsql.append("sum(n.gongrl) as gongrl,sum(n.gongrbzmh) as gongrbzmh,sum(n.rulrz) as rulrz,\n");
			sbsql.append("sum(n.fadxybml) as fadxybml,sum(n.gongrxybml) as gongrxybml,sum(n.xuybml) as xuybml,\n");
			sbsql.append("sum(n.fadxyml) as fadxyml,sum(n.gongrxyml) as gongrxyml,sum(n.xuyyml) as xuyyml,\n");
			sbsql.append("sum(n.dianhzryl) as dianhzryl,sum(jisfdl) as jisfdl,sum(gongrbml) as gongrbml,sum(biaomh) as biaomh\n");
			sbsql.append("from nianxqjhh n,(select d.id, d.mingc,d.jingjcml ,d.rijhm,dc.xuh,dc.mingc as fengs, \n");
			sbsql.append("dc.id as fuid,dc.shangjgsid from diancxxb d, diancxxb dc  \n");
			sbsql.append("where d.jib = 3  and d.fuid = dc.id(+)) dc \n");
			sbsql.append("where n.diancxxb_id=dc.id and nianf="+intyear+" and n.shujzt="+intyear+" \n");
			sbsql.append(shenhzt).append(strGongsID);
			sbsql.append("group by rollup (n.jizzt,dc.fengs,dc.mingc) \n");
			sbsql.append("having not grouping(n.jizzt)=1 \n");
			sbsql.append("order by grouping(dc.fengs) desc,fengs desc,grouping(dc.mingc) desc,dc.mingc,\n");
			sbsql.append("grouping(n.jizzt) desc,jizzt desc\n");
			
			ArrHeader=new String[1][18];
			ArrHeader[0]=new String[] {"单位","机组","发电量","发电原煤耗","综合厂用电率","供热量",
					 "供热标准煤耗","入炉热值","发电需用标煤量","供热需用标煤量",
					 "需用标煤量","发电需原煤量","供热需原煤量","需用原煤量","点火助燃油量","计算发电量","供热标煤量","标煤耗"};

			ArrWidth=new int[] {150,80,80,80,80,80,80,80,60,60,60,60,60,60,60,60,60,60};
...
//		 System.out.println(sbsql);
		 ...
		 Report rt = new Report();
			...
			rt.setTitle( "年耗用量计划", ArrWidth);//getBiaotmc()+
...
//			rt.setDefaultTitle(12, 2, "单位:(吨)" ,Table.ALIGN_RIGHT);
			...
			rt.setDefautlFooter(4, 6, "审核人：", Table.ALIGN_CENTER);
			rt.setDefautlFooter(10, 4, "填报人：", Table.ALIGN_RIGHT);
		// 设置页数
		_CurrentPage = 1;
