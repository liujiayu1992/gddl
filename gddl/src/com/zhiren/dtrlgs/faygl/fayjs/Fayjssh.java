...
public class Fayjssh extends BasePage {
	// 流程状态的类别
//	private static long WODRW = 1;// 刚刚提交未选择流程的任务
	...
	//设置默认流程
	private static String XiTongMoRenLiuCheng="发运结算";
		...
		if (!(this.getChange().equals("") || this.getChange() == null)) {
			...
			for (int i = 0; i < change.length; i++) {
				...
				if (record.length > 2) {
					...
					// renyxxb_id, "");
					try {
						BalanceLiuc.TuiHuiDianChang_jsmkb(Long.parseLong(record[0]));
...
//							BalanceLiuc.tij(record[1], Long.parseLong(record[0]),
...
//						} else {
							BalanceLiuc.tij(record[1], Long.parseLong(record[0]),
                               ...
                               long yfb_id=this.getKuangfjsyfb_idByKuangfjsymkb_id( Long.parseLong(record[0]));
						 if(yfb_id>0)
							BalanceLiuc.tij("diancjsyfb", yfb_id, renyxxb_id, false, "");
//						}
					} catch (NumberFormatException e) {
						// TODO 自动生成 catch 块
						e.printStackTrace();
...
//						} else {
						...
						if (con.getUpdate("update  " + record[1]
								+ " set RUZRQ=sysdate where id="
								+ Long.parseLong(record[0])) != -1) {// 修改入账日期
							ResultSetList rsl_fayslb = con
									.getResultSetList("select f.shr_diancxxb_id,f.id  from fayslb f  where  f.xiaosjsb_id ="
											+ Long.parseLong(record[0]));
							while (rsl_fayslb.next()) {
								fayslb_idList.add(new FaycbBean(rsl_fayslb
										.getLong("shr_diancxxb_id"), rsl_fayslb
										.getLong("id")));
							}
							FayzgInfo.CountChengb(fayslb_idList, true);//发运暂估
							BalanceLiuc
									.tij(record[1], Long.parseLong(record[0]),
											renyxxb_id, true, "");
							long yfb_id = this
									.getKuangfjsyfb_idByKuangfjsymkb_id(Long
											.parseLong(record[0]));

							BalanceLiuc.tij("diancjsyfb", yfb_id,// 审核通过
									renyxxb_id, true, "");
						}else{
							throw new Exception();
						}
...
//		 
//		 if(!BalanceLiuc.DiancUpdate(sr.getLong("diancxxb_id"),sr.getString("bianm"), 1,-1)){ 
...
//	 }
	...
	public void getSelectData() {
		...
		String sql = "";
		JDBCcon con = new JDBCcon();
		String leix =XiTongMoRenLiuCheng; //"收煤结算";
		String ShenHeDeBiaoMing="diancjsmkb";
		long renyxxb_id = ((Visit) getPage().getVisit()).getRenyID();
		String strWeizhitj = "";
		String liucztb_id="";
		if (getWeizSelectValue().getId() == LIUCZ) {// 得到刚刚从电厂上传的，还未选择流程的条目
//			strWeizhitj = BalanceLiuc.getWeixz(ShenHeDeBiaoMing, renyxxb_id, leix);
//		} else if (getWeizSelectValue().getId() == LIUCZ) {// 得到未审核的任务
			strWeizhitj = BalanceLiuc.getWeish(ShenHeDeBiaoMing, renyxxb_id, leix);
		} else {// 得到已审核的任务
			liucztb_id = BalanceLiuc.getYish(ShenHeDeBiaoMing, -1, leix);
		}

		sql = "select * \n"
				+ "  from ("
				+ "  select js.id,'"+ShenHeDeBiaoMing+"' as tabname,di.id as diancid,di.mingc as dianmc,\n"
				+ "				  getHtmlAlert('"
				+ MainGlobal.getHomeContext(this)
				+ "','FayShowJsd','jiesdbh',js.bianm,js.bianm) as bianh,"
				+ "               --gy.jianc as gongysmc, \n"
				+ "               js.gongysmc as fahdw,he.hetbh hetbh,jiessl,guohl,nvl(chengf.jiesrl,0) jiesrl,nvl(changfrl,0) changfrl,nvl(chengf.jieslf,0) jiesliuf,nvl(changflf,0) changflf,\n"
				+ "				  hansmk,meikje,shuik,"
				+ "				  buhsdj danj,js.jiesrq, \n"
				+ "				  nvl(chengf.jiesrl,0)-nvl(changfrl,0) as rezc,(nvl(jiessl,0)-nvl(guohl,0))-round((nvl(guohl,0)*0.02)) as shulc,	\n"
				...
				sql+="and he.id(+)=js.hetb_id and zt.id(+)=js.liucztb_id and lz.id(+)=zt.leibztb_id and ll.id(+)=lz.liuclbb_id\n" +
		         ...
		         if(this.getWeizSelectValue().getId()==YISH){
		        	 sql+="and jiesrq>=to_date('"+this.getStartdate()+"','yyyy-mm-dd') and jiesrq<to_date('"+this.getEnddate()+"','yyyy-mm-dd')+1\n";
		         }
...
//		}
		...
		egu.addToolbarItem("{"
				+ new GridButton(
						"删除",
						"function(){ Ext.MessageBox.confirm('警告', '确定将结算单删除吗？', function(btn) { if(btn=='yes'){ if(gridDiv_sm.hasSelection()){ "
...
//			}
			...
			if(!BalanceLiuc.isLastLeader(renyxxb_id,XiTongMoRenLiuCheng)){
			egu
					.addToolbarItem("{"
							+ new GridButton(
									"审核",
			...
			egu.addTbarText("-");
			//判断是否填写结算单
			if(BalanceLiuc.isHadThis(renyxxb_id, XiTongMoRenLiuCheng, "填写发票")){
				tishi="确认填写单子";
			 egu.addToolbarItem("{"
					 + new GridButton(
					 "填写发票",
					 "function(){ Ext.MessageBox.confirm('警告', '"+tishi+"', function(btn) { if(btn=='yes'){ if(gridDiv_sm.hasSelection()){ "
					 + " rec = gridDiv_grid.getSelectionModel().getSelections(); "
					 +"  if(rec.length!=1){"+MainGlobal.getExtMessageBox("你没有选择数据或选择多条数据,结算单只能逐个填写", false)+";return;};\n"
					 + " for(var i=0;i<rec.length;i++){ "
					 +"window.open('"+MainGlobal.getHomeContext(this)+"/app?service=page/Shoumfptx&fP_id='+rec[i].get('ID'));"
					 +"return;"
					 + " if(i==0){"
					 + "document.getElementById('CHANGE').value=rec[i].get('ID')+','+rec[i].get('TABNAME')+','+rec[i].get('BIANH')+','+rec[i].get('JIESRQ')+','+rec[i].get('DIANCID')+','+rec[i].get('HETBH')+','+rec[i].get('ZHUANGT')+','+rec[i].get('LIUCB_ID')+','+rec[i].get('LIUCZTB_ID')+';';"
					 + " }else{ "
...
//		if(BalanceLiuc.isFirstRenY(v.getRenyID(),XiTongMoRenLiuCheng)){
...
//		}
	...
	al=BalanceLiuc.getIcanliuc(v.getRenyID(),"结算");//
	if(al.size()<1){
		JDBCcon jcon=new JDBCcon();
		ResultSetList rsl=jcon.getResultSetList("select * from liucb where mingc='"+XiTongMoRenLiuCheng+"'");
		...
		if (!(this.getChange().equals("") || this.getChange() == null)) {
			...
			for (int i = 0; i < change.length; i++) {
				...
				if (record.length > 2) {
					...
					BalanceLiuc.huit(record[1], Long.parseLong(record[0]),
							renyxxb_id, "");
					long yfb_id= getKuangfjsyfb_idByKuangfjsymkb_id(Long.parseLong(record[0]));
					if(yfb_id>0)
					BalanceLiuc.huit("diancjsyfb",yfb_id,
							renyxxb_id, "");
	...
	//根据矿方结算信息表得到矿方结算煤款
	public long getKuangfjsyfb_idByKuangfjsymkb_id(long diancjsymkb_id){
		JDBCcon con=new JDBCcon();
		ResultSetList rsl=con.getResultSetList("select * from diancjsyfb where diancjsmkb_id="+diancjsymkb_id+"");
		long diancjsyfb_id=0;
		if(rsl.next()){
			diancjsyfb_id=rsl.getLong("id");
		}		
		con.Close();
		return  diancjsyfb_id;
	}
