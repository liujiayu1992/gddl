...
public class Feny_whp extends BasePage {

	/**
	 * 作者:王总兵
	 * 时间:2010-7-14 9:53:43
	 * 内容:分样车辆未回皮查询
	 */
...
//		电厂Tree
	...
	public String getYansd(){
		...
		if(meik==-1){
			...
			sbsql.append(
					"select rownum as xuh,cp.piaojh,\n" +
					"decode(cp.isjus,1,'拒收','未回皮') as zhuang,\n" + 
					"to_char(cp.lursj,'yyyy-mm-dd hh24:mi:ss') as caiysj\n" + 
					"from\n" + 
					"chepbtmp cp where cp.qingcsj is null\n" + 
					""+tiaoj+"\n"+
					"and cp.lursj>=to_date('"+kais+"','yyyy-mm-dd')\n"+
					"and cp.lursj<to_date('"+jies+"','yyyy-mm-dd')+1\n");
...
//				1120
				ArrHeader[0]=new String[] {"序号","采样号","状态","采样时间"};
				int ArrWidth[]=new int[] {50,170,150,120};
			
				  strFormat = new String[] {"", "", "",""};
				// 数据
				rt.setTitle(visit.getDiancmc()+"未回皮与拒收查询",ArrWidth);
...
//	 供应商下拉框
	...
	public void getGongysModels() {
		...
		String sql_gongys = "select cp.id,cp.piaojh\n" + 
		"from\n" + 
		"chepbtmp cp where cp.qingcsj is null\n" + 
		"and cp.lursj>=to_date('"+this.getBRiq()+"','yyyy-mm-dd')\n" + 
		"and cp.lursj<to_date('"+this.getERiq()+"','yyyy-mm-dd')+1\n";
