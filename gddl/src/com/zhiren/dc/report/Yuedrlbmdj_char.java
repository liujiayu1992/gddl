...
public class Yuedrlbmdj_char  extends BasePage implements PageValidateListener {
	...
	public String getPrintChar(){
		...
		String Sql=	
			"SELECT TO_CHAR(RIQ, 'mm') || '月' RIQ, FENX, RULZHBMDJ\n" +
			"  FROM YUEZBB\n" + 
			" WHERE TRUNC(RIQ, 'yyyy') = DATE '"+getYearValue().getValue()+"-01-01'\n" + 
			"   AND DIANCXXB_ID = "+visit.getDiancxxb_id()+" ORDER BY RIQ, FENX";
		...
		CategoryDataset dataset = cd.getRsDataChart(rstmp, "RIQ","FENX","RULZHBMDJ");//rs记录集构造生成图片需要的数据
...
//		柱子上字的倾斜度。0:水平
		...
		return ct.ChartBar3D(getPage(), dataset,  getYearValue().getValue()+"年入炉标煤单价",  800,  400);
	}
