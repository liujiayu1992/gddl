 ...
 * 2009-02-19
 * 王磊
 * 车辆信息修改时diancxxb_id取值不正确,可关联出运输单位为 -1的信息,增加车号排序
 */


/*
 * 作者:王总兵
 * 日期:2010-1-19 15:24:41
 * 描述:增加卡号字段
 * 
 * 
 */
 ...
 */
public class Chelxxgl extends BasePage implements PageValidateListener {
//	界面用户提示
	private String CustomSetKey = "Chelxxgl";
	private String msg="";
...
public void save() {
		...
		boolean ChelxxJil = MainGlobal.getXitxx_item("数量", "车辆信息管理停用车辆是否启用日志记录功能", 
		...
		while (delrsl.next()) {
			...
			sbsql.append("delete from chelxxb where id = ").append(delrsl.getString("id")).append(";\n");
		}
		delrsl.close();
		...
		while(mdrsl.next()) {
			...
			if ("".equals(mdrsl.getString("id"))||mdrsl.getString("id")==null ||mdrsl.getInt("id")==0) {
				sbsql.append("insert into chelxxb(id,diancxxb_id,yunsdwb_id,yunsfsb_id,cheph,maoz,piz,islocked,kah,chezxm,ched_id,suocr,suocsj,chang,kuan,gao,dig,gangssl,chex) values( xl_xul_id.nextval")
				...
				sbsql.append("update chelxxb set diancxxb_id =").append(diancxxb_id).append(", ").append("yunsdwb_id=(").append(yunsdwb_id).append("),").append("yunsfsb_id=(").
				     append(yunsfsb_id).append("),").append("cheph='").append(cheph).append("',").append("maoz = ").append(maoz).append(",").append("piz = ").append(piz).
				     append(",").append("islocked = '").append(islocked).append("',").append("kah = '").append(kah).append("',").append("chezxm = '").append(chezxm).
				     append("',").append("ched_id = (").append(ched_id)
				    .append("),") .append("suocr = '").append(suocr).append("',")
					.append("suocsj = ").append(suocsj).append(",")
					.append("chang = ").append(chang).append(",")
					.append("kuan = ").append(kuan).append(",")
					.append("gao = ").append(gao).append(",")
					.append("dig = ").append(dig).append(",")
					.append("gangssl = ").append(gangssl).append(",")
					.append("chex = (").append(chex).append(")")
					.append("  where id = ").append(mdrsl.getString("id")).append(";\n");
				
				if(ChelxxJil){
					
						SaveLog(con,islocked,cheph);
