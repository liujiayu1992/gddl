...
public class Zhuyzb  extends BasePage implements PageValidateListener{
//	 判断是否是集团用户
...
//******************页面初始设置********************//
	...
	//返回小于等于选择日期，离最近的最近	
	private Date getMonthQueryDate(){
		JDBCcon cn = new JDBCcon();
		Date datDate=new Date();
		ResultSetList rsl = cn.getResultSetList(" select max(riq) as riq from zhuyzb where riq<="+OraDate(_BeginriqValue) +" and diancxxb_id="+((Visit)getPage().getVisit()).getDiancxxb_id());
		while (rsl.next()){
			datDate= rsl.getDate("riq");
			if (datDate==null){
				datDate=_BeginriqValue;
			}
		}
		rsl.close();
		cn.closeRs();
...
//		String danwmc="";//汇总名称
		...
		String ri_sql = 
                 ...
                 "              round(nvl(dr.hy,0)/10000,2) as hy,\n" + 
                 "              round(nvl(dr.kuc,0)/10000,2) as kuc,\n" + 
                 "              round(nvl(s.gm,0)/10000,2) as gms,\n" + 
                 "              round(nvl(s.hy,0)/10000,2) as hys,\n" + 
                 "              round(nvl(s.kuc,0)/10000,2) as kucs \n" + 
                 "         from\n" + 
                 "              (select\n" + 
                 "                      sum(dangrgm) gm,\n" + 
                 "                      sum(haoyqkdr) hy,\n" + 
...
//		System.out.println("3333333333333"+rc_sql);
		...
		rtDay.body.setFixedRows(1);
		rtDay.body.setColAlign(1, Table.ALIGN_CENTER);
		rtDay.body.setColAlign(2, Table.ALIGN_CENTER);
		rtDay.body.setColAlign(5, Table.ALIGN_CENTER);
		
		//rtDay.body.setRowCells(1, Table.PER_BACKCOLOR, "silver");//设置背景设
		rtDay.body.setRowCells(1, Table.PER_FONTSIZE,14);//字号
		rtDay.body.setRowCells(1, Table.PER_FONTNAME,"楷体_GB2312" );//字体
		rtDay.body.setColCells(1, Table.PER_FONTSIZE,14);
		rtDay.body.setColCells(1, Table.PER_FONTNAME,"楷体_GB2312");
		...
		rtDay.body.setColAlign(4, Table.ALIGN_RIGHT);
//		合并单元格
		...
		int ArrWidth[]=new int[] {70,90,70,70,70,70,70,70,70};
			...
			rt.setTitle(DateUtil.Formatdate("yyyy年MM月月度数据",dateMonth), ArrWidth);

			rt.body.setColAlign(1, Table.ALIGN_CENTER);
			rt.body.setColAlign(2, Table.ALIGN_CENTER);
			rt.body.setColAlign(4, Table.ALIGN_CENTER);
			rt.body.setRowCells(1, Table.PER_FONTSIZE,14);//行字号
			rt.body.setRowCells(1, Table.PER_FONTNAME,"楷体_GB2312" );//行字体
			rt.body.setColCells(1, Table.PER_FONTSIZE,14);//列字号
			rt.body.setColCells(1, Table.PER_FONTNAME,"楷体_GB2312");//列字体
