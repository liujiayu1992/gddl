...
public class Rulhffsh extends BasePage implements PageValidateListener {
	private String msg = "";
	...
	public void Update1(String strchange, Visit visit) {
		...
		while (mdrsl.next()) {
			StringBuffer sql2 = new StringBuffer();
			String gyid = "select id from gongyfxb where zhillsb_id="
					+ mdrsl.getString("ZHILLSB_ID") + " and id<>"
					+ mdrsl.getString("ID") + " and fenxxmb_id=3";
	...
	private void AutoFix(String strchange, Visit visit) {
		...
		Judge ju = new Judge();
		double[] dbarrjudge = ju.getJudgeData(Judge.T_TYPE_Vad, array);
		if(dbarrjudge==null){
			return;
		}
		for (int j = 0; j < dbarrjudge.length; j++) {
			...
			if ( mdrsl.getDouble("ZHI") == dbarrjudge[j]) {
					String sql = "update gongyfxb set shenhzt=1 where id = "
							+ mdrsl.getString("ID");
		...
		for(int i=0;i<mdrsl.getRows();i++){
			mdrsl.next();
			String sql1 = "select id from gongyfxb where zhillsb_id="
				+ mdrsl.getString("ZHILLSB_ID") + " and id<>"
				+ mdrsl.getString("ID");
		ResultSetList rsl = con.getResultSetList(sql1);
		rsl.next();
		String sql3 = "update gongyfxb set shenhzt=-1 where id = "
			+ rsl.getLong("ID");
		con.getUpdate(sql3);
		...
		ResultSetList rsl = con.getResultSetList(rulid);
		if(rsl.getRows()==0){
			String sql2="inster into rulmzlb (id,diancxxb_id,vad,fenxrq,rulrq,bianm,rulbzb_id,JIZFZB_ID,LURSJ,shenhzt)values(" +
			...
			String sql2="update zhillsb set vad="+mdrsl.getDouble("ZHI")+",shenhzt=1 where id="+mdrsl.getString("ZHILLSB_ID");	
			con.getUpdate(sql2);
		}
	...
	public void getSelectData() {
		...
		JDBCcon con = new JDBCcon();
		String sql = "select distinct g.id, g.bianm,g.xuh,g.qimbh,g.qimmyzl,g.qimzl,g.meiyzl,g.honghzl1,g.zuizhhzl,g.shenhzt,\n"
				+ "g.shiqzl,g.zhi,g.zhillsb_id,to_char(g.rulrq,'yyyy-mm-dd')as rulrq,to_char(g.fenxrq,'yyyy-mm-dd')as fenxrq\n"
				+ " from gongyfxb g\n"
				+ " where fenxxmb_id=3 and g.shenhzt=0 "
				+ " and g.bianm='" + getHuaybhValue().getValue() + "'";
		...
		egu.getColumn("qimbh").setHeader("称量瓶（盘）号");
		egu.getColumn("qimmyzl").setHeader("称量瓶（盘）试样重量");
		egu.getColumn("qimzl").setHeader("称量瓶（盘）重量");
		egu.getColumn("meiyzl").setHeader("试样重量");
		egu.getColumn("honghzl1").setHeader("挥发分Vdaf(%)");
		//egu.getColumn("honghzl2").setHeader("备注");
		//egu.getColumn("honghzl3").setHeader("日期");		
		//egu.getColumn("honghzl4").setHeader("编号");
		egu.getColumn("zuizhhzl").setHeader("加热后总质量");
		egu.getColumn("shiqzl").setHeader("煤样加热后减少的质量");
		egu.getColumn("zhi").setHeader("挥发分Vad=m1/m*100-Mad(%)");
	...
	public IPropertySelectionModel getHuaybhModels() {
		...
		String sql = "select rownum,bianm from ( select distinct g.bianm from gongyfxb g "
				+ "where g.rulrq>=to_date('"
				+ this.getRiqi()
				+ "','yyyy-mm-dd')and g.rulrq<=to_date('"
				+ this.getRiq2()
				+ "',\n" + "'yyyy-mm-dd') and g.fenxxmb_id=3)";
