...
public class Gonygscx  extends BasePage implements PageValidateListener{
...
//******************页面初始设置********************//
	...
	private String getSelectData(){
		...
		if(jib==1){//选集团时刷新出所有的电厂
			...
			strGongsID ="and g.id in (select distinct f.gongysb_id from fahb f,diancxxb dc where f.diancxxb_id=dc.id and  dc.fuid="+this.getTreeid()+")";
			
		}else if (jib==3){//选电厂只刷新出该电厂
			strGongsID="and g.id in (select distinct f.gongysb_id from fahb f where f.diancxxb_id="+this.getTreeid()+")";
		...
		int iCol=0;//列数
				...
				strSQL="select getHtmlAlert('"+MainGlobal.getHomeContext(this)+"','Gonyscx_mx','gongysb_id',g.id,g.mingc) as mingc,g.quanc,g.bianm, s.quanc as shengfb_id,cs.quanc as chengsb_id,g.danwdz,g.faddbr,g.dianh,\n" +
					"g.chuanz,g.youzbm\n" + 
					"from gongysb g,shengfb s,chengsb cs\n" + 
...
// 直属分厂汇总
				 ...
				 ArrHeader[0]=new String[] {"供应商简称","供应商全称","供应商编码","所属省份","城市","地址",
						 					"联系人","电话","传真","邮编"};
				
				 
				 ArrWidth=new int[] {150,200,80,65,65,200,65,80,80,80};
				 iFixedRows=1;
			...
			rt.setTitle("供应商信息查询", ArrWidth);
			rt.setDefaultTitle(1, 2, "单位:"+this.getTreeDiancmc(this.getTreeid()), Table.ALIGN_LEFT);
