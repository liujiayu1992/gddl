...
//	}
	...
	public void doTimerTask(){
		...
		try {
			while (rs.next()){
				...
				if (Sendms.LEIB_DAY.equals(rs.getString("leib"))){
					blnIsToday=true;
				}else if (Sendms.LEIB_WEEK.equals(rs.getString("leib"))){
				...
				}else if (Sendms.LEIB_MONTH.equals(rs.getString("leib"))){
					for (int i=0;i<arrfasrq.length; i++){
				...
				if (blnIsToday){//是今天需要发送的，进一步判断时间
					...
					if (Sendms.CYCLE_HOUR.equals(rs.getString("faszqdw"))){
	...
	public boolean newAutoExcuteTask(long lngDianxxxb_id,Date datSendTime, String fujxx,String fujdh,long lngDuanxdypzb_id,long lngDuanxdyb_id){
		...
		iCount=cn.getInsert(" insert into duanxfsb(id,diancxxb_id,riq,neir,fujdh,lury,lurysj,leib,zhuangt,duanxdypzb_id,duanxdyb_id) "
					+"values("+strNewID+"," +
							lngDianxxxb_id+"," +		
							"to_date('"+DateUtil.Formatdate("yyyy-MM-dd hh:mm", datSendTime)+"','yyyy-mm-dd HH24:mi:ss')," +
							"'" +fujxx+"',"+
							"'" +fujdh+"',"+
							"'系统服务'," +
...
//		smb=getIniSMExBean();
		...
		if (!(rs==null)){
			while (rs.next()){
				doOneSendTask (rs.getDate("riq"), rs.getLong("id"),rs.getString("neir"),rs.getString("fujdh"),rs.getString("fashs"));
			}
			rs.close();
		}
		cn.Close();
	...
	//处理一行发送任务
	public String doOneSendTask(Date datSend,long lngDuanxfsbId,String strNeir,String strFujdh,String strFashs){
		JDBCcon cn = new JDBCcon();
					...
					if (!(rsNum==null)){//sql查询错误时，返回null
						while (rsNum.next()){
							...
							if (isNullString(strHansSmsInfo)){//函数返回的为空，
								blnOk =sendSms(strPhoneNumber,strNeir);//发送一条短信
							}else{
								blnOk =sendSms(strPhoneNumber,strHansSmsInfo+strNeir);//发送一条短信
							}
	...
	//发送一条短信
	public boolean sendSms(String strPhoneNumber ,String strMsg){
		try{
			...
			Sms sms1 = new Sms();
			boolean sendsucc = sms1.sendmsg(strMsg,strPhoneNumber);
			if(sendsucc){
				return true;//发送成功
			}
		}catch(Exception e) {
