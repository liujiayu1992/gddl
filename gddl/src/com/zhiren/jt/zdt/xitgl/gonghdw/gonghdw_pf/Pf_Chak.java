...
public class Pf_Chak extends BasePage implements PageValidateListener {
	...
	private Pf_Chakbean _EditValue;
	...
	public Pf_Chakbean getEditValue() {
		return _EditValue;
	}

	public void setEditValue(Pf_Chakbean EditValue) {
	...
	private void Create(){
		//根据隶属关系,省份,城市,生成小矿的供应商编码
		if(getFUIDValue().getValue() == null || getFUIDValue().getId() == -1){
			setMsg("请选择隶属关系!");
			return;
		}
		if(getShengfb_idValue().getValue() == null || getShengfb_idValue().getId() == -1){
			setMsg("请选择所属省份关系!");
			return;
		}
		if(getChengs_idValue().getValue() == null || getChengs_idValue().getId() == -1){
			setMsg("请选择所属市区关系!");
			return;
		}
		...
		try {
			...
			String  Sql_lisgx="select bianm from gongysb g where g.mingc='"+this.getFUIDValue().getValue()+"'";
			ResultSetList rs = con.getResultSetList(Sql_lisgx);
			if(rs.next()){
				if(rs.getString("bianm")==null||"".equals(rs.getString("bianm").trim())){
					lisgx="000000";
				}else{
					lisgx=rs.getString("bianm").substring(0, 6);
				}
			...
			String Sql_shengf="select bianm from shengfb s where s.quanc='"+this.getShengfb_idValue().getValue()+"'";
			ResultSetList rs1 = con.getResultSetList(Sql_shengf);
			if(rs1.next()){
				if(rs1.getString("bianm")==null||"".equals(rs1.getString("bianm").trim())){	
					shengf="00";
				}else{
					shengf=rs1.getString("bianm");
				}
			  ...
			  String Sql_chengs="select bianm from chengsb c where c.mingc='"+this.getChengs_idValue().getValue()+"'";
			  ResultSetList rs2= con.getResultSetList(Sql_chengs);
			  if(rs2.next()){
				  if(rs2.getString("bianm")==null||"".equals(rs2.getString("bianm").trim())){
					  chengs="00";	
				  }else{
					  chengs=rs2.getString("bianm");
				  }
		 ...
		 gongysbm=lisgx+shengf+chengs;
		
		 String Sql_bm="select max(bianm) as bianm from gongysb where bianm like '"+gongysbm+"%' order by bianm";
		 ResultSetList bianMa = con.getResultSetList(Sql_bm);
		 if(bianMa.next()){
			 if(bianMa.getString("bianm")==null||bianMa.getString("bianm").trim().equals("")){
				 bianm_jj=gongysbm+"00";
			 }else{
				 bianm_jj=bianMa.getString("bianm");
				 bianm_jj=String.valueOf(Long.parseLong(bianm_jj)+1);
			 }
	...
	private void Save() {
		...
		try {
			long _id = ((Visit) getPage().getVisit()).getLong8();
			int xuh = getXuh();
...
//					向gongysmzb里插入数据结束
	...
	private void getToolbars(){
		...
		ToolbarButton but1 = new ToolbarButton(null,"批复保存","function(){document.getElementById('SaveButton').click();}");
		tb.addItem(but);
		...
		tb.addText(new ToolbarText("-"));
		tb.addText(new ToolbarText("查询条件:"));
		...
		String str="function (){"+
			...
			"url = url.substring(0,end);"+
       	    ...
       	    "var rewin =  window.showModalDialog(url,'newWin','dialogWidth=520px;dialogHeight=580px;');\n" +
       	    "if   (rewin != null){document.getElementById('BIANM').value = rewin.bianh;}" +
       	    "" +
       	    "}";
		ToolbarButton chaxun = new ToolbarButton(null,"查询",str);
		tb.addItem(chaxun);
	
		tb.addText(new ToolbarText("-"));	
		ToolbarButton create = new ToolbarButton(null,"生成供应商编码","function(){" +
	...
	public void getSelectData() {
		...
		try {
			...
			String sql = "select * from gonghdwdmsqb where id = " + visit.getLong8();
			ResultSet rs = con.getResultSet(sql);
			if (rs.next()) {
				...
				String SQL = "select (select zhi\n"
						+ "          from gongysmzb\n"
						+ "         where gongysb_id = " + visit.getLong8() + "\n"
...
//	 ///////////////////////////////////////////////////////////////////////
	...
	public void beginResponse(IMarkupWriter writer, IRequestCycle cycle) {
		...
		if (zhuangt.equals("1")) {
			...
			visit.setboolean1(false);
			visit.setboolean2(true);
		}else if(zhuangt.equals("3")){//状态3为供应商代码批复页面跳转用
			visit.setboolean1(false);
			visit.setboolean2(true);
		...
		if (zhuangt.equals("1") || zhuangt.equals("2")|| zhuangt.equals("3")) {
		...
		if (!visit.getActivePageName().toString().equals(
			...
			visit.setList1(null);
			visit.setLong8(Long.parseLong(_id));
			getSelectData();
		}
		getToolbars();
	}
		...
		while (rsl.next()) {
			...
			if ("0".equals(rsl.getString("ID"))) {
				...
				sql.append("insert into gongysmkglb(id,gongysb_id,meikxxb_id) values(")
				   ...
				   .append(",").append(visit.getLong8()).append(",")
	...
	public void MeikSelectDate() {// 煤矿添加删除,修改后刷新操作
		...
		String strs = "if(document.getElementById('BIANM').value==null||document.getElementById('BIANM').value==''){ alert('请先填写供货单位编码!');}else{"
				+ " var url = 'http://'+document.location.host+document.location.pathname;"
				+ "var end = url.indexOf(';');"
				+ "url = url.substring(0,end);"
				+ "url = url + '?service=page/' + 'meik_db&bianm='+document.getElementById('BIANM').value;"
				+ " window.showModalDialog(url,'newWin','dialogWidth=930px;dialogHeight=400px;dialogLeft=45;dialogTop=300;scroll=yes;resizable=yes');}";
		egu.addToolbarItem("{"
				+ new GridButton("查询供货单位下属矿", "function (){" + strs + "}")
						.getScript() + "}");
