...
public class Shengcdsfjsd extends BasePage {
	    ...
	    try{
	        if(getEditValues()!= null && !getEditValues().isEmpty()&& !((JiesdlrBean)getEditValues().get(0)).getJiesbh().equals("")){
	        	if(Fayjsdcz.checkbh_Shih(jiesbh,((JiesdlrBean)getEditValues().get(0)).getId(),((JiesdlrBean)getEditValues().get(0)).getYid())){
	        				sql.append("begin \n");
	        				String mk="insert into diancjsmkb (id,bianm,gongysb_id,gongysmc,faz,fahksrq,fahjzrq," +
	        						...
	        						"ranlbmjbr,ranlbmjbrq,jieslx,diancxxb_id,danjc,KUANGFJSMKB_ID,YEWLXB_ID,jiesrl,jieslf,jiesrq,hetb_id,gongsqrzt)values("+newID+",'"+jiesbh+"',"+gongysb_id+",'"+gongysmc+"','"+faz+"',"
									...
									"(select nvl(max(xiaoshetb_id),0) from fahbtmp where jiesb_id="+((JiesdlrBean)getEditValues().get(0)).getId()+" ),1); \n";
			        		
			        		String yf="insert into diancjsyfb (id,bianm,gongysb_id, gongysmc,faz,fahksrq,fahjzrq,meiz,daibch,yuanshr,xianshr,yansksrq,yansjzrq," +
			        				"yansbh,shoukdw,kaihyh,zhangh,fapbh,fukfs,duifdd,ches,jiessl,guohl,guotyf,guotzf,kuangqyf,kuangqzf,jiskc," +
			        				"bukyf,hansyf,buhsyf,shuik,shuil,jiesrq,beiz,jieslx,diancxxb_id,hansdj,DIANCJSMKB_ID,YEWLXB_ID,RANLBMJBRQ,RANLBMJBR,hetb_id)values("+newID+",'"+jiesbh+"',"+gongysb_id+",'"+gongysmc+"','"+faz+"',"
	        						+"to_date('"+fahksrq+"','yyyy-MM-dd'),to_date('"+fahjzrq+"','yyyy-MM-dd'),'"+meiz+"','"+daibch+"',"
	        						+"'"+yuanshr+"','"+xianshr+"',to_date('"+yansksrq+"','yyyy-MM-dd'),to_date('"+yansjzrq+"','yyyy-MM-dd'),'"+
	        						...
	        						+"to_date('"+this.FormatDate(((JiesdlrBean)getEditValues().get(0)).getJiesrq())+"','yyyy-MM-dd'),"
	        						+"'"+beiz+"',"
	        						+jieslx+","+diancid+","+hansdj+","+newID+
	        						","+getShujlyValue().getId()+",to_date('"+ranlbmjbrq+"','yyyy-MM-dd'),'"+ranlbmjbr+"'," +
    								"(select nvl(max(xiaoshetb_id),0) from fahbtmp where jiesb_id="+((JiesdlrBean)getEditValues().get(0)).getId()+" )); \n";
...
//			        		long dancID=((JiesdlrBean)getEditValues().get(0)).getId();
			        		...
			        		if(getJieslxValue().getValue().equals("两票结算")){
			        			...
			        			sql.append("update jiesb set diancjsmkb_id="+newID+" where id="+((JiesdlrBean)getEditValues().get(0)).getId()+";\n");
			        			sql.append("update fayslb set xiaosjsb_id="+newID+" where id in(select f.fayslb_id from fahbtmp f where f.jiesb_id="+((JiesdlrBean)getEditValues().get(0)).getId()+");\n");
			        		}  		
...
//			结算时要判断有无这个id，如果有就一定要用这个id
			...
			sql="select * from jiesb where bianm='"+getJiesbhValue().getValue()+"' ";//and fuid=0";
...
//				初始化数量、硫份、热值标准
				mgongfsl=mjiessl;
				myanssl=mjiessl;
				mQnetar_js=rs.getDouble("jiesrl");
				mQnetar_kf=mQnetar_js;
				mQnetar_cf=mQnetar_js;
				mStd_js=rs.getDouble("jieslf");
				mStd_kf=mStd_js;
				mStd_cf=mStd_js;
				
				sql="select jieszbsjb.*,zhibb.bianm from jieszbsjb,jiesb,zhibb "
					 + " where jieszbsjb.jiesdid="+mid+" and zhibb.id=jieszbsjb.zhibb_id and zhibb.leib=1"
					 + " and jiesb.bianm='"+getJiesbhValue().getValue()+"' and jieszbsjb.zhuangt=1 order by jieszbsjb.id";
...
//				两票		运费
				sql="select * from jiesyfb where bianm='"+getJiesbhValue().getValue()+"' ";//and fuid=0";
				...
				sql=" select * from jiesyfb where bianm='"+getJiesbhValue().getValue()+"'";
...
//		tb1.addText(new ToolbarText("-"));
//		生成
		ToolbarButton Shengcbt=new ToolbarButton(null,"生成销售结算单","function(){HtSButton()}");
	...
	public void beginResponse(IMarkupWriter writer, IRequestCycle cycle) {
		...
		if (!visit.getActivePageName().toString().equals(
			...
			visit.setString11("");	//用于Kuangqzf返回时，跳转到正确的界面(DCBalance,Jiesdxg)
...
//		if(getShujlyValue().getStrId().equals("0")){
//			strTiaoj = "  and k.gongsqrzt=1 ";
//		}
    	
    	try{
    		int i=-1;
    		List.add(new IDropDownBean(i++, "请选择"));
    		sql="select distinct k.bianm from jiesb k, diancxxb dc where k. diancxxb_id=dc.id and k.diancjsmkb_id=0  "+strDiancID+" order by bianm";
    		ResultSet rs=con.getResultSet(sql);
    		while(rs.next()){
