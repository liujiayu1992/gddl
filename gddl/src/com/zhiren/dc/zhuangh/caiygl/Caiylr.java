...
public class Caiylr extends BasePage implements PageValidateListener {
	// 界面用户提示
	private String msg = "";
		...
		while (mdrsl.next()) {
			if ("0".equals(mdrsl.getString("ID"))) {
				...
				if(Integer.parseInt(mdrsl.getString("CAIYSJ").substring(11, 13))>=Integer.parseInt(shijd)){
					String sql_date="select to_date('"+caiysj+"','yyyy-mm-dd') as caiysj from dual";
...
//					如果出错则返回错误
	...
	public static int InsZhuanmb(JDBCcon con,long zhuanmlb_id, long diancxxb_id, 
		...
		if(Integer.parseInt(caiysj.substring(11, 13))>=Integer.parseInt(shijd)){
			String sql_date="select to_date('"+caiys+"','yyyy-mm-dd') as caiysj from dual";
			ResultSetList rs_date = con.getResultSetList(sql_date);
			while(rs_date.next()){
	...
	public void getSelectData() {
		...
		String ss="var CAIYSJ= '"+DateUtil.FormatDateTime(new Date())+"';" +
				"if(CAIYSJ>='"+DateUtil.FormatDateTime(new Date()).substring(0, 10)+" "+shijd+":00:00'&&CAIYSJ<='"+DateUtil.FormatDateTime(new Date()).substring(0, 10)+" 20:00:00') {Ext.MessageBox.alert('提示信息','请注意采样时间已经超过上午九点');}";
		egu.addToolbarButton(GridButton.ButtonType_Insert_condition,"",
				"if(Yundxx.getRawValue()=='请选择'){alert('请选择运单信息'); return;}"+ss+" ");
		String treepanel =
