 ...
 * 内容:煤矿地区复合组合查询
 */
public class Fuhmkcx extends BasePage implements PageValidateListener {
...
//		}
		...
		if(getTreeid()!=null && !"".equals(getTreeid()) && !"0".equals(getTreeid())) {
			...
			meik = " and  m.id in ( select meikxxb_id from diqmkzh dz where dz.diqmkgroup_id="+this.getTreeid()+") ";
			fahgl=" and f.meikxxb_id in ( select meikxxb_id from diqmkzh dz where dz.diqmkgroup_id="+this.getTreeid()+") ";
		}
			
		
		if(!this.getKoujValue().getStrId().equals("0")){
			fahgl+=" and f.jihkjb_id="+this.getKoujValue().getStrId()+" ";
		}
...
//		sb.append(" sum(cx.ches) ches\n");
		...
		sb.append(" (select cp.fahb_id,sum(cp.maoz) maoz,sum(cp.piz) piz,sum(cp.biaoz) biaoz,sum(cp.yuns) yuns," +
				"sum(cp.yingk) yingk,sum(cp.zongkd) zongkd,sum(cp.maoz-cp.piz-cp.zongkd) jingz, " +
		...
		sb.append(" order by grouping(fx.dc) desc,fx.dc desc,grouping(fx.gys) desc,fx.gys asc,grouping(fx.mk) desc,fx.mk asc,cx.ysdw,grouping(fx.daohrq) desc\n");
		sb.append(" \n");
		sb.append(" \n");
		sb.append(" \n");
		return sb;
...
//		sb.append(" sum(cx.ches) ches\n");
		...
		sb.append(" (select cp.fahb_id,sum(cp.maoz) maoz,sum(cp.piz) piz,sum(cp.biaoz) biaoz,sum(cp.yuns) yuns,sum(cp.yingk) yingk," +
				"sum(cp.zongkd) zongkd,sum(cp.maoz-cp.piz-cp.zongkd) jingz, count(*) ches,y.mingc ysdw\n");
		sb.append(" from chepb cp,yunsdwb y  where cp.yunsdwb_id=y.id(+) "+yunsdw+"  group by cp.fahb_id,y.mingc  ) cx where fx.id=cx.fahb_id \n");
...
//				,""+visit.getDiancxxb_id(),"",getTreeid(),getTreeid());
		...
		dt.setTree_window(this.getGYSSql().toString(), "gongysTree", ""+visit.getDiancxxb_id(), "", getTreeid(), getTreeid());
...
//	口径下拉框
	...
	public void setKoujModels() {
		setKoujModel(new IDropDownModel(" select 0 id,'全部' mingc from dual union select id,mingc from jihkjb order by id asc "));
	}
