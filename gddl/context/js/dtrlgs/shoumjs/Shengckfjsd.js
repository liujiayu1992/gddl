...
SelectFrmDiv.style.posTop=0;
	...
	if(document.all.item("ShengcButton").style.display=="none"){
		...
		document.all.item("BackButton").style.display="";
		document.all.item("SaveButton").style.display="";
	}else{
		document.all.item("RetrunsButton").style.display="";
		document.all.item("BackButton").style.display="none";
...
function droponchange(_int){
	if (objselectCol !=null){		
		...
		if(objselectCol==document.all.item("YUANSHR")){
			var Xshr=document.all.item("XianshrSelect");
   			var Yshr=document.all.item("YuanshrSelect");
			Xshr.selectedIndex=Yshr.selectedIndex;
  			document.all.item("XIANSHR").value=Xshr.options(Xshr.selectedIndex).text;
		}
...
function dropdown(_obj1,_obj2){
	...
	_obj2.style.posTop=_obj1.parentElement.offsetTop+53;
...
function check(){
	if(document.all.item("SHOUKDW").value==""||document.all.item("SHOUKDW").value=="请选择"){
		...
		alert("验收日期为空，请核对！");
		return false;
	}else if(document.all.item("ShujlyDropDown").value=="-1"){
		alert("请选择一个业务类型！");
		return false;
	};
	else{
...
function Kuangfjsd(){
		...
		document.all.item("DJC").value=eval(document.all.item("DANJC").value);
...
function SetXYS(){
	var Xshr=document.all.item("XianshrSelect");
    var Yshr=document.all.item("YuanshrSelect");
	for (i=0;i<Xshr.options.length;i++){
		if (Xshr.options(i).value=="199"){
			Xshr.selectedIndex=i;
			Yshr.selectedIndex=i;
			break;
		}
	}
  document.all.item("XIANSHR").value=Xshr.options(Xshr.selectedIndex).text;
  document.all.item("YUANSHR").value=Yshr.options(Yshr.selectedIndex).text;
  
  //var Skdw=document.all.item("SHOUKDWSelect");
  //for(i=0;i<Skdw.options.length;i++){
  //	if(Skdw.options(i).value=="20"){
  //		Skdw.selectedIndex=i;
  //		break;
  //	}
  //}
 // document.all.item("SHOUKDW").value=Skdw.options(Skdw.selectedIndex).text;
}
...
function SetDSFXYS(){
  ...
  //var Skdw=document.all.item("SHOUKDWSelect");
  //for(i=0;i<Skdw.options.length;i++){
  //	if(Skdw.options(i).value=="19911044"){
  //		Skdw.selectedIndex=i;
  //		break;
  //	}
  //}
...
function KuangfCount(){
//	alert((1+eval(document.all.item('SHUIL').value)));
    //document.all.item("JIASJE").value =Math.round((eval(document.all.item("JIESSL").value)*eval(document.all.item("SHULZJBZ").value)+eval(document.all.item("BUKYQJK").value))*100)/100;
    //document.all.item("JIAKHJ").value =Math.round((eval(document.all.item("JIASJE").value)/(1+eval(document.all.item('SHUIL').value)))*100)/100;
    //document.all.item("JIAKSK").value =Math.round((eval(document.all.item("JIASJE").value)-eval(document.all.item("JIAKHJ").value))*100)/100;
	   //清空开户银行，账号，发票编号中的值
	   // document.all.item("KAIHYH").value="";
	  //  document.all.item("ZHANGH").value="";
	   // document.all.item("FAPBH").value="";
//	    document.all.item("").value="";
    ...
    var str=document.all.item("XUANZ").value;
    if(str!=""){
	...
	for(var i=0;i<result.length;i++){
	    ...
	    if(result[i]=="qnet"){	    
	    //if(eval(document.all.item("Qnetar_ht").value)>0&&eval(document.all.item("Qnetar_js").value)>0){
	    	//document.all.item("Qnetar_yk").value=Math.round(eval(document.all.item("Qnetar_js").value)-eval(document.all.item("Qnetar_ht").value));
	    	//document.all.item("SHULZJJE").value=Math.round(eval(document.all.item("HANSDJ").value)*eval(document.all.item("Qnetar_yk").value)*100)/100;
	    //}
	    
	    if((document.all.item("Qnetar_ht").value>0||document.all.item("Qnetar_ht").value.length>4)&&eval(document.all.item("Qnetar_js").value)>0&&eval(document.all.item("JIESSL").value)>0){
	    	
	    	if(document.all.item("Qnetar_ht").value.length==4){
	    		document.all.item("Qnetar_yk").value=Math.round(eval(document.all.item("Qnetar_js").value)-eval(document.all.item("Qnetar_ht").value));
		    	//if(eval(document.all.item("Qnetar_js").value)<eval(document.all.item("Qnetar_ht").value)){
		    	//	document.all.item("Qnetar_zje").value=-Math.round(eval(document.all.item("JIESSL").value)*eval(document.all.item("Qnetar_zdj").value)*eval(document.all.item("Qnetar_yk").value)*100)/100;
		    	//}else{
		    	//}
		    	if(document.all.item("Qnetar_yk").value==0){
		    		document.all.item("Qnetar_zje").value=0;
		    	}else{
		    		document.all.item("Qnetar_zje").value=Math.round(eval(document.all.item("JIESSL").value)*eval(document.all.item("Qnetar_zdj").value)*eval(document.all.item("Qnetar_yk").value)*100)/100;
		    	}
	    		...
	    		var gongfrlxx=eval(document.all.item("Qnetar_ht").value.substring(0,4));
	    		var gongfrlsx=eval(document.all.item("Qnetar_ht").value.substring(5));
	    		
	    		if(eval(document.all.item("Qnetar_js").value)-gongfrlsx>=0){
	    		
	    			document.all.item("Qnetar_yk").value=Math.round(eval(document.all.item("Qnetar_js").value)-gongfrlsx);
	    		}else{
	    		
	    			document.all.item("Qnetar_yk").value=Math.round(eval(document.all.item("Qnetar_js").value)-gongfrlxx);
	    		}
	    ...
	    if(result[i]=="vdaf"){	    
	    if(eval(document.all.item("Vdaf_kf").value)>0&&eval(document.all.item("Vdaf_cf").value)>0){
	    	document.all.item("Vdaf_yk").value=Math.round((eval(document.all.item("Vdaf_cf").value)-eval(document.all.item("Vdaf_kf").value))*100)/100;
			if(eval(document.all.item("Vdaf_cf").value)<eval(document.all.item("Vdaf_kf").value)){
	    		document.all.item("Vdaf_zje").value=-Math.round(eval(document.all.item("JIESSL").value)*eval(document.all.item("Vdaf_zdj").value)*100)/100;
	    	}else{
				document.all.item("Vdaf_zje").value=Math.round(eval(document.all.item("JIESSL").value)*eval(document.all.item("Vdaf_zdj").value)*100)/100;
	    	}
	    }
	    ...
	    if(result[i]=="ad"){	    
	    if(eval(document.all.item("Ad_kf").value)>0&&eval(document.all.item("Ad_cf").value)>0){
	    	document.all.item("Ad_yk").value=Math.round((eval(document.all.item("Ad_cf").value)-eval(document.all.item("Ad_kf").value))*100)/100;
	    	document.all.item("Ad_zje").value=Math.round(eval(document.all.item("JIESSL").value)*eval(document.all.item("Ad_zdj").value)*100)/100;
	    }
	    }
	    if(result[i]=="mt"){	    
	    if(eval(document.all.item("Mt_kf").value)>0&&eval(document.all.item("Mt_cf").value)>0){
	    	document.all.item("Mt_yk").value=Math.round((eval(document.all.item("Mt_cf").value)-eval(document.all.item("Mt_kf").value))*100)/100;
	    	document.all.item("Mt_zje").value=Math.round(eval(document.all.item("JIESSL").value)*eval(document.all.item("Mt_zdj").value)*100)/100;
	    }
	    }
	    if(result[i]=="std"){	    
	    if(eval(document.all.item("Std_kf").value)>0&&eval(document.all.item("Std_cf").value)>0){
	    	document.all.item("Std_yk").value=Math.round((eval(document.all.item("Std_cf").value)-eval(document.all.item("Std_kf").value))*100)/100;
	    	if(document.all.item("Std_yk").value>0){
	    	
	    		document.all.item("Std_zje").value=-Math.round(eval(document.all.item("JIESSL").value)*eval(document.all.item("Std_zdj").value)*eval(document.all.item("Std_yk").value)*100)/100;
	    	}else{
	    ...
	    if (eval(document.all.item("JIESSL").value)>0){
	    	//alert(document.all.item("JIESSL").value);
	    ...
	    if(eval(document.all.item("TIELYF").value)>0){
		    ...
		    document.all.item("YUNFSK").value =Math.round((eval(document.all.item("YUNZFHJ").value)-eval(document.all.item("JISKC").value))*0.07*100)/100;
...
function MatchNum_SD(obj1,str){
	...
	if(Nums!= Numr){						// 如果不等
		...
		if(Nums==''){obj1.value=0;}				     // 否则判断是否是否为空，是返回0		
		else{obj1.value = Nums;}     	// 如果没超过上下限，将改正的数值返回到原输入框
		return true;
