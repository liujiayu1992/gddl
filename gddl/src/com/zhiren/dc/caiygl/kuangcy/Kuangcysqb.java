...
/*作者:王总兵
 *日期:2010-4-22 8:59:04
 *描述:矿采样申请表
 * 
 * 
 */
public class Kuangcysqb extends BasePage {
	...
	private String getSelectData() {
		...
		ArrHeader[0] = new String[] { "采样时间","煤矿", "采样编号", "申请单位", "采样人员", "批准人",
				"化验热值", "化验硫分","预测热值","备注"};
		

		int ArrWidth[] = new int[] { 80,120 ,80, 100, 60, 60, 60, 60,60,120};
		
		String meiktiaojian="";
		if(getMeikxxb_id()==-1){
			meiktiaojian="";
		}else{
			meiktiaojian="and k.meikxxb_id = " + getMeikxxb_id();
		}
		
		StringBuffer buffer = new StringBuffer();		
	    buffer.append(
	    		"select to_char(k.caiysj,'yyyy-mm-dd') as caiysj,decode(mk.mingc,null,k.meikdwmc,mk.mingc) as mingc,k.huaybh,k.shenqdw,\n" +
	    		"k.caiyy,k.pizr,round_new(k.qnet_ar*1000/4.1816,0) as qnet_ar,k.stad,k.ruc_rez,\n" + 
	    		"k.beiz2\n" + 
	    		" from kuangcyb k,meikxxb mk\n" + 
	    		"where mk.id(+)=k.meikxxb_id\n" + 
	    		"and k.caiysj>=to_date('"+this.getBriqi()+"','yyyy-mm-dd')\n" + 
	    		"and k.caiysj<=to_date('"+this.getEriqi()+"','yyyy-mm-dd')\n" + 
	    		""+meiktiaojian+"\n"+
		...
		rt.setTitle("矿采煤样申请登记表", ArrWidth);
