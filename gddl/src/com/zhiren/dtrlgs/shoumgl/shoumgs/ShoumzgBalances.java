...
//import com.zhiren.dtrlgs.faygl.fayjs.Shoumjsdcz;
import com.zhiren.dtrlgs.shoumgl.shoumjs.Shoumjsdcz;

import bsh.*;

public class ShoumzgBalances {
...
//			sql.append(" 		yuns,jingz,koud,kous,kouz,ches,biaoz,yingk,jiessl,(jiessl-jingz) as jieslcy,yunfjssl,yingd,yingd-yingk as kuid from (select "+Shoumjsdcz.getJiesszzh(jies_Jieslqzfs, "yuns", jies_Jsslblxs)+" as yuns,"+Shoumjsdcz.getJiesszzh(jies_Guohlqzfs,"jingz",jies_Guohlblxsw)+" as jingz,						\n");
...
//			sql.append(         Shoumjsdcz.getJiesszzh(jies_Jieslqzfs, jies_yunfjssl, jies_Jsslblxs)).append(" as yunfjssl,   \n");
//			sql.append("	 --厂方验收   																															\n");
...
//			sql.append("       			sum(cp.maoz) as maoz,sum(cp.piz) as piz,sum(cp.maoz-cp.piz-cp.zongkd) as jingz,sum(cp.biaoz) as biaoz,						\n");
...
//			sql.append(" 			from fahb f,chepb cp"+contion_table+" where f.id=cp.fahb_id "+contion_where+" and id in("+SelIds+")"+yunsdw+" 													\n"); 
...
//			sql.append("	 	(select distinct f.id as fahb_id,k.* from fahb f,chepb cp,kuangfzlzb k where f.id=cp.fahb_id and cp.kuangfzlzb_id=k.id "+yunsdw+" and f.lie_id in ("+SelIds+")) kz			\n");
...
//			sql.append("				and f.liucztb_id=1 and z.liucztb_id=1 and f.id in ("+SelIds+")) 	\n");
...
//			sql.append("	 decode(sum("+jies_Jqsl+"),0,0,round_new(sum(("+jies_Jqsl+")*z.star)/sum(("+jies_Jqsl+")),"+jies_Starblxs+")) as Star_kf, 			 	\n");
			...
			sql.append("	 	from (select distinct f.id,f.yuanid,f.diancxxb_id,f.gongysb_id,f.meikxxb_id,f.pinzb_id,f.faz_id,									\n");
			sql.append("				f.daoz_id,f.jihkjb_id,f.fahrq,f.daohrq,f.hetb_id,f.zhilb_id,f.jiesb_id,f.yunsfsb_id,f.chec,									\n");
			sql.append("       			f.maoz as maoz,f.piz as piz,f.jingz as jingz,f.biaoz as biaoz,	\n");
...
//							运费含税单价保留小数位
			...
			StringBuffer sql=new StringBuffer("");
				...
				sql.append(" 		yuns,jingz,koud,kous,kouz,ches,biaoz,yingk,jiessl,(jiessl-jingz) as jieslcy,yunfjssl,yingd,yingd-yingk as kuid from (select "+Shoumjsdcz.getJiesszzh(jies_Jieslqzfs, "yuns", jies_Jsslblxs)+" as yuns,"+Shoumjsdcz.getJiesszzh(jies_Jieslqzfs,"jingz",jies_Jsslblxs)+" as jingz,						\n");
				sql.append(			Shoumjsdcz.getJiesszzh(jies_Kdkskzqzfs, "koud", jies_Jsslblxs)+" as koud,"+Shoumjsdcz.getJiesszzh(jies_Kdkskzqzfs, "kous", jies_Jsslblxs)+" as kous,"+Shoumjsdcz.getJiesszzh(jies_Kdkskzqzfs, "kouz", jies_Jsslblxs)+" as kouz,	\n");
				sql.append(" 		sum(ches) as ches, "+Shoumjsdcz.getJiesszzh(jies_Jieslqzfs, "biaoz", jies_Jsslblxs)+" as biaoz,"+Shoumjsdcz.getJiesszzh(jies_Jieslqzfs, "jingz+yuns-biaoz", jies_Jsslblxs)+" as yingk,"+Shoumjsdcz.getJiesszzh(jies_Jieslqzfs, "yingd", jies_Jsslblxs)+" as yingd, 	\n");
				sql.append(         Shoumjsdcz.getJiesszzh(jies_Jieslqzfs, jies_Jssl, jies_Jsslblxs)).append(" as jiessl,   \n");
				sql.append(         Shoumjsdcz.getJiesszzh(jies_Jieslqzfs, jies_yunfjssl, jies_Jsslblxs)).append(" as yunfjssl,   \n");
...
//			数量(合同中以月为单位，每月存一条，暂定先取一条数)
			sql="select htsl.hetl,htb.gongfkhyh,htb.gongfzh \n"
				...
				+ " and htsl.diancxxb_id="+Diancxxb_id+" and htsl.riq<=to_date('"+Shoumjsdcz.FormatDate(Minfahrq)+"','yyyy-MM-dd')	\n"              
...
//				+ " and htb.id="+Hetb_id+"";
			...
			if(rsl.next()){
				...
				bsv.setHetmdjgs(Shoumzgcl.getTransform_Hetjgs(
...
//								获得结算数量、质量
								...
								Dbltmp=Shoumjsdcz.getUnit_transform(rsl.getString("zhib"),rsl.getString("danw"),Dbltmp);
...
//									获得结算数量、质量
									...
									do{
										...
										Dbljijzb=Shoumjsdcz.getZhib_info(bsv,rsltmp.getString("zhib"),"js");
										Dbljijzb=Shoumjsdcz.getUnit_transform(rsltmp.getString("zhib"),rsltmp.getString("danw"),Dbljijzb);
										...
										if(Dbljijzb>=rsltmp.getDouble("xiax")&&Dbljijzb<=(rsltmp.getDouble("shangx")==0?9999:rsltmp.getDouble("shangx"))){
											...
											bsh.set(rsltmp.getString("zhib")+Shoumjsdcz.getZhibbdw(rsltmp.getString("zhib"),rsltmp.getString("danw")),Dbljijzb);	//结算值
...
//									获得结算数量、质量
								...
								do{
									...
									Dbljijzb=Shoumjsdcz.getUnit_transform(rsl.getString("zhib"),rsl.getString("danw"),Dbljijzb);
...
//				    		国铁运费结算
	...
	private void CountYffromwh(){
		...
		String sql="";
		JDBCcon con=new JDBCcon();
		sql="select * from diancjskyfb jy\n" + 
			...
			do {
				...
				if(Dbljijzb>=rsl.getDouble("xiax")&&Dbljijzb<=(rsl.getDouble("shangx")==0?9999:rsl.getDouble("shangx"))){
					...
					if(Shoumjsdcz.CheckMljRz(rsl.getString("zhib"))){
						...
						bsh.set(rsl.getString("zhib")+"_上限", Shoumjsdcz.getUnit_transform(rsl.getString("zhib"), Locale.qiankmqk_danw, (rsl.getDouble("shangx")==0?9999:rsl.getDouble("shangx"))));
