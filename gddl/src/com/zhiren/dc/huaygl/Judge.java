...
import com.zhiren.common.ArrayUtil;
...
public class Judge {
	...
	public double[] getJudgeData(int T_type, double[] OriArray) {
		if (OriArray.length < 2) {
			return OriArray;
		}else if (OriArray.length==2) { //ww 2010-05-20 判断实验值有两个的时候
			for (int i = 2; i <= OriArray.length; i++) {
				...
				if ((int) (MaxRemainder * 10000) <= (int) (T * 10000)) {//用整数做比较
					return array;
				}
			}
		}
