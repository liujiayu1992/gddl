...
public class Qitwpghdtz extends BasePage implements PageValidateListener {
	...
	private static final String BAOBPZB_GUANJZ = "JILTZ_QTWP_GJZ";// baobpzb中对应的关键字
//	界面用户提示
	private String msg="";
...
//	获取相关的SQL
	...
	public StringBuffer getBaseSql() {
		...
		sb.append(" select  \n")
		  .append(" decode(grouping(d.mingc),1,'统计',d.mingc) dcmingc ,decode(grouping(g.mingc)+grouping(d.mingc),1,'合计',g.mingc) gysmingc, \n")
		  .append("  decode(grouping(g.mingc)+grouping(d.mingc)+grouping(p.mingc),1,'小计',p.mingc) pzmingc, \n")
		  .append(" decode(grouping(g.mingc)+grouping(d.mingc)+grouping(p.mingc)+grouping(q.daohrq),1,'小计',to_char(q.daohrq,'yyyy-MM-dd')) daohrq,  \n")
...
//		rt.footer.setRowHeight(1, 1);
		...
		RPTInit.getInsertSql(v.getDiancxxb_id(),getBaseSql().toString(),rt,"其他物品过衡台帐",""+BAOBPZB_GUANJZ+v.getInt1());
