...
//	 判断是否是集团用户
	...
	private boolean IsRanlgs(){//取得电厂ID
...
//		}
	...
	public String getRucmzlys(){
		...
		if(jib==1){//选集团时刷新出所有的电厂
			strgongs = "select distinct dc.fgsxh as xuh,dc.fgsid as id,dc.fgsmc as mingc from vwdianc dc ";//取得集团下的所有分公司
			strGroupID = "dc.fgsid";
		}else if (jib==2){//选分公司的时候刷新出分公司下所有的电厂
			strgongs = "select distinct dc.xuh,dc.id,dc.mingc from vwdianc dc where (dc.fgsid="+this.getTreeid() +" or dc.rlgsid="+this.getTreeid()+")";//取得分公司下的所有电厂
			strGroupID = "dc.id";
		}else if (jib==3){
		...
		ArrHeader[0]=new String[] {"入炉煤质量统计","入炉煤质量统计","入炉煤质量统计","入炉煤质量统计","入炉煤质量统计","入炉煤质量统计","入炉煤质量统计","入炉煤质量统计","入炉煤质量统计"};
		ArrHeader[1]=new String[] {"电厂","入炉煤量(吨)","入炉煤量(吨)","水分<br>Mt(%)","灰分<br>Ad(%)","挥发分<br>Vdaf(%)","硫分<br>St,d(%)","低位发热量","低位发热量"};
		ArrHeader[2]=new String[] {"电厂","入炉煤量","检质量","水分<br>Mt(%)","灰分<br>Ad(%)","挥发分<br>Vdaf(%)","硫分<br>St,d(%)","MJ/kg","Kcal/Kg"};
...
//	柱型图
	public String getChartBar(){
		...
		if(jib==1){//选集团时刷新出所有的电厂
			strgongs = "select distinct dc.fgsid as id,dc.fgsxh as xuh,dc.fgsmc as mingc from vwdianc dc ";//取得集团下的所有分公司
			strGroupID = "dc.fgsid";
		}else if (jib==2){//选分公司的时候刷新出分公司下所有的电厂
		...
		if(visit.getRuljiaql().equals(SysConstant.RuLJQL)){
			...
			sbsql.append(" select danwmc,fenx,"+strDataField+" from ( \n");
			sbsql.append(" select dc.mingc as danwmc, decode(1,1,'本期','本期') as fenx,nvl(jianzl,0) as jianzl,nvl(mt,0) as mt,nvl(ad,0) as ad,nvl(vdaf,0) as vdaf,nvl(std,0) as std,nvl(qnet_ar,0) as farl,nvl(qnet_ar1,0) as qnet_ar1  \n");
			sbsql.append(" from ( "+strgongs+" ) dc, \n");
			...
			sbsql.append(" where sj.id(+)=dc.id  \n");
			sbsql.append(" union all  \n");
			sbsql.append(" select dc.mingc as danwmc, decode(1,1,'同期','同期') as fenx,nvl(jianzl,0) as jianzl,nvl(mt,0) as mt,nvl(ad,0) as ad,nvl(vdaf,0) as vdaf,nvl(std,0) as std,nvl(qnet_ar,0) as farl,nvl(qnet_ar1,0) as qnet_ar1  \n");
			sbsql.append(" from ( "+strgongsq+" ) dc, \n");
			sbsql.append("       (select "+strGroupID+" as id, nvl(sum(fadhy+gongrhy),0) as laimzl, sum(decode(nvl(qnet_ar,0),0,0,fadhy+gongrhy)) as jianzl,");
			sbsql.append(" round_new(decode(sum(decode(nvl(qnet_ar,0),0,0,fadhy+gongrhy)),0,0,sum(round_new(mt,"+visit.getMtdec()+")*(fadhy+gongrhy))/sum(decode(nvl(qnet_ar,0),0,0,fadhy+gongrhy))),"+visit.getMtdec()+") as mt, \n");
			sbsql.append(" round_new(decode(sum(decode(nvl(qnet_ar,0),0,0,fadhy+gongrhy)),0,0,sum(ad*(fadhy+gongrhy))/sum(decode(nvl(qnet_ar,0),0,0,fadhy+gongrhy))),2) as ad, \n");
...
//	曲线图
	public String getChartLine(){
		...
		String last_nianf=""+(DateUtil.getYear(DateUtil.getLastDayOfYear(getBeginriqDate()))-1);//去年
		String date_nianf=""+DateUtil.getYear(DateUtil.getLastDayOfYear(getBeginriqDate()));//今年
		int jib=this.getDiancTreeJib();
		String strCondition=getFilerCondtion(jib);
		String strDataField=getDataField();
		Visit visit= (Visit)getPage().getVisit();
...
//	类型
	...
	public void getChartDropDownModels() {
		List list = new ArrayList();
		list.add(new IDropDownBean(1, "入厂煤质热值"));
		list.add(new IDropDownBean(2, "入炉煤质热值"));
...
//		}else if(strDataField.equals("入炉煤质热值")){
