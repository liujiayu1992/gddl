package com.zhiren.jt.diaoygl.yunsjhcx;
...
public class Yunsjhcx  extends BasePage implements PageValidateListener{
		...
		//报表内容
			if(getBaoblxValue().getValue().equals("分厂汇总")){
				...
				if(jib==1){//树为集团时,按照分公司汇总
					...
					"where ys.diancxxb_id=dc.id and ys.gongysb_id=mk.id and ys.riq=to_date('"+intyear+"-"+intMonth+"-01','yyyy-mm-dd') "+strGongsID+" and ys.leix=1\n" + 
...
//				分矿分厂汇总
				strSQL =
				 ...
				 ArrHeader=new String[1][13];
				 ArrHeader[0]=new String[] {"发货单位名称收货单位名称","收货单位名称","发到","到站","品名","批车","批吨","批准计划号","换装港","终到港","自备车车次","顺号","路局"};
