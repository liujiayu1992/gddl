 ...
 * @since 2012-04-17
 * @version V0.1
 * @discription 基于Yuejsdjsc.java 1.1.2.3修改而成
 */

public class Yuejsdjscgys extends BasePage implements PageValidateListener {
	...
	public void CreateData() {
		...
		sql =   "INSERT INTO YUEJSDJ\n" +
				"  (ID, RIQ, DIANCXXB_ID, FENX, GONGYSB_ID, MEIKXXB_ID, JIHKJB_ID, PINZB_ID, YUNSFSB_ID)\n" + 
...
//			这种情况比较特殊，所以需要在此重新关联出运费信息
		...
		sql = 
			...
			"yunj, ROUND(yunj*0.07,2) yunjs,zaf,qnet_ar from(select j.gongysb_id, j.meikxxb_id, j.jihkjb_id,\n" +
			...
			"and j.ruzrq BETWEEN "+Riq1+" AND "+Riq2+"\n" +
			" and j.diancxxb_id="+ diancxxb_id+
			" group by (j.gongysb_id,j.meikxxb_id, j.jihkjb_id, j.meiz, j.yunsfsb_id))";
...
//			逐条更新本月信息
			sb.append(  "UPDATE YUEJSDJ J\n" +
						...
						"   AND J.GONGYSB_ID = "+rs.getString("GONGYSB_ID")+"\n" + 
...
//		计算累计数据
	...
	private int CountLeij(){
		...
		sql="SELECT GONGYSB_ID,MEIKXXB_ID,JIHKJB_ID,PINZB_ID,YUNSFSB_ID,JIESL,JIESRL,JIESMJ,ROUND(JIESMJ-JIESMJ/1.17,2) JIESMJS,KUANGQYF,JIESYJ, ROUND(JIESYJ*0.07,2) JIESYJS,\n" +
			"  JIESZF, JIESZFS, \n" +
			"  ROUND(DECODE(JIESRL,0,0,(JIESMJ + JIESYJ + JIESZF + KUANGQYF)*29.271/JIESRL),2) JIESBMDJ,\n" +
			"  ROUND(DECODE(JIESRL,0,0,(JIESMJ+JIESYJ+JIESZF+KUANGQYF-ROUND(JIESMJ-JIESMJ/1.17,2)-ROUND(JIESYJ*0.07,2)-JIESZFS)*29.271/JIESRL),2)JIESBHSBMDJ \n" +
			" FROM (SELECT DJ.GONGYSB_ID,DJ.MEIKXXB_ID,\n" +
			"       DJ.JIHKJB_ID,\n" + 
			"       DJ.PINZB_ID,\n" + 
			"       DJ.YUNSFSB_ID,\n" + 
			"       SUM(DJ.JIESL) JIESL,\n" + 
			"       round(DECODE(SUM(DECODE(DJ.JIESRL, 0, 0, DJ.JIESL)),0,0,SUM(DJ.JIESRL * DECODE(DJ.JIESRL, 0, 0, DJ.JIESL)) /SUM(DECODE(DJ.JIESRL, 0, 0, DJ.JIESL))),2) JIESRL,\n" + 
			...
			"   AND DJ.FENX = FX.FENX\n" + 
			"   AND DJ.GONGYSB_ID = FX.GONGYSB_ID\n" + 
			...
			" GROUP BY (DJ.GONGYSB_ID,DJ.MEIKXXB_ID, DJ.JIHKJB_ID, DJ.PINZB_ID, DJ.YUNSFSB_ID))";
//		取得本年1月至所报月份的所有结算信息
		ResultSetList rs = con.getResultSetList(sql);
		StringBuffer sb=new StringBuffer();
		sb.append("begin \n");
...
//			计算累计数据
	...
	public void getSelectData() {
		...
		String strSql = "";
		String diancxxb_id = this.getTreeid();
		strSql ="select ID,GONGYSB_ID,MEIKXXB_ID,JIHKJB_ID,PINZB_ID,YUNSFSB_ID,FENX,JIESL,JIESRL,\n" +
				"JIESMJ, ROUND(JIESMJ-JIESMJ/1.17,2) JIESMJS,KUANGQYF,JIESYJ, ROUND(JIESYJ*0.07,2) JIESYJS,JIESZF, JIESZFS, \n" +
			"    ROUND(DECODE(JIESRL,0,0,(JIESMJ + JIESYJ + JIESZF + KUANGQYF)*29.271/JIESRL),2) JIESBMDJ,\n" +
			"    ROUND(DECODE(JIESRL,0,0,(JIESMJ+JIESYJ+JIESZF+KUANGQYF-JIESMJS-JIESYJS-JIESZFS)*29.271/JIESRL),2)JIESBHSBMDJ \n" +
			"      from (SELECT --grouping(FENX)a, grouping(GYS.MINGC)b, grouping(J.MINGC)c, grouping(P.MINGC)d, grouping(YS.MINGC)e, grouping(DJ.ID)f,\n" +
			...
			"       SUM(JIESL) JIESL,\n" + 
			"       ROUND (DECODE(SUM(DECODE(JIESRL, 0, 0, JIESL)),0,0,SUM(DECODE(JIESRL, 0, 0, JIESL) * JIESRL) /SUM(DECODE(JIESRL, 0, 0, JIESL))),2) JIESRL,\n" + 
			"       ROUND (DECODE(SUM(DECODE(JIESRL, 0, 0, JIESL)),0,0,SUM(DECODE(JIESRL, 0, 0, JIESL) * JIESMJ) /SUM(DECODE(JIESRL, 0, 0, JIESL))),2) JIESMJ,\n" + 
			"       ROUND (DECODE(SUM(DECODE(JIESRL, 0, 0, JIESL)),0,0,SUM(DECODE(JIESRL, 0, 0, JIESL) * JIESMJS) /SUM(DECODE(JIESRL, 0, 0, JIESL))),2) JIESMJS,\n" + 
			"       ROUND (DECODE(SUM(DECODE(JIESRL, 0, 0, JIESL)),0,0,SUM(DECODE(JIESMJ, 0, 0, JIESL) * KUANGQYF) /SUM(DECODE(JIESRL, 0, 0, JIESL))),2) KUANGQYF,\n" + 
			"       ROUND (DECODE(SUM(DECODE(JIESMJ, 0, 0, JIESL)),0,0,SUM(DECODE(JIESMJ, 0, 0, JIESL) * JIESYJ) /SUM(DECODE(JIESMJ, 0, 0, JIESL))),2) JIESYJ,\n" + 
			...
			"       ROUND (DECODE(SUM(DECODE(JIESRL, 0, 0, JIESL)),0,0,SUM(DECODE(JIESRL, 0, 0, JIESL) * JIESBMDJ) /SUM(DECODE(JIESRL, 0, 0, JIESL))),2) JIESBMDJ,\n" + 
			"       ROUND (DECODE(SUM(DECODE(JIESRL, 0, 0, JIESL)),0,0,SUM(DECODE(JIESRL, 0, 0, JIESL) * JIESBHSBMDJ) /SUM(DECODE(JIESRL, 0, 0, JIESL))),2) JIESBHSBMDJ\n" + 
			"  FROM YUEJSDJ DJ, MEIKXXB MK, JIHKJB J, PINZB P, YUNSFSB YS, GONGYSB GYS\n" + 
			" WHERE DJ.MEIKXXB_ID = MK.ID\n" + 
			"   AND DJ.GONGYSB_ID = GYS.ID\n" + 
			...
			" GROUP BY ROLLUP(FENX, (GYS.MINGC, MK.MINGC, J.MINGC, P.MINGC, YS.MINGC, DJ.ID))\n" + 
			" HAVING GROUPING(FENX)+GROUPING(GYS.MINGC)=1\n" + 
			" ORDER BY FENX)\n union all \n"+
			...
			"   AND DJ.DIANCXXB_ID = "+diancxxb_id+"\n" + 
			"   AND DJ.RIQ = TO_DATE('"+strDate+"', 'yyyy-mm-dd')\n" + 
			" ORDER BY JIHKJB_ID DESC, GONGYSB_ID,MEIKXXB_ID, PINZB_ID, ID, FENX)";
