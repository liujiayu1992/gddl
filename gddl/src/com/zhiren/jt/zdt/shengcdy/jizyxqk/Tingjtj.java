...
public class Tingjtj extends BasePage {
//	 判断是否是集团用户
...
//		}
	...
	public String getJiexsbztreport(){
		...
		_CurrentPage=1;
		_AllPages=1;
		String strBegin=DateUtil.FormatDate(getBeginriqDate());//日期字符
		String strEnd=DateUtil.FormatDate(getEndriqDate());
		...
		if (treejib == 1) {// 选集团时刷新出所有的电厂
			...
			str = " where  (dc.fuid = "+ getTreeid() + " or dc.shangjgsid= "+this.getTreeid()+")";
		} else if (treejib == 3) {// 选电厂只刷新出该电厂
			str = " where dc.id = " + getTreeid() + "";
		}
		String sql = "select decode(grouping(dc.mingc),1,'总计',dc.mingc) as dcmingc,jz.jizbh as jizbh,sum(a.tingjyxdl) as tingjyxdl,sum(round(a.tingjsj*24,0)) as tingjsj,sum(a.cs) as tingjcs\n" +
		"from (select * from diancxxb dc " + str+ " ) dc,jizb jz,jizyxqkb qk,(select jizb_id,count(id) as cs,sum(jiesrq-kaisrq) as tingjsj,sum(tingjyxdl) as tingjyxdl from jizyxqkb where shebzt != '运行'group by jizb_id )a\n" + 
		"where dc.id = qk.diancxxb_id and jz.id = qk.jizb_id and qk.jizb_id = a.jizb_id and qk.kaisrq>=to_date('"+strBegin+"','yyyy-mm-dd') and qk.jiesrq<=to_date('"+strEnd+"','yyyy-mm-dd') group by rollup(dc.mingc,jz.jizbh) ";
		
		String ArrHeader[][]=new String[1][5];
		ArrHeader[0]=new String[] {"电厂名称","机组编号","停机影响电量","停机时间（小时）","停机次数"};

		int ArrWidth[]=new int[] {200,100,100,100,100};
		ResultSet rs = cn.getResultSet(sql);
		
		rt.setTitle("停机统计",ArrWidth);
...
//	得到电厂树下拉框的电厂名称或者分公司,集团的名称
	...
	public void getToolBars() {
		...
		df1.setValue(DateUtil.FormatDate(this.getBeginriqDate()));
		df1.Binding("BeginriqDateSelect","forms[0]");// 与html页中的id绑定,并自动刷新
		df1.setWidth(100);
		...
		df2.setValue(DateUtil.FormatDate(this.getEndriqDate()));
		df2.Binding("EndriqDateSelect","forms[0]");// 与html页中的id绑定,并自动刷新
		df2.setWidth(100);
		//df.setListeners("select:function(){document.Form0.submit();}");
		tb1.addField(df2);
