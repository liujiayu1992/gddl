...
public class Xufhy extends BasePage implements PageValidateListener {
	// 界面用户提示
	private String msg = "";
...
//		long dcid = visit.getDiancxxb_id();
		...
		String sql = "select z.id, f.id as fid, g.mingc fahr,mk.mingc meikdw,p.mingc pinz,\n" +
			"f.chec, f.ches, f.daohrq jiexrq, f.biaoz, z.mt, z.mad, z.aar,\n" + 
			"z.aad, z.ad, z.var, z.vad, z.vdaf, z.fcad, z.qgrad,\n" + 
			"z.qnet_ar, z.stad, z.std, z.qbad, z.had, z.hdaf,z.huaysj,z.huaybh,z.huayy\n" + 
			"from fahbtmp f, zhilb z, vwfahr g,vwpinz p,meikxxb mk,diancxxb dc,fayslb fy\n" + 
			"where f.kuangfzlb_id = z.id(+)\n" + 
			"      and f.shujly='手工录入' and fy.id=f.fayslb_id\n" + 
			"      and f.fayslb_id>0\n" + 
			"      and fy.xiaosjsb_id=0\n" + 
			"	   and f.gongysb_id = g.id\n" + 
			"      and f.pinzb_id = p.id and f.jiesb_id=0 and f.meikxxb_id=mk.id\n" + 
			"      and f.leix_id=(select id from yewlxb where mingc='场地交割')\n" + 
			"      and f.diancxxb_id=dc.id"+
			str+			
		    "   and f.daohrq >= " + DateUtil.FormatOracleDate(getRiq()) + " and f.daohrq < "
			+ DateUtil.FormatOracleDate(getRiqe()) + "+1 order by jiexrq desc";
...
//		ResultSetList rsl = getExtGrid().getModifyResultSet(getChange());
		...
		String sql="begin \n" ;
//		while(rsl.next()){
			sql+="update fahbtmp set kuangfzlb_id=0 where kuangfzlb_id="+change+"; \n"+
				"delete zhilb where id="+change+"; \n";
//		}
		sql+="end;";
	...
	private void save() {
		...
		while(rsl.next()){
			...
			String huaysj=DateUtil.FormatOracleDate(rsl.getString("huaysj"));
			String huaybh=rsl.getString("HUAYBH");
			String huayy=rsl.getString("HUAYY");
			String newID=MainGlobal.getNewID(visit.getDiancxxb_id());
			if(id==0){
				sql+="insert into zhilb (id,CAIYB_ID,mt, mad, aar,aad, " +
						"ad, var, vad, vdaf, fcad, qgrad, qnet_ar, stad, std, qbad, had,hdaf,LURY,huaysj,huaybh,huayy)values("+
						newID+",0,"+
						mt+","+mad+","+aar+","+aad+","+ad+","+var+","+vad+","+vdaf+","+fcad+","+
						qgrad+","+qnet+","+stad+","+std+","+qbad+","+had+","+hdaf+",'"+visit.getRenymc()+"'," +
						huaysj+",'"+huaybh+"','"+huayy+"');\n";
				sql+="update fahbtmp set kuangfzlb_id="+newID+" where id="+fid+";\n";
				
			}else{
				sql += "update zhilb set mt = " +mt + 
				", mad = " + mad + 		", aar = " + aar +
				", aad = " + aad +		", ad = " + ad +
				", var = " + var +		", vad = " + vad + 
				", vdaf = " + vdaf + 	", fcad = " + fcad +
				...
				", stad = " + stad +	", std = " + std +
				", qbad = " + qbad +	", had = " + had +
				", hdaf = " + hdaf +	", huaysj="+huaysj+
				", huaybh='"+huaybh+	"',huayy='"+huayy+
