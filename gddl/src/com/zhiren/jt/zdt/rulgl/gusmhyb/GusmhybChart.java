...
//	 判断是否是集团用户
	...
	private String getTitle(){
		...
		title=title+DateUtil.Formatdate("yyyy年MM月",getEndriqDate());
		return title;
	}
	...
	public String getReportHrefYuemhgsb(){
		...
		return "<a href=\"#\" onclick=printReport(\""+strParam+"\") ><b>月煤耗估算表</b>";
	}
	...
	public String getReportHrefShangbmhyb(){
		...
		return "<a href=\"#\" onclick=printReport(\""+strParam+"\") ><b>上报煤耗月报</b>";
	}
	...
	public String getReportHrefMeihdb(){
		...
		return "<a href=\"#\" onclick=printReport(\""+strParam+"\") ><b>煤耗对比</b>";
	}
...
//		sbsql.append("             group by hy.diancxxb_id, to_char(hy.rulrq,'yyyy-MM') ) hy,  \n");
		...
		sbsql.append("       (select sc.diancxxb_id,to_char(riq,'yyyy-MM') riq,sum(fadl) as fadl,sum(gongdl) as gongdl,sum(gongrl) as gongrl--本期生产数据  \n");
		...
		sbsql.append("union \n");
		sbsql.append("select add_months( riq,12) as riq, decode(1,1,'同期','同期') as fenx,fady,gongryy,fardl,gongdl,gongrl,rulrl,rulrly, \n");
		sbsql.append("    round(decode(fardl,0,0,1000000*(round(fady*rulrl/29.271,0)+round(fadyy*rulrly/29.271,0))/fardl),0) as fadbzmh,   \n");
		sbsql.append("    round(decode(gongdl,0,0,1000000*(round(fady*rulrl/29.271,0)+round(fadyy*rulrly/29.271,0))/gongdl),0) as gongdbzmh,   \n");
		sbsql.append("    round(decode(gongrl,0,0,1000*(round(gongry*rulrl/29.271,0)+round(gongryy*rulrly/29.271,0))/gongrl),0) as gongrbzmh  \n");
		sbsql.append(" from  \n");
...
//	柱型图
	public String getChartBar(){
		String strCondition ="";
		String strDateStart=DateUtil.FormatDate(DateUtil.getFirstDayOfMonth(getEndriqDate()));//起始日期
		String strDateEnd=DateUtil.FormatDate(getEndriqDate());//截止日期
		
		String strDateStartP=DateUtil.FormatDate(DateUtil.AddDate(DateUtil.getFirstDayOfMonth(getEndriqDate()),-1,DateUtil.AddType_intYear));//起始日期同期
		String strDateEndP=DateUtil.FormatDate(DateUtil.AddDate(getEndriqDate(),-1,DateUtil.AddType_intYear));//起始日期同期
