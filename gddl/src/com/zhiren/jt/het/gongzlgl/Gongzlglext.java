...
public class Gongzlglext extends BasePage {
//点击行索引
...
//	  ************************流程状态***********
	    public void getSelectDataLiuczt() {
			...
			// //设置表名称用于保存
			egulczt.setHeight("bodyHeight-85");
			egulczt.frame = false;
			...
			egulczt.getColumn("leibztb_id").setComboEditor(egulczt.gridId,
					new IDropDownModel("select leibztb.id,leibztb.mingc\n" +
									" from leibztb,liucb\n" + 
									"where leibztb.liuclbb_id=liucb.liuclbb_id and  liucb.id=" +getLiucSelectValue().getId() +
									" union\n" + 
...
//		***********************流程角色************
	    public void getSelectDataLiucjs() {
			...
			// //设置表名称用于保存
			egulcjs.setHeight("bodyHeight-85");
			egulcjs.frame = false;
...
//		  ************************流程动作***********
	    public void getSelectDataLiucdz() {
			...
			ResultSetList rsl = con
					.getResultSetList(
							"select lcdz.id as id ,lcdzjs.id as liucdzjsb_id,lcjs.mingc as mingc,leibztb1.mingc as liucztqqid,leibztb2.mingc as liuczthjid,lcdz.mingc as caozmc,lcdz.dongz as dongz \n" +
			...
			// //设置表名称用于保存
			egulcdz.setHeight("bodyHeight-85");
			egulcdz.frame = false;
			...
			egulcdz.getColumn("dongz").setHeader("动作");
//			// //设置列宽度
			...
			while (rsl.next()) {
				...
				if ("0".equals(rsl.getString("ID"))) {
					liucdzb_id =  Long.parseLong(MainGlobal.getNewID(((Visit) getPage().getVisit()).getDiancxxb_id()));
					  sql.append("insert into liucdzb(id,liucztqqid,liuczthjid,mingc,dongz)values("
					  			+ liucdzb_id
					  			...
					  			+"'),(select id from (select liucztb.id,leibztb.mingc as mingc from liucztb,leibztb"
					  			+" where liucztb.leibztb_id=leibztb.id and liucztb.liucb_id="+getLiucdzSelectValue().getId()
					  			+")b where b.mingc='"+rsl.getString("liuczthjid")+"'),'"+rsl.getString("caozmc")+"','"+rsl.getString("dongz")
					  			+"');\n");
                ...
                +"'),mingc='"+rsl.getString("caozmc")+"',dongz='"+rsl.getString("dongz")
