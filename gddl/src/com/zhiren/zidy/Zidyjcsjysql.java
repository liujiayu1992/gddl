...
public abstract class Zidyjcsjysql extends BasePage {
//	进行页面提示信息的设置
	private String msg="";
...
//	页面初始化
	...
	//刷新
	 private void RefurbishChick() {
		 ...
		 sb.append("select * from zidyjcsjysql where zidyjcsjy_id = ")
...
//	确认
	private void Save() {
		...
		StringBuffer sb = new StringBuffer();
		sb.append("begin\n");
		sb.append("delete from zidyjcsjysql where zidyjcsjy_id = "+v.getLong1() + ";\n");
		for(int i = 0; i< gongs.length; i++){
			sb.append("insert into ZIDYJCSJYSQL(id,zidyjcsjy_id,xuh,z_query) values(")
			.append("getnewId(").append(v.getDiancxxb_id()).append("),").append(v.getLong1())
			.append(",").append(i+1).append(",'").append(gongs[i]).append("');");
		}
		sb.append("end;");
		JDBCcon con = new JDBCcon();
		//con.getDelete("delete from zidyjcsjysql where zidyjcsjy_id = "+v.getLong1());
		//if(getGongs().length()>0){
