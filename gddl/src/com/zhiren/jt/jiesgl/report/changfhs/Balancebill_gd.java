 ...
 * 作者：夏峥
 * 时间：2012-12-08
 * 描述：复制/gddl/src/com/zhiren/jt/jiesgl/report/changfhs/Balancebill.java		1.1.1.5 
 * 		并取消报表类型下拉框
 * 		重写供应商下拉框列表
 */
/*
 * 作者：夏峥
 * 时间：2012-12-17
 * 描述：修正条件设置弹出窗口刷新错误的问题，并将弹出窗口中关闭按钮的文字变更为取消。
 */
public class Balancebill_gd extends BasePage {
...
//				分厂别
	...
	public IPropertySelectionModel getIGongysModels() {
		...
		if (((Visit) getPage().getVisit()).isDCUser()) {
			...
			sql ="SELECT DISTINCT GONGYSB.ID, GONGYSB.MINGC\n" +
				"  FROM GONGYSB,\n" + 
				"       DIANCXXB,\n" + 
				"       ((SELECT DIANCXXB_ID, GONGYSB_ID\n" + 
				"           FROM JIESB\n" + 
				"          WHERE JIESRQ >= "+this.OraDate(this.getBeginriqDate())+"\n" + 
				"            AND JIESRQ <= "+this.OraDate(this.getEndriqDate())+")\n" + 
				"        UNION\n" + 
				"        (SELECT DIANCXXB_ID, GONGYSB_ID\n" + 
				"           FROM JIESYFB\n" + 
				...
				"   AND GONGYSB.LEIX = 1\n" + 
				"   AND GONGYSB.FUID <> 0\n" + 
				"   AND DIANCXXB.ID = A.DIANCXXB_ID\n" + 
				"   AND DIANCXXB_ID IN (SELECT ID FROM DIANCXXB WHERE FUID = "+diancxxb_id+" OR ID = "+diancxxb_id+")\n" + 
				" ORDER BY MINGC";
