...
import com.zhiren.main.Visit;
	...
	public class Fuktzdsh extends BasePage {
		private String msg = "";
		...
		public void Tij(){
			...
			if(!(this.getChange().equals("")||this.getChange()==null)){
				...
				for(int i=0;i<change.length;i++){
					...
					String record[] = change[i].split(",");		
					try {
						if(BalanceLiuc.isLastLeader(renyxxb_id, "付款单审核")){
		...
		public void getSelectData() {
			...
			String leib = "预付款";
			long renyxxb_id = ((Visit) getPage().getVisit()).getRenyID();
			String liucb_mingc="付款单审核";
			if (getWeizSelectValue().getId() ==1) {// 自己的任务
				
				String strWodrwIds;
				long liucztb_id=-1;
				try {
			     liucztb_id = BalanceLiuc.getcaozsjzt(renyxxb_id, liucb_mingc);
				} catch (Exception e) {
					// TODO 自动生成 catch 块
					e.printStackTrace();
				}
				sql="select * from ("
					+ " select fk.id,fk.fukdbh,decode(dc.mingc,null,'大唐燃料公司',dc.mingc) as dianmc,fk.fukdlx,fk.riq,gy.quanc,gy.bianm,to_char(fk.fapje,'FM9,999,999,990.09') as fapje,to_char(fk.shijfk,'FM9,999,999,990.09') as shijfk,to_char(fk.hexyfk,'FM9,999,999,990.09') as hexyfk,to_char(fk.kouyf,'FM9,999,999,990.09') as kouyf,to_char(fk.kouhkf,'FM9,999,999,990.09') as kouhkf, "
...
//					+ "         and yf.fuktzb_id in ("+strWodrwIds+")*/" +
						+	") dc \n"
					
					+" where fk.liucztb_id=zt.id and fk.gongysb_id=gy.id " 
					+"   and zt.leibztb_id=lz.id and lz.liuclbb_id=ll.id "
					+"   and fk.id=dc.fuktzb_id(+)  and fk.liucztb_id in (" +liucztb_id+ ") and fk.fukdlx='实际付款' order by fukdbh ) ";
			}else {
				...
				sql="select * from (("
					...
					+" where fk.liucztb_id=zt.id and fk.gongysb_id=gy.id " 
					+"   and zt.leibztb_id=lz.id and lz.liuclbb_id=ll.id "
					+"   and fk.id=dc.fuktzb_id(+) and fk.id in (" +strLiuczIds+ ") and fk.fukdlx='实际付款')  order by fukdbh )  ";
			}
...
//			egu.setTableName("yunslb");//要保存的表名
			...
			egu.getColumn("dianmc").setHeader("单位名称");
...
//			}
			...
			if(((Visit) getPage().getVisit()).getboolean4()){
				...
				egu.addTbarText("-");
				String biaoti="";
				if(BalanceLiuc.isLastLeader(renyxxb_id, liucb_mingc))
				{
					biaoti="审核通过";
				}else{
					biaoti="审核";
