...
function Count(){
	...
	var fukje = Round(eval(vfapje)-eval(vkouyf)-eval(vkouhkf),2);
	if(eval(vhexyfk)>=eval(fukje)){
		document.all.item("HEXYFK").value=fukje;
	}
	vshijfk = Round(eval(fukje)-eval(vhexyfk),2);
...
//	document.all.item("SHIJFK").value = vshijfk;
	...
	document.all.item("SHIJFKDX").value=tormb(eval(document.all.item("SHIJFK").value));
	Fenjsz();
}

   function getjsd(){    
      if(document.getElementById('EditTableRow').value<=0){
                     alert('请先保存');return;
      }
      	var jiesdbianm= document.getElementById('FUKTZDBH').value;  
     	var url = "http://"+document.location.host+document.location.pathname;
     		url = url + "?service=page/Fuktzdcx_dtrl&fukdbh="+jiesdbianm;
        window.open(url);
      }