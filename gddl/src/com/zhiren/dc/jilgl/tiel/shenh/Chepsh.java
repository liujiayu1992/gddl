...
public class Chepsh extends BasePage implements PageValidateListener {
	// 界面用户提示
	private String msg = "";
	...
	private void Save() {
		...
		while (rsld.next()) {
			...
			if (r.next()) {
				...
				double yunsl = Jilcz.getYunsl(diancxxb_id, pinzb_id, SysConstant.YUNSFS_HUOY,
						meikxxb_id);
				sb.delete(0, sb.length());
	...
	public void getSelectData() {
		...
		sqlbf
				.append("select c.id,f.id fahbid,f.diancxxb_id,f.zhilb_id, g.mingc as gongysb_id, m.mingc as meikxxb_id,\n");
		sqlbf
				.append("(select mingc from chezxxb fc where fc.id = f.faz_id) faz_id, \n");
		sqlbf
				.append("p.mingc as pinzb_id, fahrq, c.cheph, c.maoz, c.piz, c.yuns, c.biaoz, c.zongkd, j.mingc as jihkjb_id, \n");
		...
		sqlbf
				.append("from chepb c,fahb f ,gongysb g,meikxxb m,pinzb p,jihkjb j,chebb cb,vwyuanshdw d\n");
		...
		sqlbf.append("and f.yuanshdwb_id=d.id and f.yunsfsb_id = 1\n");

		ResultSetList rsl = con.getResultSetList(sqlbf.toString());
		if (rsl == null) {
			WriteLog
					.writeErrorLog(ErrorMessage.NullResult + "引发错误SQL:" + sqlbf);
			setMsg(ErrorMessage.NullResult);
			return;
		}
