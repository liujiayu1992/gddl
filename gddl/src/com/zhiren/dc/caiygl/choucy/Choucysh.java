...
public class Choucysh extends BasePage implements PageValidateListener {
	private String CustomSetKey = "Choucysh";
	private String msg = "";
...
//抽查样审核,把审核状态从3改到5
		...
		while (mdrsl.next()) {
			...
			sql.append("shenhzt").append(" = 5 ,shenhry='"+visit.getRenymc()+"' where id =");
	...
	public void Huit() {
		...
		while (mdrsl.next()) {
			...
			sql.append("update ").append(tableName).append(" set ");
			sql.append("shenhzt").append(" = 0 , shenhry='' ");
	...
	public void getSelectData() {
		...
		sql=
			...
			"select k.id, k.huaybh,k.huaysj,formatxiaosws(k.mt,1) as mt,\n" +
...
//		 设置页面宽度以便增加滚动条
		...
		// /设置显示列名称
		 ...
		 egu.getColumn("huaybh").setHeader("抽查样编号");
