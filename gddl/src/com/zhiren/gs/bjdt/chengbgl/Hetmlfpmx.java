...
public  class Hetmlfpmx extends BasePage implements PageValidateListener {
	private String msg = "";
	...
	public void Save() {
		...
		while(mdrsl.next()) {
			...
			if("0".equals(mdrsl.getString("ID"))) {
				...
				sql.append("insert into ").append(tableName).append("(id,riq,diancxxb_id,gongysb_id,jihkjb_id,hetl,caightb_id,beiz)" );
				sql.append("values (getnewid("+visit.getDiancxxb_id()+"),");
				sql.append("to_date((select to_char(riq,'yyyy') from caightb where id ="+visit.getString1()+"),'yyyy'),");
				sql.append((getExtGrid().getColumn("diancxxb_id").combo).getBeanId(mdrsl.getString("diancxxb_id")));
				sql.append(",").append("(select gongysb_id from caightb where id="+visit.getString1()+")");
...
//			return value==null||"".equals(value)?"null":value;
	...
	public void getSelectData() {
		...
		String chaxun = 
			"select c.id, '剩余煤量'as diancxxb_id,(c.hetl-nvl((select sum(hetl)  from caightmxb where caightb_id="+visit.getString1() +
			"),0)) hetl,beiz from caightb c\n" + 
			"where id=" + visit.getString1()+"\n"+
			"union\n" + 
			"select cm.id,d.mingc ,cm.hetl,cm.beiz from caightmxb cm,diancxxb d where d.id=cm.diancxxb_id and" +
			" cm.caightb_id="+visit.getString1()+"\n" ;
...
//	 得到登陆用户所在电厂或者分公司的名称
	...
	public String getHetl() {
		...
		String sql_hetl = "select id,hetl from caightb where id="
				+ hetlid;
