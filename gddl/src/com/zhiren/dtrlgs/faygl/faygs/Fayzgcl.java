		...
		if(LstRuccbb_id.size()>0){
			...
			for(int i=0;i<LstRuccbb_id.size();i++){
				...
				if(!String.valueOf(LstRuccbb_id.get(i)).equals("0")&&!String.valueOf(LstRuccbb_id.get(i)).equals("null")){
					...
					sql="select id,nvl(hetb_id,0) as hetb_id,nvl(gongysb_id,0) as gongysb_id,nvl(meikxxb_id,0) as meikxxb_id,nvl(faz_id,0) as faz_id from fayslb where ruccbb_id in ("+String.valueOf(LstRuccbb_id.get(i))+")";
...
//				得到发货的基础信息
				sql="select hetb_id,diancxxb_id,gongysb_id from fayslb where id="+Fahb_id.get(i);
				rsl=con.getResultSetList(sql);
				if(rsl.next()){
...
//					先检查该发货记录有无质量信息
					sql=" select z.* from fayslb f,zhilb z \n"
					...
					if(rsl.getRows()>0){
//						存在质量信息
						sql="select f.* from fayslb f,faygslsb gs where f.id=gs.fayslb_id \n"
							+ " and gs.leix=2 and f.id="+Fahb_id.get(i); 
						
						rsl=con.getResultSetList(sql);
						if(rsl.next()){
//							有类型为2的记录
							sql="select * from fayslb where XIAOSJSB_ID>0 and id="+Fahb_id.get(i);
							rsl=con.getResultSetList(sql);
							if(rsl.next()){
//								有结算表id
								sql="select * from fayslb f,faygslsb gs where f.id=gs.fayslb_id \n"
									+ " and gs.leix=4 and f.id="+Fahb_id.get(i);
								rsl=con.getResultSetList(sql);
								if(rsl.next()){
//									有leix=4的记录
									if(Flag){
...
//						不存在质量信息
						sql="select gs.* from fayslb f,faygslsb gs\n" +
...
//		首次计算暂估单价（没有质量计算暂估）
		...
		FayzgBalances bls=new FayzgBalances();
		Balances_variable bsv=new Balances_variable();
		
		try{
			
			sql="select jiesrl from (select decode(nvl(zb.jies,0),0,j.jiesrl,zb.jies) as jiesrl from diancjsmkb j,"
				+"					(select * from jieszbsjb zb where zb.zhibb_id=2) zb \n" +
				"       where j.gongysb_id="+Gongysb_id+" and j.ruzrq is not null and j.id=zb.jiesdid(+) order by ruzrq desc ) \n" + 
				"       where  rownum=1 \n" ;
			rsl=con.getResultSetList(sql);
			if(rsl.next()){
	//			从最近一次结算中取结算热量
...
//				从合同质量要求中取热量要求
				sql="select zl.xiax from hetb h,hetzlb zl\n" +
					...
					"                          and zl.zhibb_id=2\n" + 
					"                          and h.id=\n" + 
					"             (select f.hetb_id from fayslb f where id="+Fahb_id+")\n" + 
					"             order by xiax";
				rsl=con.getResultSetList(sql);
				if(rsl.next()){
...
//		首次计算暂估单价（没有质量计算暂估）
		...
		try{
			...
			sql="select distinct XIAOSJSB_ID from fayslb f where f.id="+Fahb_id;
			rsl=con.getResultSetList(sql);
			if(rsl.next()){
//				得到煤款结算id
				jiesb_id=rsl.getLong("XIAOSJSB_ID");
			}
...
//				}
				...
				if(!riq.equals("")){
					...
					sb.append("insert into faygslsb\n" +
...
//				两票结算
				...
				if(Jiesb_id>0){
					...
					sql=" select round_new(decode(jiessl,0,0,hansmk/jiessl),2) as hansdj,	\n"
						...
						+ " jiesrcrl \n" 
						+ " from diancjsmkb where id="+Jiesb_id;
				...
				if(Yunfjsb_id>0){
					...
					sql=" select round_new(hansyf/jiessl,2) as yunj," +
							...
							" from diancjsyfb where id="+Yunfjsb_id;
	...
	private static String getRuccbb_id(long jiesb_id, long yunfjsb_id) {
		...
		try{
			...
			if(jiesb_id>0){
				...
				sql="select nvl(ruccbb_id,0) as ruccbb_id from fayslb where XIAOSJSB_ID="+jiesb_id;
				rs=con.getResultSet(sql);
				while(rs.next()){
			...
			/*if(yunfjsb_id>0){
				...
				sql="select ruccbb_id	\n" 
					+ "	from fayslb f,chepb cp,danjcpb dj		\n"
...
//				 如果等于0从上次结算单中取出价格
		...
		if(bsv.getHetyj()==0){
			...
			sql="select max(id) as id from diancjsyfb where gongysmc='"+bsv.getFahdw()+"' and faz='"+bsv.getFaz()+"'";
			rsl=con.getResultSetList(sql);
			if(rsl.next()){
			...
			if(lngJiesyfb_id>0){
				...
				sql="select * from diancjsyfb where id="+lngJiesyfb_id;
				rsl=con.getResultSetList(sql);
				if(rsl.next()){
...
//		矿区地铁运费
		...
		try{
			...
			String sql="select * from diancjsyfb where gongysmc='"+Gongys+"' and faz='"+Faz+"'";
			ResultSetList rsl=con.getResultSetList(sql);
			if(rsl.next()){
