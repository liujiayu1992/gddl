 ...
 * 描述：取消备注的显示和保存功能。
 */
public class Shujshcp_hd extends BasePage implements PageValidateListener {
	// 界面用户提示
	private String msg = "";
...
//			jingz_cons = " (c.maoz-c.piz-c.koud-c.kous-c.kouz)";
		...
		sb
				.append(
						"select c.id,f.id fahbid,f.diancxxb_id,f.zhilb_id, g.mingc as gongysb_id, m.mingc as meikxxb_id,\n")
				.append(
						"f.yunsfsb_id,(select mingc from chezxxb fc where fc.id = f.faz_id) faz_id, \n")
				.append(
						"p.mingc as pinzb_id, mc.mingc as meicb_id, fahrq,to_char(c.zhongcsj,'hh24:mi:ss') as zhongcsj, c.cheph, c.maoz, c.piz, c.yuns, c.biaoz, c.sanfsl, c.zongkd,"+ kouz 
								+ jingz_cons
								+ " as jingz, "
								+ ches
				...
				.append(
						"d.mingc as yuanshdwb_id,yuanmkdw,daozch,c.lury lury,f.chex"
								+ "\n")
				.append(
						"from chepb c,fahb f ,meicb mc,gongysb g,meikxxb m,pinzb p,jihkjb j,chebb cb,vwyuanshdw d\n")
				.append("where f.id in (")
				.append(getFahids())
				.append(")\n")
				.append(
						"and c.fahb_id=f.id and f.gongysb_id=g.id(+) and f.meikxxb_id=m.id(+) \n")
				.append(
		...
		if (flag) {// 增加合计行
			hj
					.append(
							" select -1 id,-1 fahbid,-1 diancxb_id,-1 zhilb_id,'合计' gongysb_id,null meikxxb_id,")
					.append(
							" -1 yunsfsb_id,null faz_id,null pinzb_id, null meicb_id, null fahrq,null zhongcsj,'' cheph, sum(maoz) maoz, sum(piz) piz,")
					.append(
							" sum(yuns) yuns,sum(biaoz) biaoz,sum(sanfsl) sanfsl,sum(zongkd) zongd,"+ kouzsum+" sum(jingz) jingz,sum(ches) ches, null jihkjb_id,")
					.append(
...
//				if(in.next()){//添加  删除按钮显示
					...
					String check_condi1=" ";
					if(check_bool){
						check_condi1=" if(e.field!='MAOZ' && e.field!='PIZ' && e.field!='YUNS' && e.field!='BIAOZ' && e.field!='ZONGKD' && e.field!='JINGZ' && e.field!='CHES' && e.field!='CHEPH' ){ \n " +
