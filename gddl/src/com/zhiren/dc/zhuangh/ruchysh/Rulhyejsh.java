 ...
 * 	      回退时添加了日志记录
 */
public class Rulhyejsh extends BasePage implements PageValidateListener {
	...
	public void Save1(String strchange, Visit visit) {
		...
		while (mdrsl.next()) {
			...
			if(huaylb.equals("入炉")){//入炉一次审核
				String ru_sql=" select * from rulmzlzb where id="+zhillsb_id;
				ResultSetList ru_rsl=con.getResultSetList(ru_sql);
				while (ru_rsl.next()){
					String zhiyry=
						"select rb.id,rb.mingc from rulmzlzb rzb ,yangpdhb yb,zhiyryglb zb,renyxxb rb\n" +
					...
					String ruf = ru_rsl.getString("rulmzlb_id") + ",to_date('"+ru_rsl.getDateTimeString("rulrq")+"', 'YYYY-MM-DD hh24:mi:ss'),to_date('"+ru_rsl.getDateTimeString("fenxrq")+"', 'YYYY-MM-DD hh24:mi:ss'),"+ru_rsl.getString("diancxxb_id")+","//1
							+ ru_rsl.getString("rulbzb_id") + ","+jiz+","+ru_rsl.getString("qnet_ar")+","+ru_rsl.getString("aar")+","//2
							...
							+ru_rsl.getString("beiz")+"','"+ru_rsl.getString("lury")+"',to_date('"+ru_rsl.getDateTimeString("lursj")+"', 'YYYY-MM-DD hh24:mi:ss'),"+"7,'"//7
							+ru_rsl.getString("bianm")+"',"+ru_rsl.getString("har")+","+ru_rsl.getString("qgrd")+","+ru_rsl.getString("qgrad_daf")+","//8
							+ru_rsl.getString("meil")+",'"+zhiyy+"','"+visit.getRenymc()+"',"+"to_date(to_char(sysdate, 'yyyy-mm-dd hh24:mi:ss'),'yyyy-mm-dd hh24:mi:ss')"+",'"//9
...
//		)
		...
		ResultSetList rs = con.getResultSetList("select zhi from xitxxb where mingc = '化验二审回退日志' and diancxxb_id = "
				+ visit.getDiancxxb_id());
		while (rs.next()) {
