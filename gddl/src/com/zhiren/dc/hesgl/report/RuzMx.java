 ...
 * 描述：按入账日期查询到货明细 。
 */
public class RuzMx extends BasePage {
//	 判断是否是集团用户
...
//		}
	...
	public String getCaiggsb(){
		...
		sql="select decode(gongys,null,'合计',gongys) as gongys,\n"
			...
			+ " decode(sum(gusl),0,0,round(sum(gusl*stad)/sum(gusl),2)) as stad\n"
			+ " from(\n"
			+ " select g.mingc as gongys,m.mingc as meikdw,f.daohrq,f.chec,sum(round_new(maoz-piz,0)) as gusl,sum(maoz-piz) as jingz,sum(biaoz) as biaoz,sum(round(f.yuns,2)) as yuns,sum(round(f.yingk,2)) as yingk,\n"
			+ " nvl(round(decode(sum(maoz-piz),0,0,round_new(sum((maoz-piz)*qnet_ar)/sum(maoz-piz),2))/0.0041816,0),0) as qnetar ,nvl(decode(sum(maoz-piz),0,0,round_new(sum((maoz-piz)*stad)/sum(maoz-piz),2)),0) as stad \n"
			+ " from fahb f,zhilb z,meikxxb m,gongysb g,jiesb j\n"
			+ " where f.zhilb_id=z.id(+) and f.jiesb_id=j.id(+)\n"
			+ " and  ruzrq>=to_date('"+getRiq()+"','yyyy-mm-dd')\n"
			+ " and ruzrq<=to_date('"+getRiqEnd()+"','yyyy-mm-dd')\n"
			+ " and f.meikxxb_id=m.id and m.meikdq_id=g.id\n"
			+ " group by f.id,g.mingc,m.mingc,f.daohrq,f.chec\n"
			+ " )";
		
		sql=sql+" group by rollup(gongys,meikdw,daohrq,chec)\n"
			+ " having not(grouping(chec)=1 and grouping(daohrq)<>1)";

		ArrHeader=new String[1][15];
		ArrHeader[0]=new String[] {"煤矿地区","煤矿单位","到货日期","车次","估收量<br>(吨)","净重<br>(吨)","票重<br>(吨)","运损<br>(吨)","盈亏<br>(吨)","入厂发热量(Kcal/Kg)","入厂硫分<br>(%)"};
		ArrWidth=new int[] {120,80,80,50,80,80,80,75,75,75,75};
		i=3;
		ResultSet rs = cn.getResultSet(sql);
		
		rt.setTitle("入厂统计表",ArrWidth);
		rt.setDefaultTitle(1, 3, "制表单位:"+getDiancmc(), Table.ALIGN_LEFT);
		rt.setDefaultTitle(4, 5, "入账日期:"+this.getRiq()+"至"+this.getRiqEnd(), Table.ALIGN_CENTER);
