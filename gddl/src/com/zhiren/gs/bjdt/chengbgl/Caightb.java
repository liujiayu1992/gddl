...
public class Caightb extends BasePage implements PageValidateListener {
	private String msg = "";
	...
	public void Save() {
		...
		while (delrsl.next()) {
			...
			sql.append("delete from ").append("caightmxb").append(" where id =")
			.append(delrsl.getString(0)).append(";\n");
		}
		...
		while (mdrsl.next()) {
			...
			if ("0".equals(mdrsl.getString("ID"))) {
				...
				sql.append(",").append(getJihkjValue().getId()).append(",'")
				...
				sql.append(",").append("gongysb_id=").append(
						(getExtGrid().getColumn("gongysb_id").combo)
								.getBeanId(mdrsl.getString("gongysb_id")));
				// sql.append(",").append("jihkjb_id=").append(getJihkjValue().getId());
	...
	public void CreateData() {
        ...
        String zaf="";
        String qitfy="";
		String gyscount = "select cm.id,cm.jihkjb_id,cm.diancxxb_id,cm.gongysb_id from caightmxb cm"
		...
		sb.append("delete from niancghtb where riq=to_date("+getNianfValue().getValue()
				+ ",'yyyy')");
		...
		while (rs.next()) {
			...
			sb.append("select distinct\n"
							...
							+ "\n"
							+ "  from caightb c, caightmxb cm, diancxxb d, gongysb g\n"
...
//							+"   ("+getJiezrq()+"-"+getKaisrq()+"*12+"+getYuefs()+"-"+getYuef()+"+1)),2) qitfy"
			...
			while (rsl.next()) {
				...
				sb.delete(0, sb.length());
				sb.append("insert into NIANCGHTB(id,riq,diancxxb_id,gongysb_id,jihkjb_id,dinghl,daocrz,chukjg,yunj,zaf,qitfy) values(\n")
...
//				+ "	\t})}";
		...
		String chaxun = "select n.id,dc.mingc as diancxxb_id,g.mingc as gongysb_id,n.jihkjb_id,n.dinghl,n.yujdhl,"
				+ "n.caigl,round((n.daocrz),2) daocrz,n.chukjg,n.yunj,n.zaf,n.qitfy,n.daoczhjhs,n.daoczhjbhs,n.buhsbmdj,to_char(n.riq,'yyyy') riq"
				+ " from NIANCGHTB n,diancxxb dc,gongysb g where n.diancxxb_id=dc.id and "
				+ "n.gongysb_id=g.id "
				+ strdiancTreeID
				+ " "
				+ jihkj
