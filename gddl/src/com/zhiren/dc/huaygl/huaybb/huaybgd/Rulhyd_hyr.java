 ...
 * 		 把报表的格式设定为A4打印形式。
 */
public class Rulhyd_hyr extends BasePage {
...
//				StringBuffer buffer = new StringBuffer();
		...
		rt.title.setCellValue(1, 1, getImgServletPath("dt_hlj_hyr.gif"));//设置入炉化验单上显示电厂标志
		...
		rt.setDefautlFooter(4, 1, "审核：", Table.ALIGN_CENTER);
		rt.setDefautlFooter(5, 2, "化验员：", Table.ALIGN_CENTER);
...
//	电厂名称
	...
	public IDropDownBean getDiancmcValue() {
		if(((Visit)getPage().getVisit()).getDropDownBean4()==null){
			((Visit)getPage().getVisit()).setDropDownBean4((IDropDownBean)getDiancmcModel().getOption(0));
		}
...
//	得到电厂树下拉框的电厂名称或者分公司,集团的名称
	...
	private void setBianmModels() {
		...
		sb.append("select id,mingc from rulbzb where diancxxb_id="
				+ diancxxb_id);
