...
public class Shoumfptx extends BasePage {
...
//	    con.commit();
	    ...
	    try{
	        if(getEditValues()!= null && !getEditValues().isEmpty()&& !((Dcbalancebean)getEditValues().get(0)).getJiesbh().equals("")){
	        	if(Jiesdcz.checkbh("kuangfjsmkb","kuangfjsyfb",((Dcbalancebean)getEditValues().get(0)).getJiesbh(),((Dcbalancebean)getEditValues().get(0)).getId(),((Dcbalancebean)getEditValues().get(0)).getYid())){
...
//			结算时要判断有无这个id，如果有就一定要用这个id
			...
			String sql="select * from kuangfjsmkb where bianm='"+fP_bm+"'";
			ResultSet rs=con.getResultSet(sql);
			while(rs.next()){
				
				mid = rs.getLong("id");
				mtianzdw =rs.getString("xianshr");
				mkuangfjsmkbh = rs.getString("bianm");
				mdiancjsbs=mkuangfjsmkbh.substring(0,mkuangfjsmkbh.indexOf("-")+1);
...
//				mkuidjzf_je=rs.getDouble("kuidjfzf");
				...
				sql="select jieszbsjb.*,zhibb.bianm from jieszbsjb,kuangfjsmkb,zhibb "
					 + " where jieszbsjb.jiesdid=kuangfjsmkb.id and zhibb.id=jieszbsjb.zhibb_id and zhibb.leib=1"
					 + " and kuangfjsmkb.bianm='"+fP_bm+"' and jieszbsjb.zhuangt=1 order by kuangfjsmkb.id";
...
//		 4, 地铁运费
	...
	//							两票		运费
				sql="select * from kuangfjsyfb where bianm='"+fP_bm+"'";
...
//				说明存在超亏吨
			...
			_editvalues.add(new Dcbalancebean(mid, myid, mtianzdw, mkuangfjsmkbh,
					mfahdw, mmeikdw,mfaz, 
//					myunsfsb_id,
...
//		保存
	...
	private void findBianM(String id){
		...
		JDBCcon con = new JDBCcon();
		String sql="select bianm from kuangfjsmkb where id="+id;
...
//		String[] arr=Liucdzcl.parseIdStr("19915767");
