...
function colsCount(){
	    ...
	    //热量盈亏
	    if((document.all.item("HETRL").value>0||document.all.item("HETRL").value.length>4)&&eval(document.all.item("JIESRL").value)>0){
	    	
	    	if(document.all.item("HETRL").value.length==4){
	    		
	    		document.all.item("YINGKRL").value=Math.round(eval(document.all.item("JIESRL").value)-eval(document.all.item("HETRL").value));
	    	
		    	if(eval(document.all.item("JIESRL").value)<eval(document.all.item("HETRL").value)){
	    		...
	    		var hetrlxx=eval(document.all.item("HETRL").value.substring(0,4));
	    		var hetrlsx=eval(document.all.item("HETRL").value.substring(5));
	    		
	    		if(eval(document.all.item("JIESRL").value)-hetrlsx>=0){
	    		
	    			document.all.item("YINGKRL").value=Math.round(eval(document.all.item("YANSRL").value)-hetrlsx);
	    		}else{
	    		
	    			document.all.item("YINGKRL").value=Math.round(eval(document.all.item("YANSRL").value)-hetrlxx);
	    		}
	    ...
	    //硫盈亏
	    if(eval(document.all.item("HETLF").value)>0&&eval(document.all.item("JIESLF").value)>0){
	    	document.all.item("YINGKLF").value=Math.round((eval(document.all.item("JIESLF").value)-eval(document.all.item("HETLF").value))*100)/100;
...
body.onload =function(){
     ...
     if(document.all.item("Tianzdw").value==""){
...
function check(){
	...
	if(document.all.item("SHOUKDW").value==""||document.all.item("SHOUKDW").value=="请选择"){
		...
		*/
		if(document.getElementById('DIANCJSBS').value==""){
...
function checkMKJSDBH(){
	...
	if(_obj.options(_obj.selectedIndex).text=="运费结算"&&document.all.item("MEIKJSDBH").value==""){
...
function show(_obj){
	if(_obj.options(_obj.selectedIndex).text=="运费结算"){
	
		document.all.item("MKJSDBH").style.display="";
	}else{
	
		document.all.item("MKJSDBH").style.display="none";
	}
}
function Pingb(){
...
function checkSubmit(){
	...
	if(document.all.item("Zhuangt").value==2){
		...
		if(cbo_LiucmcDropDown.getRawValue()!="请选择"&&cbo_LiucmcDropDown.getRawValue()!=""){
		...
		if(cbo_HetbhDropDown.getRawValue()!="请选择"&&cbo_HetbhDropDown.getRawValue()!=""){
...
function checkJiesbh(){
	...
	if(cbo_JIESBHDropDown.getRawValue()=="请选择"){
