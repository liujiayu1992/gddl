 ...
 */
public class Nianfjjhcx  extends BasePage implements PageValidateListener {
...
//******************页面初始设置********************//
	...
	private String getSelectData(){
		...
		if (getYunsfsDropDownValue().getId()!=-1){
			strCondition ="and dh.yunsfsb_id=" +getYunsfsDropDownValue().getId() ;//运输方式
		}
		
		if (-1!=(getJihkjDropDownValue().getId())){
			strCondition=strCondition+" and dh.jihkjb_id=" +getJihkjDropDownValue().getId();
		}
		...
		String titlename=intyear+"年合同分解查询";
		...
		if (getBaoblxValue().getValue().equals("分厂")){
			...
			if(jib==1){//选集团时刷新出所有的电厂
				...
				strhaving.append("");
				strorderby.append("order by grouping(dc.fgsmc) desc,dc.fgsmc desc,grouping(dc.mingc) desc,max(dc.xuh) \n");
				...
				try{
					...
					if(rl.next()){//燃料公司
						strgrouping.append("decode(grouping(dc.rlgsmc)+grouping(dc.mingc),2,'总计',1,dc.rlgsmc,'&nbsp;&nbsp;&nbsp;&nbsp;'||dc.mingc) as diancmc,\n");
						strwhere.append(" and (dc.fgsid="+this.getTreeid()+" or dc.rlgsid= "+this.getTreeid()+") \n");
						strgroupby.append("group by rollup(dc.rlgsmc,dc.mingc)\n");
						strhaving.append("having not grouping(dc.rlgsmc)=1\n");
						strorderby.append("order by grouping(dc.rlgsmc) desc,dc.rlgsmc desc,grouping(dc.mingc) desc,max(dc.xuh) \n");
					}else{//分公司
						strgrouping.append("decode(grouping(dc.fgsmc)+grouping(dc.mingc),2,'总计',1,dc.fgsmc,'&nbsp;&nbsp;&nbsp;&nbsp;'||dc.mingc) as diancmc,\n");
						strwhere.append(" and (dc.fgsid="+this.getTreeid()+" or dc.rlgsid= "+this.getTreeid()+") \n");
						strgroupby.append("group by rollup(dc.fgsmc,dc.mingc)\n");
				...
				strorderby.append("order by grouping(dc.mingc) desc,max(dc.xuh)\n");
			}
			...
			iFixedRows=1;
			strgrouping.append("decode(grouping(g.dqid),1,'全网合计',max(g.dqmc)) as meikdqmc,\n");
			strgroupby.append("group by rollup(g.dqid)\n");
			strorderby.append("order by grouping(g.dqid) desc,g.dqid desc\n");
			...
			iFixedRows=2;
			if(jib==1){//选集团时刷新出所有的电厂
				strgrouping.append("decode(grouping(g.dqmc),1,'总计',g.dqmc) as meikmc,\n");
				strgrouping.append("decode(grouping(g.dqmc)+grouping(dc.fgsmc)+grouping(dc.mingc),3,'',2,'小计',1,dc.fgsmc,'&nbsp;&nbsp;&nbsp;&nbsp;'||dc.mingc) as diancmc,\n");
				strwhere.append("");
				strgroupby.append("group by rollup(g.dqmc,dc.fgsmc,dc.mingc)\n");
				strhaving.append("");
				strorderby.append("order by grouping(g.dqmc) desc,g.dqmc,grouping(dc.fgsmc) desc,dc.fgsmc desc,grouping(dc.mingc) desc,max(dc.xuh)\n");
			}else if (jib==2){//选分公司的时候刷新出分公司下所有的电厂
				String ranlgs = "select id from diancxxb where shangjgsid= "+this.getTreeid();
				...
				try{
					...
					if(rl.next()){//燃料公司
						strgrouping.append("decode(grouping(g.dqmc),1,'总计',g.dqmc) as meikmc,\n");
						strgrouping.append("decode(grouping(g.dqmc)+grouping(dc.rlgsmc)+grouping(dc.mingc),3,'',2,'小计',1,dc.rlgsmc,'&nbsp;&nbsp;&nbsp;&nbsp;'||dc.mingc) as diancmc,\n");
						strwhere.append(" and (dc.fgsid="+this.getTreeid()+" or dc.rlgsid= "+this.getTreeid()+") \n");
						strgroupby.append("group by rollup(g.dqmc,dc.rlgsmc,dc.mingc)\n");
						//strhaving.append("having not grouping(dc.rlgsmc)=1\n");
						strorderby.append("order by grouping(g.dqmc) desc,g.dqmc,grouping(dc.rlgsmc) desc,dc.rlgsmc desc,grouping(dc.mingc) desc,max(dc.xuh)\n");
					}else{//分公司
						strgrouping.append("decode(grouping(g.dqmc),1,'总计',g.dqmc) as meikmc,\n");
						strgrouping.append("decode(grouping(g.dqmc)+grouping(dc.fgsmc)+grouping(dc.mingc),3,'',2,'小计',1,dc.fgsmc,'&nbsp;&nbsp;&nbsp;&nbsp;'||dc.mingc) as diancmc,\n");
						strwhere.append(" and (dc.fgsid="+this.getTreeid()+" or dc.rlgsid= "+this.getTreeid()+") \n");
				...
				strgrouping.append("decode(grouping(dc.mingc),1,'总计',dc.mingc) as diancmc,\n");
				strgrouping.append("decode(grouping(dc.mingc)+grouping(g.dqmc),2,'',1,'小计',g.dqmc) as meikmc,\n");
				strwhere.append(" and dc.id=").append(this.getTreeid());
				strgroupby.append("group by rollup(dc.mingc,g.dqmc)\n");
				strhaving.append("having not grouping(dc.mingc)=1\n");
				strorderby.append("order by grouping(dc.mingc) desc,max(dc.xuh),grouping(g.dqmc) desc,g.dqmc\n");
			}
			...
			if(jib==1){//选集团时刷新出所有的电厂
				...
				strhaving.append("");
				strorderby.append("order by grouping(dc.fgsmc) desc,dc.fgsmc desc,grouping(dc.mingc) desc,max(dc.xuh),grouping(g.dqmc) desc,g.dqmc\n");
			}else if (jib==2){//选分公司的时候刷新出分公司下所有的电厂
				String ranlgs = "select id from diancxxb where shangjgsid= "+this.getTreeid();
				...
				try{
					...
					if(rl.next()){//燃料公司
						...
						strgrouping.append("decode(grouping(dc.mingc)+grouping(g.dqmc),2,'',1,'小计',g.dqmc) as meikmc,\n");
						strwhere.append(" and (dc.fgsid="+this.getTreeid()+" or dc.rlgsid= "+this.getTreeid()+") \n");
						strgroupby.append("group by rollup(dc.rlgsmc,dc.mingc,g.dqmc)\n");
						strhaving.append("having not grouping(dc.rlgsmc)=1\n");
						strorderby.append("order by grouping(dc.rlgsmc) desc,dc.rlgsmc desc,grouping(dc.mingc) desc,max(dc.xuh),grouping(g.dqmc) desc,g.dqmc\n");
					}else{//分公司
						strgrouping.append("decode(grouping(dc.fgsmc)+grouping(dc.mingc),2,'总计',1,dc.fgsmc,'&nbsp;&nbsp;&nbsp;&nbsp;'||dc.mingc) as diancmc,\n");
						strgrouping.append("decode(grouping(dc.mingc)+grouping(g.dqmc),2,'',1,'小计',g.dqmc) as meikmc,\n");
						strwhere.append(" and (dc.fgsid="+this.getTreeid()+" or dc.rlgsid= "+this.getTreeid()+") \n");
			 ...
			 "order by grouping(mk.mingc) desc,sum(dh.hej) desc,mk.mingc desc) b,\n" + 
			 "(select sum(n.hej) as hej from niandhtqkb n ,diancxxb d where to_char(n.riq,'yyyy')='"+ intyear+"' "+strGongsID+") a");
			 
			 ArrHeader=new String[3][18];
			 ArrHeader[0]=new String[] {"煤矿","年度煤炭订货数量(%)","年度煤炭订货数量(%)","年度煤炭订货数量(%)","年度煤炭订货数量(%)","年度煤炭订货数量(%)","年度煤炭订货数量(%)","年度煤炭订货数量(%)","年度煤炭订货数量(%)","年度煤炭订货数量(%)","年度煤炭订货数量(%)","年度煤炭订货数量(%)","年度煤炭订货数量(%)","年度煤炭订货数量(%)","年度煤炭订货数量(%)","年度煤炭订货数量(%)","年度煤炭订货数量(%)","年度煤炭订货数量(%)"};
		...
		if (!getBaoblxValue().getValue().equals("分矿百分比表")){
			...
			strSQL.append("from niandhtqkb dh,vwdianc dc,vwgongys g\n");
			strSQL.append("where dh.diancxxb_id=dc.id and dh.gongysb_id=g.id and to_char(dh.riq,'yyyy')='"+ intyear+"' \n");
			strSQL.append(strwhere).append(strCondition);
