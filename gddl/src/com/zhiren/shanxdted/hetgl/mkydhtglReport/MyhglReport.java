...
public class MyhglReport extends BasePage {
...
//		}
	...
	public void setLastChildNode(JDBCcon con,TreeNode ParentNode, String nianf, String yuef, StringBuffer ebuff) {
		...
		while(crs.next()) {
			...
			//添加双击事件
			ebuff.append("\n");
			ebuff.append("NavTree_root"+nianf+nianf+yuef+crs.getString("ID")+".on('click',function(node,e){\n");
			ebuff.append("document.all.myhFrame.src =\"http://\"+document.location.host+document.location.pathname + \"?service=page/Mkydbb&lx=" + crs.getString("TEXT") + "\";");
			ebuff.append("});\n");
		}
	}
