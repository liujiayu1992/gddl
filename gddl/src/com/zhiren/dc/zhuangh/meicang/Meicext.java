...
//		System.out.println(Change);
		...
		while (drsl.next()) {
			sql.append("delete from ").append("meicangb").append(" where id =")
		...
		while (rsl.next()) {
			...
			if ("0".equals(rsl.getString("ID"))) {
				sql.append("insert into ").append("meicangb").append("(id");
				...
				sql.append("update ").append("meicangb").append(" set ");
				for (int i = 1; i < rsl.getColumnCount(); i++) {
...
//								"ImageReport&&id='||m.id||'&&mk=meicxx\" target=\"_blank\">查看</a>' as meict,"
						+ "m.beiz from meicangb m,diancxxb dc where jib=3  and dc.id=m.diancxxb_id "
						+ "" + str +
						// " and m.diancxxb_id ="+ getTreeid() +
						" order by m.xuh,m.mingc");

		ExtGridUtil egu = new ExtGridUtil("gridDiv", rsl);
		egu.setTableName("meicangb");
