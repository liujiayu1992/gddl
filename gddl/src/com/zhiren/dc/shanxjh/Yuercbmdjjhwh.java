...
public class Yuercbmdjjhwh extends BasePage implements PageValidateListener {
...
//			Countlj();
	...
	private String getSql(String diancxxb_id,long tjkjid, String fenx){
		...
		String sql =
			...
			"  (id, fenx, yuetjkjb_id, meij, meijs, jiaohqzf, yunj, yunjs, daozzf, zaf, qit, qnet_ar, biaomdj, buhsbmdj)(\n" + 
			"select\n" + 
			"getnewid("+diancxxb_id+") id,nvl('"+fenx+"','') as fenx, t.id, nvl(a.meij,0) meij, nvl(a.meijs,0) meijs, \n" +
			"nvl(a.fazzf,0) jiaohqzf, nvl(a.yunj,0) yunj, nvl(a.yunjs,0) yunjs, nvl(a.daozzf,0) daozzf, nvl(a.zaf,0) zaf, nvl(a.ditf,0) qit,\n" + 
			"z.qnet_ar, decode(z.qnet_ar,0,0,round_new((a.meij+a.yunj+a.fazzf+a.zaf +a.ditf)*29.271/z.qnet_ar,2)) bmdj,\n" + 
			"decode(z.qnet_ar,0,0,round_new((a.meij-a.meijs+a.yunj-a.yunjs+a.fazzf+a.zaf +a.ditf)*29.271/z.qnet_ar,2)) bmdjbhs\n" + 
			"from(select f.diancxxb_id,m.meikdq_id,f.jihkjb_id,f.pinzb_id,f.yunsfsb_id,\n" + 
			"nvl(decode(sum(f.laimsl),0,0,round_new(sum(f.laimsl*jg.meij)/sum(f.laimsl),2)),0) meij,\n" + 
			"nvl(decode(sum(f.laimsl),0,0,round_new(sum(f.laimsl*jg.meijs)/sum(f.laimsl),2)),0) meijs,\n" + 
			"nvl(decode(sum(f.laimsl),0,0,round_new(sum(f.laimsl*jg.fazzf)/sum(f.laimsl),2)),0) fazzf,\n" + 
			"nvl(decode(sum(f.laimsl),0,0,round_new(sum(f.laimsl*jg.yunj)/sum(f.laimsl),2)),0) yunj,\n" + 
			"nvl(decode(sum(f.laimsl),0,0,round_new(sum(f.laimsl*jg.yunjs)/sum(f.laimsl),2)),0) yunjs,\n" + 
			"0 daozzf,nvl(decode(sum(f.laimsl),0,0,round_new(sum(f.laimsl*jg.zaf)/sum(f.laimsl),2)),0) zaf,\n" + 
			"nvl(decode(sum(f.laimsl),0,0,round_new(sum(f.laimsl*jg.ditf)/sum(f.laimsl),2)),0) ditf\n" + 
			" from\n" + 
			"(select wj.id,\n" + 
			...
			"from (select f.id from fahb f\n" + 
			"where f.daohrq >= "+LastODate+"\n" + 
			"and f.daohrq < add_months("+CurrODate+",1)\n" + 
			"minus select f.id from fahb f, jiesb j\n" + 
			"where f.jiesb_id = j.id\n" + 
			"and j.ruzrq >= "+LastODate+"\n" + 
			"and j.ruzrq < add_months("+CurrODate+",1)\n" + 
			"and f.daohrq >= "+LastODate+"\n" + 
			"and f.daohrq < add_months("+CurrODate+",1)) wj\n" + 
			"union\n" + 
			"select f.id,j.hansdj,\n" + 
			"decode(j.jiessl,0,0,null,0,round_new(j.shuik/j.jiessl,7)) meijs,\n" + 
			"decode(y.jiessl,0,0,null,0,round_new((nvl(y.kuangqyf,0) + nvl(y.kuangqzf,0))/y.jiessl,7)) fazzf,\n" + 
			"decode(y.jiessl,0,0,null,0,round_new(y.guotyf/y.jiessl,7)) yunj,\n" + 
			"decode(y.jiessl,0,0,null,0,round_new(y.shuik/y.jiessl,7)) yunjs,\n" + 
			"decode(y.jiessl,0,0,null,0,round_new(y.guotzf/y.jiessl,7)) zaf,\n" + 
			"0 ditf\n" + 
			"from fahb f, jiesb j, jiesyfb y\n" + 
			"where f.jiesb_id = j.id and j.id = y.diancjsmkb_id(+)\n" + 
			"and j.ruzrq >= "+LastODate+"\n" + 
			"and j.ruzrq < add_months("+CurrODate+",1)\n" + 
			"and f.daohrq >= "+LastODate+"\n" + 
			"and f.daohrq < add_months("+CurrODate+",1)) jg,\n" + 
			"fahb f, meikxxb m\n" + 
			"where f.id = jg.id and f.meikxxb_id = m.id\n" + 
			"group by f.diancxxb_id,m.meikdq_id,f.jihkjb_id,f.pinzb_id,f.yunsfsb_id) a,yuetjkjb t, yuezlb z\n" + 
			"where t.id = z.yuetjkjb_id and z.fenx = '"+fenx+"' and t.diancxxb_id = a.diancxxb_id(+)\n" + 
			"and t.gongysb_id = a.meikdq_id(+) and t.jihkjb_id = a.jihkjb_id(+)\n" + 
			"and t.pinzb_id = a.pinzb_id(+) and t.yunsfsb_id = a.yunsfsb_id(+) and t.id = '"+tjkjid+"')";
		return sql;
	}
	...
	public void CreateData() {
		...
		if(Cleaning_up_History(CurrODate,"")){
			...
			if (type.equals("JZRD")||model.equals("DTGJ")) {
				...
				while(rs.next()){
					con.getInsert(getSql(diancxxb_id,rs.getLong("id"),SysConstant.Fenx_Beny));
					con.getInsert(getSql(diancxxb_id,rs.getLong("id"),SysConstant.Fenx_Leij));
				}
				rs.close();
	...
	public void getSelectData() {
		...
		"select rcdj.id,tj.id as yuetjkjb_id,gongysb.mingc as gongysb_id,jihkjb.mingc as jihkjb_id,pinzb.mingc as pinzb_id,yunsfsb.mingc as yunsfsb_id,\n"
				+ "        fenx,meij,meijs,jiaohqzf,yunj,yunjs,daozzf,zaf,qit,qnet_ar,biaomdj,buhsbmdj\n"
				...
				+ visit.getDiancxxb_id() + " and riq=" + CurrODate + " \n" +
