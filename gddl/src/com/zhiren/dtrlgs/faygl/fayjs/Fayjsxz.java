...
public class Fayjsxz extends BasePage implements PageValidateListener {
	private String msg = "";
...
//    		GongysName=mdrsl.getString("gys");
    		mstr_fahb_id+=mdrsl.getString("id")+",";
    	}
//    	try {
//    		Gongysb_id=MainGlobal.getTableId("gongysb", "mingc", GongysName);
//		} catch (Exception e1) {
//			// TODO 自动生成 catch 块
//			e1.printStackTrace();
//		}
...
// 				Diancxxb_id=((Visit)this.getPage().getVisit()).getDiancxxb_id();
    	           ...
    	           Diancxxb_id=this.getShouhdwValue().getId();//即为收货单位的id
...
//    	发货的Lie_Idd
    	//((Visit) getPage().getVisit()).setString1(mstr_lieid.substring(0,mstr_lieid.lastIndexOf(",")));
    	 SelIds=mstr_fahb_id.substring(0,mstr_fahb_id.lastIndexOf(","));
    	 ((Visit) getPage().getVisit()).setString1(SelIds);
//    	 业务类型表i
    	  ((Visit)this.getPage().getVisit()).setLong10(MainGlobal.getProperId(this.getYewlxModel(),getYewlx()));
//    	发货Id
...
//    	更新选中发货表的hetb_id为getHthValue().getId()
    	if(setFahb_HetbId(this.getHthValue().getId(),mstr_fahb_id)){
...
//    		为防止gongysb_id=0取结算设置方案时出错，特加入gongysb_id重新赋值的语句
    		Gongysb_id=Fayjsdcz.getGongysb_id(SelIds, 
    				Diancxxb_id, Gongysb_id, 
    				Hetb_id, Double.parseDouble(getJieskdl()));
    		((Visit) getPage().getVisit()).setLong3(Gongysb_id);
    		
    		Jieszbsftz=mstr_jieszbtz=Fayjsdcz.getJiessz_item(Diancxxb_id, 
...
//   填充计算
			...
			try {
				bls.getBalanceData(SelIds, Diancxxb_id, Jieslx, Gongysb_id, Hetb_id, Jieszbsftz, Yansbh, Jieskdl, Yunsdwb_id, Shangcjsl);
...
//			4、更新老发货表
	...
	private boolean setFahb_HetbId(long HetbId,String ids) {
		// TODO 自动生成方法存根
//		更新选中发货表的hetb_id为getHthValue().getId()
		JDBCcon con=new JDBCcon(); 
		boolean flag=false;
		;String sql="update fayslb set hetb_id="+HetbId+" where id in("+ids+")";
...
//        	19、结算指标调整
       ...
       if(this.getChangb()!=null&&getHthValue().getId()!=-1){
    	   
	        if(Fayjsdcz.getJiessz_items((visit.isFencb()?MainGlobal.getProperId(getFencbModel(), this.getChangb()):visit.getDiancxxb_id()),
	        					this.getChangb_id(),this.getHthValue().getId())!=null){
	        	
	        	String JiesszArray[][]=null;
	        			
	        	JiesszArray=Fayjsdcz.getJiessz_items((visit.isFencb()?MainGlobal.getProperId(getFencbModel(), this.getChangb()):visit.getDiancxxb_id()),
	        			this.getChangb_id(),this.getHthValue().getId());
...
//             if(!getTreeid().equals("-1")){
              gongysb_id= " and f.shr_diancxxb_id="+getTreeid();
        	 ...
        	 sql = 
     			...
     			"       max( y.mingc) yewlx,max(c.mingc) as faz,sum(f.ches) as ches,sum(f.biaoz) as biaoz,	sum(f.yingk) as yingk," +
     			"		sum(f.yuns) as yuns,	\n" + 
     			"       decode(g.mingc,null,'',sum(f.maoz-f.piz-f.zongkd)) as jingz,decode(g.mingc,null,'',h.hetbh) as hetbh,\n" + 
...
//     			"       group by rollup(f.lie_id,g.mingc,m.mingc,h.hetbh,c.mingc)	\n" + 
       ...
       sql=" 	 select f.id as id,\n"+
             "g.mingc gys,\n"+
            " (select mingc from diancxxb where id=f.diancxxb_id) as fahdw,\n"+
             "(select mingc from diancxxb where id=f.shr_diancxxb_id) as shouhdw\n"+
             ",f.chec\n"+
             ",m.mingc as meikdw"+
           " ,to_char(f.fahrq, 'yyyy-MM-dd') || '--' ||\n"+
            ...
            ",c.mingc faz,\n"+
             "f.meil,\n"+
             " h.hetbh as hetbh,\n"+
             "round_new(z.qnet_ar, 2) as Qnetar,\n"+
             ...
             "round_new(z.qbad, 2) as Qbad,\n"+
             "round_new(z.qgrad, 2) as Qgrad\n"+
        "from fayslb  f,\n"+
         "    gongysb g,\n"+
         "    meikxxb m,\n"+
         "    hetb    h,\n"+
     ...
     "    and f.zhilb_id = z.id\n"+
      ...
      "   and f.xiaosjsb_id = 0\n"+
          this.getWhere(riq)+
//      "   and (di.id = 199 or di.fuid = 199)\n"+
//      "   and fahrq >= to_date('2008-07-28', 'yyyy-mm-dd')\n"+
//      "   and fahrq < to_date('2009-07-28', 'yyyy-mm-dd') + 1\n"+
     "    and f.daohrq is not null\n"+
     "  order by g.mingc, fahrq\n";
		...
		egu.getColumn("gys").setHeader("供应商");
		egu.getColumn("fahdw").setHeader("发货单位");
	     egu.getColumn("shouhdw").setHeader("收货单位");	
...
//						.getVisit()).getDiancxxb_id(), getTreeid(),"");
		...
		egu.addTbarText("收货单位:");
		ComboBox com_shr = new ComboBox();
		//JieslxDropDown
		com_shr.setTransform("Com_shrDropDown");
		com_shr.setId("Com_shr");
		com_shr.setEditable(false);
		com_shr.setLazyRender(true);// 动态绑定
		com_shr.setWidth(80);
		com_shr.setReadOnly(true);
		egu.addToolbarItem(com_shr.getScript());
		egu.addOtherScript("Com_shr.on('select',function(){document.forms[0].submit();});");
...
//			 	+ "	\t}	\n"
        ...
        egu.addToolbarButton("结算",GridButton.ButtonType_SubmitSel_condition,"JiesButton","if(Heth.getRawValue()=='请选择'&&Jieslx.getRawValue()!='地铁运费'&&Jieslx.getRawValue()!='国铁运费'){	\n	"
        																	...
        																	+ " if( Com_shr.getValue()=='大唐燃料公司'|| Com_shr.getValue()=='请选择'){Ext.MessageBox.alert('提示信息','请选择具体收货人！');};\n"
        																	+ " return ;	\n"
        																	+ " }");
//        
...
//			getYansbhModels();		//2
			...
			setShouhdwValue(null);
			setShouhdwModel(null);
			getShouhdwModels();
...
//			}
			...
			if(this.getChangb_id()+""==null||(this.getChangb_id()+"").equals("-1")){	//全部
...
//					分厂别
					if(getJieslxValue().getId()!=Locale.guotyf_feiylbb_id
						...
						sql = "  select distinct h.id,h.hetbh\n"
								...
								+ "h.id = hs.hetb_id(+) and h.xufdwmc in (\n"
								+ "   select distinct(select mingc from diancxxb where id= shr_diancxxb_id) from fayslb f,diancxxb di where fahrq>=to_date('"+this.getRiq1()+"','yyyy-MM-dd')\n "
								+ "  and fahrq<=to_date('"+this.getRiq2()+"','yyyy-MM-dd') and f.diancxxb_id=di.id\n"
								+ " -- and liucztb_id=0    \n"
								+ "and di.id="+this.getChangb_id()
								+ ") \n"
								+ where
								+ "and not(to_date('"+this.getRiq1()+"','yyyy-MM-dd') >guoqrq or to_date('"+this.getRiq2()+"','yyyy-MM-dd')<qisrq)  \n "
...
//				选择了需方
				String shr_diancxxb_id=this.getTreeid()+"";
				String xuf_condtion="";
				
//					xuf_condtion="and h.gongfdwmc=(select quanc from diancxxb where id="+this.getTreeid()+")\n";
				
					where=" and di.id="+this.getTreeid()+" \n";
...
//				 供方厂别控制
			     if((dcid=MainGlobal.getProperId(getFencbModel(), this.getChangb()))>-1){
			    	 /*    int jib=this.getJib(dcid);
			    	 if(jib==2){
			    		  xuf_condtion=" and h.gongfdwmc=(select quanc from diancxxb where id="+dcid+") \n";
			    	  }else if(jib==3){
			    		  xuf_condtion="and h.gongfdwmc=(select quanc from diancxxb where id=(select fuid from diancxxb where id="+dcid+"))\n";
//			    		  where=" and di.id=(select fuid from diancxxb where id="+dcid+")\n";
			    	  }*/
			    	 xuf_condtion =" and h.gongysb_id="+dcid+"\n"; 
			    	 
			     }	
			     // 品种的限制
			     String pinz_condition="";
			     if(MainGlobal.getProperId(getPinzModel(), this.getPinz())>-1){
			    	 pinz_condition=" and hs.pinzb_id = " + MainGlobal.getProperId(getPinzModel(), this.getPinz()) +"\n";
		         }
				
//				if(shr_diancxxb_id.equals("-1")){
//				where=
...
//					if(((Visit) getPage().getVisit()).isGSUser()){
						...
						if(getJieslxValue().getId()!=Locale.guotyf_feiylbb_id
							...
							sql = "  select distinct h.id,h.hetbh\n"
								+ "from hetb h,hetslb hs,diancxxb di\n "
								+ "where  di.id=h.diancxxb_id     and \n"
								+ "h.id = hs.hetb_id(+) and h.xufdwmc=(select mingc from diancxxb where id="+this.getTreeid()+")"
								+ where
								+ " and not(to_date('"+this.getRiq1()+"','yyyy-MM-dd') >guoqrq or to_date('"+this.getRiq2()+"','yyyy-MM-dd')<qisrq)  \n "
								+" and  liucztb_id=1"
								+ " order by hetbh ";
					...
					/*	}else{
						sql=	" select distinct hetys.id,hetbh from \n"+
							...
							" where gys.id=glb.gongysb_id and mk.id=glb.meikxxb_id\n"+	
							 "and (gys.id="+getTreeid()+" or mk.id="+getTreeid()+"))"+ where +	
							 "and not (to_date('"+this.getRiq1()+"', 'yyyy-MM-dd') > guoqrq or\n"+
						    "    to_date('"+this.getRiq2()+"', 'yyyy-MM-dd') < qisrq)\n"+
							 "order by hetbh"; 
	...
	public IDropDownBean getFencbValue() {
		...
		int optioncount=getFencbModel().getOptionCount();
			for(int i=0;i<optioncount;i++){
				if(((IDropDownBean) getFencbModel().getOption(i)).getValue().equals("大唐燃料公司"))
				{
					this.setChangb(((IDropDownBean) getFencbModel().getOption(i)).getValue());
		...
		String sql="";
		  /*  if(((Visit) getPage().getVisit()).isGSUser()){
			...
			((Visit) getPage().getVisit()).getDiancxxb_id()+"   order by mingc";
               ...
               sql="select id,mingc from gongysb  ";
			((Visit) getPage().getVisit())
...
//	收货单位下拉框
	public boolean Shouhdwchange = false;
	public IDropDownBean getShouhdwValue() {
		Visit visit=((Visit) getPage().getVisit());
		if (((Visit) getPage().getVisit()).getDropDownBean20() == null) {
			
//		int optioncount=getShouhdwModel().getOptionCount();
//			for(int i=0;i<optioncount;i++){
//				if(((IDropDownBean) getShouhdwModel().getOption(i)).getId()==visit.getDiancxxb_id())
//				{
			         ((Visit) getPage().getVisit()).setDropDownBean20((IDropDownBean) getShouhdwModel().getOption(0));
//				break;
//				}
//			}
		}
	...
	public void setShouhdwValue(IDropDownBean Value) {
		
		if(Value!=((Visit) getPage().getVisit()).getDropDownBean20()){
	...
	public IPropertySelectionModel getShouhdwModel() {
	...
	public void setShouhdwModel(IPropertySelectionModel value) {

		((Visit) getPage().getVisit()).setProSelectionModel20(value);
	}

	public IPropertySelectionModel getShouhdwModels() {
		
		String sql="select id,mingc from diancxxb d where cangkb_id=1 order by mingc";

			((Visit) getPage().getVisit())
			.setProSelectionModel20(new IDropDownModel(sql,"请选择"));
			return ((Visit) getPage().getVisit()).getProSelectionModel20();
	}
...
//			((Visit) getPage().getVisit()).setboolean3(true);
	...
	public String getTreeid(){
		return this.getShouhdwValue().getId()+"";
	}
	public long getChangb_id(){
		return MainGlobal.getProperId(getFencbModel(), this.getChangb());
	}
}