...
public class Ruchythreport extends BasePage {
//	 判断是否是集团用户
...
//	private Date _BeginriqValue = DateUtil.AddDate(new Date(), -1, DateUtil.AddType_intDay);
	...
	public String getJizyxqkreport(){
		...
		String c_sql=
			...
			"a.bianm,to_char(a.huaysj,'yyyy-mm-dd') ahuaysj," +
//			"a.qnet_ar_k,a.qnet_ar," +
			"a.mt,a.mad,a.aad,a.vad,a.stad,\n" + 
			"b.bianm,to_char(b.huaysj,'yyyy-mm-dd') bhuaysj," +
//			"b.qnet_ar_k,b.qnet_ar," +
			"b.mt,b.mad,b.aad,b.vad,b.stad\n" + 
			" from\n" + 
			...
			"  and f.faz_id=cz.id\n" + 
			"  and f.daohrq >=to_date('"+getRiq()+"','yyyy-mm-dd') and f.daohrq <=to_date('"+getRiq1()+"','yyyy-mm-dd') order by f.yundh"+
			"\n" + 
			")a,\n" + 
			"(\n" + 
			"select  f.id,f.yundh,lc.mingc lcmingc,gs.mingc gmingc,mk.mingc mmingc,p.mingc pmingc,cz.mingc cmingc,(f.maoz-f.piz) xiehl,\n" + 
			...
			"    z.stad as stad\n" + 
			" from fahb f,zhillsb z ,zhuanmb m,zhuanmlb l,luncxxb lc,gongysb gs,meikxxb mk,pinzb p,chezxxb cz\n" + 
			"  where f.zhilb_id=z.yuanzhilb_id and z.zhilb_id=0\n" + 
			"  and z.id=m.zhillsb_id and m.zhuanmlb_id=l.id and l.jib=3\n" + 
			"  and f.luncxxb_id=lc.id and f.gongysb_id=gs.id and f.meikxxb_id=mk.id and f.pinzb_id=p.id\n" + 
			"  and f.faz_id=cz.id\n" + 
			"  and f.daohrq >=to_date('"+getRiq()+"','yyyy-mm-dd') and f.daohrq <=to_date('"+getRiq1()+"','yyyy-mm-dd') order by f.yundh"+
			")b\n" +  
...
//			"select *  from (select rc.yundh,rc.chuanm,rc.gongys,rc.meikmc,rc.pinz,rc.faz,rc.yundl,rc.xiehl,rc.shuicl,to_char(rc.daohrq,'yyyy-mm-dd') as daohrq,rc.qnet_ar_k as qnet_ar_k_rc,rc.qnet_ar as qnet_ar_rc,\n" +
//			" rc.mt as mt_rc,rc.mad as mad_rc,rc.aad as aad_rc,rc.vad as vad_rc,rc.stad as stad_rc,rc.xiemkssj,\n" +
//			" rc.xiemjssj,rc.meic,rc.duowmc,rc.meicml,\n" +
//			" rl.shangmkssj,rl.shangmjssj,rl.jizlh,rl.meich,rl.meil ,\n" +
//			" rl.qnet_ar_k as qnet_ar_k_rl,rl.qnet_ar as qnet_ar_rl,rl.mt as mt_rl,rl.mad as mad_rl,rl.aad as aad_rl,\n" +
...
// 
		...
		String ArrHeader[][]=new String[1][23];//27
		ArrHeader[0]=new String[] {"运单号","船名","供应商","煤矿名称","品种","发站港",//
				"卸货量","运单量","水尺量","到货日期","化验编码","化验时间",
//				"Qnet,ar(Kcal/kg)","Qnet,ar(MJ/kg)",
				"Mt(%)","Mad(%)","Aad(%)","Vad(%)","Stad(%)"
				//19
				,"原化验编码","原化验时间"//5
//				,"Qnet,ar(Kcal/kg)","Qnet,ar(MJ/kg)"
				,"Mt(%)","Mad(%)","Aad(%)","Vad(%)","Stad(%)"};//7

		int ArrWidth[]=new int[] {70,80,80,60,45,75,45,45,45,90,90,90,
//				45,45,
				45,45,45,45,
				45,90,90,
//				45,45,
		...
		rt.setTitle("入厂化验值替换查询",ArrWidth);
		//footer
