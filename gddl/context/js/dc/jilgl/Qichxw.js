...
//          国电宣威1,2,3,4,5,7号衡器配置        		
        	case "51": str = Instring.substring(str.indexOf("") + 5 , str.indexOf("") + 10);
        		zhongl = parseInt(str,10);
        		break;
//          国电宣威6号衡器配置
        	case "52": str =  Instring.substring(str.indexOf("") + 4 , str.indexOf("") + 9);
        		zhongl = parseInt(str,10);
        		break;
...
//			数据原型< =00935 > 实际值为53.90
			case "60": str = Instring.substring(0, 16);
				...
				break;
//			国电邯郸电厂				
			case "170": str = Instring.substring(0, 20);
				for(var i=0;i<=20;i++){
					if(str.substr(i,1)==""){
...
case "110": str = Instring.substring(0, 18);
//alert(str);
				for(var i=0;i<=9;i++){
					if(str.substr(i,1)=='='){
...
case "120": str = Instring.substring(0, 18);
				for(var i=0;i<=9;i++){
					if(str.substr(i,1)=='='){
				...
				for(var i =(temp1.length-1);i>= 0;i--){
					if(temp1.substr(i,1)==' '||temp1.substr(i,1)=='='||temp1.substr(i,1)=='-'){
						Stemp=Stemp+'0';
					}else Stemp = Stemp + temp1.substr(i,1);
				}
//alert(Stemp);
...
case "130": str = Instring.substring(0, 9);
                                ...
                                for(var i=17;i>=0;i--)
                                {
                                 ...
                                 {
                                   continue;
                                 }else temp1=temp1+str.substr(i,1);}
                               for(var i=0;i<=17;i++)
                                {
                                 if(temp1.substr(i,1)=='=')
                                 {
...
function caij1(){
	...
	if(row_maoz_index==-1) return true;
...
//	设置汽车衡测试参数
	...
	if(Ext.isReady){
		...
		if(DataIndex=="MAOZ"){
			if(Mode == "sel")
//				rec = grid.getSelectionModel().getSelected();
				rec=grid.getStore().getAt(row_maoz_index);
			else
				rec = grid.getStore().getAt(0);
			rec.set("ZHONGCHH",Jilhh);
		}else
...
//				rec = gridp.getSelectionModel().getSelected();
				rec=gridp.getStore().getAt(row_maoz_index);
			else
		...
		if(DataIndex=="MAOZ"){
			grid.getView().getRow(row_maoz_index).style.backgroundColor='red';
		}else{
			gridp.getView().getRow(row_maoz_index).style.backgroundColor='red';
		}
