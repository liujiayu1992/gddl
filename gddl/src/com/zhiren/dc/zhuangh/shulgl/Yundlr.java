				...
				for (int i = 1; i < mdrsl.getColumnCount(); i++) {
					...
					if (mdrsl.getColumnNames()[i].equals("FUID")) {
						if (mdrsl.getString("fuid").equals("请选择")
							...
							sql
									.append(
											(egu.getColumn(mdrsl.getColumnNames()[i]).combo)
													.getBeanId(mdrsl
															.getString("fuid")))
									.append(",");
						}
...
//		if(ToAddMsg.equals("toAdd")){
//			egu.addOtherScript("var p=new gridDiv_plant("+DataSource+"); gridDiv_ds.insert(0,p); ");
//			StringBuffer sb=new StringBuffer();
//			sb.append("\n ").append(egu.gridId).append("_ds.getAt(0).beginEdit();	\n")
//				.append(egu.gridId).append("_ds.getAt(0).dirty=true;	\n")
//				.append(egu.gridId).append("_ds.getAt(0).endEdit();	\n");
//			egu.addOtherScript(sb.toString());
//		}
