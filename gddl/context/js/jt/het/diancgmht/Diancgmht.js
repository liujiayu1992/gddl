...
SelectFrmDivzf.style.position="relative";
SelectHeadDivzf.style.position="relative";
SelectDataDivzf.style.position="relative";
SelectFrmDivzf.style.height=trhetzxxHeight-70;
SelectFrmDivzf.style.width=tablemainWidth;
SelectHeadDivzf.style.width=tablemainWidth;
SelectHeadDivzf.style.height=tableTileHeight;
SelectHeadDivzf.style.posLeft=0;
SelectDataDivzf.style.width=tablemainWidth;
SelectDataDivzf.style.height=trhetzxxHeight-100-12;
oDatazf.style.borderRight="gray 1px solid";
EditHeadTabelzf.style.borderRight="gray 1px solid";
EditHeadTabelzf.style.height=tableTileHeight;
...
function classchangezf(_str1,_str2){

	var rows=oDatazf.rows ;

	if(_str2=="onclick"){
		if (oldClickrowzf !=-1){
			oDatazf.rows(oldClickrowzf).className="edittableTrOut";
		}
		oldClickrowzf=_str1.rowIndex;
		oDatazf.rows(oldClickrowzf).className="edittableTrClick";
	}

	if(_str2=="onmouseout"){
		if (oldClickrowzf!=_str1.rowIndex){
			_str1.className="edittableTrOut";
		}else{
			_str1.className="edittableTrClick";
		}
	...
	document.all.item("EditTableRowzf").value=oldClickrowzf;
}
...
function saveCheck(){
    ...
    if(	document.all.shuljiaoy.value=='true')//不加入校验
...
//						}
	...
	for(i=0;i<oDatag.rows.length;i++){
		...
		if(document.all.item("jijlx",i).value==''){
			alert("<价格>标签页中第<"+(i+1)+">行<价格类型>不能为空！");
			return false;
		}
