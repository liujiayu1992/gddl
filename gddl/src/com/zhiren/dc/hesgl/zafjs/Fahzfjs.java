...
public class Fahzfjs extends BasePage implements PageValidateListener {
...
//	杂费名称下拉框_结束
	...
	public void save() {
		...
		while(mdrsl.next()) {
			sbsql.append("insert into zafjsxxb(id, diancxxb_id, jiesrq, fahb_id, gongysb_id, meikxxb_id, zafmc_item_id, shuil, shuik, zongje, shifds, beiz) values(getnewid(")
				 .append(visit.getDiancxxb_id()).append("), ").append(visit.getDiancxxb_id()).append(", to_date(to_char(sysdate, 'yyyy-mm-dd hh24:mi:ss'),'yyyy-mm-dd hh24:mi:ss'), ")
				 .append(mdrsl.getString("fahb_id")).append(", ").append(mdrsl.getString("gongysb_id")).append(", ").append(mdrsl.getString("meikxxb_id")).append(", ")
				 .append(getZafmcValue().getStrId()).append(", ")
				 .append(getSqlValue(mdrsl.getString("shuil"))).append(", ")
				 .append(getSqlValue(mdrsl.getString("shuik"))).append(", ")
				 .append(getSqlValue(mdrsl.getString("zafje"))).append(", ")
				 .append(getComboxValue(String.valueOf((getExtGrid().getColumn("shifds").combo).getBeanId(mdrsl.getString("shifds"))))).append(", '")
	...
	public void getSelectData() {
		...
		String sql = 
			...
			"       to_number('') as zafje,\n" +
			"       '' as beiz\n" +
			"  from fahb fh, gongysb gys, meikxxb mk, hetb ht, chezxxb cz, zhilb zl, hetzfb zf\n" + 
			...
			"   and not exists(select * from zafjsxxb js where js.fahb_id = fh.id and js.zafmc_item_id = zf.item_id)\n" + 
			"union\n" + 
			"select '' as fahb_id,\n" + 
			"       '合计' as fahdw,\n" + 
...
//		在第2列加入复选框
		...
		egu.addTbarText("-");
		egu.addToolbarButton("结算", GridButton.ButtonType_SubmitSel_condition, "SaveButton", 
