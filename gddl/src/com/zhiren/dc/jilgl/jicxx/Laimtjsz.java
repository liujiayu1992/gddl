...
public class Laimtjsz extends BasePage implements PageValidateListener {
	private String msg = "";
	...
	private void Save() {
		...
		while(rs.next()){
			if(rs.getInt("id") == 0){
				sql += "update meiltjkjb set zhuangt = 0,shixsj=sysdate where meikxxb_id=" 
				...
				+ " ;\n";
				sql += "insert into meiltjkjb(id,diancxxb_id,meikxxb_id,jihkjb_id,yunsfsb_id,tongjkj,lurry) values(getnewId("
				...
				sql += "update meiltjkjb set meikxxb_id=" 
					+getExtGrid().getColumn("meikxxb_id").combo.getBeanId(rs.getString("meikxxb_id")) 
					+",jihkjb_id ="
					+getExtGrid().getColumn("jihkjb_id").combo.getBeanId(rs.getString("jihkjb_id")) 
					+",yunsfsb_id ="
	...
	public void getSelectData() {
		...
		JDBCcon con = new JDBCcon();
		String sql = 
			"select t.id, t.diancxxb_id, m.mingc meikxxb_id,\n" +
			"j.mingc jihkjb_id, y.mingc yunsfsb_id, t.tongjkj, t.lurry\n" + 
			"  from meiltjkjb t, meikxxb m, yunsfsb y, jihkjb j\n" + 
			"where t.diancxxb_id = "+getTreeid()+" and t.zhuangt = 1"+
			" and t.meikxxb_id = m.id and t.jihkjb_id =j.id(+) and t.yunsfsb_id = y.id(+)";
