...
public class Niancgjhcx_hd  extends BasePage implements PageValidateListener {
	...
	private String getSelectData(){
		...
		if (getBaoblxValue().getValue().equals("分厂")){
			...
			if(jib==1){//选集团时刷新出所有的电厂
				...
				strwhere.append(filterDcid(visit,"dc"));//append(" and dc.id=").append(this.getTreeid()).
...
//			 ArrWidth=new int[] {80,45,40,40,40,40,45,40,40,40,45,40,40,40,45,40,40,40};
			...
			iFixedRows=1;
			strgrouping.append("decode(grouping(g.mingc),1,'全网合计',g.mingc) as meikdqmc,\n");
			strgrouping.append("decode(grouping(g.mingc)+grouping(m.mingc),2,'',1,'小计',m.mingc) as meikmc,\n");
			strgroupby.append("group by rollup(g.mingc,m.mingc)\n");
			strorderby.append("order by grouping(g.mingc) desc,g.mingc desc,grouping(m.mingc) desc,m.mingc desc\n");
			...
			ArrHeader=new String[3][19];
			 ArrHeader[0]=new String[] {"供货单位","煤矿单位","年度煤炭订货数量(单位:万吨)","年度煤炭订货数量(单位:万吨)","年度煤炭订货数量(单位:万吨)","年度煤炭订货数量(单位:万吨)","年度煤炭订货数量(单位:万吨)","年度煤炭订货数量(单位:万吨)","年度煤炭订货数量(单位:万吨)","年度煤炭订货数量(单位:万吨)","年度煤炭订货数量(单位:万吨)","年度煤炭订货数量(单位:万吨)","年度煤炭订货数量(单位:万吨)","年度煤炭订货数量(单位:万吨)","年度煤炭订货数量(单位:万吨)","年度煤炭订货数量(单位:万吨)","年度煤炭订货数量(单位:万吨)","年度煤炭订货数量(单位:万吨)","年度煤炭订货数量(单位:万吨)"};
			 ArrHeader[1]=new String[] {"供货单位","煤矿单位","全年合计","一季度","一季度","一季度","一季度","二季度","二季度","二季度","二季度","三季度","三季度","三季度","三季度","四季度","四季度","四季度","四季度"};
			 ArrHeader[2]=new String[] {"供货单位","煤矿单位","全年合计","一季度","一月","二月","三月","二季度","四月","五月","六月","三季度","七月","八月","九月","四季度","十月","十一月","十二月"};
			 //说明:如果报表不用设置页面距,横向打印正好答应完,ArrWidth的总宽度要小于971
			 ArrWidth=new int[] {120,100,55,55,48,48,48,55,48,48,48,55,48,48,48,55,48,48,48};
			...
			if(jib==1){//选集团时刷新出所有的电厂
				strgrouping.append("decode(grouping(dc.fgsmc)+grouping(dc.mingc),2,'总计',1,dc.fgsmc,'&nbsp;&nbsp;&nbsp;&nbsp;'||dc.mingc) as diancmc,\n");
				strgrouping.append("decode(grouping(dc.mingc)+grouping(g.mingc),2,'',1,'小计',g.mingc) as meikdqmc,\n");
				strgrouping.append("decode(grouping(g.mingc)+grouping(m.mingc),2,'',1,'小计',m.mingc) as meikmc,\n");
				strwhere.append("");
				strgroupby.append("group by rollup(dc.fgsmc,dc.mingc,g.mingc,m.mingc)\n");
				strhaving.append("");
				strorderby.append("order by grouping(dc.fgsmc) desc,dc.fgsmc desc,grouping(dc.mingc) desc,dc.mingc desc,grouping(g.mingc) desc,g.mingc,grouping(m.mingc) desc,m.mingc desc\n");
			}else if (jib==2){//选分公司的时候刷新出分公司下所有的电厂
				String ranlgs = "select id from diancxxb where shangjgsid= "+this.getTreeid();
				...
				try{
					...
					if(rl.next()){//燃料公司
						...
						strgrouping.append("decode(grouping(g.mingc)+grouping(m.mingc),2,'',1,'小计',m.mingc) as meikmc,\n");
						strwhere.append(" and (dc.fgsid="+this.getTreeid()+" or dc.rlgsid= "+this.getTreeid()+") \n");
						strgroupby.append("group by rollup(dc.rlgsmc,dc.fgsmc,dc.mingc,g.mingc,m.mingc)\n");
						strhaving.append("having not grouping(dc.rlgsmc)=1\n");
						strorderby.append("order by grouping(dc.rlgsmc) desc,dc.rlgsmc desc,grouping(dc.fgsmc) desc,dc.fgsmc desc,grouping(dc.mingc) desc,dc.mingc desc,grouping(g.mingc) desc,g.mingc,grouping(m.mingc) desc,m.mingc desc\n");
					}else{//分公司
						strgrouping.append("decode(grouping(dc.fgsmc)+grouping(dc.mingc),2,'总计',1,dc.fgsmc,'&nbsp;&nbsp;&nbsp;&nbsp;'||dc.mingc) as diancmc,\n");
						strgrouping.append("decode(grouping(dc.mingc)+grouping(g.mingc),2,'',1,'小计',g.mingc) as meikdqmc,\n");
						strgrouping.append("decode(grouping(g.mingc)+grouping(m.mingc),2,'',1,'小计',m.mingc) as meikmc,\n");;
				...
				strgrouping.append("decode(grouping(g.mingc)+grouping(m.mingc),2,'',1,'小计',m.mingc) as meikmc,\n");
				strwhere.append(filterDcid(visit,"dc"));//append(" and dc.id=").append(this.getTreeid()).
				strgroupby.append("group by rollup(dc.mingc,g.mingc,m.mingc)\n");
				strhaving.append("having not grouping(dc.mingc)=1\n");
				strorderby.append("order by grouping(dc.mingc) desc,dc.mingc desc,grouping(g.mingc) desc,g.mingc,grouping(m.mingc) desc,m.mingc desc\n");
			}
			
			 ArrHeader=new String[3][20];
			 ArrHeader[0]=new String[] {"电厂名称","供货单位","煤矿单位","年度煤炭订货数量(单位:万吨)","年度煤炭订货数量(单位:万吨)","年度煤炭订货数量(单位:万吨)","年度煤炭订货数量(单位:万吨)","年度煤炭订货数量(单位:万吨)","年度煤炭订货数量(单位:万吨)","年度煤炭订货数量(单位:万吨)","年度煤炭订货数量(单位:万吨)","年度煤炭订货数量(单位:万吨)","年度煤炭订货数量(单位:万吨)","年度煤炭订货数量(单位:万吨)","年度煤炭订货数量(单位:万吨)","年度煤炭订货数量(单位:万吨)","年度煤炭订货数量(单位:万吨)","年度煤炭订货数量(单位:万吨)","年度煤炭订货数量(单位:万吨)","年度煤炭订货数量(单位:万吨)"};
			 ArrHeader[1]=new String[] {"电厂名称","供货单位","煤矿单位","全年合计","一季度","一季度","一季度","一季度","二季度","二季度","二季度","二季度","三季度","三季度","三季度","三季度","四季度","四季度","四季度","四季度"};
			 ArrHeader[2]=new String[] {"电厂名称","供货单位","煤矿单位","全年合计","一季度","一月","二月","三月","二季度","四月","五月","六月","三季度","七月","八月","九月","四季度","十月","十一月","十二月"};

			 ArrWidth=new int[] {120,100,100,55,55,48,48,48,55,48,48,48,55,48,48,48,55,48,48,48};
			 ...
			 if(jib==1){//选集团时刷新出所有的电厂
				...
				strtiaoj="and dh.diancxxb_id=dc.id "+filterDcid(visit,"dc");//and dc.id=+this.getTreeid()
				strGongsID = "and n.diancxxb_id=d.id "+filterDcid(visit,"d"); //and d.id=+  this.getTreeid();
			 }
...
//				strgrouping.append("decode(grouping(dc.mingc)+grouping(g.mingc),2,'',1,'小计',g.mingc) as meikmc,\n");
				...
				strgrouping.append("decode(grouping(g.mingc),1,'总计',g.mingc) as meikdqmc,\n");
				strgrouping.append("decode(grouping(g.mingc)+grouping(m.mingc),2,'',1,'小计',m.mingc) as meikmc,\n");
				strgrouping.append("decode(grouping(m.mingc)+grouping(dc.fgsmc)+grouping(dc.mingc),3,'',2,'',1,dc.fgsmc||'小计',dc.mingc) as diancmc,\n");
				
				
				strwhere.append("");
				strgroupby.append("group by rollup(g.mingc,m.mingc,dc.fgsmc,dc.mingc)\n");
				strhaving.append("");
				strorderby.append("order by grouping(g.mingc) desc,g.mingc,grouping(m.mingc) desc,m.mingc,grouping(dc.fgsmc) desc,dc.fgsmc desc,grouping(dc.mingc) desc,dc.mingc desc\n");
			}else if (jib==2){//选分公司的时候刷新出分公司下所有的电厂
				String ranlgs = "select id from diancxxb where shangjgsid= "+this.getTreeid();
...
//						strgrouping.append("decode(grouping(dc.mingc)+grouping(g.mingc),2,'',1,'小计',g.mingc) as meikmc,\n");
						...
						strgrouping.append("decode(grouping(g.mingc),1,'总计',g.mingc) as meikdqmc,\n");
						strgrouping.append("decode(grouping(g.mingc)+grouping(m.mingc),2,'',1,'小计',m.mingc) as meikmc,\n");
						strgrouping.append("decode(grouping(m.mingc)+grouping(dc.rlgsmc)+grouping(dc.mingc),3,'',2,'',1,dc.rlgsmc||'小计',dc.mingc) as diancmc,\n");
						
						
						strwhere.append(" and (dc.fgsid="+this.getTreeid()+" or dc.rlgsid= "+this.getTreeid()+") \n");
						strgroupby.append("group by rollup(g.mingc,m.mingc,dc.rlgsmc,dc.fgsmc,dc.mingc)\n");
						strhaving.append("having not grouping(dc.rlgsmc)=1\n");
						strorderby.append("order by grouping(g.mingc) desc,g.mingc,grouping(m.mingc) desc,m.mingc,grouping(dc.rlgsmc) desc,dc.rlgsmc desc,grouping(dc.mingc) desc,dc.mingc desc\n");
					}else{//分公司
//						strgrouping.append("decode(grouping(dc.fgsmc)+grouping(dc.mingc),2,'总计',1,dc.fgsmc,'&nbsp;&nbsp;&nbsp;&nbsp;'||dc.mingc) as diancmc,\n");
//						strgrouping.append("decode(grouping(dc.mingc)+grouping(g.mingc),2,'',1,'小计',g.mingc) as meikmc,\n");
						...
						strgroupby.append("group by rollup(g.mingc,m.mingc,dc.fgsmc,dc.mingc)\n");
						strhaving.append("");
						strorderby.append("order by grouping(g.mingc) desc,g.mingc,grouping(m.mingc) desc,m.mingc,grouping(m.mingc) desc,m.mingc,grouping(dc.fgsmc) desc,dc.fgsmc desc,grouping(dc.mingc) desc,dc.mingc desc\n");
					}
					rl.close();
				...
				strgrouping.append("decode(grouping(g.mingc),1,'总计',g.mingc) as meikdqmc,\n");
						strgrouping.append("decode(grouping(g.mingc)+grouping(m.mingc),2,'',1,'小计',m.mingc) as meikmc,\n");
				strgrouping.append("decode(grouping(dc.mingc)+grouping(m.mingc),2,'',1,'小计',dc.mingc) as diancmc,\n");
				
				
				strwhere.append(filterDcid(visit,"dc"));//append(" and dc.id=").append(this.getTreeid()).
				strgroupby.append("group by rollup(g.mingc,m.mingc,dc.mingc)\n");
			 ...
			 ArrHeader=new String[3][20];
			 ArrHeader[0]=new String[] {"供货单位","煤矿单位","电厂名称","年度煤炭订货数量(单位:万吨)","年度煤炭订货数量(单位:万吨)","年度煤炭订货数量(单位:万吨)","年度煤炭订货数量(单位:万吨)","年度煤炭订货数量(单位:万吨)","年度煤炭订货数量(单位:万吨)","年度煤炭订货数量(单位:万吨)","年度煤炭订货数量(单位:万吨)","年度煤炭订货数量(单位:万吨)","年度煤炭订货数量(单位:万吨)","年度煤炭订货数量(单位:万吨)","年度煤炭订货数量(单位:万吨)","年度煤炭订货数量(单位:万吨)","年度煤炭订货数量(单位:万吨)","年度煤炭订货数量(单位:万吨)","年度煤炭订货数量(单位:万吨)","年度煤炭订货数量(单位:万吨)"};
			 ArrHeader[1]=new String[] {"供货单位","煤矿单位","电厂名称","全年合计","一季度","一季度","一季度","一季度","二季度","二季度","二季度","二季度","三季度","三季度","三季度","三季度","四季度","四季度","四季度","四季度"};
			 ArrHeader[2]=new String[] {"供货单位","煤矿单位","电厂名称","全年合计","一季度","一月","二月","三月","二季度","四月","五月","六月","三季度","七月","八月","九月","四季度","十月","十一月","十二月"};
		...
		if (!getBaoblxValue().getValue().equals("分矿百分比表")){
			...
			strSQL.append("from niancgjhb dh,vwdianc dc,gongysb g,meikxxb m\n");
			strSQL.append("where dh.diancxxb_id=dc.id(+) and dh.gongysb_id=g.id(+) and dh.meikxxb_id=m.id(+) and to_char(dh.riq,'yyyy')='"+ intyear+"' \n");
		...
		rt.setDefaultTitle(16, 3, "单位:万吨", Table.ALIGN_RIGHT);
		// 数据
		...
		rt.setDefautlFooter(16,3,"打印日期:"+DateUtil.FormatDate(new Date()),Table.ALIGN_RIGHT);
