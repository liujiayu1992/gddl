 ...
 * 时间：2013年06月29日
 * 作者：李凯洋
 * 修改内容：加权硫分图表  指标值使用 收到基硫分st,ar
 * 
 */
public class Yuedstd_char  extends BasePage implements PageValidateListener {
	...
	public String getPrintChar(){
		...
		String Sql=
			...
			"                    0,\n" + 
			"                    0,\n" + 
			"                    SUM(SL.JINGZ * round_new(z.stad*(100-z.mt)/(100-z.mad),2)) / SUM(SL.JINGZ)),\n" + 
			"             2) STAR\n" + 
		...
		CategoryDataset dataset = cd.getRsDataChart(rstmp, "RIQ","FENX","STAR");//rs记录集构造生成图片需要的数据
...
//		柱子上字的倾斜度。0:水平
		...
		return ct.ChartBar3D(getPage(), dataset,  getYearValue().getValue()+"年入厂加权硫份（St,ar）",  900,  400);
	}
