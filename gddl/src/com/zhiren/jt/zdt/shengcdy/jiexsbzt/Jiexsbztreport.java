...
/*作者:ll
 * 时间:2009-8-13 
 * 说明:增加接卸设备状态下拉框，分别查询不同状态：运行、检修、异常等设备状态
 */
/*作者:ll
 * 时间:2009-8-19 
 * 说明:修改查询sql.
...
//	private Date _BeginriqValue = DateUtil.AddDate(new Date(), -1, DateUtil.AddType_intDay);
	...
	public String getJiexsbztreport(){
		...
		String sql = "select a.diancmc,a.shebmc,a.bianh,a.shebzt,a.kaisrq,a.jiesrq,a.shuom \n"
			+ "from (select dc.mingc as diancmc,dc.shebmc,dc.bianh,decode(yx.shebzt,null,'运行','好用','运行',yx.shebzt) as shebzt,\n"
			+ "       case when shebzt like '%修%' then to_char(kaisrq, 'yyyy-mm-dd hh24') else '' end as kaisrq,   \n"//decode(shebzt,'运行','',to_char(kaisrq,'yyyy-mm-dd hh24')) as kaisrq,
			+ "       case when shebzt like '%修%' then to_char(jiesrq, 'yyyy-mm-dd hh24') else '' end as jiesrq,   \n"//decode(shebzt,'运行','',to_char(jiesrq,'yyyy-mm-dd hh24')) as jiesrq,
			+ "			yx.shuom \n"
		...
		ArrHeader[0]=new String[] {"单位","设备名称","设备编号","设备状态","开始时间","结束时间","说明"};
