 ...
 * 2009-05-21
 */
public class Yunftjtz  extends BasePage implements PageValidateListener{
	...
	private static final String BAOBPZB_GUANJZ = "YUNFTJTZ";// baobpzb中对应的关键字
...
//		得到年、月、日
		...
		String sql_mx = 
			...
			"       j.ches,\n" + 
			"       j.yunj,\n" + 
			"		decode(j.jiessl,0,0,decode(j.yunj,0,0,round_new(j.hansyf/j.jiessl/j.yunj,2))) as yunjl,\n" +
			"       decode(j.jiessl,0,0,round_new(j.hansyf/j.jiessl,2)) as shijyj,\n" + 
			"       j.buhsyf as shijyf,\n" + 
			"       j.shuik as shuij,\n" + 
			"       j.hansyf as hej\n" + 
			"from jiesyfb j,gongysb g,meikxxb m,hetys h,yunsdwb y\n" + 
		...
		String sql = 
			...
			"       '' as yunj,\n" + 
			"       0 as yunjl,\n" + 
			"       decode(sum(j.jiesml),null,0,round_new(sum(j.shijyj*j.jiesml)/sum(j.jiesml),0)) as shijyj,\n" + 
			"       sum(j.shijyf) as shijyf,\n" + 
			...
			")\n" + 
			") x\n" + 
			"order by decode(x.gongys,'合计',0,1) desc,x.gongys,x.kuangm,x.chengydw\n";
		...
		if (rsl.getRows() > 0) {
			...
			rt.setDefaultTitle(1, 11, title_bt, Table.ALIGN_CENTER);
			rsl.close();
		} else {
			rs = rstmp;
			
			ArrHeader =new String[1][11];
			ArrHeader[0]=new String[] {"供应商","矿名","承运单位<br> (名称)","结算煤量<br> (吨)","车数<br> (辆)","运距<br> (公里)","运价率<br> (元/吨)","实际运价<br> (元)","实际运费<br> (元)","税金<br> (元)","合计<br> (元)"};
			
			ArrWidth = new int[] {150,150,90,75,75,75,75,75,75,75,90};

			rt.setTitle("汽车小窑煤运费结算表", ArrWidth);
			rt.setDefaultTitle(1, 11, title_bt, Table.ALIGN_CENTER);
		}
...
//	***************************报表初始设置***************************//
	...
	public void getToolBars() {
		...
		String gongys_sql = 
			...
			"          from gongysb gys, gongysdcglb gysgl,jiesyfb j\n" + 
			...
			"          from meikxxb m, gongysmkglb glb, gongysb gys, gongysdcglb glbdc,jiesyfb j\n" + 
