...
//	界面用户提示
	...
	private void Save() {
		...
		while(rsl.next()) {
			id=rsl.getLong("id");
			if(id==0){
				sb.append("insert into kaohbzb (id,kaohxm_id,shangx,xiax,danw,fangs,chaocjz,biaoz,beiz)");
				...
				sb.append(rsl.getDouble("xiax")).append(",'").append(rsl.getString("danw")).append("','").append(rsl.getString("fangs")).append("',").append(rsl.getDouble("chaocjz")).append(",").append(rsl.getDouble("biaoz")).append(",'").append(rsl.getString("beiz")).append("');\n") ;
			}else{
				...
				sb.append(rsl.getDouble("shangx")).append(",xiax=").append(rsl.getDouble("xiax")).append(",danw='").append(rsl.getString("danw")).append("'").append(",fangs='").append(rsl.getString("fangs")).append("'").append(",chaocjz=").append(rsl.getDouble("chaocjz")).append(",biaoz=").append(rsl.getDouble("biaoz")).append(",beiz='").append(rsl.getString("beiz")).append("'");
	...
	public void getSelectData() {
		...
		sbsql.append("select k.id,i.mingc,k.shangx,k.xiax,danw,fangs,k.chaocjz,k.biaoz,k.leib,k.beiz ");
		...
		egu.getColumn("danw").setWidth(50);
		egu.getColumn("fangs").setHeader("增减方式");
		egu.getColumn("fangs").setWidth(70);
...
//		 设置项目下拉框
		...
		egu.getColumn("danw").setEditor(c2);
		c2.setEditable(true);
		String Sql1 = " select 1 as id ,'Mj/kg' as mingc from dual union select 2 as id ,'Kcal/Kg' as mingc from dual union select 3 as id ,'%' as mingc from dual";
		egu.getColumn("danw").setComboEditor(egu.gridId,new IDropDownModel(Sql1));
		ComboBox c3 = new ComboBox();
		egu.getColumn("fangs").setEditor(c3);
		c3.setEditable(true);
		String Sql2 = " select 1 as id ,nvl('每升高',0) as mingc from dual union select 2 as id ,nvl('每降低',0) as mingc from dual ";
		egu.getColumn("fangs").setComboEditor(egu.gridId,new IDropDownModel(Sql2));
