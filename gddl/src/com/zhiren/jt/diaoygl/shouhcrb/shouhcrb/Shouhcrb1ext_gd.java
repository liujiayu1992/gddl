 ...
 * 说明:修改页面显示内容的排列顺序
 */
public  class Shouhcrb1ext_gd extends BasePage implements PageValidateListener {
...
//		    "      sum(nvl(dt.kuc,0)) as kuc,\n" + 
		    "      sum(nvl(dt.bukdml,0)) as bukdml,\n" + 		    
		    "      sum(nvl(dt.kedkc,0)) as kedkc,\n" +
		    "      sum(nvl(dt.changwml,0)) as changwml,"+
...
//				---------------修改后-------------------------------------
				...
				egu.getColumn("kuc").setHeader("库存(t)");
				egu.getColumn("kuc").setEditor(null);
				egu.getColumn("bukdml").setHeader("不可调煤量(t)");
				egu.getColumn("kedkc").setHeader("可调库存(t)");
				egu.getColumn("kedkc").setEditor(null);
				egu.getColumn("changwml").setHeader("厂外存煤(t)");
				egu.getColumn("quemtjts").setHeader("缺煤停机台数(台)");
				egu.getColumn("quemtjrl").setHeader("缺煤停机容量(万千瓦)");
				egu.getColumn("fadl").setHeader("发电量(万kWh)");		
				egu.getColumn("fady").setHeader("发电耗用(t)");
		...
		egu.getColumn("kedkc").setWidth(70);
		egu.getColumn("changwml").setWidth(70);	
		...
		egu.getColumn("kedkc").setRenderer("function(value,metadata){metadata.css='tdTextext'; return value;}");
		...
		sb.append("if(e.field=='BUKDML'){ \n");
		sb.append("rec.set('BUKDML', eval(rec.get('BUKDML')||0) + eval(e.value||0) - eval(e.originalValue||0)); \n");
		sb.append("rec.set('KEDKC', eval(rec.get('KEDKC')||0) + eval(e.value||0) - eval(e.originalValue||0));} \n");
...
//			可调库存=账面库存-不可调煤量
			sb2.append("e.record.set('KEDKC',parseFloat(e.record.get('KUC')==''?0:e.record.get('KUC'))-parseFloat(e.record.get('BUKDML')==''?0:e.record.get('BUKDML')));");
            ...
            //不可调煤量发生变化时，合计行的(不可调煤量，可调库存)发生变化
 			
 			sb2.append("if(e.field=='BUKDML'){").append("rec.set('BUKDML', eval(rec.get('BUKDML')||0) + eval(e.value||0) - eval(e.originalValue||0));").append("rec.set('KEDKC', eval(rec.get('KEDKC')||0) + eval(e.value||0) - eval(e.originalValue||0));").append("}");
 	        //
 			
 			sb2.append("if(e.field=='CHANGWCML'){").append("rec.set('CHANGWCML', eval(rec.get('CHANGWCML')||0) + eval(e.value||0) - eval(e.originalValue||0));").append("}");
