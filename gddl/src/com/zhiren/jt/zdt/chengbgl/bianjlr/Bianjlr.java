package com.zhiren.jt.zdt.chengbgl.bianjlr;
...
* 		   
*/ 
public class Bianjlr  extends BasePage implements PageValidateListener{
...
//******************页面初始设置********************//
	...
	private String getSelectData(){
		...
		if(jib==1){//选集团时刷新出所有的电厂
			...
			notHuiz=" having not grouping(gs.mingc)=1 ";//当电厂树是分公司时,去掉集团汇总
		...
		if(getBaoblxValue().getValue().equals("本月")){
		  ...
		  strSQL = 
			  "select  decode(grouping(gs.mingc)+grouping(dc.mingc),2,'总计',1,'&nbsp;&nbsp;'||gs.mingc,'&nbsp;&nbsp;&nbsp;&nbsp;'||dc.mingc) as mingc\n" +
			  ...
			  "        ,decode(sum(l.fadhbzml+l.gongrhbzml),0,0,round(sum((l.fadhbzml+l.gongrhbzml)*fadzhbmdj)/sum(l.fadhbzml+l.gongrhbzml),2)) as fadzhbmdj\n" + 
			  ...
			  "          /decode(sum(l.gongdl),0,0,round(sum(l.gongdl*l.gongdlbzmh)/sum(l.gongdl),2))*1000)),2)  as fadzhbmdj_yg \n" + 
			  "       ,(round(decode(decode(sum(l.gongdl),0,0,round(sum(l.gongdl*l.gongdlbzmh)/sum(l.gongdl),2)),0,0,\n" + 
			  "       ((decode(sum(l.shoudl),0,0,round(sum(l.shoudl*l.shouddj)/sum(l.shoudl),2))\n" + 
			  "       -(decode(sum(l.shoudl),0,0,round(sum(l.shoudl*l.shouddwbdcb)/sum(l.shoudl),2))\n" + 
			  "          -decode(sum(l.shoudl),0,0,round(sum(l.shoudl*l.shouddwrlcb)/sum(l.shoudl),2))))\n" + 
			  "          /decode(sum(l.gongdl),0,0,round(sum(l.gongdl*l.gongdlbzmh)/sum(l.gongdl),2))*1000)),2)-"+getRulqqtfy()+")  as rucbmdj_yg \n"+
			  "from\n" + 
			  "(\n" + 
			  " select yzb.diancxxb_id as id\n" + 
//			  "       ,yzb.shoudl\n" + 
//			  "       ,yzb.gongdl\n" + 
//			  "       ,yzb.fadhbzml\n" + 
//			  "       ,yzb.fadhyzbzml\n" + 
...
//			  "       ,rcmb.laimsl\n" + 
//			  "       ,rcmb.buhsbmdj\n" + 
//			  "       ,(decode(yzb.fadhbzml,0,0,yzb.fadmcb/yzb.fadhbzml)+decode(yzb.fadhyzbzml,0,0,yzb.fadycb/yzb.fadhyzbzml)) as fadzhbmdj\n" + 
//			  "       ,yzb.shouddwbdcb\n" + 
//			  "       ,yzb.shouddwrlcb\n" + 
//			  "       ,yzb.shouddwgdcb\n" + 
//			  "       ,yzb.shouddj\n" + 
//			  "       ,(yzb.shouddj-yzb.shouddwbdcb) as bianjlr\n" + 
//			  "       ,yzb.lirze\n" + 
//			  "       ,round(decode(yzb.gongdlbzmh,0,0,((yzb.shouddj-(yzb.shouddwbdcb-yzb.shouddwrlcb))/yzb.gongdlbzmh*1000)),2) as fadzhbmdj_yg\n" + 
//			  "       ,round((decode(yzb.gongdlbzmh,0,0,((yzb.shouddj-(yzb.shouddwbdcb-yzb.shouddwrlcb))/yzb.gongdlbzmh*1000))-15),2) as rucbmdj_yg\n" + 
//				-------------------------------修改调整公式后yuezbb中的字段------------------------------------	
			  "      ,yzb.shoudl\n" +
			  ...
			  "      ,rcmb.laimsl\n" + 
			  "      ,rcmb.buhsbmdj\n" + 
			  "      ,(decode(yzb.fadmzbml,0,0,yzb.fadmcb*10000/yzb.fadmzbml)+decode(yzb.fadyzbzml,0,0,yzb.fadycb*10000/yzb.fadyzbzml)) as fadzhbmdj\n" + 
			  "      ,yzb.shouddwbdcb\n" + 
			  "      ,yzb.shouddwrlcb_meid+yzb.shouddwrlcb_qid as shouddwrlcb\n" + 
			  "      ,yzb.shouddwgdcb\n" + 
			  "      ,yzb.shouddj\n" + 
			  "      ,(yzb.shouddj-yzb.shouddwbdcb) as bianjlr\n" + 
			  "      ,yzb.lirze\n" + 
			  "      ,round(decode(yzb.gongdbzmh,0,0,((yzb.shouddj-(yzb.shouddwbdcb-(yzb.shouddwrlcb_meid+yzb.shouddwrlcb_qid)))/yzb.gongdbzmh*1000)),2) as fadzhbmdj_yg\n" + 
			  "      ,round((decode(yzb.gongdbzmh,0,0,((yzb.shouddj-(yzb.shouddwbdcb-(yzb.shouddwrlcb_meid+yzb.shouddwrlcb_qid)))/yzb.gongdbzmh*1000))-15),2) as rucbmdj_yg\n "+
//			---------------------------------------------------------------------------------------------
			  "from yuezbb yzb,diancxxb dc\n" + 
			  ...
			  "       )rcmb\n" + 
//			  "where yzb.diancxxb_id=dc.id  and yzb.diancxxb_id=rcmb.diancxxbid(+)\n" + 
			  "where yzb.diancxxb_id=dc.id  and yzb.diancxxb_id=rcmb.diancxxbid\n" + 
			  "      and yzb.riq=to_date('"+intyear+"-"+intMonth+"-01','yyyy-mm-dd') \n" + 
			  "       "+strGongsID+"and yzb.fenx='"+fenx+"' \n" + 
...
// 直属分厂汇总
				 ...
				 ArrHeader[0]=new String[] {"单位名称","售电量（万千瓦时）","实际完成供电煤耗（克/千瓦时）","入厂标煤单价（不含税，元/吨）","发电综合标煤单价（不含税，元/吨）","售电单位变动成本（元/千千瓦时）","其中：售电单位燃料成本（元/千千瓦时）","售电单位固定成本（元/千千瓦时）","售电单价（不含税，元/千千瓦时）","边际利润（元/千千瓦时）","利润总额（万元）","基于边际利润为0时的发电综合标煤单价预估（元/吨）","基于边际利润为0时的入厂标煤单价预估（元/吨）"};
...
//					  "        ,decode(sum(l.gongdl),0,0,round(sum(l.gongdl*l.gongdlbzmh)/sum(l.gongdl),2)) as gongdlbzmh\n" + 
//					  "        ,decode(sum(l.laimsl),0,0,round(sum(l.laimsl*l.buhsbmdj)/sum(l.laimsl),2)) as rucbmdj\n" + 
//					  "        ,decode(sum(l.fadhbzml+l.gongdlbzmh),0,0,round(sum((l.fadhbzml+l.gongdlbzmh)*fadzhbmdj)/sum(l.fadhbzml+l.gongdlbzmh),2)) as fadzhbmdj\n" + 
//					  "        ,decode(sum(l.shoudl),0,0,round(sum(l.shoudl*l.shouddwbdcb)/sum(l.shoudl),2)) as shouddwbdcb\n" + 
//					  "        ,decode(sum(l.shoudl),0,0,round(sum(l.shoudl*l.shouddwrlcb)/sum(l.shoudl),2)) as shouddwrlcb\n" + 
//					  "        ,decode(sum(l.shoudl),0,0,round(sum(l.shoudl*l.shouddwgdcb)/sum(l.shoudl),2)) as shouddwgdcb\n" + 
//					  "        ,decode(sum(l.shoudl),0,0,round(sum(l.shoudl*l.shouddj)/sum(l.shoudl),2)) as shouddj\n" + 
...
//					  "     ,(select tj.diancxxb_id as diancxxbid,sl.laimsl,rcm.buhsbmdj from yuercbmdj rcm,yuetjkjb tj,yueslb sl,diancxxb dc\n" + 
//					  "      where sl.yuetjkjb_id=tj.id and rcm.yuetjkjb_id=tj.id and tj.riq=to_date('"+intyear+"-"+intMonth+"-01','yyyy-mm-dd')\n" + 
//					  "            and rcm.fenx='累计' and sl.fenx='累计' and tj.diancxxb_id=dc.id  "+strGongsID+"\n" + 
//					  "       )rcmb\n" + 
////					  "where yzb.diancxxb_id=dc.id  and yzb.diancxxb_id=rcmb.diancxxbid(+)\n" + 
//					  "where yzb.diancxxb_id=dc.id  and yzb.diancxxb_id=rcmb.diancxxbid\n" + 
//					  "      and yzb.riq=to_date('"+intyear+"-"+intMonth+"-01','yyyy-mm-dd') \n" + 
//					  "       "+strGongsID+"and yzb.fenx='累计' \n" + 
//					  "      )l,diancxxb dc,diancxxb gs\n" + 
//					  "      where l.id=dc.id and gs.id=dc.fuid\n" + 
//					  "      group by rollup(gs.mingc,dc.mingc)\n" + 
//					  "      order by grouping(gs.mingc)desc,grouping(dc.mingc) desc";
