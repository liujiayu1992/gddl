...
public class Youg extends BasePage implements PageValidateListener{
	...
	public void save() {
		...
		if (rsl == null) {
			WriteLog.writeErrorLog(ErrorMessage.NullResult 
					+ "yougb.Save 中 getExtGrid().getDeleteResultSet(getChange()) 方法没有正确返回记录集。");
		...
		if(rsl == null) {
			WriteLog.writeErrorLog(ErrorMessage.NullResult 
					+ "yougb.Save 中 getExtGrid().getModifyResultSet(getChange()) 方法没有正确返回记录集。");
			setMsg(ErrorMessage.NullResult);
			return;
		}
		while (rsl.next()) {
