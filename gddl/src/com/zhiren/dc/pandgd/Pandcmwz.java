...
public class Pandcmwz extends BasePage implements PageValidateListener {
//	界面用户提示
	private String msg="";
...
//	 页面变化记录
	...
	public void save() {
		...
		while (rsl.next()) {
			id = rsl.getLong("id");
			sSql = "delete from pandgdcmwz where id=" + id;
		...
		while (rsl.next()) {
			id = rsl.getLong("id");
			if (id == 0) {
				sSql = "insert into pandgdcmwz values(getNewId(" + visit.getDiancxxb_id() + "),"
	...
	public void getSelectData() {
		...
		String sSql = "select y.id,dc.mingc as diancxxb_id,y.xuh,y.bianm,y.mingc,y.piny,y.beiz\n"
				+ "from pandgdcmwz y,diancxxb dc where y.diancxxb_id=dc.id\n"
