...
//	***************设置消息框******************//
	...
	private String getSelectData(){
		...
		sbsql.append("AND dz.leib = '车站' AND dz.diancxxb_id = dc.ID)) AS daoz,\n");
		sbsql.append("DECODE ((SELECT cz.quanc FROM chezxxb cz, diancdzb dz\n");
		sbsql.append(" WHERE cz.ID = dz.chezxxb_id AND dz.leib = '港口' AND dz.diancxxb_id = dc.ID),NULL, '',\n");
		sbsql.append("(SELECT cz.quanc FROM chezxxb cz, diancdzb dz WHERE cz.ID = dz.chezxxb_id AND dz.leib = '港口'\n");
		...
		//定义表头数据
		 ...
		 ArrHeader[0]=new String[] {"编码","名称","全称","省份","上级公司","到站","到港","电厂类别"};
...
//		rt.body.mergeFixedCol(1);
		...
		rt.setDefautlFooter(1,8,"打印日期:"+DateUtil.FormatDate(new Date()),Table.ALIGN_RIGHT);
