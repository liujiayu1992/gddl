...
public class Baobpz extends BasePage implements PageValidateListener {
//	进行页面提示信息的设置
	private String _msg;
...
//	保存分组的改动
	...
	public void Save1(String strchange,Visit visit) {
		...
		while(delrsl.next()) {
			...
			sql.append("delete from ").append("baobpzzb").append(" where baobpzb_id =").append(delrsl.getString(0)).append(";\n");
		}
...
//		System.out.println(sql.toString());
	...
	public void getSelectData() {
		// Visit visit = (Visit) getPage().getVisit();
		JDBCcon con = new JDBCcon();
		ResultSetList rsl = con.getResultSetList("select id,guanjz,biaot from Baobpzb");

		
		ExtGridUtil egu = new ExtGridUtil("gridDiv", rsl);
		egu.getColumn("id").setHeader("ID");
		egu.getColumn("id").setHidden(true);
		egu.getColumn("guanjz").setHeader("报表关键字");		
		...
		sb.append("{text:' 添加',icon:'imgs/btnicon/insert.gif',cls:'x-btn-text-icon',minWidth:75,handler:function() {	\n")
			...
			.append("var plant = new gridDiv_plant({ID: '0',GUANJZ: '',BIAOT: ''});	\n")
