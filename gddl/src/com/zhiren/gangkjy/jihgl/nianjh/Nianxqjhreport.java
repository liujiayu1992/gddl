...
//	界面用户提示
	...
	private String REPORT_NAME_REZC = "nianxqjhreport";
...
//		String riq = getNianfValue().getValue() + "-01-01";
		...
		StringBuffer sb = new StringBuffer();
		sb.append("select decode(grouping(d.mingc),1,'总计',d.mingc) as xuqdc_id,\n")
		.append("sum(n.qickc) as qickc,\n")
		.append("sum(n.jihdl) as jihdl,\n")
		.append("sum(n.jihhyl) as jihhyl,\n")
		.append("sum(n.qimkc) as qimkc,\n")
		.append("sum(n.xuql) as xuql,\n")
		.append("n.beiz      as beiz\n")
		.append("from nianxqjhb n, vwxuqdw d\n")
		
		.append("where n.xuqdc_id=d.id\n")
		.append("and n.diancxxb_id="+ visit.getDiancxxb_id()+"\n")
		.append("and riq = "+ CurrentDate+"\n")
		.append("group by rollup(d.mingc,n.beiz)\n")
		.append("having grouping(d.mingc) = 1 or grouping(n.beiz) = 0\n")
		.append("order by d.mingc desc");
...
//		String riq = getNianfValue().getValue() + " 年 " ;
		...
		ArrHeader[0]=new String[] {Local.xuqdw,Local.qickc,Local.jihdl,Local.jihhyl,Local.qimkc,Local.xuql,Local.beiz};
		
		int ArrWidth[]=new int[] {120,120,120,120,120,120,150};
		
		rt.setTitle("年需求计划", ArrWidth);
		...
		rt.setDefaultTitle(1, 3, "制表单位：" + ((Visit) this.getPage().getVisit()).getDiancqc(),
				Table.ALIGN_LEFT);
		rt.setDefaultTitle(4, 2, "报表年份：" + getNianfValue().getValue() + " 年 ",
				Table.ALIGN_LEFT);
