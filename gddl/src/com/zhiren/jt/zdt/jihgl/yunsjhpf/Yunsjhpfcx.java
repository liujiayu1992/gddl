...
public class Yunsjhpfcx extends BasePage {
	...
	public String getPrintTable() {
		...
		if (mstrReportName.equals(RT_DR1)) {// 运输计划提报
			leib = 1;
			tablename = "yunsjhtbb";
			title = "运输计划提报查询";
			return getSelectData(title, tablename, leib);
		} else if (mstrReportName.equals(RT_DR2)) {// 运输计划补充提报
			leib = 0;
			tablename = "yunsjhtbb";
			title = "运输计划补充提报查询";
			return getSelectData(title, tablename, leib);
		} else if (mstrReportName.equals(RT_DR3)) {// 运输计划批复
			leib = 1;
			tablename = "yunsjhb";
			title = "运输计划批复查询";
			return getSelectData(title, tablename, leib);
		} else if (mstrReportName.equals(RT_DR4)) {// 运输计划补充批复
			leib = 0;
			tablename = "yunsjhb";
			title = "运输计划补充批复查询";
			return getSelectData(title, tablename, leib);
...
//		str_SQL.append(" where dc.fuid = gs.fengsid or dc.shangjgsid=gs.fengsid) dianc \n");
	...
	private String getSelectData(String title, String tablename, long leix) {
		...
		if(getLeixSelectValue().getValue().equals("分厂")){
			...
			ArrWidth = new int[] { 120, 65, 65, 65, 65, 65, 65,  65, 80 };
		}else if(getLeixSelectValue().getValue().equals("分矿")){
			ArrHeader = new String[1][9];
			ArrHeader[0] = new String[] { "单位", "发站(港)", "到站(港)", "品种", "车数",
					"吨数", "批复车数", "批复吨数", "备注" };//"批准号",

			ArrWidth = new int[] { 150, 65, 65, 65, 65, 65, 65,  65, 80 };
		}else if(getLeixSelectValue().getValue().equals("分厂分矿")){
			ArrHeader = new String[1][10];
			ArrHeader[0] = new String[] { "单位","供货单位", "发站(港)", "到站(港)", "品种", "车数",
					"吨数", "批复车数", "批复吨数", "备注" };//"批准号",

			ArrWidth = new int[] { 120,150, 65, 65, 55, 55, 55, 55, 55, 80 };
		}else if(getLeixSelectValue().getValue().equals("分矿分厂")){
			ArrHeader = new String[1][10];
			ArrHeader[0] = new String[] { "供货单位","单位", "发站(港)", "到站(港)", "品种", "车数",
					"吨数", "批复车数", "批复吨数", "备注" };//"批准号",

			ArrWidth = new int[] { 150,120, 65, 65, 55, 55, 55, 55, 55, 80 };
		}
	}
		if (getLeixSelectValue() != null) {
			where = " and y.diancxxb_id=diancid and y.gongysb_id=g.id and y.pinm=pz.id(+) "
					+ "and y.faz_id=cz.id(+) and y.leix="
					+ leix
		...
		rt.setDefaultTitle(1, 3,""+ ((Visit) getPage().getVisit()).getDiancqc(),Table.ALIGN_LEFT);
		rt.setDefaultTitle(4, 2,date, Table.ALIGN_LEFT);
...
//		增加长度的拉伸
		...
		rt.setDefautlFooter(1, 2, "制表时间:" + DateUtil.FormatDate(new Date()),
				Table.ALIGN_LEFT);
		rt.setDefautlFooter(3,2,"审批:",Table.ALIGN_CENTER);
		
		if(((Visit) getPage().getVisit()).getDiancmc().equals("河北大唐")){
			
		rt.setDefautlFooter(9, 1, "制表:",Table.ALIGN_RIGHT);
		}else{
			
		rt.setDefautlFooter(9, 1, "制表:"+((Visit) getPage().getVisit()).getDiancmc(),Table.ALIGN_RIGHT);
		}
	...
	public void getGongysDropDownModels() {
		String sql = "select id,mingc\n" + "from gongysb\n"
			+"where gongysb.leix<>0 and zhuangt=1";
