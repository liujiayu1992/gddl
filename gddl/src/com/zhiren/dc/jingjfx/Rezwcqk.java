...
public class Rezwcqk extends BasePage implements PageValidateListener {
//	界面用户提示
	private String msg="";
	...
	//删除
		...
		con.getDelete(strSql);
		strSql=
		"insert into REZWCQKFKB(id,riq,gongysb_id,rucrz,qunpjrz,hetrz,jiesrz,diancxxb_id)(\n" + 
		"select "+MainGlobal.getNewID(visit.getDiancxxb_id())+",sysdate,5,20000,20000,0,222,"+visit.getDiancxxb_id()+" from dual\n" + 
		"\n" + 
		")";
	...
	public void getSelectData() {
		...
		strSql=
			...
			"select REZWCQKFKB.id,riq,gongysb.mingc  gongysb_id,rucrz,qunpjrz,hetrz,jiesrz\n" +
			"from REZWCQKFKB,gongysb\n" + 
			"where REZWCQKFKB.gongysb_id=gongysb.id and REZWCQKFKB.diancxxb_id="+visit.getDiancxxb_id()+" and to_char(riq,'YYYYMM')='"+getNianf()+getYuef()+"'";
		JDBCcon con = new JDBCcon();
...
//		egu.setHeight("bodyHeight");
		...
		egu.getColumn("rucrz").setHeader("计划量(吨)");
		egu.getColumn("rucrz").setWidth(120);
		egu.getColumn("qunpjrz").setHeader("到货量(吨)");
		egu.getColumn("qunpjrz").setWidth(120);
		egu.getColumn("hetrz").setHeader("年合同量(吨)");
		egu.getColumn("hetrz").setWidth(120);
		egu.getColumn("jiesrz").setHeader("结算热值(千焦/千克)");
