...
public class Hetlwcqkfk extends BasePage implements PageValidateListener {
//	界面用户提示
	private String msg="";
	...
	//删除
		...
		strSql=
		"insert into HETLWCQKFKB(id,riq,gongysb_id,jihl,daohl,nianhtl,diancxxb_id)(\n" + 
		"select "+MainGlobal.getNewID(visit.getDiancxxb_id())+",sysdate,5,20000,20000,0,"+visit.getDiancxxb_id()+" from dual\n" + 
		"\n" + 
		")";
	...
	public void getSelectData() {
		...
		String strSql="";
		strSql=
			"select HETLWCQKFKB.id,riq,gongysb.mingc gongysb_id,jihl,daohl,nianhtl\n" +
			"from HETLWCQKFKB,gongysb\n" + 
			" where HETLWCQKFKB.gongysb_id=gongysb.id and  to_char(riq,'YYYYMM')='"+getNianf()+getYuef()+"' and diancxxb_id="+visit.getDiancxxb_id();
		JDBCcon con = new JDBCcon();
...
//		egu.setHeight("bodyHeight");
		...
		egu.getColumn("jihl").setHeader("计划量(吨)");
		egu.getColumn("jihl").setWidth(120);
		egu.getColumn("daohl").setHeader("到货量(吨)");
		egu.getColumn("daohl").setWidth(120);
		egu.getColumn("nianhtl").setHeader("年合同量(吨)");
		egu.getColumn("nianhtl").setWidth(120);
