 ...
 * 作者：赵胜男
 * 时间：2012-09-07
 * 描述：修改08-1表显示总厂合计 MonthReport&lx=diaor08b-1
 */
public class MonthReport_dk extends BasePage implements PageValidateListener {
...
//				+ "               /sum(decode(j.jiesl,0,1,j.jiesl)),2) as bhsbmj,\n"
				...
				"                         and (diancxxb_id =" + getTreeid() + " or diancxxb_id in (select id from diancxxb where fuid=" + getTreeid() + " )) \n" + 
				"                      union\n" + 
				"                      select yuetjkjb.id,yunsfsb_id, gongysb_id, diancxxb_id, '累计' as fenx, JIHKJB_ID, DQID, DQMC\n" + 
				"                        from yuetjkjb, VWGONGYSDQ DQ\n" + 
				"                       where riq = to_date('" + strDate + "','yyyy-mm-dd')\n" + 
				"                         AND YUETJKJB.GONGYSB_ID = DQ.ID\n" + 
				"                        and (diancxxb_id =" + getTreeid() + " or diancxxb_id in (select id from diancxxb where fuid=" + getTreeid() + " )) ) tj,\n" + 
				...
				" having not (grouping(j.fenx)=1 AND GROUPING(gongysb.mingc)=1 )\n" + 
