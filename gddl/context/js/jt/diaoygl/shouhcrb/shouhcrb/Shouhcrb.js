...
SelectDataWidth=tablemainWidth+13 ;
//QueryFrameClsssend
/*
...
SelectHeadDiv.style.width=SelectDataWidth-10-10;
...
SelectDataDiv.style.height=SelectDataHeight;
...
function RealDelSel(_str1,_str2){
	...
	var falge=0;
	var sizedelet = oData.rows.length+1;
	if(document.all.item("Jib").value!=3){
		for(var i=0; i < sizedelet-1;i++){
			if(document.all.item("Select",i).checked){
				falge=1;
			}else{
				continue;
		...
		if(falge==0){
			...
			if(_str2=="Tijiao"){
				...
				if(document.all.item("wait").style.display=="none"){
					var paraId = "";
					var size = oData.rows.length+1;
			
					for(var i=0; i < size-1;i++){
						if(document.all.item("Select",i).checked){
								_danwname ="单位名称：";
							danwmc = danwmc + " 序号(" + document.all.item("XUH",i).value+")-" 
							+ document.all.item("DIANCHANG_ID",i).value+"-" 
							+ document.all.item("MEIKUANG_ID",i).value +"\n";;
						}
					}
					if(confirm('您选择了：\n\n' +_danwname+'\n' + danwmc + '\n\n执行：' +  _name  )){
						//Showwait();
...
function createTabIndex(){
	j=0;
    for(i=0;i<oData.rows.length;i++){
         document.all.item("GONGM",i).tabIndex=++j;
         document.all.item("HAOY",i).tabIndex=++j;
         document.all.item("TIAOZL",i).tabIndex=++j;
         if(document.all.item("KUC",i).value==0){
         	document.all.item("KUC",i).value=document.all.item("SHANGYKC",i).value;
         }
    }
    /*
    for(i=0;i<oData.rows.length;i++){
    ...
    for(i=0;i<oData.rows.length;i++){
         document.all.item("KUC",i).tabIndex=++j;
    }
    */
}
...
function myMatchNum(obj1){
	...
	//str=/^0\.[0-9]{1,2}|[1-9][0-9]{0,9}\.[0-9]{1,2}|0|[1-9][0-9]{0,9}$/;小数,整数,零的情况
	//str=/^[-]{0,1}0\.[0-9]{1,2}|[-]{0,1}[1-9][0-9]{0,9}\.[0-9]{1,2}|0|[-]{0,1}[1-9][0-9]{0,9}$/;//小数,负数,整数,零的情况
	str=/^[-]{0,1}[1-9][0-9]{0,9}|0$/;//只允许输入整数或者负的整数
	var Numr, Numre;
	var Nums = obj1.value;
