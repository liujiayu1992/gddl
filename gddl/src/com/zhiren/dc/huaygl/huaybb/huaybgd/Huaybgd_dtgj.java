 ...
 * 时间：2010-05-25
 * 描述：选择化验编号下拉框时自动刷新页面
 * 	     添加系统参数 化验编号只从zhillsb、yangpdh表中检索
 *		特殊情况：大坝电厂化验先于数量进入系统，故作此调整
 * 		
 */
/*
 * 作者：张立东
 * 时间：2010-01-18
 * 描述：根据参数配置，判断是否显示主管。默认否。
 */
/*
 * 作者：张立东
 * 时间：2009-08-17
 ...
 * 描述：修改报表中车号显示数量
 */
public class Huaybgd_dtgj extends BasePage {

	private static final String REPORTNAME_HUAYBGD_ZHILB="Huaybgd_zhilb";
	private static final String REPORTNAME_HUAYBGD_ZHILLSB="Huaybgd_zhillsb"; //当没有设置资源或者 资源不正确也以此默认取数
			...
			if (rs.next()) {
				...
				if(this.getReportName().equals(REPORTNAME_HUAYBGD_ZHILLSB)){
					...
					if (isKuangb){
						...
						if(isMeikdm){
							try{
								String sql="select decode(c.meikdm,null,' ',c.meikdm) as meikdm from caiyb c,zhillsb z where c.zhilb_id=z.zhilb_id and z.id="+getBianmValue().getId();
								ResultSet rec=con.getResultSet(sql);
								if (rec.next()){
									strKuangb=rec.getString("meikdm");
								}else{
									ResultSet rskb=con.getResultSet("select decode(meikmc,null,'',meikmc) as kuangb,decode(caiyry,null,'',caiyry) as caiyry,decode(zhiyry,null,'',zhiyry) as zhiyry,decode(caiyrq,null,'',to_char(caiyrq,'yyyy-mm-dd')) caiysj from zhillsb z,qitycyb q where z.id=q.zhillsb_id(+) and z.beiz='"+getBianmValue().getValue()+"'");
									if (rskb.next()){
...
//						"" + strLury + ""};
		...
		boolean isZhug = MainGlobal.getXitxx_item("化验", "化验报告增加主管", 
				String.valueOf(((Visit) this.getPage().getVisit()).getDiancxxb_id()),
				"否").equals("是");
		if (isZhug) {
			rt.setDefautlFooter(1, 1, "负责人：", 1);
			rt.setDefautlFooter(2, 1, "主管：", 2);
		}else{
...
//		}
	...
	public void beginResponse(IMarkupWriter writer, IRequestCycle cycle) {
		...
		if (cycle.getRequestContext().getParameters("lx") != null) {
			...
			if(!cycle.getRequestContext().getParameters("lx")[0].equals(this.getReportName())){
				setChangbValue(null);
				setChangbModel(null);
				setBianmValue(null);
				setBianmModel(null);
...
//						"(select id from zhuanmlb where jib = (select nvl(max(jib),0) from zhuanmlb))");
		...
		if(!this.hasDianc(this.getTreeid_dc())){
			...
			s=" and f.diancxxb_id in ( select distinct d.id from diancxxb d start with d.id="+this.getTreeid_dc()+" connect by prior d.id=d.fuid )";
		}
		
		if(this.getReportName().equals(REPORTNAME_HUAYBGD_ZHILB)){
			
			sb.append(" select distinct z.id,z.huaybh  from zhilb z,fahb f,caiyb c   \n")
			...
			// ww 添加系统参数 化验编号只从zhillsb、yangpdh表中检索
			// 特殊情况：大坝电厂化验先于数量进入系统，故作此调整
			boolean isHyOutSl = MainGlobal.getXitxx_item("化验", "查询化验报告单脱离数量", 
					String.valueOf(((Visit) this.getPage().getVisit()).getDiancxxb_id()),
					"否").equals("是");
			if (!isHyOutSl) {
				sb
				...
				sb.append(
						"SELECT distinct l.id,z.bianm FROM zhuanmb z,zhillsb l,yangpdhb  yd\n" + 
						"WHERE l.id=yd.zhilblsb_id\n" + 
						"AND l.id = z.zhillsb_id\n" + 
						"and l.huaysj = " + DateUtil.FormatOracleDate(getRiq()) + "\n" + 
						"and l.shenhzt>-1\n" + 
						"and z.zhuanmlb_id =\n" + 
						"(select id from zhuanmlb where jib =\n" + 
						"(select nvl(max(jib),0) from zhuanmlb))"
					);
			}
