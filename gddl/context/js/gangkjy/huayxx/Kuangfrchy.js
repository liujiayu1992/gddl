...
var gridDiv_cm = new Ext.grid.ColumnModel([new Ext.grid.RowNumberer(),{header:'ID',dataIndex:'ID',hidden :true},
...
{header:'煤矿单位',dataIndex:'MEIKDW',width:100,editor :new Ext.form.TextField({maxLength:50,width:'auto',selectOnFocus:true}),hidden :false},
...
var gridDiv_ds = new Ext.data.Store({proxy : new Ext.zr.data.PagingMemoryProxy(gridDiv_data),pruneModifiedRecords:true,reader: new Ext.data.ArrayReader({}, [{name:'ID'},{name:'FID'},{name:'FAHR'},{name:'MEIKDW'},{name:'PINZ'},{name:'CHEC'},{name:'CHES'},{name:'JIEXRQ'},{name:'BIAOZ'},{name:'MT'},{name:'MAD'},{name:'AAR'},{name:'AAD'},{name:'AD'},{name:'VAR'},{name:'VAD'},{name:'VDAF'},{name:'FCAD'},{name:'QGRAD'},{name:'QNET_AR'},{name:'STAD'},{name:'STD'},{name:'QBAD'},{name:'HAD'},{name:'HDAF'}])});
var gridDiv_plant = Ext.data.Record.create([{name: 'ID', type:'string'},{name: 'FID', type:'string'},{name: 'FAHR', type:'string'},{name: 'MEIKDW', type:'string'},{name: 'PINZ', type:'string'},{name: 'CHEC', type:'string'},{name: 'CHES', type:'float'},{name: 'JIEXRQ', type:'date'},{name: 'BIAOZ', type:'float'},{name: 'MT', type:'float'},{name: 'MAD', type:'float'},{name: 'AAR', type:'float'},{name: 'AAD', type:'float'},{name: 'AD', type:'float'},{name: 'VAR', type:'float'},{name: 'VAD', type:'float'},{name: 'VDAF', type:'float'},{name: 'FCAD', type:'float'},{name: 'QGRAD', type:'float'},{name: 'QNET_AR', type:'float'},{name: 'STAD', type:'float'},{name: 'STD', type:'float'},{name: 'QBAD', type:'float'},{name: 'HAD', type:'float'},{name: 'HDAF', type:'float'}]);
...
if(record.get('ID')=='' || record.get('ID')==null){Ext.MessageBox.alert('提示信息','化验值为空不能删除!');return;}
else{
Ext.MessageBox.confirm('警告', '确定将化验单删除吗？', 
function(btn) {  
if(btn=='yes'){
...
+ '<MEIKDW update="true">' + Mrcd[i].get('MEIKDW').replace(/[<]/g,'&lt;').replace(/[>]/g,'&gt;')+ '</MEIKDW>'
...
var Rpt_window =new Ext.Window({width:550,height:360,closable:false,modal:true,layout:'fit',items:[ValueForm]});
...
if(shenhztChk()){
	...
	Rpt_window.show();
	CountValue();
}else{Ext.MessageBox.alert('警告','票重为0不能编辑!');return;}
}
function EditGrid(){
	var rec = gridDiv_sm.getSelected();
...
function shenhztChk(){
	var rec=gridDiv_sm.getSelected();
	//对票重判断
	var IsequalsZero=rec.get('BIAOZ');
