 ...
 */
public class Meicywh extends BasePage implements PageValidateListener {
	private final static String Customkey = "Meicygl";
	private String msg = "";
...
//		visit.getExtGrid1().Save(getChange(), visit);
		...
		while (delrsl.next()) {
			sbsqlDel.append("delete from meicyb where id = ").append(delrsl.getString("id")).append(";\n");
			String IsHaveQnet_ar="select * from meicyb where id="+delrsl.getString("id")+" and (shenhzt=3 or shenhzt=5 )";
			if(con.getHasIt(IsHaveQnet_ar)){//化验值已经提交,不允许删除
				this.setMsg("编号:"+delrsl.getString("huaybh")+" 化验值已录入,不允许删除!");
				con.Close();
				return;
		...
		while(mdrsl.next()) {
			if("0".equals(mdrsl.getString("id"))) {
				sbsql.append("insert into MEICYB(id, meikxxb_id,cheh,caiysj, huaybh, caiyy,pizr,  ruc_liuf,ruc_rez,yuc_biz,yuc_rez,beiz2) values(getnewid(")
				...
				sbsql.append("update MEICYB set ")
	...
	public void getSelectData() {
		...
		"select c.id,c.caiysj,mk.mingc as meikxxb_id,c.cheh,c.huaybh,c.caiyy,c.pizr,decode(c.shenhzt,5,c.stad,'') as chouy_liuf,\n" +
		" decode(c.shenhzt,5,round_new(c.qnet_ar*1000/4.1816,0),'') as chouy_rez,c.ruc_liuf,c.ruc_rez,\n" + 
		"c.yuc_biz,c.yuc_rez,c.beiz2\n" + 
		" from meicyb c ,meiccyjb mk\n" + 
		...
		egu.getColumn("meikxxb_id").setHeader("取煤位置");
        // 煤场和采样机下拉框
		ComboBox meik = new ComboBox();
		egu.getColumn("meikxxb_id").setEditor(meik);
		meik.setEditable(true);
		sql = "select id, mingc from meiccyjb order by mingc";
		...
		meik.setEditable(true);
		sql = "select r.id,r.quanc from renyxxb r where r.bum='验煤员' order by quanc";
		...
		egu.getColumn("chouy_liuf").setHeader("化验硫分");
		...
		egu.getColumn("chouy_rez").setHeader("化验热值");
		...
		//抽查样编码是否以"H201008045"作为编码规则,字母"H"+年份+月份+三位顺序号,
		boolean ChoucyBm = MainGlobal.getXitxx_item("采样", "煤场样编码是否以字母H+年份+月份+三位顺序号作为编码规则", 
				String.valueOf(((Visit) this.getPage().getVisit()).getDiancxxb_id()),
				"是").equals("是");
		String sPwHandler="";
		 String newBianh="";
		if(ChoucyBm){//H201008045
				 newBianh=GetBianh();
				 sPwHandler = "function(){"
					+"var zhi3;"
	...
	//得到当月最大的编号C201008120，字母C+年份+月份+三位顺序号
	   private String GetBianh() {
	    	...
	    	if (rsl.next()) {
	    		if (rsl.getString("bianh") == null || rsl.getString("bianh").equals("")) {
	    			newBianh = 'H'+getRiqi().substring(0, 4) + getRiqi().substring(5, 7) + "001";
	    		} else {
	   ...
	   private String GetBianh_new() {
	    	...
	    	if (rsl.next()) {
	    		if (rsl.getString("bianh") == null || rsl.getString("bianh").equals("")) {
	    			newBianh = 'H'+ getRiqi().substring(5, 7) + "001";
	    		} else {
	    			bianh = Long.parseLong(rsl.getString("bianh").substring(1,rsl.getString("bianh").length())) + 1 + "";
