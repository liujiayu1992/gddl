...
public class Faycyxg extends BasePage implements PageValidateListener {
	// 界面用户提示
	private String msg = "";
...
//		 卸煤日期的ora字符串格式
		...
		String sql = 
			"select c.id, z.zhilb_id,l.huaybh,to_char(z.CHUKJSSJ,'yyyy-mm-dd') zhuangcsj,\n" +
			...
			"             and c.id=l.caiyb_id\n" + 
			"             and z.CHUKJSSJ >= "+strxmrqOra+"\n" + 
			"             and z.CHUKJSSJ <"+strxmrqeOra+"+ 1\n" + 
			"             and x.id=z.shr_diancxxb_id\n" + 
			"             and p.id=z.pinzb_id";
...
//		设置grid隐藏列
		...
		// 设置grid列标题
		egu.getColumn("zhuangcsj").setHeader("发运时间");
...
//		 详细过衡按钮
	...
	private void save() {
		...
		while (rs.next()) {
			...
			sql += "update fayslb set zhilb_id = 0 where zhilb_id =" + zhilbid + ";\n";
