...
tablemainHeight=bodyHeight-bodyTop-13;
tablemainWidth =bodyWidth ;
ConditionHeight =20;
ConditionWidth=tablemainWidth ;
xiaox=100;
document.getElementById("xiaox").style.height=xiaox;
SelectDataHeight=tablemainHeight - ConditionHeight  - 20-xiaox;
...
SelectDataDiv.style.height=SelectDataHeight-22+15;
...
function setXuh(){
	...
	if(oData.rows.length>0){
		rowscount=document.all.item("XUH").length;
		if(oData.rows.length==1){
			document.all.item("XUH").value=1;
		}else{
			for (i=0;i<rowscount;i++){
				document.all.item("XUH",i).value=i+1;
			}
		}
	}
}
...
var rows=oData.rows ;
	...
	if(_str2=="onclick"){
		...
		if(document.getElementById("tijButton")!=null){
			if(document.all.item("shengdzt",oldClickrow).value==1){
				document.getElementById("tijButton").value="审定";
			}else{
				document.getElementById("tijButton").value="提交";
			}
		}
	}
...
function chakrz(){
    ...
    if(document.getElementById("EditTableRow").value!=-1){
    	var indexrow=document.getElementById('EditTableRow').value;
   		var url = "http://"+document.location.host+document.location.pathname;
        var end = url.indexOf(";");
        var changk="changf";
        if(document.getElementById("ZHONGL").value=="矿方结算单"){
        	
        	changk="kuangf";
        }
        
		url = url.substring(0,end);
   	    url = url + "?service=page/" + "Jiesdcz&"+changk+"_bianm="+document.all.item("JIESBH",indexrow).value;
