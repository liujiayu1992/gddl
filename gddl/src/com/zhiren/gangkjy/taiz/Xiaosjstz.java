...
public class Xiaosjstz  extends BasePage implements PageValidateListener{
...
//******************页面初始设置********************//
	...
	private String getSelectData(){
		...
		String sql = 
			"select decode(grouping(d.mingc),1,'总计',d.mingc) as shouhr,\n" +
			"       k.hangc,\n" + 
			"       k.chuanm,\n" + 
			"       decode(grouping(d.mingc)+grouping(k.bianm),1,'小计',k.bianm) as jiesbh,\n" + 
...
//			"	   and k.diancxxb_id = " + visit.getDiancxxb_id() + "\n" +
			yewlx+
			"group by rollup(d.mingc,k.hangc,k.chuanm,k.bianm,k.jiesrq,(k.ruzrq,k.meiz,k.buhsdj))\n" + 
			"having not (grouping(k.hangc)||grouping(k.meiz))=1\n" + 
			"order by grouping(d.mingc) desc,d.mingc,grouping(k.hangc) desc,k.hangc,k.chuanm,k.bianm,k.jiesrq,k.ruzrq,k.meiz";

		strSQL.append(sql);
		
		String ArrHeader[][]=new String[1][20];
		ArrHeader[0]=new String[] {Local.shouhr_jies,Local.hangc_zhuangcb,Local.chuanm_zhuangcb,Local.jiesbh_jies,Local.jiesrq_jies,Local.ruzrq_jies,Local.pinz,Local.ches,Local.meil_jies,"发热量(Kcal/kg)",Local.liuf_jies,"不含税单价",Local.hej_jies,Local.buhsmk_jies,Local.meis_jies,Local.buhsyf_jies,Local.yunfs_jies,Local.biaomdj_jies,Local.buhsbmdj_jies};

		int ArrWidth[]=new int[] {120,60,80,170,80,80,60,40,60,50,40,50,90,90,90,50,50,50,50};
		...
		rt.setTitle("结算台账(销售)", ArrWidth);
		rt.setDefaultTitle(1, 20, getBRiq() + " 至 " + getERiq(), Table.ALIGN_CENTER);
