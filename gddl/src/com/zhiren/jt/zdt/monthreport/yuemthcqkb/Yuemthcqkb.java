package com.zhiren.jt.zdt.monthreport.yuemthcqkb;
...
public class Yuemthcqkb extends BasePage implements PageValidateListener {
	private String msg = "";
...
while (drsl.next()) {
	sql_delete.append("delete from ").append("yuemthcqkb").append(
	...
	sql_delete
			...
			.append(" where id =")
			.append(
					"(select id from yuemthcqkb where diancxxb_id=(select id from diancxxb dc where dc.mingc='"
...
while (rsl.next()) {
	...
	if ("0".equals(rsl.getString("ID"))) {
		...
		sql
				.append("insert into yuemthcqkb("
						+ "id,riq,diancxxb_id,fadl,quanbhyyml,gongrl,rulrl,fadhml,gongrhml,qithyl,chucshl,shuifctz,shijkc,zhangmkc,fenx)values("
						+ id
						+ ",to_date('"
						+ rsl.getString("riq")
						...
						+ rsl.getString("diancxxb_id") + "'),"
						 +rsl.getDouble("fadl")
						 +","+rsl.getDouble("quanbhyyml")
						 +","+rsl.getDouble("gongrl")
						 +","+rsl.getDouble("rulrl")
						 +","+rsl.getDouble("fadhml")
						 +","+rsl.getDouble("gongrhml")
		...
		sql
				.append("update yuemthcqkb set diancxxb_id=(select id from diancxxb dc where dc.mingc='"
						+ rsl.getString("diancxxb_id")
						+ "'),fadl="
						+ rsl.getDouble("fadl")
						+ ",quanbhyyml="
	...
	con.getUpdate(sql.toString());
		...
		ResultSetList rsllj = LeijSelect(diancxxb_id);
		 ...
		 while (rsllj.next()) {
			 sqllj.append("delete from ").append("yuemthcqkb").append(
				" where id =").append("(select id from yuemthcqkb where diancxxb_id="+rsllj.getString("diancxxb_id")
						+" and riq=to_date('"+ intyear + "-"+ StrMonth + "-01','yyyy-mm-dd') and fenx='累计')").append(";\n");
			 long yuesyhcqkb_id= 0;
			 yuesyhcqkb_id = Long.parseLong(MainGlobal.getNewID(((Visit)
			 getPage().getVisit()).getDiancxxb_id()));
			 sqllj.append(
			 "insert into yuemthcqkb(id,riq,diancxxb_id,fadl,quanbhyyml,gongrl," +
			 "rulrl,fadhml,gongrhml,qithyl,chucshl,shuifctz,shijkc,zhangmkc,fenx)values("
			 + yuesyhcqkb_id
			 +",to_date('"+ intyear + "-"+ StrMonth + "-01','yyyy-mm-dd'),"
			 + rsllj.getString("diancxxb_id")
			 +","+rsllj.getDouble("fadl")
			 +","+rsllj.getDouble("quanbhyyml")
			 +","+rsllj.getDouble("gongrl")
			 +","+rsllj.getDouble("rulrl")
	...
	public ResultSetList LeijSelect(String diancxxb_id) {
		...
		String sqllj =
			...
			"      sum(ymhc.fadl) as fadl,\n" + 
			"            sum(ymhc.quanbhyyml) as quanbhyyml,\n" + 
			"            sum(ymhc.gongrl) as gongrl,\n" + 
			"      decode(sum(ymhc.quanbhyyml),0,0,round(sum(ymhc.rulrl)*sum(ymhc.quanbhyyml)/sum(ymhc.quanbhyyml),3)) as rulrl,\n" + 
			"      sum(ymhc.fadhml) as fadhml,\n" + 
			"      sum(ymhc.gongrhml) as gongrhml,\n" + 
			"             sum(ymhc.qithyl) as qithyl,\n" + 
			"             sum(ymhc.chucshl) as chucshl,\n" + 
			"             sum(ymhc.shuifctz) as shuifctz,\n" + 
			"                   sum(ymhc.shijkc) as shijkc,\n" + 
			"             sum(ymhc.zhangmkc) as zhangmkc\n" + 
			"  from yuemthcqkb ymhc, diancxxb dc\n" + 
			"  where ((ymhc.riq = add_months(to_date('"+ intyear + "-"+ StrMonth + "-01', 'yyyy-mm-dd'), -1) and\n" + 
			"       fenx = '累计') or\n" + 
			"       (ymhc.riq = to_date('"+ intyear + "-"+ StrMonth + "-01', 'yyyy-mm-dd') and fenx = '本月'))\n" + 
			"   and ymhc.diancxxb_id = dc.id\n" + 
			"   and dc.id= (select id from diancxxb dc where dc.mingc='"+diancxxb_id+"')\n" + 
			" group by (dc.id)\n";
	...
	public void getSelectData() {
		...
		String chaxun = 
			"select ymthc.id,ymthc.riq,dc.mingc as diancxxb_id ,ymthc.fadl," +
			"ymthc.gongrl,ymthc.rulrl,ymthc.quanbhyyml,ymthc.fadhml,ymthc.gongrhml,ymthc.qithyl,"+
			"ymthc.chucshl,ymthc.shuifctz,ymthc.shijkc,ymthc.zhangmkc,ymthc.fenx" + 
			"  from yuemthcqkb ymthc, diancxxb dc \n" + 
			" where  ymthc.diancxxb_id = dc.id\n" + 
			"   and ymthc.fenx='本月' and to_char(ymthc.riq,'yyyy-mm') ='" + intyear + "-" + StrMonth+ "'  \n"+
			"    "+str+"  "+
			"   order by dc.mingc,ymthc.id";
	...
	egu.getColumn("rulrl").setHeader("入炉热量");
	egu.getColumn("quanbhyyml").setHeader("全部耗用原煤量");
	egu.getColumn("quanbhyyml").setEditor(null);
	egu.getColumn("fadhml").setHeader("发电耗煤量");
	egu.getColumn("gongrhml").setHeader("供热耗煤量");
	egu.getColumn("qithyl").setHeader("其它耗用量");
	...
	//设置不可编辑的颜色
	egu.getColumn("quanbhyyml").setRenderer("function(value,metadata){metadata.css='tdTextext'; return value;}");
	...
	egu.getColumn("rulrl").setWidth(80);
	egu.getColumn("quanbhyyml").setWidth(100);
	egu.getColumn("fadhml").setWidth(80);
	egu.getColumn("gongrhml").setWidth(80);
...
//		
		...
		sb.append("gridDiv_grid.on('afteredit',function(e){");
			...
			sb.append("e.record.set('QUANBHYYML',parseFloat(e.record.get('FADHML')==''?0:e.record.get('FADHML'))+parseFloat(e.record.get('GONGRHML')==''?0:e.record.get('GONGRHML'))+parseFloat(e.record.get('QITHYL')==''?0:e.record.get('QITHYL')));");
		sb.append("});");
