...
public class Zhiyzhybm extends BasePage implements PageValidateListener {
//	界面用户提示
	private String msg="";
...
//	绑定日期
	...
	private void Show(IRequestCycle cycle) {
		...
		String zhiyh="";
		while(rsl.next()) {
			zhiyh+="'"+rsl.getString("zhiybm")+"',";
			
		}
		String aa=zhiyh.substring(0,zhiyh.length()-1);
		setChepid(aa);
		cycle.activate("Zhiyzhybm_mx");
	}
	...
	public void getSelectData() {
		...
		sb.append( "select distinct z.zhillsb_id as id,z.zhiybm, h.huaybm\n")
				.append("  from (select bianm as zhiybm, zhillsb_id\n")
				.append("          from zhuanmb\n")
				...
				.append("               (select id from zhuanmlb where mingc = '制样编码') and bianm like '%%') z,\n")
				.append("       (select bianm as huaybm, zhillsb_id\n")
				.append("          from zhuanmb\n")
				.append("         where zhillsb_id in\n")
				.append("               (select zm.zhillsb_id as id\n")
				.append("                  from zhuanmb zm, zhuanmlb lb, yangpdhb y, zhillsb z, fahb f\n")
				...
				.append("                   and z.id = zm.zhillsb_id)\n")
				.append("           and zhuanmlb_id =\n")
				.append("               (select id from zhuanmlb where mingc = '化验编码')) h,\n")
				...
				.append("+1) s\n")
				.append(" where z.zhillsb_id = h.zhillsb_id\n")
				.append("   and h.zhillsb_id = s.zid\n")
				.append("   and z.zhillsb_id = s.zid\n")
				.append(" and  s.diancxxb_id = "+visit.getDiancxxb_id()+" order by zhiybm,huaybm");
...
//		设置多选框
		...
		"url = url.substring(0,end);"+
   	    "url = url + '?service=page/' + 'Zhiyzhybm_son&tiaoj='+TIAOJ.getValue();\n"+
