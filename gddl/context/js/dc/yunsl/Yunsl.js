...
SelectHeadDiv.scrollLeft=SelectDataDiv.scrollLeft;
	...
	if(xianszt){
		EditHeadTabel.width = 690;
		trheadTabel.width = 690;
		oData.width = 690;
		if(oData.rows.length > 0){
			trData.width = 690;
		}
	}else{
		EditHeadTabel.width = 490;
		trheadTabel.width = 490;
		oData.width = 490;
		if(oData.rows.length > 0){
			trData.width = 490;
			document.all.item("thhead").style.display = "none";
			var rows=oData.rows.length ;
			for(k=0;k<rows;k++){
				document.all.item("tdData",k).style.display = "none";
			}
		}
	}
...
//	var rows=oData.rows.length ;
//	for(k=0;k<rows;k++){
//		if(document.all.item("ZHUANGT",k).value=="不可编辑"){
//			InitCss(k);
//		}
//	}
...
//function InitCss(row){
//	var obj=oData.rows(row) ;
//	var aReturn ;
//	var l ;
//	var i ;
...
//		var v;
//		//alert(aReturn[i].Id);
//		aReturn[i].parentElement.className="tdText";
//		if(i != 5){
//			aReturn[i].onclick="";
//		}
//	}
