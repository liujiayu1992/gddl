 ...
 * @since 2012-04-09
 * @version V0.1
 * @discription 基于Yuercdjsc.java 1.1.2.3修改而成的月入厂标煤单价生成界面
 */

public class Yuercdjscgys extends BasePage implements PageValidateListener {
	...
	private void DelData() {
		...
		strSql.append("begin \n");
		strSql.append("delete from yuercbmdj_gys where riq="+ CurrODate + " and diancxxb_id=" + diancxxb_id+";\n");
		strSql.append("end;");
	...
	public void CreateData() {
		...
		if(intYuef!=1){
			tiaoj=" union\n " +
			"select y.diancxxb_id,y.meikxxb_id,y.gongysb_id,y.pinzb_id,y.jihkjb_id,y.yunsfsb_id\n" + 
			"from yuercbmdj_gys y where y.riq = add_months("+CurrODate+",-1)\n" + 
			"AND y.fenx='本月' and y.diancxxb_id = "+diancxxb_id+"\n";
		}
		
		sql.append("insert into yuercbmdj_gys(id,riq,fenx,diancxxb_id,gongysb_id,meikxxb_id,pinzb_id,jihkjb_id,yunsfsb_id)\n" +
		"(select getnewid(t.diancxxb_id) id,"+CurrODate+" riq,fx.fenx,\n" + 
		"t.diancxxb_id,t.gongysb_id as gongysb_id,t.meikxxb_id AS meikxxb_id, t.pinzb_id,t.jihkjb_id,t.yunsfsb_id from\n" + 
		"(SELECT distinct f.diancxxb_id,m.id as meikxxb_id,gys.id gongysb_id,f.pinzb_id,f.jihkjb_id,f.yunsfsb_id\n" + 
		"from fahb f, meikxxb m, gongysb gys\n" + 
		"where f.daohrq >= "+Riq1+" \n" + 
		"and f.daohrq <="+Riq2+"\n" + 
		"and f.meikxxb_id = m.id  AND f.gongysb_id=gys.id and f.diancxxb_id = "+diancxxb_id+"\n" + 
		tiaoj+ 
		")t, vwfenxyue fx)");
...
//		写入表头信息
		...
		if(Flag<0){
			setMsg(CurrZnDate + "的头数据生成失败！");
			con.Close();
			return;
		}
		sql.setLength(0);
		sql.append(
				"SELECT RC.ID, SL.JINGZ\n" +
				"  FROM (SELECT RC.ID,\n" + 
				"               RC.DIANCXXB_ID,\n" + 
				"               RC.GONGYSB_ID,\n" + 
				"               RC.MEIKXXB_ID,\n" + 
				...
				"               RC.JIHKJB_ID,\n" + 
				"               RC.YUNSFSB_ID\n" + 
				"          FROM YUERCBMDJ_GYS RC\n" + 
				"         WHERE RC.DIANCXXB_ID = "+diancxxb_id+"\n" + 
				"           AND RC.RIQ = "+CurrODate+"\n" + 
				"           AND RC.FENX = '本月') RC,\n" + 
				...
				"               F.JIHKJB_ID,\n" + 
				"               F.YUNSFSB_ID,\n" + 
				"               ROUND(SUM(F.LAIMSL + DECODE(F.BIAOZ, F.LAIMSL, 0, F.YUNS)), 0) JINGZ\n" + 
				"          FROM FAHB F, MEIKXXB M, GONGYSB GYS\n" + 
				"         WHERE F.DAOHRQ >= "+Riq1+"\n" + 
				"           AND F.DAOHRQ <= "+Riq2+"\n" + 
				"           AND F.MEIKXXB_ID = M.ID\n" + 
				"           AND F.GONGYSB_ID = GYS.ID\n" + 
				...
				"                  F.JIHKJB_ID,\n" + 
				"                  F.YUNSFSB_ID) SL\n" + 
				" WHERE RC.DIANCXXB_ID = SL.DIANCXXB_ID\n" + 
				"   AND RC.GONGYSB_ID = SL.GONGYSB_ID\n" + 
				"   AND RC.MEIKXXB_ID = SL.MEIKXXB_ID\n" + 
				"   AND RC.PINZB_ID = SL.PINZB_ID\n" + 
				"   AND RC.JIHKJB_ID = SL.JIHKJB_ID\n" + 
				"   AND RC.YUNSFSB_ID = SL.YUNSFSB_ID");
		...
		sql.append("begin\n");
		while(rsl.next()){
			sql.append("UPDATE YUERCBMDJ_GYS SET RUCSL = "+rsl.getDouble("JINGZ")+" WHERE ID = "+rsl.getDouble("ID")+";\n");
		}
		sql.append("end;\n");
		if(sql.length()>20){
			Flag=con.getUpdate(sql.toString());
			if(Flag<0){
				setMsg(CurrZnDate + "的入厂数量信息生成失败！");
				con.Close();
				return;
			}
		}
...
//		更新本月入厂标煤单价信息
		sql.setLength(0);
		sql.append("SELECT id,rcrl,rcmj,round(rcmj-rcmj/1.17,2)rcmjs,rcyj,round(rcyj*0.07,2)rcyjs,rczf,rczfs,\n" +
					...
					"FROM (SELECT id,\n" + 
					"DECODE((gusl+jiesl),0,0, ROUND((gusl*gusrl+jiesl*jiesrl)/(gusl+jiesl),2))rcrl,\n" + 
					"DECODE((gusl+jiesl),0,0, ROUND((gusl*gusmj+jiesl*jiesmj)/(gusl+jiesl),2))rcmj,\n" + 
					"DECODE((gusl+jiesl),0,0, ROUND((gusl*gusmjs+jiesl*jiesmjs)/(gusl+jiesl),2))rcmjs,\n" + 
					"DECODE((gusl+jiesl),0,0, ROUND((gusl*gusyj+jiesl*jiesyj)/(gusl+jiesl),2))rcyj,\n" + 
					"DECODE((gusl+jiesl),0,0, ROUND((gusl*gusyjs+jiesl*jiesyjs)/(gusl+jiesl),2))rcyjs,\n" + 
					"DECODE((gusl+jiesl),0,0, ROUND((gusl*guszf+jiesl*jieszf)/(gusl+jiesl),2))rczf,\n" + 
					"DECODE((gusl+jiesl),0,0, ROUND((gusl*guszfs+jiesl*jieszfs)/(gusl+jiesl),2))rczfs\n" + 
					"  FROM YUERCBMDJ_GYS\n" + 
					" WHERE DIANCXXB_ID = "+ diancxxb_id+" AND FENX='"+SysConstant.Fenx_Beny+"'\n" + 
					"   AND RIQ = "+CurrODate+")rc");
		ResultSetList rs = con.getResultSetList(sql.toString());
		StringBuffer sb=new StringBuffer();
		sb.append("begin\n");
		while(rs.next()){
//			逐条更新信息
			sb.append("UPDATE YUERCBMDJ_GYS\n" +
...
//			这种情况比较特殊，所以需要在此重新关联出运费信息
		...
		sql.append( 
			"select gongysb_id,meikxxb_id,jihkjb_id,pinzb_id,yunsfsb_id,jiesl,meij, ROUND(meij-meij/1.17,2) meijs,\n" +
			"yunj, ROUND(yunj*0.07,2) yunjs,zaf,qnet_ar from(\n" +
			" select f.gongysb_id gongysb_id,f.meikxxb_id,\n" +
			" f.jihkjb_id,f.pinzb_id, f.yunsfsb_id,\n" +
			"round(sum(f.jingz),0) as jiesl,\n" + 
			...
			"decode(sum(j.jiessl),0,0,round(sum(j.jiessl*j.jiesrl*0.0041816)/sum(j.jiessl),2)) as qnet_ar\n" + 
			"from jiesb j, jiesyfb jy,fahb f,meikxxb mk, gongysb gys\n" + 
			"where j.id = f.jiesb_id and j.id = jy.diancjsmkb_id(+) \n" +
			" AND f.meikxxb_id=mk.id AND f.gongysb_id=gys.id\n" + 
			" and f.diancxxb_id="+ diancxxb_id+"\n"+
			"and j.ruzrq BETWEEN "+Riq1+" AND "+Riq2+"\n" +
			"and f.daohrq BETWEEN "+Riq1+" AND "+Riq2+"\n" +
			"group by (f.gongysb_id,f.meikxxb_id,f.jihkjb_id, f.pinzb_id, f.yunsfsb_id))");
		...
		while(rs.next()){
//			逐条更新信息
			sb.append("UPDATE (SELECT RC.JIESL,RC.JIESRL,RC.JIESMJ,RC.JIESMJS,RC.JIESYJ,RC.JIESYJS,RC.JIESZF\n" + 
					"                  FROM YUERCBMDJ_GYS RC\n" + 
					"                 WHERE RC.DIANCXXB_ID = "+diancxxb_id+"\n" + 
					"                   AND RC.RUCSL > 0\n" + 
					"                   AND RC.RIQ = "+CurrODate+"\n" + 
					"                   AND RC.FENX = '"+Fenx+"'\n" + 
					"                   AND RC.GONGYSB_ID = "+rs.getString("GONGYSB_ID")+"\n" + 
					"                   AND RC.MEIKXXB_ID = "+rs.getString("MEIKXXB_ID")+"\n" + 
					"                   AND RC.JIHKJB_ID = "+rs.getString("JIHKJB_ID")+"\n" + 
					"                   AND RC.PINZB_ID = "+rs.getString("PINZB_ID")+"\n" + 
					"                   AND RC.YUNSFSB_ID = "+rs.getString("YUNSFSB_ID")+")\n" + 
...
//		取得已到货且未结算的信息，其中数量信息为来煤数量-结算数量
		...
		sql.append("SELECT RC.ID,\n" +
				...
				"       GUS.RUCZF\n" + 
				"  FROM (SELECT FH.GONGYSB_ID,\n" + 
				"               FH.MEIKXXB_ID,\n" + 
				"               FH.JIHKJB_ID,\n" + 
				"               FH.PINZB_ID,\n" + 
				"               FH.YUNSFSB_ID,\n" + 
				"               ROUND(SUM(FH.JINGZ), 2) JINGZ,\n" + 
				"               ROUND(DECODE(SUM(FH.JINGZ),0,0,SUM(FH.JINGZ * ZL.QNET_AR) / SUM(FH.JINGZ)),2) RUCRL,\n" + 
				...
				"         GROUP BY FH.GONGYSB_ID,FH.MEIKXXB_ID,FH.JIHKJB_ID,FH.PINZB_ID,FH.YUNSFSB_ID) GUS,\n" + 
				"       YUERCBMDJ_GYS RC\n" + 
				" WHERE RC.FENX = '"+Fenx+"'\n" + 
				"   AND RC.DIANCXXB_ID = "+diancxxb_id+"\n" + 
				"   AND RC.RIQ = "+CurrODate+"\n" + 
				"   AND RC.RUCSL > 0\n" + 
				"   AND RC.GONGYSB_ID = GUS.GONGYSB_ID\n" + 
				"   AND RC.MEIKXXB_ID = GUS.MEIKXXB_ID\n" + 
				"   AND RC.JIHKJB_ID = GUS.JIHKJB_ID\n" + 
				"   AND RC.PINZB_ID = GUS.PINZB_ID\n" + 
				"   AND RC.YUNSFSB_ID = GUS.YUNSFSB_ID");
...
//	更新结算单价表中的累积数据后，根据结算单价表中的累积数据更新相应的月入厂标煤单价表中的累积数据
	private int CountLJ(JDBCcon con,String diancxxb_id,String year,String CurrODate){
		...
		"       ROUND(SUM(GS.RUCSL), 2) RUCSL\n" + 
		"  FROM YUERCBMDJ_GYS GS,\n" + 
		"       (SELECT *\n" + 
		"          FROM YUERCBMDJ_GYS\n" + 
		"         WHERE DIANCXXB_ID = "+diancxxb_id+"\n" + 
		"           AND RIQ = "+CurrODate+" AND FENX = '累计') T\n" + 
		" WHERE GS.FENX = '本月'\n" + 
		"   AND GS.GONGYSB_ID = T.GONGYSB_ID\n" + 
		"   AND GS.MEIKXXB_ID = T.MEIKXXB_ID\n" + 
		"   AND GS.JIHKJB_ID = T.JIHKJB_ID\n" + 
		"   AND GS.PINZB_ID = T.PINZB_ID\n" + 
		"   AND GS.YUNSFSB_ID = T.YUNSFSB_ID\n" + 
		"   AND GS.RIQ BETWEEN "+year+" AND "+CurrODate+"\n" + 
		"   GROUP BY T.ID)");
...
//			逐条更新信息
			sb.append(
					"UPDATE (SELECT * FROM YUERCBMDJ_GYS GS WHERE GS.ID = "+rs.getString("ID")+")\n" + 
					"   SET GUSL   = "+rs.getDouble("GUSL")+",\n" + 
					"       RUCSL  = "+rs.getDouble("RUCSL")+",\n" + 
...
//		更新累积入厂标煤单价信息
		sql.setLength(0);
		sb.setLength(0);
		sql.append("SELECT ID,RCSL,rcrl,rcmj,round(rcmj-rcmj/1.17,2)rcmjs,rcyj,round(rcyj*0.07,2)rcyjs,rczf,rczfs,\n" +
					"DECODE(rc.rcrl,0,0,ROUND((rc.rcmj+rc.rcyj+rc.rczf)*29.271/rc.rcrl,2))rcbmdj,\n" + 
					"DECODE(rc.rcrl,0,0,ROUND((rc.rcmj+rc.rcyj+rc.rczf-round(rcmj-rcmj/1.17,2)-round(rcyj*0.07,2)-rc.rczfs)*29.271/rc.rcrl,2))rcbhsbmdj\n" + 
					"FROM (SELECT js.ID,\n" + 
					"(gusl+jiesl) rcSl,\n" + 
					...
					"DECODE((gusl+jiesl),0,0, ROUND((gusl*guszf+jiesl*jieszf)/(gusl+jiesl),2))rczf,\n" + 
					"DECODE((gusl+jiesl),0,0, ROUND((gusl*guszfs+jiesl*jieszfs)/(gusl+jiesl),2))rczfs\n" + 
					"  FROM YUERCBMDJ_GYS JS\n" + 
					" WHERE JS.DIANCXXB_ID = "+ diancxxb_id+" AND JS.FENX='"+SysConstant.Fenx_Leij+"'\n" + 
					"   AND JS.RIQ = "+CurrODate+")rc");
...
//			计算并更新结算单价和月入厂标煤单价的累积信息
	...
	public void getSelectData() {
		...
		strSql =
			"SELECT RCID,GONGYSB_ID,MEIKXXB_ID,JIHKJB_ID,PINZB_ID,YUNSFSB_ID,FENX,\n" +
			"JIESL,JIESRL,JIESMJ,ROUND((JIESMJ-JIESMJ/1.17),2)JIESMJS,JIESYJ,ROUND((JIESYJ*0.07),2) JIESYJS,\n" + 
			"JIESZF,JIESZFS,GUSL,GUSRL,GUSMJ,ROUND((GUSMJ-GUSMJ/1.17),2)GUSMJS,GUSYJ,ROUND((GUSYJ*0.07),2)GUSYJS,\n" + 
			"GUSZF,GUSZFS,RUCSL,RUCRL,RUCMJ,ROUND((RUCMJ-RUCMJ/1.17),2)RUCMJS,RUCYJ,ROUND((RUCYJ*0.07),2) RUCYJS,RUCZF,RUCZFS,\n" + 
			"ROUND(DECODE(RUCRL,0,0,(RUCMJ + RUCYJ + RUCZF)*29.271/RUCRL),2) RUCBMDJ,\n" + 
			"ROUND(DECODE(RUCRL,0,0,(RUCMJ+RUCYJ+RUCZF-ROUND(RUCMJ-RUCMJ/1.17,2)-ROUND(RUCYJ*0.07,2)-RUCZFS)*29.271/RUCRL),2)RUCBHSBMDJ\n"+
			"FROM (\n" +
			"SELECT -1 RCID,\n" + 
			...
			"       YUNSFSB   YS,\n" + 
			"       PINZB     PZ\n" + 
			" WHERE RC.GONGYSB_ID = GYS.ID\n" + 
			"   AND RC.MEIKXXB_ID=MK.ID\n" + 
			"   AND RC.JIHKJB_ID = JH.ID\n" + 
			"   AND RC.YUNSFSB_ID = YS.ID\n" + 
			"   AND RC.PINZB_ID = PZ.ID\n" + 
			"   AND RC.DIANCXXB_ID = "+diancxxb_id+"\n" + 
			"   AND RC.RIQ = TO_DATE('"+strDate+"', 'yyyy-mm-dd')\n" + 
			"   GROUP BY  ROLLUP  (RC.FENX,(GYS.MINGC,MK.MINGC,JH.MINGC ,PZ.MINGC ,YS.MINGC))\n" + 
			"   HAVING GROUPING(RC.FENX)+GROUPING(GYS.MINGC)=1 ORDER BY FENX)\n" + 
			"   UNION ALL\n" + 
		...
		((NumberField)egu.getColumn("JIESRL").editor).setDecimalPrecision(2);
		...
		((NumberField)egu.getColumn("GUSRL").editor).setDecimalPrecision(2);
		...
		((NumberField)egu.getColumn("RUCSL").editor).setDecimalPrecision(2);
...
//			保存按钮
		...
		sb.append(
					...
					"   var zafs=0;\n" +
					"	if(jsl+gsl!=0){\n" +
					"   	rez=Round_new((eval(gridDiv_ds.getAt(i).get('JIESRL')||0)*jsl+eval(gridDiv_ds.getAt(i).get('GUSRL')||0)*gsl)/(jsl+gsl),2);\n" +
		...
		sb.append("if(e.record.get('RCID')=='-1'){e.cancel=true;}\n");//合计行不允许编辑
		sb.append(condition);
		sb.append("});\n");
