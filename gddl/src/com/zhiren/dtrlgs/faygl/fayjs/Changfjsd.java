...
//			 String stryuns = "";
			 ...
			 if(rs.next()){
				 ...
				 strHetbh = Fayjsdcz.nvlStr(MainGlobal.getTableCol("hetb", "hetbh", "id", rs.getString("hetb_id")));
				 }
//				 danjc = rs.getDouble("danjc");
				 ...
				 strChaokdlx=Fayjsdcz.nvlStr(rs.getString("chaokdlx"));	//超亏吨类型
				 ...
				 while(rs2.next()){
					 ...
					 if(rs2.getString("mingc").equals(Locale.jiessl_zhibb)){
						 ...
						 strgongfbz_Qnetar = String.valueOf(Fayjsdcz.getUnit_transform(rs2.getString("mingc"),Locale.qiankmqk_danw,rs2.getDouble("GONGF")));	    //供方热量
						 strchangfys_Qnetar = String.valueOf(Fayjsdcz.getUnit_transform(rs2.getString("mingc"),Locale.qiankmqk_danw,rs2.getDouble("CHANGF")));	//验收热量
						 strjiesbz_Qnetar = String.valueOf(Fayjsdcz.getUnit_transform(rs2.getString("mingc"),Locale.qiankmqk_danw,rs2.getDouble("jies")));		//结算热量
...
//			4, 地铁运费
	...
	//					 strfahrq = rs.getString("fahrq");//发货日期
						 ...
						 strHetbh = Fayjsdcz.nvlStr(MainGlobal.getTableCol("hetys", "hetbh", "id", rs.getString("hetb_id")));
						 strfaz=rs.getString("faz");
						 strdiabch=rs.getString("daibch");
						 stryuanshr = rs.getString("yuanshr");//原收货人
						 strshoukdw = rs.getString("shoukdw");//收款单位
...
//			 控制低位热、挥发分、灰分、水分隐藏显示
			 ...
			 type=MainGlobal.getXitxx_item("发运结算", "结算单所属单位", String.valueOf(lgdiancxxb_id), "DTRL");
			  if(type.equals("DTRL")){
					...
					String kfsql="select *  from liucgzb where id in(\n"+
						  ...
						  while(kfrs.next()){
							 ...
							 kfrs.close();
//				 中国大唐
				 String tianbdw=getTianzdw(199);//填制单位。（可根据条件来填入单位）  
				 int ArrWidth[]=new int[] {125,70,65,65,65,65,73,70,86,75,87,59};
				 
				 String ArrHeader[][]=new String[33][12];
				 ArrHeader[0]=new String[] {"供货单位:"+tianbdw,"","","发站:",strfaz,"代表车号:",strdiabch,"","收款单位:",strshoukdw,"",""};
...
//				 ArrHeader[3]=new String[] {"发运数量(吨):",strfahsl," 车数:"+strches,"验收编号:",this.nvlStr(stryansbh),"发票编号:",this.nvlStr(strfapbh),"","兑付地点:",strduifdd,"付款方式:",strfukfs};
				 ...
				 ArrHeader[23]=new String[] {strjiesbz_sl,strbuhsdj,strjine,"",strbukouqjk," ",formatq(strjiakhj),"",strshuil_mk,formatq(strshuik_mk),formatq(strjialhj),"660165.61"};
...
//					 不计算超\亏吨
				 ...
				 ArrHeader[29]=new String[] {"备注:",strbeiz,"","","","","","","过衡重量(吨):",strguohzl,""+Locale.jiesslcy_title+"",""};
				 ArrHeader[30]=new String[] {"经销部:(盖章)","经销部:(盖章)","经销部:(盖章)","经销部领导审核:(签章)","经销部领导审核:(签章)","经销部领导审核:(签章)","领导审核:(签章)","领导审核:(签章)","领导审核:(签章)","财务部:(签章)","财务部:(签章)","财务部:(签章)"};
...
//				 定义页Title
//				 Report rt=new Report();
				 rt.setTitle("燃 料 销 售 结 算 单",ArrWidth);
