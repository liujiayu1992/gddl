...
public class Rucmslfkmx_zdt extends BasePage {
	...
	private String getPrintDataTz(){
		...
		String sql_biaotmc = "select  zhi from xitxxb where mingc='入厂数量验收查询单车明细'";
		...
		if (dancmx.equals("是")){
			sbsql.append("select decode(grouping(y.mingc),1,'总计',y.mingc) as danwmc,");
...
//			sbsql.append("         and fh.jihkjb_id="+kouj+"  \n");
			...
			sbsql.append("select decode(grouping(y.mingc),1,'总计',y.mingc) as danwmc,to_char(fahrq,'yyyy-mm-dd') as fahrq,to_char(daohrq,'yyyy-mm-dd') as daohrq,\n");
			//sbsql.append("decode(grouping(y.mingc)+grouping(fh.lieid),2,'',1,'小计',to_char(max(fahrq),'yyyy-mm-dd')) as fahrq,max(to_char(fh.daohrq,'yyyy-mm-dd'))  as daohrq, \n");
			sbsql.append(" sum(fh.ches) as ches,\n");
			sbsql.append(" sum(round_new(laimsl,0)) as laimsl, \n");
			sbsql.append(" sum(round_new(fh.biaoz,0)) as biaoz, \n");
			sbsql.append(" sum(round_new(fh.jingz,0)) as jingz, \n");
		...
		ArrHeader[0]=new String[] {"单位","发货日期","收煤日期","车数","实收量<br>(吨)","票重<br>(吨)","净重<br>(吨)","运损<br>(吨)","盈吨<br>(吨)","亏吨<br>(吨)","扣吨<br>(吨)"};
