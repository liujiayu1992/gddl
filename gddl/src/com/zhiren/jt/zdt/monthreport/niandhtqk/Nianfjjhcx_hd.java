...
public class Nianfjjhcx_hd  extends BasePage implements PageValidateListener {
...
//******************页面初始设置********************//
	...
	private String getSelectData(){
		...
		String titlename=intyear+"年计划分解查询";
			...
			if(jib==1){//选集团时刷新出所有的电厂
				...
				try{
					...
					if(rl.next()){//燃料公司
						...
						strorderby.append("order by grouping(dc.rlgsmc) desc,dc.rlgsmc desc,grouping(dc.mingc) desc,dc.mingc desc\n");
					}else{//分公司
						strgrouping.append("decode(grouping(dc.fgsmc)+grouping(dc.mingc),2,'总计',1,dc.fgsmc,'&nbsp;&nbsp;&nbsp;&nbsp;'||dc.mingc) as diancmc,\n");
						strwhere.append(" and (dc.fgsid="+this.getTreeid()+" or dc.rlgsid= "+this.getTreeid()+") \n");
						strgroupby.append("group by rollup(dc.fgsmc,dc.mingc)\n");
			...
			if(jib==1){//选集团时刷新出所有的电厂
				...
				try{
					...
					if(rl.next()){//燃料公司
						...
						strhaving.append("having not grouping(dc.rlgsmc)=1\n");
						strorderby.append("order by grouping(dc.rlgsmc) desc,dc.rlgsmc desc,grouping(dc.mingc) desc,dc.mingc desc,grouping(g.mingc) desc,g.mingc\n");
					}else{//分公司
						strgrouping.append("decode(grouping(dc.fgsmc)+grouping(dc.mingc),2,'总计',1,dc.fgsmc,'&nbsp;&nbsp;&nbsp;&nbsp;'||dc.mingc) as diancmc,\n");
						strgrouping.append("decode(grouping(dc.mingc)+grouping(g.mingc),2,'',1,'小计',g.mingc) as meikmc,\n");
						strwhere.append(" and (dc.fgsid="+this.getTreeid()+" or dc.rlgsid= "+this.getTreeid()+") \n");
		...
		if (!getBaoblxValue().getValue().equals("分矿百分比表")){
			...
			strSQL.append("from niandhtqkb dh,vwdianc dc,gongysb g\n");
