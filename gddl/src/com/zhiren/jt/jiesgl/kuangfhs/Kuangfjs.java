...
public class Kuangfjs extends BasePage {
...
//	合计大写
	...
	private long Save(IRequestCycle cycle) {
		...
		String sql = "";
		double jieslx = ((Kuangfjsbean) getEditValues().get(0)).getJieslx();
		try {
			sql=" begin \n";
			if(jieslx == 1){//两票结算
				sql += " update kuangfjsmkb set \n" +
					" gongysmc = '" + ((Kuangfjsbean) getEditValues().get(0)).getFahdw() + "',\n" +
					" shoukdw = '" + ((Kuangfjsbean) getEditValues().get(0)).getShoukdw() + "',\n" +
					" kaihyh = '" + ((Kuangfjsbean) getEditValues().get(0)).getKaihyh() + "',\n" +
					" zhangh = '" + ((Kuangfjsbean) getEditValues().get(0)).getZhangh() + "',\n" +
					" fapbh = '" + ((Kuangfjsbean) getEditValues().get(0)).getFapbh() + "',\n" +
					" fukfs = '" + ((Kuangfjsbean) getEditValues().get(0)).getFukfs() + "',\n" +
					" hetb_id = "+getHetbhValue().getId()+",\n" +
					" beiz = '" + ((Kuangfjsbean) getEditValues().get(0)).getBeiz() + "'\n" +
					" where id = " + jiesmkid + ";\n";
				
				sql += " update kuangfjsyfb set \n" +
				" gongysmc = '" + ((Kuangfjsbean) getEditValues().get(0)).getFahdw() + "',\n" +
				" shoukdw = '" + ((Kuangfjsbean) getEditValues().get(0)).getShoukdw() + "',\n" +
				" kaihyh = '" + ((Kuangfjsbean) getEditValues().get(0)).getKaihyh() + "',\n" +
				" zhangh = '" + ((Kuangfjsbean) getEditValues().get(0)).getZhangh() + "',\n" +
	...
	private long SaveKuangfjsmkb() {
		...
		try {
			...
			sql = " insert into kuangfjsmkb(id, diancxxb_id, bianm, gongysb_id, gongysmc, faz, fahksrq, fahjzrq, meiz, daibch, yuanshr, xianshr, yansksrq, yansjzrq, yansbh, shoukdw, kaihyh, zhangh, fapbh, fukfs, duifdd, ches, jiessl, guohl, hansdj, bukmk, hansmk, buhsmk, meikje, shuik, shuil, buhsdj, jieslx, jiesrq, ruzrq, hetb_id, liucztb_id, liucgzid, kuangfjsb_id, diancjsmkb_id, beiz, danjc)"
					...
					+ getHetbhValue().getId()
					+ ", 0, 0, 0,"+getDiancjsId("diancjsmkb",getJiesbhValue().getValue())+",'"
					+ ((Kuangfjsbean) getEditValues().get(0)).getBeiz()
					+ "',"
					+ ((Kuangfjsbean) getEditValues().get(0)).getDanjc()+")";
	...
	private long SaveKuangfjsyfb(long Mkid) {
		// 存储运费表
		JDBCcon con = new JDBCcon();
		Visit visit = new Visit();
		String sql = "";
		...
		try {
			...
			sql = " insert into kuangfjsyfb (id, diancxxb_id, bianm, gongysb_id, gongysmc, faz, fahksrq, fahjzrq, meiz, daibch, yuanshr, xianshr, yansksrq, yansjzrq, yansbh, shoukdw, kaihyh, zhangh, fapbh, fukfs, duifdd, ches, jiessl, guohl, guotyf, dityf, jiskc, hansdj, bukyf, hansyf, buhsyf, shuik, shuil, buhsdj, jieslx, jiesrq, ruzrq, hetb_id, liucztb_id, liucgzid, kuangfjsb_id, kuangjsmkb_id, diancjsyfb_id, beiz, danjc)"
					...
					+ ((Kuangfjsbean) getEditValues().get(0)).getJingz()
					+ ","
					+ ((Kuangfjsbean) getEditValues().get(0)).getTielyf()
					+ ","
					+ ((Kuangfjsbean) getEditValues().get(0)).getQitzf()
					+ ","
					+ ((Kuangfjsbean) getEditValues().get(0)).getJiskc()
					+ ","
					+ ((Kuangfjsbean) getEditValues().get(0)).getShulzjbz()
					+ ", "
					+ ((Kuangfjsbean) getEditValues().get(0)).getBukyqyzf()
					+ ","
					+ " "
					+ ((Kuangfjsbean) getEditValues().get(0)).getYunzfhj()
					+ ", "
					+ ((Kuangfjsbean) getEditValues().get(0)).getBuhsyf()
					+ ", "
					+ ((Kuangfjsbean) getEditValues().get(0)).getYunfsk()
					+ ", "
					+ ((Kuangfjsbean) getEditValues().get(0)).getYunfsl()
					+ ",0, "
					+ ((Kuangfjsbean) getEditValues().get(0)).getJieslx()
					+ ", to_date('"+this.FormatDate(new Date())+"','yyyy-MM-dd'), null, "
					+ getHetbhValue().getId()
					+ ", 0, 0, 0, "+Mkid+","+getDiancjsId("diancjsyfb",getJiesbhValue().getValue())+"'"
					+ ((Kuangfjsbean) getEditValues().get(0)).getBeiz()
					+ "',"
					+ ((Kuangfjsbean) getEditValues().get(0)).getDanjc()+")";
			...
			if (con.getInsert(sql) == 1) {
				...
				((Kuangfjsbean) getEditValues().get(0)).setYid(Id);
				return Id;
			}
	...
	private long SaveKuangfjsb(long kuangfjsbId) {
		...
		try {
			...
			if (kuangfjsbId == 0) {
				...
				sql = "insert into kuangfjsb (id, diancxxb_id, gongysb_id, gongysmc, faz, fahksrq, fahjzrq, meiz, daibch, yuanshr, xianshr, yansksrq, yansjzrq, yansbh, ches, jiessl, guohl, hansdj, bukmk, hansmk, buhsmk, meisk, guotyf, dityf, jiskc, yunfsk, yunzfhj, hanshj, buhsdj, jieslx, ruzrq, hetb_id, diancjsb_id, beiz)"
						+ " values"
						+ " ("
						+ Id
						+ ", "
						...
						+ "',"
						+ " to_date('"
						+ this.FormatDate(((Kuangfjsbean) getEditValues()
								.get(0)).getFahksrq())
						+ "','yyyy-MM-dd'), to_date('"
						+ this.FormatDate(((Kuangfjsbean) getEditValues().get(0)).getFahjzrq())
						+ "','yyyy-MM-dd'), '"
						+ ((Kuangfjsbean) getEditValues().get(0)).getPinz()
						+ "', '"
						+ ((Kuangfjsbean) getEditValues().get(0)).getDaibcc()
						...
						+ ((Kuangfjsbean) getEditValues().get(0)).getYunzfhj()
						+ ", "
						+ ((Kuangfjsbean) getEditValues().get(0)).getHej()						
						+ ", 0, "
						+ ((Kuangfjsbean) getEditValues().get(0)).getJieslx()
						+ ", null, "
						+ getHetbhValue().getId()
						+ ", "+getDiancjsId("diancjsb",getJiesbhValue().getValue())+",'"
						+ ((Kuangfjsbean) getEditValues().get(0)).getBeiz()
						+ "')";
				...
				sql = " update kuangfjsb set hansmk = hansmk+"
	...
	private void Retruns() {
		...
		// ((Kuangfjsbean) _list.get(i)).getXXX();
		getSelectData();
		Fuz();
	}
...
//	}
//
//	private boolean _TijldChick = false;
//
//	public void TijldButton(IRequestCycle cycle) {
//		_TijldChick = true;
//	}
...
//				说明存在超亏吨
	...
	public void setHetxx(){
		...
		String mkaihyh = "";
		String mzhangh = "";
		sql = "select distinct h.gongfdwmc,h.gongfkhyh,h.gongfzh from hetb h where id=" + getHetbhValue().getId() + "\n";
		ResultSet rs = con.getResultSet(sql);	
		try {
			if(rs.next()){
				mfahdw = rs.getString("gongfdwmc");
				mshoukdw = rs.getString("gongfdwmc");
		...
		String js = "document.getElementById('Fahdw').value = '"+mfahdw+"';\n" +
					"document.getElementById('SHOUKDW').value = '"+mshoukdw+"';\n" +
	...
	public void beginResponse(IMarkupWriter writer, IRequestCycle cycle) {
		...
		if (!visit.getActivePageName().toString().equals(
			...
			getSelectData();
		}else if(((Visit) getPage().getVisit()).getboolean1()==true){
			setHetxx();
	...
	private void getToolbars() {
		...
		//保存
		ToolbarButton save = new ToolbarButton(null, "保存",
				"function(){ document.Form0.SaveButton.click();}");
		save.setId("baocbt");
		tb1.addItem(save);
		tb1.addText(new ToolbarText("-"));
		//		
		tb1.addText(new ToolbarText("采购合同编号:"));
		ComboBox het = new ComboBox();
		het.setTransform("HetbhDropDown");
		het.setEditable(true);
		het.setWidth(150);
		het.setListWidth(200);
		het.setListeners("select:function(){document.Form0.submit();}");
		tb1.addField(het);
		tb1.addText(new ToolbarText("-"));
...
//	 工具条_begin
	...
	public IPropertySelectionModel getIHetbhModels() {
		...
		String qisrq="";
		String jiezrq="";
		String[] raw1=((Visit) getPage().getVisit()).getString3().split(";");
		for(int i=0;i<raw1.length;i++){
			String[] raw2=raw1[i].split(","); //分别获取 jiesmkbid jiesyfbid
		...
		if(!jiesmkid.equals("")){
			...
			JDBCcon con = new JDBCcon();
			sql = "select diancxxb_id,to_char(yansksrq,'yyyy-MM-dd') as qisrq,to_char(yansjzrq,'yyyy-MM-dd') as jiezrq from kuangfjsmkb where id="+jiesmkid;
			ResultSet rs = con.getResultSet(sql);
			try{
				if(rs.next()){
					
					diancxxb_id = rs.getString("diancxxb_id");
					qisrq = rs.getString("qisrq");
					jiezrq = rs.getString("jiezrq");
				}
				rs.close();
		...
		if (((Visit) getPage().getVisit()).getRenyjb() == 3) {
			...
			sql = "select id,hetbh from hetb where diancxxb_id="
					+ ((Visit) getPage().getVisit()).getDiancxxb_id()
					+ "  and leib=2 order by hetbh";

		} else if (((Visit) getPage().getVisit()).getRenyjb() == 2) {

			sql =
				"select distinct h.id,h.hetbh||'_'||g.mingc as hetbh\n" + 
				"       from hetb h,hetslb hs,gongysb g\n" + 
				"       where h.id = hs.hetb_id\n" + 
				"             and h.gongysb_id = g.id (+)\n" + 
				"             and hs.diancxxb_id = "+diancxxb_id+"\n" + 
				"             and h.qisrq<=to_date('"+qisrq+"','yyyy-MM-dd')\n" + 
				"             and h.guoqrq>=to_date('"+jiezrq+"','yyyy-MM-dd')\n" + 
				"             and h.leib = 2\n" + 
				"union\n" + 
				"select distinct h.id,h.hetbh||'_'||g.mingc as hetbh\n" + 
				"       from hetb h,gongysb g\n" + 
				"            where h.fuid in\n" + 
			...
			sql = "select id,hetbh from hetb where leib=2 order by hetbh";
		}
		((Visit) getPage().getVisit())
