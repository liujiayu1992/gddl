 ...
 * 作者：张森涛
 * 时间：2009-09-04
 * 描述：增加xitxxb设置，休约盘点密度保留小数位 默认为4位设置如下
 * 		mingc	= '盘点密度保留小数位'
 * 		zhi		= 小数位
 * 		leib 	= '盘点'
 * 		zhuangt = 1
...
//					+ "Pandtj.Save 中 getExtGrid().getDeleteResultSet(getChange()) 方法没有正确返回记录集。");
	...
	public void getSelectData() {
		...
		if (_AddChick) {
			...
			sSql = "select 0 as id,a.meicb_id,pandff,cunml,duig,dingc,dic,dingk,dik,b.mid,tij from\n"
				+ "	  (select meicb.xuh,meicb.id,meicb.mingc as meicb_id,nvl('人工','') as pandff,p.cunml,p.duig,p.dingc,p.dic,p.dingk,p.dik,p.mid,p.tij from meicb,\n"
			...
			//wangzongbing,修改提示方式,只要有一个mid不会0,就不提示
			int i=0;
			while(rs.next()){
...
//		gbt = new GridButton("删除","function(){document.getElementById('DeleteButton').click();}");
