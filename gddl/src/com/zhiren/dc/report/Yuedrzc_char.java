...
public class Yuedrzc_char  extends BasePage implements PageValidateListener {
	...
	public String getPrintChar(){
		...
		String Sql=
			...
			"SELECT SR.RIQ, SR.FENX, SR.REZ\n" +
			"  FROM (SELECT Z.FENX XUH,\n" + 
			"               TO_CHAR(T.RIQ, 'mm') || '月' RIQ,\n" + 
			"               nvl('入厂','')||Z.FENX FENX,\n" + 
			...
			"               nvl('入炉','')||FENX FENX,\n" + 
			"               ROUND(RULTRMPJFRL, 2) REZ\n" + 
			"          FROM YUEZBB\n" + 
			"         WHERE TRUNC(RIQ, 'yyyy') = DATE '"+getYearValue().getValue()+"-01-01'\n" + 
			"           AND DIANCXXB_ID = "+visit.getDiancxxb_id()+") SR\n" + 
			" ORDER BY SR.RIQ, SR.XUH,SR.FENX";
		...
		CategoryDataset dataset = cd.getRsDataChart(rstmp, "RIQ","FENX","REZ");//rs记录集构造生成图片需要的数据
...
//		柱子上字的倾斜度。0:水平
		...
		return ct.ChartBar3D(getPage(), dataset,  getYearValue().getValue()+"年入厂入炉热值",  900,  400);
	}
