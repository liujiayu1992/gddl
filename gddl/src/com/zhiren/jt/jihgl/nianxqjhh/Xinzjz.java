...
public class Xinzjz extends BasePage implements PageValidateListener {
	private String msg = "";
...
//		 工具栏的年份和月份下拉框
		...
		String sbsql="select * from nianxqjhh where nianf="+nextyear+" and jizzt=1 and diancxxb_id="+diancxxb_id;
...
//	定义两维数组-存储去年和今年的数据
	...
	public void getQus(JDBCcon con2,long year,long shujlx,String yuefen,long diancxxb_id,int row){//得到上年和去年的数据(参数有年份，电厂ID)
		String sql_sn="select to_char(n.gongdbzmh) as gongdbzmh,to_char(n.zonghcydl) as zonghcydl," +
		  ...
		  " from nianxqjhh n where n.nianf="+(year)+" and n.jizzt=1 and n.shujzt='"+(shujlx)+"' " +
	...
	public void Insert(){
		 ...
		 * 11 xuymzbml_xj 需用煤折标煤量--计算（总需求量-油发热量）
		 * 12 rulrz_xj 入炉热值－录入
		...
		getQus(con1,intyear,intyear,yuefen,diancxxb_id,1);
		...
		String insert_sql1="insert into nianxqjhh(ID,DIANCXXB_ID,NIANF,FADL,FADBHM,ZONGHCYDL," +
				...
				"GONGRBML,BIAOMH,ZHUANGJRL,MIAOS,TOUCRQ,SHEJMZ,BEIZ,QITY,YUNS,QICKC,QIMKC,ZONGXQL,GONGDBZMH,YOUFRL) values (" +
					...
					shuj[0][9]+","+zhuangt+",1,"+lastyear+",'"+gongsshr+"',to_date('"+gongsshsj+"','yyyy-mm-dd'),'"+
		...
		String insert_sql2="insert into nianxqjhh(ID,DIANCXXB_ID,NIANF,FADL,FADBHM,ZONGHCYDL," +
				...
				"GONGRBML,BIAOMH,ZHUANGJRL,MIAOS,TOUCRQ,SHEJMZ,BEIZ,QITY,YUNS,QICKC,QIMKC,ZONGXQL,GONGDBZMH,YOUFRL) values (" +
					...
					shuj[1][9]+","+zhuangt+",1,"+intyear+",'"+gongsshr+"',to_date('"+gongsshsj+"','yyyy-mm-dd'),'"+
		...
		"GONGRBML,BIAOMH,ZHUANGJRL,MIAOS,TOUCRQ,SHEJMZ,BEIZ,QITY,YUNS,QICKC,QIMKC,ZONGXQL  ) values (" +
			...
			"0,1,"+nextyear+",'"+gongsshr+"',to_date('"+gongsshsj+"','yyyy-mm-dd'),'"+
...
//	----------------------------------------------------------------------	
		...
		String sbsql1="select * from nianxqjhh yx,diancxxb dc where yx.diancxxb_id=dc.id and  nianf="+nextyear+" and jizzt=1 "+zhuangt+" and diancxxb_id="+getTreeid();
		...
		sbsql.append("12,'需用煤折标煤量',13,'入炉热值',14,'发电需原煤量',15,'供热需用标煤量',16,'需用原煤量',17,'其他用',18,'运损',19,'期初库存',\n");
		...
		sbsql.append("union \n");
		sbsql.append("select t.lanc as lanc,t.mingc as mingc,z1.zhi as lastyear,z2.zhi as intyear,z3.zhi as nextyear,'Mj/kg' as danw,nvl(z1.zhuangt,0) as zhuangt from \n");
		sbsql.append("(select 12 as lanc,'需用煤折标煤量' as mingc,'万吨' dun,0 as zhuangt from dual) t,\n");
		sbsql.append("(select 12 as lanc,decode(to_char((yx.zongxql-yx.youfrl),'FM99999990.00'),0,'0',to_char((yx.zongxql-yx.youfrl),'FM99999990.00')) as zhi,nvl(yx.zhuangt,0) as zhuangt from nianxqjhh yx,diancxxb dc where yx.diancxxb_id=dc.id and yx.shujzt="+lastyear+" and yx.diancxxb_id="+getTreeid()+" and yx.nianf="+nextyear+" "+jizzt+" "+zhuangt+"   ) z1, \n");
...
//		计算发电需用标煤量-新增机组 （发电量*供电标准煤耗）
		sb.append("fadxybml_xj.set('LASTYEAR',Round((eval(fadl_xj.get('LASTYEAR')||0)*10000)*eval(gongdbzmh_xj.get('LASTYEAR')||0)/1000000,2));\n");
		sb.append("fadxybml_xj.set('INTYEAR',Round((eval(fadl_xj.get('INTYEAR')||0)*10000)*eval(gongdbzmh_xj.get('INTYEAR')||0)/1000000,2));\n");
		sb.append("fadxybml_xj.set('NEXTYEAR',Round((eval(fadl_xj.get('NEXTYEAR')||0)*10000)*eval(gongdbzmh_xj.get('NEXTYEAR')||0)/1000000,2));\n");

		// 计算供热需用标煤量-新增机组 （供热量*供热标准煤耗）
		sb.append("gongrxybml_xj.set('LASTYEAR',Round(eval(gongrl_xj.get('LASTYEAR')||0)*eval(gongrbzmh_xj.get('LASTYEAR')||0)/1000,2));\n");
		sb.append("gongrxybml_xj.set('INTYEAR',Round(eval(gongrl_xj.get('INTYEAR')||0)*eval(gongrbzmh_xj.get('INTYEAR')||0)/1000,2));\n");
		sb.append("gongrxybml_xj.set('NEXTYEAR',Round(eval(gongrl_xj.get('NEXTYEAR')||0)*eval(gongrbzmh_xj.get('NEXTYEAR')||0)/1000,2));\n");
		...
		// 计算需用煤折标煤量 （总需求量-油发热量）
		sb.append("xuymzbml_xj.set('LASTYEAR',Round(eval(zongxql_xj.get('LASTYEAR')||0)-eval(youfrl_xj.get('LASTYEAR')||0),2));\n");
		sb.append("xuymzbml_xj.set('INTYEAR',Round(eval(zongxql_xj.get('INTYEAR')||0)-eval(youfrl_xj.get('INTYEAR')||0),2));\n");
		sb.append("xuymzbml_xj.set('NEXTYEAR',Round(eval(zongxql_xj.get('NEXTYEAR')||0)-eval(youfrl_xj.get('NEXTYEAR')||0),2));\n");
		...
		sb.append("}else{\n ");
		sb.append("		fadxyml_xj.set('INTYEAR',Round((eval(fadxybml_xj.get('LASTYEAR')||0)*1000*29.271)/eval(rulrz_xj.get('INTYEAR')||0)/1000,2));\n");
		sb.append("}\n ");
		sb.append("if(eval(rulrz_xj.get('NEXTYEAR')||0)==0){ \n");
		sb.append("		fadxyml_xj.set('NEXTYEAR',0);\n ");
		sb.append("}else{\n ");
		sb.append("		fadxyml_xj.set('NEXTYEAR',Round((eval(fadxybml_xj.get('LASTYEAR')||0)*1000*29.271)/eval(rulrz_xj.get('NEXTYEAR')||0)/1000,2));\n");
		sb.append("}\n ");
		...
		sb.append("}\n");
		sb.append("var zongxql2=Round(eval(xuyyml_xj.get('LASTYEAR')||0)+eval(qity_xj.get('LASTYEAR')||0)+eval(yuns_xj.get('LASTYEAR')||0)+eval(qimkc_xj.get('LASTYEAR')||0)-eval(qickc_xj.get('LASTYEAR')||0),2);\n");
		sb.append("if (zongxql2=='0.00'){\n");
		sb.append(" 	zongxql2='0';\n");
		sb.append("}\n");
		sb.append("var zongxql3=Round(eval(xuyyml_xj.get('LASTYEAR')||0)+eval(qity_xj.get('LASTYEAR')||0)+eval(yuns_xj.get('LASTYEAR')||0)+eval(qimkc_xj.get('LASTYEAR')||0)-eval(qickc_xj.get('LASTYEAR')||0),2);\n");
