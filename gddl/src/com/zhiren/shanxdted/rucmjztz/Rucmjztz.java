 ...
 * 内容:入厂煤质量检验报表
 */
public class Rucmjztz extends BasePage implements PageValidateListener {
...
//	获取相关的SQL
	...
	public StringBuffer getBaseSql() {
		...
		sb.append("  from fahb f where f.daohrq>="+briq+"\n");
		sb.append(" and  f.daohrq<="+eriq+" "+fahgl+"\n");
		sb.append(" )    )fh ,zhilb z,meikxxb m,yunsfsb y,vwdianc d \n");
		sb.append(" where fh.diancxxb_id=d.id "+diancid+"and fh.zhilb_id=z.id and fh.meikxxb_id=m.id "+meik+" and fh.yunsfsb_id=y.id and y.id in(1,2) "+this.getRZTJ()+"\n");
		
		sb.append(" and (select count(distinct cp.yunsdwb_id) from chepb  cp where cp.fahb_id=fh.id"+yunsdw+" )=1\n");
		
		sb.append(" group by grouping sets('1',(fh.id,y.mingc,m.mingc,fh.ysdw,rownum),(y.mingc,m.mingc,fh.ysdw),(y.mingc))\n");
		sb.append(" having grouping(fh.id)+grouping(y.mingc)+grouping(m.mingc)+grouping(fh.ysdw)!=4\n");
		sb.append(" order by y.mingc desc,m.mingc asc,fh.ysdw asc,grouping(fh.id) asc\n");
	...
	private StringBuffer getTJByYunsfsSql(){
		...
		sb.append(" select \n");
		sb.append("   decode(grouping(m.mingc)+grouping(fh.ysdw),2, decode(y.mingc,'铁路','火车','公路','汽车'),'') xuh,\n");
		sb.append("  decode(y.mingc,'铁路','火车','公路','汽车') ysfs,\n");
	...
	private StringBuffer getTJSql(){
		...
		sb1.append("  from fahb f where f.daohrq>="+briq+"\n");
		sb1.append(" and  f.daohrq<="+eriq+" "+fahgl+"\n");
		sb1.append(" )    )fh ,zhilb z,meikxxb m,yunsfsb y,vwdianc d \n");
		sb1.append(" where fh.diancxxb_id=d.id "+diancid+"and fh.zhilb_id=z.id and fh.meikxxb_id=m.id "+meik+" and fh.yunsfsb_id=y.id and y.id in(1,2) "+this.getRZTJ()+"\n");
...
//				"document.getElementById('RefurbishButton').click();\n" 
	...
	public String getPrintTable(){
		...
		if(this.getGesValue().getStrId().equals("1")){//简报
			...
			sql=this.getTJByYunsfsSql().toString();
		}else{
...
//		System.out.println(sql);
		...
		int[] ArrWidth=null;
        	...
        	rs = rstmp;
        	 ArrHeader = new String[][] {{"序号","运输","煤矿","运输单位","车数","Qnet,ar<br>(MJ/KJ 大卡)","Qnet,ar<br>(MJ/KJ 大卡)",
        		 "Mt<br>(%)","Mad<br>(%)","Aar<br>(%)","Ad<br>(%)","Vdaf<br>(%)","Stad<br>(%)","Std<br>(%)"},};
    
    		 ArrWidth = new int[] {50,  50, 140,  120, 50, 50, 50, 50, 50, 50 , 50, 50, 50, 50};
...
//    		rt.title.setRowCells(2, Table.PER_FONTSIZE, 19);
    		...
    		rt.setDefaultTitle(1,3, "单位：" +dcmc ,
    				Table.ALIGN_LEFT);
    		rt.setDefaultTitle(4, 6,  DateUtil.Formatdate("yyyy年MM月dd日", DateUtil.getDate(this.getBRiq()))+"至"+ DateUtil.Formatdate("yyyy年MM月dd日", DateUtil.getDate(this.getERiq())),
    				Table.ALIGN_CENTER);
