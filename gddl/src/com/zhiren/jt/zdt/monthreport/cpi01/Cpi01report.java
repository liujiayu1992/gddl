package com.zhiren.jt.zdt.monthreport.cpi01;
...
* 时间：2009-06-12
* 作者： ll
* 修改内容：在分矿汇总查询中，增加电厂数据上报状态为条件，进行查询。
* 		   
*/ 
/* 
* 时间：2009-06-17
* 作者： ll
* 修改内容：按照上海燃料要求，上海杨树浦发电厂、上海吴泾热电厂、上海闵行发电厂上报集团数据，
* 与上海全资三厂数据冲突，现修改查询程序，过滤掉以上三个电厂数据。
...
* 时间：2009-06-20
* 作者： sy
* 修改内容：修改Guoldcid()中的sql，原来要过滤的电厂名称不对，在数据库中查不出值
* 		   
*/ 
/* 
* 时间：2009-07-28
...
* 时间：2009-07-31
* 作者： ll
* 修改内容：修改热值显示，由原来设置3位小数，改为设置2位小数
* 		   
*/ 
/* 
* 时间：2009-08-5
...
* 		   
*/ 
public class Cpi01report  extends BasePage implements PageValidateListener{
...
//******************页面初始设置********************//
	...
	private String getSelectData(){
		...
		if(getBaoblxValue().getValue().equals("分厂分矿汇总")){
		  ...
		  "    sum(decode(kj.jihkjb_id,1,sl.laimsl,0)) as daohl_zd,\n" + 
		  "    Round(decode(sum(decode(kj.jihkjb_id,1,sl.laimsl,0)),0,0, sum(decode(kj.jihkjb_id,1,sl.laimsl,0)*decode(kj.jihkjb_id,1,dj.meij,0))/sum(decode(kj.jihkjb_id,1,sl.laimsl,0))),2) as chebj_zd,\n" + 
		  "    Round(decode(sum(decode(kj.jihkjb_id,1,sl.laimsl,0)),0,0, sum(decode(kj.jihkjb_id,1,sl.laimsl,0)*decode(kj.jihkjb_id,1,zl.qnet_ar,0))/sum(decode(kj.jihkjb_id,1,sl.laimsl,0))),2) as farl_zd,\n" + 
		  "    Round(decode(sum(decode(kj.jihkjb_id,1,sl.laimsl,0)),0,0, sum(decode(kj.jihkjb_id,1,sl.laimsl,0)*decode(kj.jihkjb_id,1, (dj.meij+dj.yunj+dj.daozzf+dj.zaf+dj.qit+dj.jiaohqzf),0))/sum(decode(kj.jihkjb_id,1,sl.laimsl,0))),2) as daocj_zd,\n" + 
		  "    sum(decode(kj.jihkjb_id,3,sl.laimsl,0)) as daohl_qy,\n" + 
		  "    Round(decode(sum(decode(kj.jihkjb_id,3,sl.laimsl,0)),0,0, sum(decode(kj.jihkjb_id,3,sl.laimsl,0)*decode(kj.jihkjb_id,3,dj.meij,0))/sum(decode(kj.jihkjb_id,3,sl.laimsl,0))),2) as chebj_qy,\n" + 
		  "    Round(decode(sum(decode(kj.jihkjb_id,3,sl.laimsl,0)),0,0, sum(decode(kj.jihkjb_id,3,sl.laimsl,0)*decode(kj.jihkjb_id,3,zl.qnet_ar,0))/sum(decode(kj.jihkjb_id,3,sl.laimsl,0))),2) as farl_qy,\n" + 
		  "    Round(decode(sum(decode(kj.jihkjb_id,3,sl.laimsl,0)),0,0, sum(decode(kj.jihkjb_id,3,sl.laimsl,0)*decode(kj.jihkjb_id,3, (dj.meij+dj.yunj+dj.daozzf+dj.zaf+dj.qit+dj.jiaohqzf),0))/sum(decode(kj.jihkjb_id,3,sl.laimsl,0))),3) as daocj_qy,\n" + 
		  "    sum(decode(kj.jihkjb_id,2,sl.laimsl,0)) as daohl_sc,\n" + 
		  "    Round(decode(sum(decode(kj.jihkjb_id,2,sl.laimsl,0)),0,0, sum(decode(kj.jihkjb_id,2,sl.laimsl,0)*decode(kj.jihkjb_id,2,dj.meij,0))/sum(decode(kj.jihkjb_id,2,sl.laimsl,0))),2) as chebj_sc,\n" + 
		  "    Round(decode(sum(decode(kj.jihkjb_id,2,sl.laimsl,0)),0,0, sum(decode(kj.jihkjb_id,2,sl.laimsl,0)*decode(kj.jihkjb_id,2,zl.qnet_ar,0))/sum(decode(kj.jihkjb_id,2,sl.laimsl,0))),2) as farl_sc,\n" + 
		  "    Round(decode(sum(decode(kj.jihkjb_id,2,sl.laimsl,0)),0,0, sum(decode(kj.jihkjb_id,2,sl.laimsl,0)*decode(kj.jihkjb_id,2, (dj.meij+dj.yunj+dj.daozzf+dj.zaf+dj.qit+dj.jiaohqzf),0))/sum(decode(kj.jihkjb_id,2,sl.laimsl,0))),2) as daocj_sc,\n" + 
		  "    sum(sl.laimsl) as daohl_zh,\n" + 
		  "    Round(decode(sum(sl.laimsl),0,0,sum(sl.laimsl*dj.meij)/sum(sl.laimsl)),2) as chebj_zh ,\n" + 
		  "    Round(decode(sum(sl.laimsl),0,0,sum(sl.laimsl*zl.qnet_ar)/sum(sl.laimsl)),2) as farl_zh,\n" + 
		  "    Round(decode(sum(sl.laimsl),0,0,sum(sl.laimsl* (dj.meij+dj.yunj+dj.daozzf+dj.zaf+dj.qit+dj.jiaohqzf))/sum(sl.laimsl)),2) as daocj_zh\n" + 
		  " from yueslb sl ,yuezlb zl,yuercbmdj dj,yuetjkjb kj, diancxxb dc, gongysb g, jihkjb j,vwfengs f\n" + 
...
// 直属分厂汇总
				 ...
				 arrFormat= new String []{"","0","0.00","0.00","0.00","0","0.00","0.00","0.00","0","0.00","0.00","0.00","0","0.00","0.00","0.00"};
				...
				biaoti="月电煤价格明细表(分购煤地区)";
				strSQL = 
//					"select decode(grouping(g.mingc)+grouping(f.mingc),2,'总计',1,f.mingc,'&nbsp;&nbsp;'||g.mingc) as diancmc,\n" +
					biaot+
					"    sum(decode(kj.jihkjb_id,1,sl.laimsl,0)) as daohl_zd,\n" + 
					"    Round(decode(sum(decode(kj.jihkjb_id,1,sl.laimsl,0)),0,0, sum(decode(kj.jihkjb_id,1,sl.laimsl,0)*decode(kj.jihkjb_id,1,dj.meij,0))/sum(decode(kj.jihkjb_id,1,sl.laimsl,0))),2) as chebj_zd,\n" + 
					"    Round(decode(sum(decode(kj.jihkjb_id,1,sl.laimsl,0)),0,0, sum(decode(kj.jihkjb_id,1,sl.laimsl,0)*decode(kj.jihkjb_id,1,zl.qnet_ar,0))/sum(decode(kj.jihkjb_id,1,sl.laimsl,0))),2) as farl_zd,\n" + 
