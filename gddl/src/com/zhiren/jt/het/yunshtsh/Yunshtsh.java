...
//***************************************************************************//
	...
	public void getSelectData() {
		...
		String diancWhere="";
//显示需方
		boolean flag=MainGlobal.getXitxx_item("合同", "显示需方", "0", "是").equals("是");
		if(flag){//){
				...
				if(this.isTijsh()){
					 str=
						...
						"url = url.substring(0,end);"+
			       	    "url = url + '?service=page/' + 'Yunshtshrz&hetys_id='+record.data['ID']+'&leix='+((record.data['LEIX']=='合同')?'1':'2');";
				}else{
					 str=
			       		" var url = 'http://'+document.location.host+document.location.pathname;"+
			            "var end = url.indexOf(';');"+
...
//			Hetshbean bean=(Hetshbean)list.get(getEditTableRow());
	...
	//		String leib="合同";
			...
			if(ExtGrid1.griddata[getEditTableRow()][10]!=null && ExtGrid1.griddata[getEditTableRow()][10].equals("合同")){
				Liuc.tij(tableName,Long.parseLong(ExtGrid1.griddata[getEditTableRow()][1]) , renyxxb_id, getXiaox());
			}else if(ExtGrid1.griddata[getEditTableRow()][10]!=null && ExtGrid1.griddata[getEditTableRow()][10].equals("模板")){
