...
public class Meitxqfahz  extends BasePage implements PageValidateListener{
//
...
//	
	...
	private String ZONGDH="ZONGDH";//重点订货
	private String QUYDY="QUYDY";//区域订货
...
//	 得到电厂树的值的级别,(jib:1,2,3,) 1是集团,2是分公司,3是电厂
	...
	public String getPrintTable() {
		...
		String table="";
		if (mstrReportName.equals(ZONGDH)){
			leib=" and n.jihkjb_id=1 ";
			table="（棋盘表分厂）";
			return getQibb(leib,table);
		}else if (mstrReportName.equals(QUYDY)){
			leib=" and n.jihkjb_id=3 ";
			table="（棋盘表分区域）";
			return getQibb(leib,table);
		}
...
//			_BeginriqValue = DateUtil.AddDate(new Date(), -1, DateUtil.AddType_intDay);
		...
		if(cycle.getRequestContext().getParameters("lx") !=null) {
			...
			mstrReportName = visit.getString9();
        }else{
	...
	private String getQibb(String leib,String table) {
		...
		sqlRow = 
			"select decode(grouping("+danwmc+"),0,"+danwmc+",'合计') as 电厂\n" +
			"from vwdianc vdc,niancgjh n,diancxxb dc where n.diancxxb_id=dc.id and dc.id=vdc.id "+ leib 
			+where+" and to_char(n.riq,'yyyy')="+intyear+"\n" + 
			"group by rollup ("+danwmc+")\n" + 
			"order by grouping("+danwmc+") desc,"+danwmc+"";
//		得到列数据：供应商
		
		sqrCol = 
			"select decode(grouping(sf.quanc) + grouping(g.mingc),\n" +
			"              2,\n" + 
			"              '合计',\n" + 
			"              1,\n" + 
			"              sf.quanc || '小计',\n" + 
			"              g.mingc) as 供应商\n" + 
			"from niancgjh n,diancxxb dc,gongysb g,shengfb sf\n" + 
			"where n.diancxxb_id = dc.id\n" + 
			"      and n.gongysb_id = g.id\n" + 
			"      and dc.shengfb_id = sf.id\n" +  where + " and to_char(n.riq,'yyyy')="+ intyear +leib+
			"group by rollup(sf.quanc, g.mingc)\n" + 
			"order by grouping(sf.quanc) desc,sf.quanc,grouping(g.mingc) desc,g.mingc ";
		
//		 得到全部数据 棋盘表
		
		sbsql = 
			"select grouping("+danwmc+") as rowjb,\n" +
			"       grouping(g.mingc) as coljb,\n" + 
			"       decode(grouping("+danwmc+"), 0, "+danwmc+", '合计') as 电厂,\n" + 
			"       decode(grouping(sf.quanc) + grouping(g.mingc), 1, sf.quanc || '小计', 0, g.mingc, '合计') as 供应商,\n" + 
			"       round(sum(n.nianjhcgl)/10000,2) as nianjhcgl\n" + 
			"from niancgjh n,diancxxb dc,vwdianc vdc,gongysb g,shengfb sf\n" + 
			"where n.diancxxb_id = dc.id\n" + 
			"   and dc.id = vdc.id\n" + 
			"   and dc.shengfb_id = sf.id\n" + 
			"   and n.gongysb_id = g.id\n" + where + " and to_char(n.riq,'yyyy')="+ intyear + leib+
			"group by cube("+danwmc+", sf.quanc, g.mingc)";
		sb.append(sbsql);
...
//		System.out.println(sb);
		...
		rt.setTitle(intyear+"年采购计划"+table, ArrWidth);
