 ...
 * @author huochaoyuan
 *  陕西分公司日报程序用
 *新建“收耗存维护”(Shouhcrb_tz)
 *在原有页面上添加“外报数量”(fahb.biaoz)列，自动取数且不能修改；
 *添加“昨日外报库存”(shouhcrbb.shangbkc)列，自动取数且不能修改；
 *显示“外报库存”(shouhcrbb.shangbkc)列，自动取数且不能修改；
 *页面录入来煤量、耗煤量后其他数据（库存，外报库存）自动计算，
 *其中“损耗”(shouhcrbb.cuns)列只影响实际库存；为“保存”按钮添加修改时限控制；
 */
...
//	}
	...
	public void getSelectData() {
		...
		egu.getColumn("cuns").setHeader("损耗(t)");
		...
		egu.getColumn("shangbkc").setHeader("外报库存(t)");
		...
		egu.getColumn("wbsl").setHeader("外报数量");
		egu.getColumn("wbsl").setEditor(null);
		egu.getColumn("wbsl").setUpdate(false);
		egu.getColumn("wbkczr").setHeader("昨日外报库存");
...
//		bc.append("function (){")
//		.append("alert('保存');")
//	//	.append(MainGlobal.getExtMessageBox("'正在保存数据,请稍候！'",false))
//		.append("document.getElementById('SaveButton').click();}");
//		GridButton gbb = new GridButton("保存",bc.toString());
