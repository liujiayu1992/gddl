...
public class Luzfhreport extends BasePage {
//	 判断是否是集团用户
...
//	private Date _BeginriqValue = DateUtil.AddDate(new Date(), -1, DateUtil.AddType_intDay);
	...
	public String getJizyxqkreport(){
		...
		String c_sql=
			...
			" select  to_char(CAIYRQ,'yyyy-mm-dd hh24:mi:ss') caiyrq,to_char(HUAYRQ,'yyyy-mm-dd hh24:mi:ss') huayrq,HUAYY,LUZJZY," +
			"LUZJZE,FEIHJZY,FEIHJZE,LURY,to_char(LURSJ,'yyyy-mm-dd hh24:mi:ss') lursj,SHENHRYYJ,to_char(SHENHSJYJ,'yyyy-mm-dd hh24:mi:ss') SHENHSJYJ," +
			"SHENHRYEJ,to_char(SHENHSJEJ,'yyyy-mm-dd hh24:mi:ss')  from LUZFHB    " +
			" where shenhzt=7  and "
			+"  HUAYRQ>=to_date('"+getRiq()+"'||' '||'00:00:00','yyyy-mm-dd hh24:mi:ss') and HUAYRQ<=to_date('"+getRiq1()+"'||' '||'23:59:59','yyyy-mm-dd hh24:mi:ss') ";

		
		String ArrHeader[][]=new String[1][12];
		ArrHeader[0]=new String[] {"采样日期","化验日期","化验员","1#炉渣","2#炉渣","1#飞灰","2#飞灰","录入员",
				"录入时间","一级审核员","一级审核时间","二级审核员","二级审核时间"
				};

		int ArrWidth[]=new int[] {120,120,60,45,45,45,45,50,120,
				70,120,70,120};
...
//		页脚 
		 ...
		 rt.setDefautlFooter(1, 2,"主管:"+"",Table.ALIGN_LEFT);//DateUtil.FormatDate(new Date()),Table.ALIGN_LEFT
		 rt.setDefautlFooter(5, 2,"审核:", Table.ALIGN_CENTER);
		 
					
		rt.setDefautlFooter(10, 2, "化验:",Table.ALIGN_RIGHT);//+((Visit) getPage().getVisit()).getDiancmc()
