...
//
// $Id: TimePicker.js,v 1.1.1.1 2014/05/12 05:30:27 xiaz Exp $
//
//
...
//	this._calDiv.style.paddingRight="1px";
	...
	for(var i=2004; i <= this._currentDate.getFullYear(); ++i) {
...
//			td.style.backgroundColor ="#ffffcc";
	...
	td.appendChild(document.createTextNode(" 时"));
	...
	td.appendChild(document.createTextNode(" 分"));
	...
	td.appendChild(document.createTextNode(" 秒"));
/*
	td = document.createElement("td");
	...
	td.appendChild(document.createTextNode("今天是：" + today.getFullYear() + "年" +(today.getMonth() +1) + "月"  +  today.getDate() +"日"));
...
Calendar.prototype.setMinute = function(Minute) {
	var d = new Date(this._currentDate);
	d.setMinutes(Minute);
	this.setCurrentDate(d);
}

Calendar.prototype.setSecond = function(Second) {
	var d = new Date(this._currentDate);
	d.setSeconds(Second);
	this.setCurrentDate(d);
}
