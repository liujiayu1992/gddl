...
public class Rulhfsh extends BasePage implements PageValidateListener {
	private String msg = "";
	...
	public void Update1(String strchange, Visit visit) {
		...
		while (mdrsl.next()) {
			...
			String gyid = "select id from gongyfxb where zhillsb_id="
					...
					+ mdrsl.getString("ID") + "and fenxxmb_id=4";
	...
	private void AutoFix(String strchange, Visit visit) {
		...
		Judge ju = new Judge();
		double[] dbarrjudge = ju.getJudgeData(Judge.T_TYPE_Aad, array);
		if(dbarrjudge==null){
			return;
		}
		...
		ResultSetList rsl = con.getResultSetList(rulid);
		if(rsl.getRows()==0){
			String sql2="inster into rulmzlb (id,diancxxb_id,aad,fenxrq,rulrq,bianm,rulbzb_id,JIZFZB_ID,LURSJ,shenhzt)values(" +
			...
			String sql2="update zhillsb set aad="+mdrsl.getDouble("ZHI")+",shenhzt=1 where id="+mdrsl.getString("ZHILLSB_ID");	
			con.getUpdate(sql2);
		}
	...
	public void getSelectData() {
		...
		String sql = "select distinct g.id, g.bianm,g.xuh,g.qimbh,g.qimmyzl,g.qimzl,g.meiyzl,g.honghzl1,g.zuizhhzl,g.shenhzt,\n"
				+ "g.shengyzl,g.zhi,g.zhillsb_id,to_char(g.rulrq,'yyyy-mm-dd')as rulrq,to_char(g.fenxrq,'yyyy-mm-dd')as fenxrq\n"
				+ " from gongyfxb g\n"
				+ " where fenxxmb_id=4 and g.shenhzt=0"
		...
		egu.getColumn("shengyzl").setHeader("残留物的质量");
		egu.getColumn("zhi").setHeader("灰分");
