...
public class Yunsdwsx extends BasePage implements PageValidateListener {
	private String msg = "";
	...
	private void Save() {
		...
		while (rsl.next()) {
			sql.append("update yunsdwb set HENGQSXZT = '"+
	...
	public void getSelectData() {
		...
		String sql = "";
		if(Key){
    		sql = "select id,mingc,quanc,decode(hengqsxzt,0,'_',1,'默认使用',2,'正常使用') hengqsxzt from yunsdwb  \n" +
		     	  " where diancxxb_id="+getTreeid()+" and (mingc like '%" + gettheKey() + "%' or quanc like '%" + gettheKey() + "%')" ;
    		Key=false;
    	}else{
    		sql = "select id,mingc,quanc,decode(hengqsxzt,0,'_',1,'默认使用',2,'正常使用') hengqsxzt from yunsdwb where diancxxb_id="+getTreeid()+"  \n" ;
    	}
