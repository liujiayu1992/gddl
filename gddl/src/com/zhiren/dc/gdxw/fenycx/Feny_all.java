...
public class Feny_all extends BasePage {

	/**
	 * 作者:王总兵
	 * 时间:2011-1-16
	 * 内容:根据煤矿名称查询采样号,桶号,车号
	 */
...
//		电厂Tree
	...
	public String getYansd(){
		...
		if(meik==-1){
			...
			tiaoj=" and cp.meikdwmc='"+this.getGongysValue().getValue()+"'";
		}
			...
			sbsql.append(
					"select rownum as xuh,a.meikdwmc,a.mingc,a.piaojh,a.cheph,a.maoz,a.piz,a.koud,a.maoz-a.piz-a.koud as jing,\n" +
					"to_char(a.lursj,'yyyy-mm-dd hh24:mi:ss') as caiysj ,\n" + 
					"to_char(a.zhongcsj,'yyyy-mm-dd hh24:mi:ss') as zhongcsj ,\n" + 
					"to_char(a.qingcsj,'yyyy-mm-dd hh24:mi:ss') as qingcsj\n" + 
					"from (\n" + 
					"select cp.piaojh,cp.meikdwmc,cp.cheph,\n" + 
					"decode(wz.mingc,null,decode(cp.isjus,1,'拒收','正在卸煤'),wz.mingc) as mingc,\n" + 
					"cp.zhongcsj,cp.qingcsj,cp.lury,cp.zhongcjjy,cp.qingcjjy,cp.maoz,cp.piz,cp.koud\n" + 
					",cp.lursj\n" + 
					...
					" and cp.meikdwmc  in (select  distinct xg.meikmc from   rengcymkmcb  xg)\n"+
...
//				1120
				ArrHeader[0]=new String[] {"序号","煤矿名称","桶号","采样号","车号","毛重","皮重","扣吨","净重","采样时间","重车时间","空车时间"};
				int ArrWidth[]=new int[] {50,140,60,80,80,55,55,55,55,180,180,180};
			
				  strFormat = new String[] {"", "", "","","","", "", "","","","", ""};
				// 数据
				rt.setTitle(kais+"人工采样煤矿、桶号、采样号、车号查询",ArrWidth);
...
//	 供应商下拉框
	...
	public void getGongysModels() {
		...
		//只有在rengcymkmcb表里设置的煤矿名称才能查询到
		String sql_gongys = "select rownum as xuh,meikdwmc from (\n" +
		"select distinct c.meikdwmc\n" + 
		"  from chepbtmp c\n" + 
		" where c.lursj >= to_date('"+this.getBRiq()+"', 'yyyy-mm-dd')\n" + 
		" and c.lursj<to_date('"+this.getBRiq()+"', 'yyyy-mm-dd')+1\n" + 
		" and c.meikdwmc in (select  distinct xg.meikmc from   rengcymkmcb  xg)"+
		" and c.piz>0)";
