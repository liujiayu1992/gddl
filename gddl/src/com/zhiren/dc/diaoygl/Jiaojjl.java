...
//	 页面变化记录
	...
	public void save() {
		...
		if (rsl == null) {
			WriteLog.writeErrorLog(ErrorMessage.NullResult 
					+ "jiaojjl.Save 中 getExtGrid().getDeleteResultSet(getChange()) 方法没有正确返回记录集。");
		...
		if(rsl == null) {
			WriteLog.writeErrorLog(ErrorMessage.NullResult 
					+ "jiaojjl.Save 中 getExtGrid().getModifyResultSet(getChange()) 方法没有正确返回记录集。");
			setMsg(ErrorMessage.NullResult);
			return;
		}
		while (rsl.next()) {
	...
	public void getSelectData() {
		...
		egu.getColumn("changckc").setHeader("厂存空车");
		egu.getColumn("changczc").setHeader("厂存重车");
		egu.getColumn("jiaojzc").setHeader("交接站存");
		egu.getColumn("zhanc").setHeader("站存");
		if (treejib == 1) {
