...
public class Yunltj extends BasePage implements PageValidateListener {
...
//			sbsql.append("grouping(dc.mingc) desc,min(dc.xuh) ,dc.mingc,p.mingc,fx.fenx \n");
		...
		ArrHeader[0]=new String[] {"序号","日期","红柳末煤","石槽村末煤","羊二末煤","灵新末煤","梅花井末煤","石槽村煤泥","金凤煤泥","红柳煤泥"};
...
//			rt.setDefautlFooter(11, 2, "制表:", Table.ALIGN_RIGHT);
	...
	private String getZhiltj_bfpz() {//不分品种
		String yunsdwmc=this.getYunsdwValue().getValue();
		JDBCcon con = new JDBCcon();
		StringBuffer sbsql = new StringBuffer("");
		String riq=getBeginriqDate();
		String riq2=getEndriqDate();
		int jib=this.getDiancTreeJib();
		String sql="select\n" +
				"decode(grouping(to_char(f.daohrq, 'mm') || '月' || to_char(f.daohrq, 'dd') || '日')+grouping(m.mingc || p.mingc),1,'小计',\n" +
				"2,'合计',m.mingc || p.mingc)\n" +
				" as mingc,\n" +
				"to_char(f.daohrq, 'mm') || '月' || to_char(f.daohrq, 'dd') || '日' as riq,\n" +
				"sum(c.maoz - c.piz) jingz\n" +
				"  from fahb f\n" +
				" inner join meikxxb m\n" +
				"    on f.meikxxb_id = m.id\n" +
				" inner join pinzb p\n" +
				"    on f.pinzb_id = p.id\n" +
				" inner join chepb c\n" +
				"    on c.fahb_id = f.id\n" +
				"where f.daohrq between date'"+riq+"'  and date'"+riq2+"'"+(this.getYunsdwValue().getId()==-1?"":" and c.yunsdwb_id=   " +this.getYunsdwValue().getId())+
				" group by rollup(m.mingc || p.mingc,to_char(f.daohrq, 'mm') || '月' || to_char(f.daohrq, 'dd') || '日')"
				;
		...
		Report rt = new Report();
		 ...
		 ArrHeader[0]=new String[] {"煤矿","日期","运量"};

		String ArrTitle[][]=new String[4][3];
		ArrTitle[0]=new String[] {"","",""};

		ArrTitle[1]=new String[] {"运量统计表","",""};
		ArrTitle[2]=new String[] {"结算单位:"+yunsdwmc,"",""};
		ArrTitle[3]=new String[] {"结算日期:"+FormatDate(DateUtil.getDate(riq))+"-"+FormatDate(DateUtil.getDate(riq2)),"",""};

		 int ArrWidth[]=new int[] {240,240,240};
		 int aw=rt.paperStyle(((Visit) this.getPage().getVisit()).getDiancxxb_id(),((Visit) this.getPage().getVisit()).getString1());//取得报表纸张类型
			rt.getArrWidth(ArrWidth, aw);//添加报表的A4控制
			Table bt=new Table(rs,2,0,2);
...
//		    rt.setTitle( "运量统计表", ArrWidth);
			/*rt.title.setRowHeight(2, 50);
			rt.title.setRowCells(2, Table.PER_FONTSIZE, 19);
			rt.title.setRowCells(2, Table.PER_VALIGN, Table.VALIGN_TOP);*/
//			rt.setDefaultTitle(1, 2, "结算单位:" +((Visit) getPage().getVisit()).getDiancqc(),Table.ALIGN_LEFT);
//
//			rt.setDefaultTitle(2, 5, "结算日期:" +FormatDate(DateUtil.getDate(riq))+"-"+FormatDate(DateUtil.getDate(riq2)),Table.ALIGN_CENTER);
//			rt.setDefaultTitle(10, 2, "单位:吨" ,Table.ALIGN_RIGHT);
//-------------------------
			rt.body.setPageRows(rt.PAPER_ROWS);
			rt.createDefautlFooter(ArrWidth);
			//rt.setDefautlFooter(1, 3, "制表时间:"+FormatDate(DateUtil.getDate(DateUtil.FormatDate(new Date()))),Table.ALIGN_LEFT);
			rt.setDefautlFooter(1, 1, "审核：", Table.ALIGN_CENTER);
		    rt.setDefautlFooter(2, 1, "统计：", Table.ALIGN_CENTER);




		rt.title.mergeCell(1, 1, 1, 3);

		rt.title.mergeCell(2, 1, 2, 3);
		rt.title.mergeCell(3, 1, 3, 3);
		rt.title.mergeCell(4, 1, 4, 3);
		rt.title.setBorder(0,0,0,0);
		for(int i=0;i<rt.title.getCols();i++){
			for(int j=0;j<4;j++){
				rt.title.getCell(j+1,i+1).setBorderNone();
			}
		}

		rt.title.setCells(1, 1, 2, 3, Table.PER_FONTNAME, "隶书");
		rt.title.setCells(1, 1, 2, 3, Table.PER_FONTSIZE, 20);
...
//			stra.add(Calendar.DATE,-1);
	...
	public IPropertySelectionModel getYunsdwModels() {
		String sql = "";
			sql = "select id,mingc from yunsdwb order by mingc /*where id in " +
					"(select yunsdwb_id from chepb c inner join fahb f " +
					"on c.fahb_id=c.id where trunc(f.daohrq) between '"+this.getBeginriqDate()+"' and '"+this.getEndriqDate()+"')*/";
...
//-------------------------------------------------
		...
		ys.setEditable(true);
		ys.setWidth(200);
		ys.setListeners("select:function(own,rec,index){Ext.getDom('YunsdwDropDown').selectedIndex=index}");
		tb1.addField(ys);
