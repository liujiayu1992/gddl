 ...
 */
public class Shultj extends BasePage implements PageValidateListener {
...
//		String[] arrDc =  getTreeid_dc().split(",");
//		if(arrDc.length > 1) {
//			if (gs.equals("普通")) {
//				havingSql = 
...
//				havingSql = 
//					" GROUP BY GROUPING SETS(1,(CP.ID, FH.DC, FH.YUNSFSB_ID, FH.MK, CP.YSDW, CP.CHEPH),(FH.DC, FH.YUNSFSB_ID, FH.MK, CP.YSDW),(FH.DC, FH.YUNSFSB_ID),(FH.DC),(FH.YUNSFSB_ID))\n" +
//					"ORDER BY FH.DC,FH.YUNSFSB_ID,FH.MK,CP.YSDW,CH DESC,MAX(DZSJ)";
//
...
//			if (gs.equals("普通")) {
//				havingSql = 					
//					"GROUP BY GROUPING SETS((FX.DC, FX.YUNSFSB_ID, FX.MK, FX.MKDQ, CX.YSDW),(FX.DC, FX.YUNSFSB_ID),(fx.dc))\n" +
//					"ORDER BY fx.dc,yunsfsb_id,FX.MKDQ,GROUPING(fx.mk)";
//				
//			} else if (gs.equals("明细")) {
//				havingSql = 
//					"GROUP BY GROUPING SETS((CP.ID, FH.DC, FH.YUNSFSB_ID, FH.MK, CP.YSDW, CP.CHEPH),(FH.DC, FH.YUNSFSB_ID, FH.MK, CP.YSDW),(FH.DC, FH.YUNSFSB_ID),(FH.DC))\n" +
//					"ORDER BY FH.DC,FH.YUNSFSB_ID,FH.MK,CP.YSDW,CH DESC,MAX(DZSJ)";
//
//			} else if (gs.equals("简报")) {
//				havingSql = 					
	...
	private String getBaseSql(){
		...
		if (gs.equals("普通") || gs.equals("简报")){
			SQL = 
				...
				"       SUM(ROUND_NEW(CX.PIZ, "+visit.getShuldec()+")) PIZ,\n" + 
			...
			SQL = 
				...
				"       SUM(ROUND_NEW(CP.PIZ, "+visit.getShuldec()+")) AS PZ,\n" + 
...
//		System.out.println(sql);
		...
		if (isMx) {
			...
			ArrHeader[0] = new String[] {"电厂名称","煤矿地区","煤矿单位","运输单位","车号/车数","毛重","皮重","净重","扣吨","票重","运损","盈亏","复磅时间","复磅员"};
			int[] ArrWidth = new int[] {100, 70, 140, 100,  60, 60, 60, 60, 50, 60, 50, 50, 130, 55};
			...
			rt.setDefaultTitle(4, 5, getBRiq() + " 至 " + getERiq(),Table.ALIGN_CENTER);
			rt.setDefaultTitle(9, 6, "单位：吨、车" + "&nbsp;&nbsp;打印日期：" 
...
//			运输方式
			...
			rt.footer.setCellValue(1, 9, "审核：");
			rt.footer.setCellValue(1, 11, "班长：");
			rt.footer.setCellValue(1, 13, "制表：");
			
			rt.footer.setCells(1, 9, 1, 9, Table.PER_ALIGN, Table.ALIGN_LEFT);
			rt.footer.setCells(1, 11, 1, 11, Table.PER_ALIGN, Table.ALIGN_LEFT);
			rt.footer.setCells(1, 13, 1, 13, Table.PER_ALIGN, Table.ALIGN_LEFT);
			...
			ArrHeader[0] = new String[] {"电厂名称","煤矿地区","煤矿单位","运输单位","车数","毛重","皮重","净重","扣吨","票重","运损","盈亏"};
			int[] ArrWidth = new int[] {100, 70, 150, 110,  60, 70, 70, 70, 70, 70, 60, 60 };
			...
			rt.title.setRowCells(2, Table.PER_FONTSIZE, 19);
			rt.setDefaultTitle(1, 3, "制表单位：" + diancqc, Table.ALIGN_LEFT);
			rt.setDefaultTitle(4, 4, getBRiq() + " 至 " + getERiq(),Table.ALIGN_CENTER);
			rt.setDefaultTitle(8, 5, "单位：吨、车" + "&nbsp;&nbsp;打印日期：" 
					+ DateUtil.Formatdate("yyyy-MM-dd HH:mm:ss", new Date()), 
					Table.ALIGN_RIGHT);
...
//			运输方式
			...
			rt.footer.setCellValue(1, 7, "审核：");
			rt.footer.setCellValue(1, 9, "班长：");
			rt.footer.setCellValue(1,11, "制表：");
			
			rt.footer.setCells(1, 7, 1, 7, Table.PER_ALIGN, Table.ALIGN_RIGHT);
			rt.footer.setCells(1, 9, 1, 9, Table.PER_ALIGN, Table.ALIGN_RIGHT);
			rt.footer.setCells(1, 11, 1, 11, Table.PER_ALIGN, Table.ALIGN_RIGHT);
