...
public class Kuangfrchy extends BasePage implements PageValidateListener {
	// 界面用户提示
	private String msg = "";
...
//		long dcid = visit.getDiancxxb_id();
		...
		String sql = "select z.id, f.id as fid, g.mingc fahr,mk.mingc meikdw,p.mingc pinz,\n" +
			...
			" z.qnet_ar, z.stad, z.std, z.qbad,\n" + 
			"z.had,\n" + 
			"z.hdaf\n" + 
			"from fahb f, kuangfzlb z, vwfahr g,vwpinz p,meikxxb mk,diancxxb dc\n" + 
			"where f.kuangfzlb_id = z.id (+) and f.diancxxb_id=dc.id\n" + 
			"and f.gongysb_id = g.id\n" + 
			"and f.pinzb_id = p.id and f.jiesb_id=0 and f.meikxxb_id=mk.id\n" + 
			"and f.yewlxb_id<>(select id from yewlxb where mingc='直达') \n " +
			str+			
...
//		ResultSetList rsl = getExtGrid().getModifyResultSet(getChange());
		...
		String sql="begin \n" ;
//		while(rsl.next()){
			sql+="update fahb set kuangfzlb_id=0 where kuangfzlb_id="+change+"; \n"+
...
//		}
	...
	private void save() {
		...
		while(rsl.next()){
			...
			if(id==0){
				sql+="insert into kuangfzlb (id,gongysb_id,meikxxb_id,mt, mad, aar,aad, " +
						"ad, var, vad, vdaf, fcad, qgrad, qnet_ar, stad, std, qbad, had,hdaf,LURY)values("+
						newID+","+
						"(select id from gongysb where mingc='"+rsl.getString("FAHR")+"'),"+
						"(select id from meikxxb where mingc='"+rsl.getString("MEIKDW")+"'),"+
						mt+","+mad+","+aar+","+aad+","+ad+","+var+","+vad+","+vdaf+","+fcad+","+
						qgrad+","+qnet+","+stad+","+std+","+qbad+","+had+","+hdaf+",'"+visit.getRenymc()+"');\n";
				sql+="update fahb set kuangfzlb_id="+newID+" where id="+fid+";\n";
