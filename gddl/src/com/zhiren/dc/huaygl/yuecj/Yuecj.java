...
import com.zhiren.dc.monthReport.MonthReportOp;
 ...
 * @version 1.0
 */
public class Yuecj extends BasePage implements PageValidateListener {
	/* 月报填报自定义配置关键字 */
	...
	ExtGridUtil extGridUtil=new ExtGridUtil();
	...
	public void Copysykj() {
		...
		//String meiyybm=mdrsl.getString("meiyybm");
//		String choujbm=meiyybm;//meiyybm.substring(0,2).replace("JG","RG");//+meiyybm.substring(2,12);
		String sql = "begin \n";
		while(mdrsl.next()){
				if(mdrsl.getString("ID")!=null){
					sql += "update zhuanmldb set choujbm='" + mdrsl.getString("meiyybm").replaceAll("JG","RG")+"'"
	...
	private void Save() {
		...
		ResultSetList drsl = getExtGrid().getModifyResultSet(getChange());
		ResultSetList deleteResultSet = getExtGrid().getDeleteResultSet(getChange());

		String sql = "begin \n";
		while (drsl.next()) {
			//drsl.getString("id") == null
			if ("0".equals(drsl.getString("ID"))) {

				sql += "insert  into zhuanmldb (id,meiyybm,lid,xuh,riq,beiz)values(getnewid(" + visit.getDiancxxb_id() + "),'" +
						drsl.getString("meiyybm") + "'," +
						"to_char(" + drsl.getString("lid") + ",'fm90.09')," +
						drsl.getString("xuh") + "," +
						"to_date('" + getNianf() + "-" + getYuef() + "','yyyy-mm'),'" +
						drsl.getString("beiz") +
						"');";
			}else{
				sql += "update zhuanmldb set xuh=" + drsl.getString("xuh") + ","
						+ "meiyybm='" + drsl.getString("meiyybm") + "',"
						+ "lid=to_char(" + drsl.getString("lid") + ",'fm90.09'),"
						+ "beiz='" + drsl.getString("beiz") + "'\n" +
						"where ID=" +  drsl.getString("ID") + ";";
			}
			sql += "\n" + "end;";
			con.getUpdate(sql);
		}
		while(	deleteResultSet.next()) {
			//删除
			sql += "delete from zhuanmldb where ID=" + deleteResultSet.getString("ID") + ";";
			sql += "\n" + "end;";
			con.getUpdate(sql);
		}
	}
	...
	private void Save1() {
		...
		sql = "select t.id,tj.id yuetjkjb_id,gongysb.id as meikdq_id,jihkjb.id as jihkjb_id,pinzb.id as pinzb_id,\n"
				+ "fenx,jiesl,meij,meijs,kuangqyf,yunj,yunjs,daozzf,zaf,zafs,qit,qnet_ar,biaomdj,buhsbmdj\n"
	...
	private String getYuejsdjSql_by(String CODate, String data_c,
		...
		String sql = "select\n"
			...
			+ "from\n"
			+ "(select t.diancxxb_id, t.gongysb_id, t.jihkjb_id, t.pinzb_id\n"
...
//			+ SysConstant.Fenx_Beny
			+ ")\n"
			+ "group by diancxxb_id,gongysb_id,jihkjb_id,pinzb_id\n"
			+ "";
	return sql;
	}
	...
	private String getTjkjXuh(JDBCcon con,String diancId,String riq){
		int num=0;
		//String sql="select max(xuh+1) xuh from zhuanmldb where diancxxb_id="+diancId+" and riq="+riq;
		String sql="select max(xuh) xuh from zhuanmldb";
		ResultSetList rs=con.getResultSetList(sql);
		if (rs.next()) {
			num=rs.getInt("xuh")+1;
		}
	...
	private void initYuetjkj(JDBCcon con, String diancxxb_id, String CurrODate) {
		String sql = "select id,xuh,meiyybm,choujbm,lid,beiz from zhuanmldb order by id asc";
				/*+ "from zhuanmldb zb,yuetjkjb yb \n"
				+ "where yb.diancxxb_id ="
				+ diancxxb_id
				+ " and yb.riq = "
				+ CurrODate +" ORDER BY zb.id";*/
		ResultSetList rsl = con.getResultSetList(sql);
		...
		//egu.getColumn("xuh").setHeader("选择");
		egu.setGridSelModel(ExtGridUtil.GridselModel_Check);
		egu.addColumn(1,new GridColumn(GridColumn.ColType_Check));
		egu.getColumn("id").setHidden(true);
		egu.getColumn("id").setEditor(null);
		egu.getColumn("xuh").setHeader("序号");
		egu.getColumn("meiyybm").setHeader("样品编码");
		egu.getColumn("choujbm").setHeader("样品编码");
		egu.getColumn("lid").setHeader("粒度");
...
//		egu.getColumn("diancxxb_id").setWidth(0);
		...
		egu.getColumn("meiyybm").setWidth(120);
		egu.getColumn("choujbm").setWidth(120);
		egu.getColumn("lid").setWidth(100);

		egu.getColumn("xuh").setDefaultValue(getTjkjXuh(con,diancxxb_id,CurrODate));

		// 设置粒度下拉框
		egu.getColumn("lid").setEditor(new ComboBox());
//		egu.getColumn("lid").setComboEditor(egu.gridId,
//				new IDropDownModel(SysConstant.SQL_Lid));
		// 设置口径下拉框
...
//				new IDropDownModel(SysConstant.SQL_Kouj));
		...
		//------------------
//		GridButton gbt = new GridButton("生成","function(){document.getElementById('CopyButton').click();}");
//		gbt.setIcon(SysConstant.Btn_Icon_Copy);
//		egu.addTbarBtn(gbt);
		//------------------
		egu.addToolbarButton("生成", GridButton.ButtonType_Sel, "CopyButton",null, SysConstant.Btn_Icon_Show);
	...
	private void initYuesl(JDBCcon con, String diancxxb_id, String CurrODate) {
		...
		// 设置列标题
		egu.getColumn("meiyybm").setHeader("样品编码");
		egu.getColumn("choujbm").setHeader("抽检编码");
		egu.getColumn("lid").setHeader("粒度");
		egu.getColumn("beiz").setHeader("备注");
		...
		// 设置不可编辑
		egu.getColumn("choujbm").setEditor(null);
	...
	private void initYuezl(JDBCcon con, String diancxxb_id, String CurrODate) {
		...
		// 设置列标题
		egu.getColumn("gongysb_id").setHeader("样品编码");
		egu.getColumn("gongysb_id").setHeader("粒度");
		egu.getColumn("gongysb_id").setHeader("备注");
		//*egu.getColumn("gongysb_id").setHeader("供应商");
		egu.getColumn("jihkjb_id").setHeader("计划口径");
		egu.getColumn("pinzb_id").setHeader("品种");
		egu.getColumn("yunsfsb_id").setHeader("运输方式");//*
...
//				+ CurrODate + "\n" + "order by t.yuetjkjb_id,t.fenx ";
		...
		// 设置列标题
		egu.getColumn("gongysb_id").setHeader("样品编码");
		egu.getColumn("jihkjb_id").setHeader("粒度");
		egu.getColumn("pinzb_id").setHeader("备注");
		//egu.getColumn("gongysb_id").setHeader("供应商");
		//egu.getColumn("jihkjb_id").setHeader("计划口径");
		//egu.getColumn("pinzb_id").setHeader("品种");
		//egu.getColumn("yunsfsb_id").setHeader("运输方式");
