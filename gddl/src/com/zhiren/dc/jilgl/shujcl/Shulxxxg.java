 ...
 * 修改人:tzf
 * 时间:2009-06-26
 * 内容:铁路运输   时  发货数据的  运输单位 有可能不存在，用外关联数据
 */
/*
 * 2009-05-15
 * 王磊
 ...
 * 2009-8-6 
 * 王总兵
 * 1.增加"数量修改是否可以编辑皮重"的参数设置,默认为不可编辑
 * 2.去掉保存时必须填入运输单位的限制
 * 
 * 
 * 
 ...
 * 作者：王磊
 * 时间：2009-10-16
 * 描述：数量信息修改中增加按车号选择的颜色识别
 */
/*
 * 作者：王磊
 * 时间：2009-12-03
 * 描述：调整保存时总扣杂算法,原算法：this.isZongkdBfb()?(rsl.getDouble("koud")+rsl.getDouble("kous")+rsl.getDouble("kouz")):rsl.getDouble("koud")
 */
/*
 * 作者：赵胜男
 * 时间：2013-03-06
...
public class Shulxxxg extends BasePage implements PageValidateListener {
	...
	public void getSelectData(String tt) {
		...
		if (!boo_hejrows) {
			...
			sb.append(ordersql);*/
			//yuss 2012-03-29 将“原煤矿单位”默认取值为相应的煤矿名称，避免在修改时每次都要人工去选择原煤矿单位且保存时提示“原煤矿不能为空”
			sb
			.append("select c.id,f.id fahbid,f.yunsfsb_id,d.mingc  diancxxb_id,f.zhilb_id, cheph, c.biaoz, c.koud,c.kous,c.kouz, c.maoz, c.piz, c.yuns, c.zhongcsj, g.mingc as gongysb_id, m.mingc as meikxxb_id,mc.mingc as meicb_id,\n");
	sb
			.append("(select mingc from chezxxb fc where fc.id = f.faz_id) faz_id, \n");
	...
	sb.append(ordersql);
		...
		// GridButton("（模糊）查找/查找下一个","function(){SearCH(null,null);}");
		// chazhao.setIcon(SysConstant.Btn_Icon_Search);
		// egu.addTbarBtn(chazhao);
		// egu.addTbarText("-");
