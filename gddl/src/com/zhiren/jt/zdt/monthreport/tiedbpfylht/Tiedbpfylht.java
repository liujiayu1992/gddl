...
public  class Tiedbpfylht extends BasePage implements PageValidateListener {
	private String msg = "";
	...
	private void Save() {
		...
		while (rsl.next()){
			...
			if(!"0".equals(rsl.getString("id"))){
				...
				String bb="select diancxxb_id,gongysb_id,jihkjb_id,pinzb_id,faz_id,daoz_id,yunsfsb_id from tiedbpfyl where id="+rsl.getLong("id")+"";
				...
				String aa="delete from tiedbpfyl " +
			...
			for (int i=1;i<=12;i++){
				...
				//if (htl!=0){
					...
					String bb="insert into tiedbpfyl(id,diancxxb_id,gongysb_id,jihkjb_id,pinzb_id,faz_id,daoz_id,beiz,hej,riq,yunsfsb_id) values(" +
		...
		while (rsl.next()){
			...
			strYear=getNianfValue().getValue() +"-01-01";
			strYearEnd=getNianfValue().getValue() +"-12-01";
			String cc ="delete from tiedbpfyl " +
			"where (riq>=to_date('"+strYear +"','yyyy-mm-dd') and riq<=to_date('"+strYearEnd +"','yyyy-mm-dd'))" +
	...
	public void CoypLastYearData(){
		...
		while(rslcopy.next()){
			...
			con.getInsert("insert into tiedbpfyl(id,gongysb_id,diancxxb_id,hej,riq) values(" +
	...
	public void getSelectData() {
		...
		if(jib==1){//选集团时刷新出所有的电厂
			...
			strdiancTreeID = "  and (dc.fuid=  " +this.getTreeid()+" or dc.shangjgsid="+this.getTreeid()+")";
...
//		电厂下拉框
		...
		String GongysSql = "select id,mingc from gongysb where fuid in (0,-1) order by mingc";
