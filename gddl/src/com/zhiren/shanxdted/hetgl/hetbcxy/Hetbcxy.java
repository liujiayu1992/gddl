package com.zhiren.shanxdted.hetgl.hetbcxy;
...
import com.zhiren.jt.het.diancgmht.Zhilyqbean;
import com.zhiren.jt.het.diancgmht.jijbean;
 ...
 */
public class Hetbcxy extends BasePage implements PageValidateListener {
	...
	public void getmobmcSelectModels() {
		...
		if (this.isTijsh()) {
			sql = "select id,mingc " + "from hetb_mb "
					...
					+ "connect by prior id=fuid)" + " and nvl(liucztb_id,0)=1 order by hetb_mb.mingc ";
		} else {
			sql = "select id,mingc " + "from hetb_mb "
					+ "where diancxxb_id in(" + "select id from diancxxb\n"
					+ "start with id="
					+ ((Visit) getPage().getVisit()).getDiancxxb_id()
					+ "connect by prior id=fuid)   order by hetb_mb.mingc ";
		}
	...
	public void gethetSelectModels() {
		String sql = "";
		sql = "select id,hetbh\n" + "from hetb\n"
				+ "where hetb.liucztb_id=0 and fuid="
				+ getzhuhtSelectValue().getId() +"  order by hetbh ";
		((Visit) getPage().getVisit())
...
//		String sql = "";
//		//		
//		// if(((Visit) getPage().getVisit()).getString17().equals("XS")){
//		//			
//		// sql ="select h.id,h.hetbh\n" +
//		// " from hetb h,diancxxb d\n" +
//		// " where h.diancxxb_id=d.id\n" +
//		// " and (h.leib=0 or h.leib=1)\n" +
//		// " and h.liucztb_id=0\n" +
//		// " and h.fuid="+ getzhuhtSelectValue().getId()+"\n"+
//		// " and h.diancxxb_id="+((Visit)
//		// getPage().getVisit()).getDiancxxb_id();
//		// }else if(((Visit) getPage().getVisit()).getString17().equals("CG")){
//		// sql ="select h.id,h.hetbh\n" +
//		// " from hetb h,diancxxb d\n" +
//		// " where h.diancxxb_id=d.id\n" +
//		// " and h.leib=2\n" +
...
//			sql += " union select id,hetbh\n" + "from hetb\n"
//					+ "where hetb.liucztb_id=0 and fuid="
//					+ getzhuhtSelectValue().getId();
//			// 置空合同ID
...
//	}
	...
	public void getgongfModels() {
		String sql = "select id,piny||' '||mingc from gongysb where leix=1 order by mingc";
		((Visit) getPage().getVisit())
	...
	public void getxufModels() {// 显示该用户单位下所有孩子
		...
		if (((Visit) getPage().getVisit()).getString17().equals("CG")) {
			//专为外二设定需方单位
			...
			sql = "select id,mingc from diancxxb where  fuid="
					...
					+ diancId + ")  order by xuh";
			}
		} else if (((Visit) getPage().getVisit()).getString17().equals("XS")) {
			if (diancqc.equals("上海外高桥第二发电有限责任公司")) {
		...
		// + ((Visit) getPage().getVisit()).getDiancxxb_id()+" or id in(select
		// fuid from diancxxb where id="+((Visit)
		// getPage().getVisit()).getDiancxxb_id()+")";
		((Visit) getPage().getVisit())
...
//	 月份
	...
	// 主合同
	public IDropDownBean getzhuhtSelectValue() {
		if (((Visit) getPage().getVisit()).getDropDownBean25() == null) {
			((Visit) getPage().getVisit())
					.setDropDownBean25((IDropDownBean) getzhuhtSelectModel()
							.getOption(0));
		}
		return ((Visit) getPage().getVisit()).getDropDownBean25();
	}

	public void setzhuhtSelectValue(IDropDownBean Value) {
		if (Value != null
	...
	public void setzhuhtSelectModel(IPropertySelectionModel value) {
		((Visit) getPage().getVisit()).setProSelectionModel25(value);
	}

	public IPropertySelectionModel getzhuhtSelectModel() {
		if (((Visit) getPage().getVisit()).getProSelectionModel25() == null) {
			getzhuhtSelectModels();
		}
		return ((Visit) getPage().getVisit()).getProSelectionModel25();
	}

	public void getzhuhtSelectModels() {
		String sql = "";

		if (((Visit) getPage().getVisit()).getString17().equals("XS")) {

			sql = "select ht.id, hetbh\n"
					+ "       from hetb ht, diancxxb dc\n"
					+ "       where (leib = 0 or leib = 1)\n"
					+ "             and ht.fuid = 0\n"
					+ "             and to_char(ht.qiandrq, 'YYYY') = '" + getNianfValue().getId() + "'\n"
					+ "             and to_char(ht.qiandrq, 'MM') = " + getYuefValue().getId() + "\n"
					+ "             and diancxxb_id = dc.id\n"
					+ "             and (diancxxb_id = "
					+ ((Visit) getPage().getVisit()).getDiancxxb_id()
					+ "  or dc.fuid = "
					+ ((Visit) getPage().getVisit()).getDiancxxb_id()
		...
		// " and (diancxxb_id ="+((Visit)
		// getPage().getVisit()).getDiancxxb_id()+" or diancxxb_id in (select
		// fuid from diancxxb where id="+((Visit)
		// getPage().getVisit()).getDiancxxb_id()+")" +
		// "or diancxxb_id in (select id from diancxxb where fuid="+((Visit)
		// getPage().getVisit()).getDiancxxb_id()+"))";
		((Visit) getPage().getVisit())
				.setProSelectionModel25(new IDropDownModel(sql, ""));
		return;
	}
	...
	public IDropDownBean getJijlxValue() {
		if (((Visit) getPage().getVisit()).getDropDownBean26() == null) {
			((Visit) getPage().getVisit())
					.setDropDownBean26((IDropDownBean) getIJijlxModel()
							.getOption(0));
		}
	...
	public void beginResponse(IMarkupWriter writer, IRequestCycle cycle) {
		...
		if (!visit.getActivePageName().toString().equals(
			...
			if (visit.getActivePageName().toString().equals("Hetfz")) {
				value = visit.getString14();
				if (value != null) {
					isFuz = value.equals("") ? false : true;
				}
			}

			((Visit)this.getPage().getVisit()).setLong5(0);
			...
			if (!visit.getActivePageName().toString().equals("Hetfz")) {
				...
				if (cycle.getRequestContext().getParameters("lx") != null) {
					...
					((Visit) this.getPage().getVisit()).setString17(cycle
							.getRequestContext().getParameters("lx")[0]);
				} else {
					// 认为是电厂和煤矿或电厂和分公司签订的合同
					((Visit) this.getPage().getVisit()).setString17("XS");
				}
			}
		...
		if (cycle.getRequestContext().getParameters("lx") != null
			...
			if (!((Visit) this.getPage().getVisit()).getString17().equals(
		...
		if (((Visit) getPage().getVisit()).getboolean4() || ((Visit) getPage().getVisit()).getboolean10()) {// 年份刷新合同
			// gethetSelectModels();
			getzhuhtSelectModels();
		}
		...
		if (((Visit) getPage().getVisit()).getboolean7()) {// 主合同刷新补充合同
			Clear();
			gethetSelectModels();
		}
	}
...
//		合同变量定义Grid_end
	...
	public String getGridScript() {
		...
		if(getExtGrid()!=null){
			...
			return "Ext.MessageBox.alert('提示信息','请先选择或保存补充合同!');	\n";
		}
	}
...
//	合同变量设置_Grid_end
	...
	public void submit(IRequestCycle cycle) {
		...
		if (fuzButton) {
			fuzButton = false;
			if(getzhuhtSelectValue().getId()==-1){
				return;
			}
			fuz(String.valueOf(getzhuhtSelectValue().getId()));
		}
		if(_SaveButton){
			_SaveButton = false;
...
//			未保存了合同
			this.setMsg("请先保存补充合同!");
		}
	}

	public void fuz(IRequestCycle cycle) {
		if (this.getzhuhtSelectValue().getId() == -1) {
			this.setMsg("请选择主合同");
			return;
		}
		((Visit)this.getPage().getVisit()).setLong5(this.getzhuhtSelectValue().getId());
	...
	private void fuz(String hetxxb_id) {
		...
		String sql = "begin\n"
				...
				+ "  QISRQ,GUOQRQ,HETB_MB_ID,JIHKJB_ID,LIUCZTB_ID,LIUCGZID,LEIB,HETJJFSB_ID,MEIKMCS,XIAF,hetmtxzb_id) \n"
				+ "  (select "
				+ newid
				+ ",ID,DIANCXXB_ID,HETBH||'补充合同'||"+(gethetSelectModel().getOptionCount())+",QIANDRQ,QIANDDD,GONGFDWMC,GONGFDWDZ,GONGFDH, \n"
				+ "  GONGFFDDBR,GONGFWTDLR,GONGFDBGH,GONGFKHYH,GONGFZH,GONGFYZBM,GONGFSH,XUFDWMC,XUFDWDZ, \n"
				+ "  XUFFDDBR,XUFWTDLR,XUFDH,XUFDBGH,XUFKHYH,XUFZH,XUFYZBM,XUFSH,HETGYSBID,GONGYSB_ID, \n"
				+ "  QISRQ,GUOQRQ,HETB_MB_ID,JIHKJB_ID,0,LIUCGZID,LEIB,HETJJFSB_ID,MEIKMCS,XIAF,hetmtxzb_id \n"
				+ "  from hetb where hetb.id="
				+ hetxxb_id
				+ "); \n"
				+ "  insert into hetslb(ID,PINZB_ID,YUNSFSB_ID,FAZ_ID,DAOZ_ID,DIANCXXB_ID,RIQ,HETL,HETB_ID,ZHUANGT) \n"
				+ "  (select ID+100000,PINZB_ID,YUNSFSB_ID,FAZ_ID,DAOZ_ID,DIANCXXB_ID,RIQ,HETL,"
				+ newid
				+ ",ZHUANGT \n"
...
//		visit.setString17(""); // 页面参数
	...
	private void Xinj() {
		...
		((Visit) getPage().getVisit()).setExtGrid1(null);	//清楚变量
	}
	...
	private void Dak(long hetmb_id) {
		...
		try {
			...
			sql = "select hetjgb.ID,zhibb.mingc zhibmc,tiaojb.mingc tiaojmc,SHANGX,XIAX,zhibdw.mingc zhibdwmc,JIJ,jijdw.mingc jijdwmc,jijgs,\n"
					...
					+ hetmb_id + "order by xiax,hetjgb.id";
			rs = con.getResultSet(sql);
			i = 0;
			while (rs.next()) {
				long id = rs.getLong("id");
			...
			sql = "select z.ID,zhibb.mingc zhibmc,tiaojb.mingc tiaojmc,z.SHANGX,z.XIAX,zhibdw.mingc zhibdwmc,JIS,jisdw.mingc jisdwmc,\n"
					...
					+ "z.BEIZ,decode(z.SHIYFW,0,'全部','超出部分') as SHIYFW,p.mingc as pinz, decode(z.jijlx, 0, '含税单价', '不含税单价') jijlx\n"
					+ "from hetzkkb z,zhibb,tiaojb,danwb zhibdw,danwb jisdw,danwb koujdw,danwb zengfjdw,zhibb canzxm,danwb canzxmdw,hetjsxsb,yunsfsb,pinzb p\n"
					...
					+ "and z.yunsfsb_id=yunsfsb.id(+) and z.pinzb_id=p.id(+) and z.hetb_id="
					+ hetmb_id;
			rs = con.getResultSet(sql);
			i = 0;
			while (rs.next()) {
				...
				list.add(new Zengkkbean(id, ++i, zhibmc, tiaojmc, SHANGX, XIAX,
						...
						canzxmdwmc, CANZSX, CANZXX, hetjsxsmc, yunsfsmc, BEIZ,
...
//			打开变量
	...
	private void Baoc() {// 补充协议的类别在主合同中（值得注意）
		JDBCcon con = new JDBCcon();
		...
		if (gethetSelectValue().getId() == -1) {// 如果是新合同
			// 插入（新增）保存
			// 保存合同信息（包括供方需方）
			rs = con.getResultSetList("select fuid from hetb where id="
					+ gethetSelectValue().getId());
			if (rs.next()) {
				fuid = rs.getInt("fuid");
			}
			rs.close();
			if (getzhuhtSelectValue().getId() == -1 && fuid == 0) {
				this.setMsg("请选择主合同！");
			} else {
				...
				long hetb_id = Long.parseLong(MainGlobal
				...
				try {
					sql = "insert into hetb(ID,DIANCXXB_ID,HETBH,QIANDRQ,QIANDDD,GONGFDWMC,GONGFDWDZ,GONGFDH,GONGFFDDBR,GONGFWTDLR,"
							...
							+ "XUFYZBM,XUFSH,HETGYSBID,GONGYSB_ID,QISRQ,GUOQRQ,JIHKJB_ID,hetb_mb_id,liucgzid,meikmcs,leib,fuid)\n"
							+ "values("
							+ hetb_id
							+ ","
							+ getxufValue().getId()
							...
							+ getFahr()
							+ "',"
							+ MainGlobal.getTableCol("hetb", "leib", "id",
									String.valueOf(getzhuhtSelectValue()
											.getId()))
							+ ","
							+ getzhuhtSelectValue().getId() + ")";
				} catch (Exception e) {
				...
				for (int i = 0; i < list.size(); i++) {// 遍历容器
					...
					Shouhr = Shouhr.substring(Shouhr.lastIndexOf(";") + 1,
							Shouhr.length());
				...
				for (int i = 0; i < list.size(); i++) {// 遍历容器
					...
					String hetjsfsb_id = ((jijbean) list.get(i))
							.getHetjsfsb_id();
					String Hetjsxsb_id = ((jijbean) list.get(i))
							.getHetjsxsb_id();
					...
					String hetjjfsb_id = ((jijbean) list.get(i))
							.getHetjjfsb_id();
					double fengsjj = ((jijbean) list.get(i)).getFengsjj(); // 分公司加价
					String fengsjjdw = ((jijbean) list.get(i)).getZuigmjdw();
				...
				for (int i = 0; i < list.size(); i++) {// 遍历容器
					...
					String tiaojb_id = ((Zengkkbean) list.get(i))
							.getTIAOJB_ID();
					...
					String hetjsxsb_id = ((Zengkkbean) list.get(i))
							.getJIESXXB_ID();
					String yunsfsb_id = ((Zengkkbean) list.get(i))
							.getYUNSFSB_ID();
...
//				新建合同_保存变量_end
			...
			if (getzhuhtSelectValue().getId() == -1 && fuid == 0) {
				...
				sql = "update hetb set HETBH='" + bean.getHetbh()
						...
						+ ",meikmcs='" + getFahr() + "',fuid="
						+ getzhuhtSelectValue().getId() + " where id="
						+ hetb_id;
				con.getUpdate(sql);
						...
						for (int j = 0; j < 12; j++) {
							if (Y[j] != 0) {
								sql = "insert into hetslb(ID,PINZB_ID,YUNSFSB_ID,FAZ_ID,DAOZ_ID,DIANCXXB_ID,RIQ,HETL,HETB_ID)values("
										...
										+ Shouhr
										+ "'),to_date('"
										+ String.valueOf(bean.getShengxsj()
												.getYear() + 1900)
										+ (j + 1)
										+ "','YYYYMM'),"
										+ Y[j]
				...
				for (int i = 0; i < list.size(); i++) {// 遍历容器
					...
					if (hetjgb_id == 0) {
						...
						sql = "insert into hetjgb(ID,ZHIBB_ID,TIAOJB_ID,SHANGX,XIAX,DANWB_ID,JIJ,JIJDWID,JIJGS,HETJSFSB_ID,HETJSXSB_ID,YUNJ,YUNJDW_ID,\n"
								...
								+ "'),"
								+ this
										.getProperId(this.getIJijlxModel(),
												jijlx)
								+ ",(select max(id) from pinzb where mingc='"
								+ pinz + "'))";
						con.getInsert(sql);
				...
				for (int i = 0; i < list.size(); i++) {// 遍历容器
					...
					if (hetzkkb_id == 0) {
						...
						sql = "insert into hetzkkb( ID,ZHIBB_ID,TIAOJB_ID,SHANGX,XIAX,DANWB_ID,JIS,JISDWID,KOUJ,KOUJGS,KOUJDW,ZENGFJ,"
								+ "ZENGFJGS,ZENGFJDW,XIAOSCL,JIZZKJ,JIZZB,CANZXM,CANZXMDW,CANZSX,CANZXX,HETJSXSB_ID,YUNSFSB_ID,BEIZ,"
								+ "HETB_ID,SHIYFW,PINZB_ID,JIJLX)"
								+ "values("
								+ "xl_xul_id.nextval"
								+ ",(select id from zhibb where mingc='"
								+ zhibb_id
								...
								+ zengfjdw
								+ "'),"
								+ getProperId(getxiaoswcljSelectModel(),
										xiaoscl)
								+ ","
								+ jizzkj
								+ ","
						...
						sql = "update hetzkkb set ZHIBB_ID=(select id from zhibb where mingc='"
								...
								+ beiz
								+ "',pinzb_id=(select max(id) from pinzb where mingc='"
								+pinzb_id+"'), jijlx="+ this.getProperId(this.getIJijlxModel(), jijlx) 
								+" where id=" + hetzkkb_id;
						con.getUpdate(sql);
					}
				}
	...
	private void gethetmb(long hetmb_id) {
		...
		try {
			...
			sql = "select z.ID,zhibb.mingc zhibmc,tiaojb.mingc tiaojmc,z.SHANGX,z.XIAX,zhibdw.mingc zhibdwmc,JIS,jisdw.mingc jisdwmc,\n"
					...
					+ "decode(XIAOSCL,1,'进位',2,'舍去',3,'四舍五入',4,'四舍五入(0.1)',5,'四舍五入(0.01)','')XIAOSCL,JIZZKJ,JIZZB\n"
					+ ",CANZXM.Mingc canzxmmc,CANZXMDW.Mingc canzxmdwmc,CANZSX,CANZXX,hetjsxsb.mingc hetjsxsmc,yunsfsb.mingc yunsfsmc,\n"
					+ "z.BEIZ,decode(z.SHIYFW,0,'全部','超出部分')SHIYFW,p.mingc as pinz,decode(z.jijlx, 0, '含税单价', '不含税单价') jijlx\n"
					+ "from hetzkkb z,zhibb,tiaojb,danwb zhibdw,danwb jisdw,danwb koujdw,danwb zengfjdw,zhibb canzxm,danwb canzxmdw,hetjsxsb,yunsfsb,pinzb p\n"
					+ "where z.zhibb_id=zhibb.id  and z.tiaojb_id=tiaojb.id and z.danwb_id=zhibdw.id and z.jisdwid=jisdw.id(+) and z.koujdw=koujdw.id(+)\n"
					+ "and z.zengfjdw=zengfjdw.id(+) and z.canzxm=canzxm.id(+) and z.canzxmdw=canzxmdw.id(+) and z.hetjsxsb_id=hetjsxsb.id(+)\n"
					+ "and z.yunsfsb_id=yunsfsb.id(+) and z.pinzb_id=p.id(+) and z.hetb_id="
