...
//			getSelectData();
	...
	public void getSelectData() {
		...
		JDBCcon con = new JDBCcon();
		ResultSetList rsl = con.getResultSetList("select y.id,y.riq,y.bianh,g.mingc as gongysb_id ,s.mingc as shoukdwb_id,y.jine,y.kaihyh,y.zhangh,decode(leib,1,'煤款',2,'运费',3,'杂费',4,'其他')as leib,y.fapbh as fapbh,y.jingbr as jingbr,y.beiz from yufkb y , gongysb g, shoukdw s where y.riq>=to_date('"+getRiqi()+
		...
		egu.getColumn("shoukdwb_id").setHeader("收款单位");
...
//		egu.setWidth(1000);
		...
		sb.append("{text:' 添加',icon:'imgs/btnicon/insert.gif',cls:'x-btn-text-icon',minWidth:75,handler:function() {	\n")
			...
			.append("var plant = new gridDiv_plant({ID: '0',RIQ: '',BIANH: '',GONGYSB_ID: '"+gongys+"',SHOUKDWB_ID: '请选择',JINE: '',KAIHYH: '"+kaihyh+"',ZHANGH: '"+zhangh+"',LEIB: '请选择',FAPBH: '',JINGBR: '"+((Visit) getPage().getVisit()).getRenymc()+"',BEIZ: ''});	\n")
