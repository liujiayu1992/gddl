...
public class Jiesmx_gdds extends BasePage implements PageValidateListener {
	...
	public void beginResponse(IMarkupWriter writer, IRequestCycle cycle) {
		...
		String binam = cycle.getRequestContext().getParameter("bianm");
		visit.setString19(binam);
...
//    	上限
	...
	public String getPrintTable() {
		...
		String sql = 
			...
			"       (select bianm from zhuanmb where zhillsb_id = d.zhilblsb_id and zhuanmlb_id = 100661) 采样编码,\n" + 
			"       (select bianm from zhuanmb where zhillsb_id = d.zhilblsb_id and zhuanmlb_id = 100662) 制样编码,\n" + 
			"       (select bianm from zhuanmb where zhillsb_id = d.zhilblsb_id and zhuanmlb_id = 100663) 化验编码,\n" + 
			"       z.qnet_ar as qnet_ar,\n" + 
			"       z.mt as Mt,\n" + 
			"       round_new(nvl(z.stad * (100 - z.mt) / (100 - z.mad), 2),2)as Star,\n" + 
			"       z.aar as Aar,\n" + 
			"       z.vdaf as Vdaf,\n" + 
			"       decode(z.shenhzt, 0, '未化验', '已化验') shenhzt\n" + 
			"  from caiyb c, yangpdhb d, bumb b, zhillsb z, fahb f,meikxxb m\n" + 
			...
			"   AND z.zhilb_id IN\n" + 
			"       (SELECT zhilb_id\n" + 
			"          FROM fahb f, diancxxb d, jiesb j\n" + 
			"         WHERE f.diancxxb_id = d.id\n" + 
			"           and d.id = "+diancxxb_id+"\n" + 
			"          and f.jiesb_id=j.id\n" + 
			"          and j.bianm='"+visit.getString19()+"'\n" + 
			"               )\n" + 
			" order by  daohrq";
		...
		ArrHeader[0] = new String[] {"煤矿","来煤数量","到货日期","采样编码","制样编码","化验编码","Qnet_ar","Mt","Star","Aar","Vdaf","状态"};
		int ArrWidth[]=new int[] {100,60,80,100,60,60,50,50,50,50,50,50};
		rt.setTitle("结算编码明细", ArrWidth);
