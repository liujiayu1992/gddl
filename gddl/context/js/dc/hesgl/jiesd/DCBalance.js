...
function colsCount(){
	...
	if(eval(document.getElementById("JIAKJE").value)>0){
		...
		if(document.getElementById("SHIFCSDJ").value!="否"){
			...
			document.all.item("JIASJE").value = Math.round((eval(document.all.item("JIASJE").value)+eval(document.all.item("BUKYQJK").value))*100)/100;
			...
			document.all.item("JIAKJE").value =eval(document.all.item("JIAKHJ").value);
			document.all.item("BUHSDJ").value=Math.round(eval(document.all.item("JIAKHJ").value)/eval(document.all.item("JIESSL").value)*10000000)/10000000;
    ...
    if(eval(document.all.item("TIELYF").value)>0
    	...
    	if(document.getElementById("SHIFCSDJ").value!="否"
	    	...
	    	document.all.item("YUNFSK").value = Math.round((eval(document.all.item("YUNZFHJ").value)-eval(document.all.item("TIELZF").value)-eval(document.all.item("KUANGQZF").value))*eval(document.all.item("YUNFSL").value)*100)/100;
...
//		判断合同
		if(document.all.item("JINGZ").value<=0||document.all.item("JIESSL").value<=0||document.all.item("GONGFSL").value<=0){
			alert("结算数量,供方数量，过衡重量 小于或等于零，请核对！");
			return false;
		}			
		return true;
	}
