...
function Tianzdwdrop(_obj1,_obj2){
	if (oldSelectobj!=null){
		oldSelectobj.style.display="none";
	}
	_obj2.style.borderRight="#FC0303 1px solid";
	...
	_obj2.style.borderTop="#FC0303 1px solid";
	_obj2.style.borderBottom="#FC0303 1px solid";
	_obj2.style.posLeft =_obj1.offsetLeft+5;
	_obj2.style.posTop=_obj1.offsetTop+28;
	_obj2.style.posWidth=_obj1.offsetWidth+1;
	_obj2.style.posHeight=_obj1.offsetHeight ; 
	_obj2.style.position="absolute";
...
function Count(){
	if(eval(document.all.item("GONGFL").value)>0){
    	...
    	document.all.item("JINGZ").value=eval(document.all.item("GONGFL").value);
...
function CountYK(){
	if(eval(document.all.item("YINGK").value)>0 & eval(document.all.item("SHULZJBZ").value)>0){
		document.all.item("SHULZJJE").value = eval(document.all.item("YINGK").value)*eval(document.all.item("SHULZJBZ").value)
	}
}
...
function check(){
	if(document.all.item("SHOUKDW").value==""||document.all.item("SHOUKDW").value=="请选择"){
		...
		alert("原收货人为空，请核对！");
		return false;
	}else if(document.all.item("XIANSHR").value=="" || document.all.item("XIANSHR").value=="请选择"){
		alert("现收货人为空，请核对！");
		return false;
...
//	else if(document.all.item("HETONGDropDown").value==""||document.all.item("HETONGDropDown").value=="-1"){
//		alert("合同编号为空，请核对！");
//		return false;
//	};
