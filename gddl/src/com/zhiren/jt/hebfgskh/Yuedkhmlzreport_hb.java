...
public class Yuedkhmlzreport_hb  extends BasePage implements PageValidateListener{
...
//		
		...
		String ss=
			...
			"SELECT gmingc,mmingc,pmingc,fl,rez,stad,jiz,beiz,\n" + 
			"decode(sign(jif),-1,'<font color=red >'||Formatxiaosws(jif,2)||'</font>',1,'<font color=blue >'\n" + 
			"||Formatxiaosws(jif,2)||'</font>',0,'0')  jif \n" + " FROM (\n" + 
//			"rez,stad,jiz,beiz,jif FROM (\n" + 
			"(\n" + 
			"\n" + 
			"select  fdl.gmingc, '当月综合硫分(不含煤泥、中煤等)' mmingc,fdl.pmingc ,\n" + 
			"\n" + 
			...
			"                 ,getjiz(round_new(decode(sum(ah), 0, 0, round_new(sum(fdl.stad * ah) / sum(ah), 2)),0),'KHXM_ZHLF','原煤') jiz\n" + 
			"                 ,getbiaoz(round_new(decode(sum(ah), 0, 0, round_new(sum(fdl.stad * ah) / sum(ah), 2)),0),'KHXM_ZHLF','原煤') beiz,\n" + 
			"           round_new(round_new(sum(ah)/3300,0)*nvl(getbiaoz(round_new(sum(fdl.stad * ah) / sum(ah), 2),'KHXM_ZHLF','原煤')*\n" + 
			"           getchaocjz(abs(getchaz(round_new(sum(stad * ah) / sum(ah), 2),'KHXM_ZHLF','原煤')),round_new(sum(stad * ah) / sum(ah),2),'KHXM_ZHLF')\n" + 
			"\n" + 
			"            ,0),3) jif,2 as xuh\n" + 
			" from\n" + 
			"(select f.id,g.mingc gmingc,m.mingc mmingc,p.mingc pmingc,f.laimsl,f.jingz,z.stad\n" + 
			...
			"(\n" + 
			"\n" + 
			"select  fdl.gmingc, decode(fdl.mmingc,'','当月综合硫分',fdl.mmingc) mmingc,fdl.pmingc ,\n" + 
			"\n" + 
			"round_new(sum(fdl.jingz),0) as fl,\n" + 
			"\n" + 
			"         decode(sum(ah), 0, 0, round_new(sum(fdl.stad * ah) / sum(ah), 2)) as stad,\n" + 
			...
			"                 round_new(decode(sum(ah), 0, 0, round_new(sum(fdl.rez * ah) / sum(ah), 2)),0) as rez\n" + 
			"                 ,\n" + 
			"            decode(fdl.mmingc,'','0', getjiz(round_new(decode(sum(ah), 0, 0, round_new(sum(fdl.stad * ah) / sum(ah), 2)),0),'KHXM_FKLF',fdl.pmingc))     jiz\n" + 
			"                 ,getbiaoz(round_new(decode(sum(ah), 0, 0, round_new(sum(fdl.stad * ah) / sum(ah), 2)),0),'KHXM_FKLF',fdl.pmingc) beiz,\n" + 
			"           round_new(round_new(sum(ah)/3300,5)*nvl(getbiaoz(round_new(sum(fdl.stad * ah) / sum(ah), 5),'KHXM_FKLF',fdl.pmingc)*\n" + 
			"           getchaocjz(abs(getchaz(round_new(sum(stad * ah) / sum(ah), 5),'KHXM_FKLF',fdl.pmingc)),round_new(sum(stad * ah) / sum(ah),5),'KHXM_FKLF')\n" + 
			"\n" + 
			"            ,0),3) jif,1 as xuh\n" + 
			" from\n" + 
			"(select f.id,g.mingc gmingc,m.mingc mmingc,p.mingc pmingc,f.laimsl,f.jingz,z.stad\n" + 
			...
			"                 ,'0' jiz\n" + 
			"                 ,0 beiz,\n" + 
//			"          SUM( round_new(round_new(sum(ah)/3300,0)*nvl(getbiaoz(round_new(sum(fdl.stad * ah) / sum(ah), 2),'KHXM_FKLF','原煤')*\n" + 
//			"           getchaocjz(abs(getchaz(round_new(sum(stad * ah) / sum(ah), 2),'KHXM_FKLF','原煤')),round_new(sum(stad * ah) / sum(ah),2),'KHXM_FKLF')\n" + 
			"\n" + 
...
//			"having (grouping(fdl.gmingc)+grouping(mmingc)+grouping(fdl.pmingc)) =3\n" + 
		...
		ArrHeader[0]=new String[]{"供应商","煤矿","煤种","净重","热值","硫分","硫分","硫分","硫分"};
	    ArrHeader[1]=new String[] {"供应商","煤矿","煤种","吨","Kcal/Kg","Stad(%)","考核基准","扣分标准(分/列)","计分"};
		ArrWidth =new int[] {120,75,75,75,75,75,75,75,75};
		ResultSetList rs = cn.getResultSetList(ss);
		int col=0;
		if(rs.getRows()>0){
			col=rs.getRows();
		}
//		System.out.println(ss);
		...
		rt.setDefaultTitle(5, 1, "填报日期:"+intyear+"年"+intMonth+"月", Table.ALIGN_CENTER);
...
//		rt.body.merge(col, 1, col, 3);
		if(col>1){
			rt.body.merge(col, 1, col, 3);
//			rt.body.merge(col-3, 1, col-3, 3);
		}
