...
public class GongysChakext extends BasePage implements PageValidateListener {
	...
	private void Save() {
		...
		try {
			long _id =((Visit) getPage().getVisit()).getLong10() ;
			int xuh = getXuh();
			...
			if (_id == -1) {
				String SQL = "select * from gongysb where mingc = '" + mingc
						+ "'";
				...
				SQL = "select * from gongysb where quanc = '" + quanc + "'";
				...
				String sql = "insert into gongysb(id,fuid,xuh,mingc,quanc,piny,bianm,danwdz,faddbr," +
						" weitdlr,kaihyh,zhangh,dianh,shuih,youzbm,chuanz,meitly,meiz,chubnl," +
						"kaicnl,kaicnx,shengcnl,gongynl,liux,yunsfs,shiccgl,zhongdht,yunsnl,heznx,rongqgx," +
						"xiny,gongsxz,kegywfmz,kegywfmzzb,shengfb_id,shifss,shangsdz,zicbfb,shoumbfb,qitbfb,beiz) values (" + _id + "," + fuid
						+ "," + xuh + " ,'" + mingc + "','" + quanc + "','"
						+ piny + "','" + bianm + " ','" + danwdz + " ','"
				...
				if (flag == -1) {
					...
					con.getInsert("insert into gongysdcglb values("
...
//					向gongysmzb里插入数据结束
	...
	public void Save(String strchange, Visit visit) {
		...
		for (int i = 0; i < change.length; i++) {
			...
			String record[] = change[i].split(",", getDataColumnCount() + 1);
			if ("D".equals(record[0])) {
				sql.append("delete from ").append("meikxxb").append(
						" where id =").append(record[1]).append(";\n");
				sql.append("delete from ").append("gongysmkglb").append(
						" where meikxxb_id =").append(record[1]).append(
						" and gongysb_id = " + getMId() + ";\n");
				sql3.append("delete from kuangzglb where meikxxb_id = "+record[1]+";\n");
			} else if ("U".equals(record[0])) {
				StringBuffer sql2 = new StringBuffer();
				if ("0".equals(record[1])) {
					int index = 0;
					sql.append("insert into ").append("meikxxb").append("(");
					sql3.append("insert into ").append("kuangzglb").append(
							"(id,meikxxb_id,chezxxb_id) values (");
					for (int c = 0; c < getExtGrid().getGridColumns().size(); c++) {
						GridColumn gc = (GridColumn) getExtGrid()
								.getGridColumns().get(c);
									...
									sql3.append(
											Long.parseLong(MainGlobal
													.getNewID(visit
															.getDiancxxb_id()))
													+ "," + _id + ",").append(
											getValueSql(gc, record[9])).append(
											");");
								} else {
									sql.append(gc.dataIndex).append(",");
									if (index == 1) {
										_id = Long.parseLong(MainGlobal
												.getNewID(visit
														.getDiancxxb_id()));
										sql2.append("" + _id + ",");
									} else {
										sql2.append(
												getValueSql(gc, record[index]))
												.append(",");
									}
								}
							}
					...
					sql.append("insert into ").append("gongysmkglb")
							.append("(");
					sql.append("id,gongysb_id,meikxxb_id) values(").append("")
							.append(
									""
											+ Long.parseLong(MainGlobal
													.getNewID(visit
					...
					int index = 0;
					sql.append("update ").append("meikxxb").append(" set ");
					for (int c = 0; c < getExtGrid().getGridColumns().size(); c++) {
						GridColumn gc = (GridColumn) getExtGrid()
								.getGridColumns().get(c);
						if (gc.coltype == GridColumn.ColType_default) {
							...
							if (index > 1) {
								if (gc.update) {
									if (index == 9) {
										sql3
												.append("update kuangzglb set chezxxb_id = "
														+ getValueSql(gc,
																record[index])
														+ " where meikxxb_id = "
														+ record[1] + ";\n");
					...
					sql.deleteCharAt(sql.length() - 1);
					sql.append(" where id =").append(record[1]).append(";\n");
				}
			}
		}
		sql.append("end;");
	...
	private void getToolbars(){
		...
		ToolbarButton but1 = new ToolbarButton(null,"保存","function(){if(!validatecombox()){return;};document.getElementById('SaveButton').click();"+MainGlobal.getExtMessageShow("提示","正在保存....",200)+"}");
		tb.addItem(but);
	...
	public void getFuid_idModels() {
		String sql = "select f.*\n"
				+ "  from (select 0 as id, decode(1, 1, '大供应商') as fuid\n"
				+ "          from dual\n"
				+ "        union\n"
				+ "        select 1 as id, decode(1, 1, '小供应商') as fuid from dual) f\n"
				+ " order by id";
		((Visit) getPage().getVisit())
	...
	public void getFUIDModels() {
		String sql = "select id,mingc from gongysb where fuid = 0 order by mingc,xuh";
		((Visit) getPage().getVisit())
	...
	public void beginResponse(IMarkupWriter writer, IRequestCycle cycle) {
		...
		if (!visit.getActivePageName().toString().equals(
			...
			visit.setList1(null);
			visit.setLong10(Long.parseLong(_id));
			getSelectData();
		}
		getToolbars();
	}
		...
		while (rsl.next()) {
			...
			if ("0".equals(rsl.getString("ID"))) {
				...
				sql.append("insert into gongysmkglb(id,gongysb_id,meikxxb_id) values(")
				   ...
				   .append(",").append(visit.getLong10()).append(",")
