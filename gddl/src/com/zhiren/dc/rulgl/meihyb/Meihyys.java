...
public class Meihyys extends BasePage implements PageValidateListener {
	private String msg = "";
	...
	private void Save(int num) {
		...
		if (DefaultShjb.equals("不审核")) {
			...
			if (DefaultShjb.equals("一级审核")) {
				DefaultShzt = "5";
				msgstr = "数据已封存！";
			} else {
				if (DefaultShjb.equals("两级审核")) {
					DefaultShzt = "3";
					msgstr = "数据已提交！";
				}
			}
		...
		int shenhzt = 0;
		if (num == 1) {
			shenhzt = Integer.parseInt(DefaultShzt);
		} else if (num == 2) {
			shenhzt = 2;
			msgstr = "数据已回退！";
		}
	...
	public void getSelectData() {
		...
		String chaxun = "select m.id,m.diancxxb_id, m.rulrq,m.rulmzlb_id, rb.mingc as rulbzb_id,j.mingc as jizfzb_id,\n"
				...
				+ "   and d.id ="
				+ visit.getDiancxxb_id()
				+ "   and (m.shenhzt = 1 or m.shenhzt = 4)";
