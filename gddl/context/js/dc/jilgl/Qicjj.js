	...
	if (MSComm1.InBufferCount > 7) {
		...
		switch (DataMode) {
			...
			case "120" :
				...
				zhongl = eval(zhongl);
				break;
			// 数据原型< Aa+00856000000000` B > 实际值为8.560 韩城二电二期表头4#
			case "220" :
				str = Instring.substring(0, 44);
				// alert(str);
				for (var i = 0; i <= 9; i++) {
					if (str.substr(i, 1) == '+') {
						temp1 = str.substring(i + 1, i + 7);
				...
				// for(var i =(temp1.length-1);i>= 0;i--){
				// if(temp1.substr(i,1)=='
				// '||temp1.substr(i,1)=='='||temp1.substr(i,1)=='-'||temp1.substr(i,1)=='+'){
				// Stemp=Stemp+'0';
				// }else Stemp = Stemp + temp1.substr(i,1);
				// }
				// alert(temp1);
			...
			case "210" :
				...
				blen = Stemp.indexOf("");
				if (blen != -1) {
					str = Stemp.substring(blen + 3, blen + 9);
					zhongl = parseInt(str, 10);
				}
				break;
			//大唐鲁北发电
...
function caij1() {
	...
	if (Ext.isReady) {
		...
		if (jingzXt < 0) {
			...
			if (DataIndex == 'PIZ') {
				var jin_va = (parseFloat(rec.get('MAOZ'))
						- parseFloat(rec.get('PIZ'))
						- parseFloat(rec.get('KOUD'))
						- parseFloat(rec.get('KOUS')) - parseFloat(rec
						.get('KOUZ')))
						+ '';
				// if(jin_va.lastIndexOf(".")>=0){
				// jin_va=jin_va.substring(0,jin_va.lastIndexOf("."))+jin_va.substr(jin_va.lastIndexOf("."),2);}
				// rec.set('BIAOZ',jin_va);
				rec.set('JINGZ', jin_va);
			} else {
				// rec.set('BIAOZ',rec.get('MAOZ'));
			}
			...
			if (DataIndex == 'PIZ') {
				var jin_va = (parseFloat(rec.get('MAOZ'))
						- parseFloat(rec.get('PIZ')) - parseFloat(rec
						.get('KOUD')))
						+ '';
				var koud_va = "0";
				var s = "0.";
				var km = "";
				jin_va = jin_va.substring(0, jin_va.lastIndexOf("."))
						+ jin_va.substr(jin_va.lastIndexOf("."), jingzXt + 5);
				...
				if (jin_va.lastIndexOf(".") >= 0) {
					...
					if (jin_va.substring(jin_va.lastIndexOf(".") + 1).length > jingzXt) {// 尾数足够，要保留尾数到koud
						...
						var num_tem = Math.ceil(parseFloat("0."
								+ jin_va.substr(jin_va.lastIndexOf(".") + 1
												+ jingzXt + 1, 2)));
						var k_t = parseFloat(jin_va.substr(jin_va
										.lastIndexOf(".")
										+ 1 + jingzXt, 1))
								+ num_tem;
						// alert(k_t);
					...
					jin_va = parseFloat(jin_va.substring(0, jin_va
									.lastIndexOf(".")))
							+ parseFloat(km
									+ "0"
									+ jin_va.substr(jin_va.lastIndexOf("."),
											jingzXt + 1)) + parseFloat(s) + "";

					if (jin_va.lastIndexOf(".") != -1
							&& jin_va.substring(jin_va.lastIndexOf(".")).length > jingzXt
									+ 1) {
						var kl = "0.";
						for (var i = 0; i < jingzXt; i++) {
							kl += "0";
