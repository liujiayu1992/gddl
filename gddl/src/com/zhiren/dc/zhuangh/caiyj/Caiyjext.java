package com.zhiren.dc.zhuangh.caiyj;
 ...
 * 
 */
public class Caiyjext extends BasePage implements PageValidateListener {
	private String msg = "";
...
//		System.out.println(Change);
		...
		while (drsl.next()) {
			sql.append("delete from ").append("caiyjb").append(" where id =")
		...
		while (rsl.next()) {
			...
			if ("0".equals(rsl.getString("ID"))) {
				sql.append("insert into ").append("caiyjb").append("(id");
				...
				sql.append("update ").append("caiyjb").append(" set ");
				for (int i = 1; i < rsl.getColumnCount(); i++) {
	...
	public void getSelectData() {
		...
		ResultSetList rsl = con
				.getResultSetList("select distinct m.id,m.xuh,dc.mingc as diancxxb_id,m.mingc,m.bianm," 
...
//								"ImageReport&&id='||m.id||'&&mk=meicxx\" target=\"_blank\">查看</a>' as meict,"
						+ "m.beiz from caiyjb m,diancxxb dc where jib=3  and dc.id=m.diancxxb_id "
						+ "" + str +
						// " and m.diancxxb_id ="+ getTreeid() +
						" order by m.xuh,m.mingc");

		ExtGridUtil egu = new ExtGridUtil("gridDiv", rsl);
		egu.setTableName("caiyjb");
