 ...
 * 描述：处理总金额显示为科学计数法的问题
 */
public class Jiesmx_xb_v1 extends BasePage implements PageValidateListener {
	private String yunj = "";
...
//		rt.title.setRowCells(2, Table.PER_FONTBOLD, true);
	...
	public Paragraph getMingx(JDBCcon con){
		...
		String sql = 
			...
			"		title.huayz,\n" +
			"		zhejbz,round(body.koud*jiesdj,2) kouk,title.koud,round(ches,0) from\n" +
...
//		substr(title.bianm,5,length(title.bianm)),
		...
		ArrHeader[0] = new String[] { "样本编号", "矿点", "化验员", "样本容量", "折扣方式",
				"标准值", "化验项目", "化验值", "扣价", "扣吨<br>折价", "扣矸<br>石量", "车数" };
		int ArrWidth[] = new int[] { 60, 70, 50, 50, 50, 80, 105, 40, 40, 40, 40, 40 };
		Report rt = new Report();
	...
	public Paragraph getJiesqk(JDBCcon con){
		...
		String sql = 
//			"select decode(grouping(bianm),0,substr(bianm,4,length(bianm)),'合计') bianm,\n" +
//			"          sum(gongfsl) gongfsl,\n" + 
//			"          sum(yanssl) yanssl,\n" + 
//			"          round(avg(shulzk),2) zhulzk,\n" + 
//			"          round(avg(shulkk),2) shulkk,\n" + 
//			"          round(avg(kouj),2) kouj,\n" + 
//			"          sum(zhejje) zhejje,\n" + 
//			"          sum(koudzkk) koudzkk,\n" + 
//			"          sum(gongfsl*"+yunj+") yunf,\n" + 
//			"          sum(jiashj) jiashj\n" + 
//			"  from\n" + 
//			"(\n" + 
//			"select z.bianm,d.gongfsl,d.yanssl,\n" + 
//			"         dayuzerodisp(sum(decode(zhibb_id,21,zhejbz,0))) shulzk,xiaoyuzerodisp(sum(decode(zhibb_id,21,zhejbz,0))) shulkk,\n" + 
//			"         d.hetj-d.jiesdj kouj,sum(d.zhejje) zhejje,\n" + 
//			"         round((d.yanssl-d.jiessl)*d.jiesdj*-1,2) koudzkk,0 yunf,d.jiashj\n" + 
//			"        from danpcjsmxb d,jiesb j,fahb f,zhuanmb z,zhuanmlb lb,zhillsb zl\n" + 
//			"         where d.jiesdid=j.id\n" + 
//			"           and to_char(d.lie_id) =to_char(f.lie_id)\n" + 
//			"           and j.id="+getJiesID()+"\n" + 
//			"           and z.zhuanmlb_id=lb.id\n" + 
//			"           and lb.mingc='化验编码'\n" + 
//			"           and zl.id=z.zhillsb_id\n" + 
//			"           and f.zhilb_id=zl.zhilb_id\n" + 
//			"group by z.bianm,gongfsl,yanssl,d.jiessl,d.jiesdj,d.hetj,jiashj\n" + 
//			")\n" + 
//			"group by rollup ((bianm,gongfsl,yanssl,shulzk,shulkk,kouj,zhejje,koudzkk,yunf,jiashj))\n" + 
//			"order by substr(bianm,2,length(bianm)-1)";
			//2012-11-18备


		"select decode(grouping(bianm),0,substr(bianm,4,length(bianm)),'合计') bianm,\n" +
		"          sum(gongfsl) gongfsl,\n" + 
		"          decode(grouping(bianm),1,0,round(avg(kouj),2)*-1) kouj,\n" + 
		"          round(avg(shulzk),2) zhulzk,\n" + 
		"          round(avg(shulkk),2) shulkk,\n" + 
		"          sum(yanssl) yanssl,\n" + 
		"          sum(zhejje) zhejje,\n" + 
//		"          round(sum(yanssl)*round(avg(kouj),2)*-1,2) zhejje,\n" + 
		"          sum(koudzkk) koudzkk,\n" + 
		"          sum(gongfsl*"+yunj+") yunf,\n" + 
		"          sum(jiashj) jiashj\n" + 
		"  from\n" + 
		"(\n" + 
		"    select bianm,gongfsl,yanssl,dayuzerodisp((hetj-kouj-jiesdj)*-1) shulzk,xiaoyuzerodisp((hetj-kouj-jiesdj)*-1) shulkk,kouj," +
//		"			zhejje," +
		"			round(yanssl*kouj*-1,2) zhejje," +
		"			koudzkk,0 yunf,jiashj\n" + 
		"    from (\n" + 
		"            select z.bianm,d.gongfsl,d.yanssl,\n" + 
		"                     dayuzerodisp(sum(decode(zhibb_id,21,zhejbz,0))) shulzk,\n" + 
		"                     xiaoyuzerodisp(sum(decode(zhibb_id,21,zhejbz,0))) shulkk,\n" + 
		"                     --d.hetj-d.jiesdj kouj,\n" + 
		"                     sum(decode(zhekfs,0,decode(zhibb_id,21,0,zhejbz),0))*-1 kouj,\n" + 
		"                     sum(d.zhejje) zhejje,\n" + 
		"                     round((d.yanssl-d.jiessl)*d.jiesdj*-1,2) koudzkk,d.jiashj,\n" + 
		"                     max(d.hetj) hetj,jiesdj\n" + 
		"                    from danpcjsmxb d,jiesb j,fahb f,zhuanmb z,zhuanmlb lb,zhillsb zl\n" + 
		"                     where d.jiesdid=j.id\n" + 
		"                       and to_char(d.lie_id) =to_char(f.lie_id)\n" + 
		"                       and j.id="+getJiesID()+"\n" + 
		...
		ArrHeader[0] = new String[] { "样本编号", "样本容量", "扣价", "数量奖价", "数量扣价", "结算数量", "热值扣款", "其它指标扣款","运费", "结算金额"};
		int ArrWidth[] = new int[] { 67, 60, 60, 60, 60, 50, 60, 80, 80, 85 };
		Report rt = new Report();
...
//		tb.setCellValue(1, 1, "运费：");
		...
		tb.setCellValue(3, 5, "燃料分管领导：");
		tb.setCellValue(3, 8, "账务审核：");
//		tb.setCellValue(3, 7, "计划审核：");
		
		tb.setCellValue(5, 1, "财务结算审核：");
//		tb.setCellValue(5, 3, "质价审核：");
		tb.setCellValue(5, 5, "部门审核：");
		
		tb.setCellValue(5, 8, "结算人：");
...
//		tb.setCells(3 ,6 ,3 ,6, Table.PER_ALIGN, Table.ALIGN_RIGHT);
...
//		}
		...
		String sql1 = 
			"select round(sum(sl.kouj*sl.yanssl),2) +\n" +
			"round(SUM(round((sl.yanssl-sl.jiessl)*sl.jiesdj,2))*-1,2) kouk,\n" +
			"sum(round(sl.jiessl*sl.jiesdj,2))+sum(f.maoz-f.piz)*"+yunj+" hej\n" +
		...
		if(rs1.next()){
			...
			tb.setCellValue(1, 9, new DecimalFormat("0.00").format(CustomMaths.Round_new(rs1.getDouble("hej"),2))+"");
		}
		...
		rt.body.setCellAlign(1, 3, Table.ALIGN_RIGHT);
		rt.body.setCellAlign(1, 7, Table.ALIGN_RIGHT);
		bt.addText(rt.getAllPagesHtml());
		return bt;
	}
