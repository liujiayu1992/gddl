 ...
 * 作者:tzf
 * 时间:2010-03-10
 * 修改内容: 化验报告单(质量临时表)  只有当二级审核后 才可以查看。
 */
/*
 * 作者:卞昕
 * 时间:2010-11-8
 * 修改内容: 化验报告单增加运输单位和固定碳。运输单位需要在xitxxb表里面添加sql语句来实现。
 */
/*
 * 作者:赵胜男
 * 时间:2012-07-17
 * 修改内容: 使用系统参数判断，‘是否判断车皮表中运输单位’
 */
/*
 * 作者:夏峥
 * 时间:2013-07-10
...
public class Huaybgd extends BasePage {
    ...
    private boolean getShifejsh() {//是否二级审核后才可以查看(zhillsb)
        JDBCcon con = new JDBCcon();

        String sql = " select * from xitxxb where mingc='检质报告二级审核后查看(临时表)' and zhi='是' and leib='化验' and zhuangt=1 ";
...
////		使用系统参数判断，‘是否判断车皮表中运输单位’
//        if (MainGlobal.getXitxx_item("化验报告单", "是否判断车皮表中运输单位", "0", "是").equals("否")) {
    ...
    private String getHuaybgd() {
        ...
        try {
            if (rs.next()) {
                ...
                String sql = "select * from xitxxb where mingc='化验报告单是否显示审核人' and leib='化验' and zhuangt =1 ";
                ...
                if (this.getReportName().equals(REPORTNAME_HUAYBGD_ZHILLSB)) {
                    ...
                    if (rs.getLong("shenhzt") == 7) {
                        ...
                        cheph = "";//如果是从zhillsb取数据,并且质量临时表没有经过最终审核,审核状态7代表最终审核,那么为防止化验人员通过车号推断出矿别,
                        //所以在这里把车号自动隐藏.只要当经过最终审核后才能显示车号
                    }
                } else if (this.getReportName().equals(REPORTNAME_HUAYBGD_ZHILB)) {
                    strYunsdw = "(" + rs.getString("yunsdwmc") + ")";
                    strKuangb = rs.getString("MEIKDWMC");
                ...
                ArrHeader[0] = new String[]{"化验编号",
                        ...
                        "" + strKuangb + strYunsdw, "发站", "" + strChez + ""};
                ...
                ArrHeader[16] = new String[]{"固定碳Fcad(%)", "固定碳Fcad(%)",
                        "固定碳Fcad(%)", "" + rs.getDouble("FCAD") + "", "", ""};
                ArrHeader[17] = new String[]{"空气干燥基弹筒热值Qb,ad(J/g)",
                        "空气干燥基弹筒热值Qb,ad(J/g)", "空气干燥基弹筒热值Qb,ad(J/g)",
                        "" + rs.getDouble("QBAD") + "", "", ""};
                ArrHeader[18] = new String[]{"干燥基高位热值Qgr,d(J/g)",
                        "干燥基高位热值Qgr,d(J/g)", "干燥基高位热值Qgr,d(J/g)",
                        "" + rs.getDouble("QGRD") + "", "", ""};
                ArrHeader[19] = new String[]{"空气干燥基高位热值Qgr,ad(J/g)",
                        "空气干燥基高位热值Qgr,ad(J/g)", "空气干燥基高位热值Qgr,ad(J/g)",
                        "" + rs.getDouble("QGRAD") + "", "", ""};
                ArrHeader[20] = new String[]{"干燥无灰基高位热值Qgr,daf(J/g)",
                        "干燥无灰基高位热值Qgr,daf(J/g)", "干燥无灰基高位热值Qgr,daf(J/g)",
                        "" + rs.getDouble("GANZWHJGWRZ") + "", "", ""};
                ArrHeader[21] = new String[]{"收到基低位热值Qnet,ar(J/g)",
        ...
        rt.setDefautlFooter(2, 1, "负责人：" + shenhryej, -1);
