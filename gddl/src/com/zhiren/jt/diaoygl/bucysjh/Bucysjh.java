package com.zhiren.jt.diaoygl.bucysjh;
...
public class Bucysjh extends BasePage implements PageValidateListener {
//	 判断是否是集团用户
	...
	private void Insert() {
		...
		try {
			...
			/*
			sql.append("select dc.jianc,cz.jianc as daoz from diancxxb dc,chezxxb cz ");
			sql.append(" where dc.chezxxb_id=cz.id and dc.jianc='");
			sql.append(diancmc).append("'");
			*/
		...
		Bucysjhbean yb = new Bucysjhbean();
		yb.setNianf(intYear);
		yb.setYuef(intMonth);
		...
		for (int i = 0; i < size; i++) {
			((Bucysjhbean) getEditValues().get(i)).setXuh(i + 1);
		}
	}
		...
		if (introw != -1) {
			...
			if (_value != null) {
				if (getEditValues() != null && !getEditValues().isEmpty()) {
					long id = ((Bucysjhbean) getEditValues().get(introw))
							.getId();
					if (id != 0) {
			...
			for (int i = 0; i < _value.size(); i++) {
				((Bucysjhbean) _value.get(i)).setXuh(i + 1);
			}
		}
	}
		...
		try {
			...
			if (_list != null || _list.size() != 0) {
				for (int i = 0; i < _list.size(); i++) {
					id = ((Bucysjhbean) _list.get(i)).getId();
					dianchangid = getDiancmcValue().getId();
					meikdqid = ((Bucysjhbean) _list.get(i)).getMeikdqid();
					// nianf=((Yunsjhhtbean) _list.get(i)).getNianf();
					// yuef=((Yunsjhhtbean) _list.get(i)).getYuef();
					tielj = getProperId(_ITieljModel, ((Bucysjhbean) _list
							.get(i)).getTielj());
					
					fazid = getProperId(_IChezmcModel, ((Bucysjhbean) _list
							.get(i)).getFazmc());
					
					daozid = getProperId(_IChezmcModel, ((Bucysjhbean) _list
							.get(i)).getDaozmc());
					pinm = getProperId(_IMeizModel, ((Bucysjhbean) _list.get(i))
							.getPinm());
					
					pic = ((Bucysjhbean) _list.get(i)).getPic();
					pid = ((Bucysjhbean) _list.get(i)).getPid();
					pizjhh = ((Bucysjhbean) _list.get(i)).getPizjhh();
					huanzg = getProperId(_IGangkmcModel, ((Bucysjhbean) _list
							.get(i)).getHuanzg());
					zhongdg = getProperId(_IGangkmcModel, ((Bucysjhbean) _list
							.get(i)).getZhongdg());
					zibbcc = ((Bucysjhbean) _list.get(i)).getZibbcc();
					shunh = ((Bucysjhbean) _list.get(i)).getShunh();
	...
	public List getSelectData() {
		...
		try {
			...
			if (getMeikdqmcValue() == null || getMeikdqmcValue().getId() == -1) {
				...
				sql.append("   and yh.diancxxb_id = " + diancid+ " and yh.gongysb_id = " + getMeikdqmcValue().getId()+ " and yh.leix=0 order by yh.id");
				...
				while (rs.next()) {
					...
					_editvalues.add(new Bucysjhbean(id, ++mxuh, dianchangid,
		...
		if (_editvalues == null) {
			_editvalues.add(new Bucysjhbean());
		}
		setEditTableRow(-1);
	...
	// //////////////////////////////////////////////////////////////////
	private Bucysjhbean _EditValue;
	...
	public Bucysjhbean getEditValue() {
		return _EditValue;
	}

	public void setEditValue(Bucysjhbean EditValue) {
