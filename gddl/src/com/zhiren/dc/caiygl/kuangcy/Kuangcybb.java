...
public class Kuangcybb extends BasePage implements PageValidateListener {
	...
	public String getPrintTable() {
		...
		String strSql = 
			"select decode(mk.mingc,null,k.meikdwmc,mk.mingc)  as mkmc,k.huaybh as bianm,k.huayy,k.shenhry,to_char(k.huaysj,'yyyy-mm-dd') as huaysj,k.caiyy,\n" +
...
//			
		...
		if(visit.getDiancmc().equals("宣威发电")){
			rt.setTitle(visit.getDiancmc()+"监控样检验报表", ArrWidth);
		}else{
			rt.setTitle(visit.getDiancmc()+"矿采煤质检验报表", ArrWidth);
		}
...
//	}
	...
	private void setBianmModels() {
		...
		String sss="select k.id,k.huaybh from kuangcyb k " +
				" where k.huaysj=to_date('"+getBRiq()+"','yyyy-mm-dd') 	 and k.shenhzt=5 order by k.huaybh ";
