...
//	合计行的ID默认为-1，保存的时候不保存合计行
	   chaxun = "select case when grouping(vw.gongysmy)=1 then -1 else nvl(fk.id,0) end as id,\n"
				+ "       dc.mingc as diancxxb_id,decode(grouping(vw.gongysmy),1,'合计',vw.gongysmy) as gongysb_id,\n"
				+ "       to_date('"+ this.getRiqi()+"', 'yyyy-mm-dd') as riq,nvl(fk.leix,'"+getGongmLxValue().getValue()+"') as leix,\n"
				+ "       sum(nvl(fk.rijjhl,0)) as rijjhl,sum(fk.ches) as ches,sum(fk.duns) as duns,\n"
				+ "       nvl(fk.shangbzt,0) as shangbzt \n"
				+ " from fenkshcrb fk,diancxxb dc,vwdcgysmykjgl vw,dianckjpxb px\n"
				+ " where dc.id=fk.diancxxb_id and fk.gongysb_id=vw.id and fk.riq=to_date('"+ this.getRiqi()+"', 'yyyy-mm-dd')\n"
				+ "   and dc.id=px.diancxxb_id and px.kouj='月报' "+ strdiancTreeID +" \n"
				+ " group by rollup(dc.mingc,(vw.gongysmy,fk.id,fk.leix,fk.shangbzt))\n"
				+ " having not grouping(dc.mingc)=1 order by min(px.xuh),grouping(vw.gongysmy) desc,vw.gongysmy";
...
//		   chaxun=
//		   		"select nvl(fk.id,0) as id,dc.mingc as diancxxb_id,vw.gongysmy as gongysb_id,to_date('"+this.getRiqi()+"', 'yyyy-mm-dd') as riq,\n" +
//		   		"       jh.leix,\n" + 
//		   		"       decode(nvl(jh.rijjhl,0),0,nvl(fk.rijjhl,0),nvl(jh.rijjhl,0)) as rijjhl,\n" + 
//		   		"       nvl(fk.ches,0) as ches,nvl(fk.duns,0) as duns,nvl(fk.shangbzt,0) shangbzt\n" + 
//		   		" from\n" + 
//		   		"\n" + 
//		   		"     (select nvl(fk.id,0) as id,\n" + 
//		   		"       fk.diancxxb_id,\n" + 
//		   		"       fk.gongysb_id,\n" + 
//		   		"       fk.riq,\n" + 
//		   		"       fk.leix,\n" + 
//		   		"       nvl(fk.rijjhl,0) as rijjhl,\n" + 
//		   		"       fk.ches,\n" + 
//		   		"       fk.duns,\n" + 
//		   		"       fk.shangbzt\n" + 
//		   		"     from fenkshcrb fk\n" + 
...
//		   		"\n" + 
//		   		"    ( select jh.diancxxb_id,jh.gongysb_id,\n" + 
//		   		"          round(nvl(jh.hej,0) * 10000 /daycount(to_date('"+this.getRiqi()+"','yyyy-mm-dd')),0) as rijjhl,\n" + 
//		   		"          decode(1,1,'重点','重点') as leix\n" + 
//		   		"      from niancgjhb jh,gongysb gy\n" + 
//		   		"      where jh.gongysb_id=gy.id  and jh.riq=First_day(to_date('"+this.getRiqi()+"','yyyy-mm-dd')) and jh.hej>0 ) jh,\n" + 
//		   		"\n" + 
//		   		"      diancxxb dc,dianckjpxb px, \n" + 
//		   		"      vwdcgysmykjgl vw \n" + 
//		   		"\n" + 
//		   		"   where fk.gongysb_id(+)=jh.gongysb_id and fk.diancxxb_id(+)=jh.diancxxb_id \n"+ strdiancTreeID +
//		   		"     and vw.diancxxb_id=dc.id and jh.gongysb_id=vw.gongysb_id and jh.diancxxb_id=vw.diancxxb_id \n" +
//		   		"	  and dc.id=px.diancxxb_id and px.kouj='月报' order by px.xuh,vw.gongysmy ";
		...
		egu.setTableName("fenkshcrb");//将上面的sql查询出来的数据存储到shouhcrbb这张表中。
...
//		egu.setWidth(1000);
		...
		String GongysSql = "select vw.id,vw.gongysmy from vwdcgysmykjgl vw,diancxxb dc where vw.kjshiyzt=1  and vw.diancxxb_id=dc.id "+strDiancId+" and vw.gongyskjb_id="+getGongmLxValue().getId()+" order by vw.gongysmy ";
...
//		sb.append("if(e.row==0){e.cancel=true;}");
		//当日供煤发生变化时,合计行的(当日供煤,累计供煤,库存)发生计算
//			sb.append("rec = gridDiv_ds.getAt(gridDiv_ds.getCount()-1);").append("if(e.field=='RIJJHL'){").append("rec.set('RIJJHL', eval(rec.get('RIJJHL')||0) + eval(e.value||0) - eval(e.originalValue||0));").append("}");
