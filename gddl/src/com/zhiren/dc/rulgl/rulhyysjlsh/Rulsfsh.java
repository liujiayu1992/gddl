...
public class Rulsfsh extends BasePage implements PageValidateListener {
	private String msg = "";
	...
	public void Update1(String strchange, Visit visit) {
		...
		while (mdrsl.next()) {
			...
			String gyid = "select id from gongyfxb where zhillsb_id="
					...
					+ mdrsl.getString("ID") + "and fenxxmb_id=2";
	...
	private void AutoFix(String strchange, Visit visit) {
		...
		ResultSetList rsl = con.getResultSetList(rulid);
		if(rsl.getRows()==0){
			String sql2="inster into rulmzlb (id,diancxxb_id,mad,fenxrq,rulrq,bianm,rulbzb_id,JIZFZB_ID,LURSJ,shenhzt)values(" +
			...
			String sql2="update zhillsb set mad="+mdrsl.getDouble("ZHI")+",shenhzt=1 where id="+mdrsl.getString("ZHILLSB_ID");	
			con.getUpdate(sql2);
		}
	...
	public void getSelectData() {
		...
		String sql = "select distinct g.id, g.bianm,g.xuh,g.qimbh,g.qimmyzl,g.qimzl,g.meiyzl,g.honghzl1,g.zuizhhzl,g.shenhzt,\n"
				...
				+ " from gongyfxb g\n"
				+ " where fenxxmb_id=2 and g.shenhzt=0 "
		...
		egu.getColumn("honghzl1").setHeader("检查性试验后总质量");
		// egu.getColumn("honghzl2").setHeader("备注");
		// egu.getColumn("honghzl3").setHeader("日期");
		// egu.getColumn("honghzl4").setHeader("编号");
		egu.getColumn("zuizhhzl").setHeader("烘干后总质量");
		egu.getColumn("shiqzl").setHeader("干燥后煤样减少的质量");
		egu.getColumn("zhi").setHeader("水分");
