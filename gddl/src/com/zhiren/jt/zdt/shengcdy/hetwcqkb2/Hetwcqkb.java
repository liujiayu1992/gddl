...
public class Hetwcqkb extends BasePage {
	...
	public String getPrintTable() {
		...
		if(getLeixSelectValue().getValue().equals("棋盘表")){
			return getQibb();
		}else if (getLeixSelectValue().getValue().equals("分厂分供应商")){
			return getFencGys();
		}else {
			return getZhiltj();
		}
	...
	//分厂，分供应商
	public String getSQLCK(String Beginriq,String Endriq,String Jihkj_fh,String Jihkj_j,String Gongsid){//获得主SQL
	...
	private String getFencGys() {
		...
		sbsql.append(getSQLCK(Beginriq,Endriq,jihkj_fh,jihkj_j,Gongsid));
		...
		rt.setDefautlFooter(10, 3,"制表:"+((Visit)getPage().getVisit()).getRenymc(), Table.ALIGN_RIGHT);
		// 设置页数
		_CurrentPage = 1;
...
//			***********************************************************************************//			
	...
	public String getSQL(String Beginriq,String Endriq,String Jihkj_fh,String Jihkj_j,String Gongsid){//获得主SQL
		...
		row.append(strgrouping);
		row.append(
			"sum(nvl(nianjhl,0))*10000 nianjhl,\n" +
			"sum(jihl_bq)*10000 jihl_bq,sum(jihl_tq)*10000 jihl_tq,sum(shidl_bq)shidl_bq,\n" + 
			"sum(shidl_tq)shidl_tq,sum(chaz_bq)chaz_bq,sum(chaz_tq)chaz_tq,\n" + 
			"--sum(daohl_bn)daohl_bn,\n" + 
			"round_new(decode(sum(nvl(nianjhl,0)),0,0,sum(shidl_bq)/(sum(nvl(nianjhl,0))*10000)*100),2) as daohl_bn,\n" + 
			"round_new(decode(sum(jihl_bq),0,0,sum(shidl_bq)/sum(jihl_bq*10000)*100),2) as daohl_bq,\n" + 
			"round_new(decode(sum(jihl_tq),0,0,sum(shidl_tq)/sum(jihl_tq*10000)*100),2) as daohl_tq\n" + 
			"--sum(daohl_bq)daohl_bq,sum(daohl_tq)daohl_tq\n" + 
			"from\n" + 
			"------------同期--------------\n" + 
			"(select  nianjhl,biaot.diancxxb_id,biaot.gongysb_id,\n" + 
			"0 jihl_bq,jihlb.jihl jihl_tq,0 shidl_bq,shisl.daohl shidl_tq,0 chaz_bq,\n" + 
			"nvl(jihlb.jihl,0)*10000-nvl(shisl.daohl,0)chaz_tq,\n" + 
			"decode(shisl.daohl,null,0,nianjhl,0,0,round(shisl.daohl/(nianjhl*10000)*100,2)) daohl_bn,0 daohl_bq,\n" + 
			"decode(jihlb.jihl,null,0,0,0,decode(shisl.daohl,null,0,round(shisl.daohl/(jihlb.jihl*10000)*100,2))) daohl_tq\n" + 
			"from\n" + 
			"--实收量\n" + 
			"(select diancxxb_id,gongysb_id,sum(daohl) as daohl from (\n" + 
			"select fh.diancxxb_id,fh.gongysb_id,\n" + 
			"       (round_new(sum(fh.biaoz), 0) + round_new(sum(fh.yingd), 0) -\n" + 
			"       round_new(sum(fh.yingd - fh.yingk), 0)) daohl\n" + 
			"  from fahb fh\n" + 
			" where "+Jihkj_fh+"\n" + 
			"   and fh.daohrq >=ADD_MONTHS(to_date('"+Beginriq+"', 'yyyy-mm-dd'), -12)\n" + 
			"   and fh.daohrq <=ADD_MONTHS(to_date('"+Endriq+"', 'yyyy-mm-dd'), -12)\n" + 
			" group by fh.diancxxb_id,fh.gongysb_id, fh.lieid)\n" + 
			" group by diancxxb_id,gongysb_id) shisl,\n" + 
			"--计划量\n" + 
			"(select 0 nianjhl,\n" + 
			"       j.diancxxb_id,j.gongysb_id,\n" + 
			"       sum(getHetl_Sjd(ADD_MONTHS(to_date('"+Beginriq+"', 'yyyy-mm-dd'), -12),\n" + 
			"                       ADD_MONTHS(to_date('"+Endriq+"', 'yyyy-mm-dd'), -12),\n" + 
			"                       j.riq,\n" + 
			"                       j.hej)) jihl\n" + 
			"  from niandhtqkb j\n" + 
			" where "+Jihkj_j+"\n" + 
			"   and j.riq >= ADD_MONTHS(to_date('"+Beginriq+"', 'yyyy-mm-dd'), -12)\n" + 
			"   and j.riq <= ADD_MONTHS(to_date('"+Endriq+"', 'yyyy-mm-dd'), -12)\n" + 
			" group by diancxxb_id,gongysb_id) jihlb,\n" + 
			"--表头\n" + 
			"(select distinct fh.diancxxb_id,fh.gongysb_id from fahb fh where "+Jihkj_fh+"\n" + 
			"   and fh.daohrq >=ADD_MONTHS(to_date('"+Beginriq+"', 'yyyy-mm-dd'), -12)\n" + 
			"   and fh.daohrq <=ADD_MONTHS(to_date('"+Endriq+"', 'yyyy-mm-dd'), -12)\n" + 
			"union\n" + 
			"select distinct j.diancxxb_id,j.gongysb_id from niandhtqkb j where "+Jihkj_j+"\n" + 
			"   and j.riq >= ADD_MONTHS(to_date('"+Beginriq+"', 'yyyy-mm-dd'), -12)\n" + 
			"   and j.riq <= ADD_MONTHS(to_date('"+Endriq+"', 'yyyy-mm-dd'), -12)) biaot\n" + 
			"where biaot.diancxxb_id=shisl.diancxxb_id(+) and biaot.diancxxb_id=jihlb.diancxxb_id(+)\n" + 
			"  and biaot.gongysb_id=shisl.gongysb_id(+) and biaot.gongysb_id=jihlb.gongysb_id(+)\n" + 
			"union\n" + 
			"--------------------本期-------------------\n" + 
			"select  nianjhl,biaot.diancxxb_id,biaot.gongysb_id,\n" + 
			"jihlb.jihl  jihl_bq,0 jihl_tq,shisl.daohl shisl_bq,0 shisl_tq,\n" + 
			"nvl(jihlb.jihl,0)*10000-nvl(shisl.daohl,0)chaz_bq,0 chaz_tq,\n" + 
			"decode(shisl.daohl,null,0,nianjhl,0,0,round(shisl.daohl/(nianjhl*10000)*100,2)) daohl_bn,\n" + 
			"decode(jihlb.jihl,null,0,0,0,\n" + 
			"decode(shisl.daohl,null,0,round(shisl.daohl/(jihlb.jihl*10000)*100,2))) daohl_bq,0 daohl_tq\n" + 
			"from\n" + 
			"--实收量\n" + 
			"(select diancxxb_id,gongysb_id,sum(daohl) as daohl from (\n" + 
			"select fh.diancxxb_id,fh.gongysb_id,\n" + 
			...
			"\n" + 
			"--计划\n" + 
			"(select diancxxb_id,gongysb_id,sum(nianjhl) nianjhl, sum(jihl) jihl\n" + 
			"  from (select 0 nianjhl,\n" + 
			"               j.diancxxb_id,j.gongysb_id,\n" + 
			"               sum(getHetl_Sjd(to_date('"+Beginriq+"', 'yyyy-mm-dd'),\n" + 
			"                               to_date('"+Endriq+"', 'yyyy-mm-dd'),\n" + 
			"                               j.riq,\n" + 
			...
			"          from niandhtqkb j\n" + 
			"         where "+Jihkj_j+"\n" + 
			"           and j.riq >= to_date('"+Beginriq+"', 'yyyy-mm-dd')\n" + 
			"           and j.riq <= to_date('"+Endriq+"', 'yyyy-mm-dd')\n" + 
			"         group by diancxxb_id,gongysb_id\n" + 
			"        union\n" + 
			"        select sum(hej) nianjhl,\n" + 
			"               j.diancxxb_id,j.gongysb_id,\n" + 
			"               0 jihl\n" + 
			"          from niandhtqkb j\n" + 
			"         where "+Jihkj_j+"\n" + 
			"           and to_char(j.riq, 'yyyy') =to_char(to_date('"+Beginriq+"', 'yyyy-mm-dd'), 'yyyy')\n" + 
			"         group by diancxxb_id,gongysb_id)\n" + 
			" group by diancxxb_id,gongysb_id) jihlb,\n" + 
			"\n" + 
			"--表头\n" + 
			...
			"select distinct j.diancxxb_id,j.gongysb_id from niandhtqkb j where "+Jihkj_j+"\n" + 
			"   and j.riq >= to_date('"+Beginriq+"', 'yyyy-mm-dd')\n" + 
			"   and j.riq <= to_date('"+Endriq+"', 'yyyy-mm-dd')) biaot\n" + 
			"\n" + 
			"where biaot.diancxxb_id=shisl.diancxxb_id(+) and biaot.diancxxb_id=jihlb.diancxxb_id(+)\n" + 
			"  and biaot.gongysb_id=shisl.gongysb_id(+) and biaot.gongysb_id=jihlb.gongysb_id(+)\n" + 
			") shuj,vwdianc dc,vwgongys g\n" + 
...
//	    	}
	    ...
	    public void getLeixSelectModels() {
	        ...
	        list.add(new IDropDownBean(5,"分厂分供应商"));
