...
public class Shoumbalance extends BasePage {
	// List1
	// dropdwon1,2
...
//					故结算表的id应取danpcjsmxb的jiesbid。
			...
			sql = "insert into kuangfjsmkb (id, diancxxb_id, bianm, gongysb_id, gongysmc,  \n" +
...
//					+ ", "
					...
					+","
					+((Visit)this.getPage().getVisit()).getLong10()//业务类型id
					+ ")";
...
//	}
	...
	private long SaveDiancjsyfb(long Meikid,JDBCcon con) {
		...
		try {
			...
			sql = " insert into kuangfjsyfb  (id,diancxxb_id, bianm, gongysb_id, gongysmc, meikxxb_id, "
					...
					+ " kuangfjsmkb_id, RANLBMJBR, RANLBMJBRQ, kuidjfyf,kuidjfzf,yewlxb_id)"
					+ " values("
					+ Id
					+ ", "
					+ ((Visit) getPage().getVisit()).getLong1()
...
//	}
	...
	private boolean Save() {
		...
		try {
			...
			if (Shoumjsdcz.CheckHetshzt(((Dcbalancebean) getEditValues().get(0))
					.getHetb_id())) {
				// 判断合同的审核状态
		...
		if (((Dcbalancebean) getEditValues().get(0)).getId() == 0
			...
			if (Save() && this.getProperId(this.getILiucmcModel(), getLiucmc()) > -1) {
				...
				if (((Visit) getPage().getVisit()).getLong2() == 1) {// 两票结算
					...
					Shoumjsdcz.SubmitGsDiancjsmkb(((Dcbalancebean) getEditValues()
							.get(0)).getId());
					Shoumjsdcz.SubmitGsDiancjsyfb(((Dcbalancebean) getEditValues()
							.get(0)).getYid());
...
//			设置超/亏吨的显示	
        ...
        //设置默认值begin
			mxianshr=myuanshr=MainGlobal.getTableCol("diancxxb", "mingc", "id", "199");
			 if(bsv.getGongysb_Id()==12){
			       JDBCcon con_nei=new JDBCcon();
  		           ResultSetList rsl=con_nei.getResultSetList("select * from gongysb where id=20" );
  		          if(rsl.next()){
  		        	mshoukdw =rsl.getString("quanc");
  		        	mkaihyh   =rsl.getString("kaihyh");
  		        	mzhangh=   rsl.getString("zhangh");
  		           }
  		           con_nei.Close();
     	     }
