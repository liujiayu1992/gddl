...
public class Rullfsh extends BasePage implements PageValidateListener {
	private String msg = "";
	...
	public void Update1(String strchange, Visit visit) {
		...
		while (mdrsl.next()) {
			StringBuffer sql2 = new StringBuffer();
			String gyid = "select id from liufhyb where zhillsb_id="
					+ mdrsl.getString("ZHILLSB_ID") + " and id<>"
					+ mdrsl.getString("ID") + "and fenxxmb_id=6";
	...
	private void AutoFix(String strchange, Visit visit) {
		...
		Judge ju = new Judge();
		double[] dbarrjudge = ju.getJudgeData(Judge.T_TYPE_Stad, array);
		if(dbarrjudge==null){
			return;
		}
		for (int j = 0; j < dbarrjudge.length; j++) {
			...
			if ( mdrsl.getDouble("ZHI") == dbarrjudge[j]) {
					String sql = "update liufhyb set shenhzt=1 where id = "
							+ mdrsl.getString("ID");
		...
		for(int i=0;i<mdrsl.getRows();i++){
			mdrsl.next();
			String sql1 = "select id from liufhyb where zhillsb_id="
				+ mdrsl.getString("ZHILLSB_ID") + " and id<>"
				+ mdrsl.getString("ID");
		ResultSetList rsl = con.getResultSetList(sql1);
		rsl.next();
		String sql3 = "update liufhyb set shenhzt=-1 where id = "
			+ rsl.getLong("ID");
		con.getUpdate(sql3);
	...
	ResultSetList rsl = con.getResultSetList(rulid);
		...
		for (int i=0;i<mdrsl.getRows();i++){
			mdrsl.next();
			if(rsl.getRows()==0){
			String sql2="inster into rulmzlb (id,diancxxb_id,stad,fenxrq,rulrq,bianm,rulbzb_id,JIZFZB_ID,LURSJ,shenhzt)values(" +
			...
			String sql2="update zhillsb set stad="+mdrsl.getDouble("ZHI")+",shenhzt=1 where id="+mdrsl.getString("ZHILLSB_ID");	
			con.getUpdate(sql2);
		}
	...
	public void getSelectData() {
		...
		JDBCcon con = new JDBCcon();
		String sql = "select distinct g.id,g.rulrq,g.fenxrq, g.bianm,g.xuh,g.qimmyzl,g.qimzl,g.meiyzl,\n"
				+ "g.zhi,g.lury,g.zhillsb_id\n"
				+ " from liufhyb g\n"
				+ " where fenxxmb_id=6 and shenhzt=0"
				+ " and g.bianm='"
		...
		egu.getColumn("zhi").setHeader("硫分");
		...
		egu.addToolbarButton("自动审核", GridButton.ButtonType_Sel, "AutoButton",
				null, SysConstant.Btn_Icon_Show);
		setExtGrid(egu);
	...
	public IPropertySelectionModel getHuaybhModels() {
		...
		String sql = "select rownum,bianm from (select distinct l.bianm from liufhyb l "
				+ "where l.rulrq>=to_date('"
				+ this.getRiqi()
				+ "','yyyy-mm-dd')and l.rulrq<=to_date('"
				+ this.getRiq2()
				+ "',\n" + "'yyyy-mm-dd') and l.fenxxmb_id=6) ";
