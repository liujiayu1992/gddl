...
public class Yuedkhmqsreport_hb  extends BasePage implements PageValidateListener{
...
//	"rez,jiz,beiz,jif FROM (\n" + 
	...
	"                 ,\n" + 
	"      decode(sum(ah), 0, 0, round_new(sum(fdl.mt * ah) / sum(ah), 1)) as mt,\n" + 
	"            decode(fdl.mmingc,'','0', getjiz(round_new(decode(sum(ah), 0, 0, round_new(sum(fdl.mt * ah) / sum(ah), 2)),0),'KHXM_FKSF',fdl.pmingc))     jiz\n" + 
	"                 ,getbiaoz(round_new(decode(sum(ah), 0, 0, round_new(sum(fdl.mt * ah) / sum(ah), 2)),0),'KHXM_FKSF',fdl.pmingc) beiz,\n" + 
	"           round_new(round_new(sum(ah)/3300,5)*nvl(getbiaoz(round_new(sum(fdl.mt * ah) / sum(ah), 5),'KHXM_FKSF',fdl.pmingc)*\n" + 
	"           getchaocjz(abs(getchaz(round_new(sum(mt * ah) / sum(ah), 5),'KHXM_FKSF',fdl.pmingc)),round_new(sum(mt * ah) / sum(ah),5),'KHXM_FKSF')\n" + 
	"\n" + 
	"            ,0),3) jif,1 as xuh\n" + 
	" from\n" + 
	"(select f.id,g.mingc gmingc,m.mingc mmingc,p.mingc pmingc,f.laimsl,f.jingz,z.mt\n" + 
	...
	"                 ,'0' jiz\n" + 
	"                 ,0 beiz,\n" + 
//	"          SUM( round_new(round_new(sum(ah)/3300,0)*nvl(getbiaoz(round_new(sum(fdl.mt * ah) / sum(ah), 2),'KHXM_FKSF','原煤')*\n" + 
//	"           getchaocjz(abs(getchaz(round_new(sum(mt * ah) / sum(ah), 2),'KHXM_FKSF','原煤')),round_new(sum(mt * ah) / sum(ah),2),'KHXM_FKSF')\n" + 
	"\n" + 
...
//	"having (grouping(fdl.gmingc)+grouping(mmingc)+grouping(fdl.pmingc)) =3\n" + 
	...
	"ORDER BY XUH,gmingc";
		...
		ArrHeader[0]=new String[]{"供应商","煤矿","煤种","净重","热值","全水","全水","全水","全水"};
	    ArrHeader[1]=new String[] {"供应商","煤矿","煤种 ","吨","Kcal/Kg","Mt","考核基准","扣分标准(分/列)","计分"};
		ArrWidth =new int[] {120,80,80,80,80,80,80,70,70};
...
//		System.out.println(ss);
		...
		rt.setDefaultTitle(6, 2, "填报日期:"+intyear+"年"+intMonth+"月", Table.ALIGN_CENTER);
