...
public class Rulrcmrz extends BasePage {
	...
	private String getHetltj() {
		...
		buffer.append(
				"select decode(grouping(ruc.ruc_riq),1,'累计', to_char(ruc.ruc_riq,'yyyy-mm-dd')) as riq,\n" +
				"       rul.rul_banz,\n" + 
		...
		ArrHeader[0] = new String[] { "日期", "入炉煤采样时间", "入炉数量<br>吨","总入炉数量<br>吨","入炉煤热值<br>cal/kg","入炉煤加权平均热值<br>cal/kg","入厂总煤量<br>吨","入厂煤平均热值<br>cal/kg","热值差<br>cal/kg","入炉煤含硫量<br>%","入炉煤含硫平均<br>%","入厂煤含硫平均<br>%","备注"};
//		ArrHeader[1] = new String[] { "日期", "入炉煤采样时间", "吨","总吨","cal/kg","kcal/kg","吨","cal/kg","star","star","star","备注"};
		...
		rt.setTitle("入 炉 入 厂 煤 热 值", ArrWidth);
...
//		rt.body.mergeCol(12);
		...
		if ("累计".equals(rt.body.getCell(rt.body.getRows(),1).value)) {
