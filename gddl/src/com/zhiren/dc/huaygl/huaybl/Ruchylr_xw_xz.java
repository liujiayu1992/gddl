 ...
 * 
 */
public class Ruchylr_xw_xz extends BasePage implements PageValidateListener {

	private String CustomSetKey = "Ruchylr_xw_xz";
	...
	public void getSelectData() {
		...
		if(isXuanw){//宣威发电入厂录入
			...
			sql = "select l.id,\n"
				...
				+ "       l.t4,\n" + "       huayy,\n" + "       l.lury,\n"
				+ "       l.beiz,\n" + "       l.huaylb\n"
				+"from zhuanmb z, zhillsb l,caiyb cy,\n" +
				...
				"      and fh.zhilb_id=cy.zhilb_id\n" + 
				"      and cy.caiyrq>=to_date('"+this.getZhiy1()+"','yyyy-mm-dd') and cy.caiyrq<=to_date('"+this.getZhiy2()+"','yyyy-mm-dd')\n" + 
				"      and l.huaylbb_id in (select id from leibb where  beiz='新增')\n" + 
				"order by cy.caiyrq,l.id, l.huaylb";
		...
		if(rsl3.next()){
			...
			sb.append("if(e.field=='STAD'){if(e.record.get('STAD')>3||e.record.get('STAD')<0.5){Ext.MessageBox.alert('提示信息','请确认硫分是否正确！');}}\n");
...
//			大同：各期所有矿点固定had值，只是各期不一样
			...
			sql.append(Compute.ComputeValue(Mt, Mad, Aad, Vad, Qbad, sign_H, H,
					sign_S, S, sacle));
			sql.append("t1")
					.append("=" + Judgment(mdrsl.getString("t1")) + ",");
			sql.append("t2")
					.append("=" + Judgment(mdrsl.getString("t2")) + ",");
			sql.append("t3")
					.append("=" + Judgment(mdrsl.getString("t3")) + ",");
			sql.append("t4")
					.append("=" + Judgment(mdrsl.getString("t4")) + ",");
			if (!failed) {
