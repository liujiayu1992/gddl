...
public class Kucgjreport extends BasePage {
//	 判断是否是集团用户
...
//	private Date _BeginriqValue = DateUtil.AddDate(new Date(), -1, DateUtil.AddType_intDay);
	...
	private String REPORT_KUCGJ="kucgj";
	...
	public String getPrintTable(){
		if(mstrReportName.equals(REPORT_KUCGJ)){
			return getKucgj();
	...
	public String getKucgj(){
		...
		if (treejib == 1) {// 选集团时刷新出所有的电厂
			...
			if (IsZongc(cn,getTreeid())){
				str = "and dc.fuid = " + getTreeid() + "";
			}else{
				str = "and dc.id = " + getTreeid() + "";
			}
		}
		String sql = "select * from (select dc.mingc,rb.kuc,dc.jingjcml,dc.xianfhkc,dc.rijhm,\n"
					...
					+ " where rb.diancxxb_id=dc.id and rb.riq=to_date('"+strDate+"','yyyy-mm-dd')\n"
					+ "   and rb.kuc<dc.jingjcml "+str+"  order by dc.xuh) ";
 
		String ArrHeader[][]=new String[1][7];
		ArrHeader[0]=new String[] {"电厂","库存煤量","警戒库存","限负荷库存","日均耗煤","低于警戒库存<br>比例(%)","可用天数"};

		int ArrWidth[]=new int[] {120,80,80,80,80,100,80};
		...
		rt.setTitle("库存告警",ArrWidth);
		rt.setDefaultTitle(1, 2, "制表单位:"+getDiancmc(), Table.ALIGN_LEFT);
		rt.setDefaultTitle(3, 3, FormatDate(DateUtil.getDate(strDate)) , Table.ALIGN_CENTER);
