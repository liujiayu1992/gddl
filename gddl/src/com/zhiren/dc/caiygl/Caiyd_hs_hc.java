 ...
 * 合山采样单查询(火车)
 */
public class Caiyd_hs_hc  extends BasePage implements PageValidateListener{
...
//******************页面初始设置********************//
	...
	private String getCaiySql(){
		...
		String	sSql =  
			...
			"       to_char(max(c.zhongcsj),'yyyy-MM-dd HH24:mi:ss') as jiessj,\n" + 
			"       count(c.id) as ches,\n" + 
			"       g.quanc as gongmdw\n" + 
			"from fahb f,meikxxb m,hetb h,chepb c,gongysb g,caiyb cy,zhillsb z,zhuanmb zm,chezxxb cz\n" + 
	...
	private String getCaizrSql(){
		...
		String	sSql =
			"select r.quanc as caizr,y.beiz\n" +
			"from yangpdhb y,caiyryglb gl,renyxxb r,zhillsb ls,zhuanmb z\n" + 
			"where gl.yangpdhb_id=y.id\n" + 
			"and gl.renyxxb_id=r.id\n" + 
			"and y.zhilblsb_id=ls.id\n" + 
			"and z.zhillsb_id=ls.id\n" + 
			"and z.zhuanmlb_id=(select id from zhuanmlb where jib=1)\n" + 
			"and z.bianm ='" + getBianhValue().getValue() + "'\n" + 
			"order by y.caiysj desc";
	...
	private String getSelectData(){
		...
		String[][] CAIY=new String[][]{
			{"发站","合同编号","过衡开始时间","过衡结束时间","车数"},
			{faz,hetbh,kaissj,jiessj,ches},
			{"供煤单位",kuangb,kuangb,kuangb,kuangb},
		...
		//rt.setDefaultTitle(2,2,"制样日期:"+getBRiq(),Table.ALIGN_LEFT);
		...
		rt.body.setCellAlign(5, 4, Table.ALIGN_LEFT);
		
		
		rt.createFooter(1, ArrWidth);
		//rt.setDefautlFooter(1, 3, "采样人员：", Table.ALIGN_LEFT);
		//rt.setDefautlFooter(1, 2, "采制样日期:"+getBRiq(),Table.ALIGN_LEFT);
...
//	编号
	...
	public IPropertySelectionModel getBianhModels() {
		...
		String sql =
			"select distinct zm.id,zm.bianm\n" +
