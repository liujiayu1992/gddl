...
public class Yunsjhpf extends BasePage implements PageValidateListener {
	...
	public void CoypLastYueData() {
		...
		String chaxun = "select y.id,d.mingc as diancxxb_id,g.mingc as gongysb_id,y.riq,\n"
			...
			+ " where y.gongysb_id=g.id and y.diancxxb_id=d.id and y.pinm=p.id and y.leix=1 \n"
...
//		egu.getColumn("diancxxb_id").setDefaultValue(getDiancmc());
		...
		String GongysSql = "select id,mingc from gongysb where leix<>0 and zhuangt=1 order by mingc";
...
//			"		var yunsfsb1=rec1.get('YUNSFSB_ID');\n" + 
			...
			"		if(i==e.row) {\n" + 
			"			continue;\n" + 
			"		} else if (gonghdw==gonghdw1 && diancxxb_id==diancxxb_id1 && pinzb==pinzb1 " +
			"					 && faz_id==faz_id1 && daoz_id==daoz_id1) {\n" + 
			"			falge=1;\n " + 
			...
			" 			if(Mr[i].get('DIANCXXB_ID')==Mr[j].get('DIANCXXB_ID') && Mr[i].get('GONGYSB_ID')==Mr[j].get('GONGYSB_ID') " +
			"					&& Mr[i].get('PINM')==Mr[j].get('PINM') && Mr[i].get('FAZ_ID')==Mr[j].get('FAZ_ID') " +
			"					&& Mr[i].get('DAOZ_ID')==Mr[j].get('DAOZ_ID')) { \n" +
			"				falge=1;\n " + 
			"				Ext.MessageBox.alert('提示信息',\"您录入的数据与第\"+(i+1)+\"行数据条件完全相同，请您修改数据！\");\n" +
			" 				break;" + 
			" 			} \n" + 
