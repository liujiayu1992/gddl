package com.zhiren.jt.zdt.monthreport.tiedbpfylht;
...
public class TiedbpfylhtReport  extends BasePage implements PageValidateListener {
...
//******************页面初始设置********************//
	...
	private String RT_HET="tiedbpfylcx";
	private String mstrReportName="tiedbpfylcx";
	...
	private String getSelectData(){
		...
		String titlename=intyear+"铁道部批复运力合同查询";
		...
		if (getBaoblxValue().getValue().equals("分厂")){
			...
			if(jib==1){//选集团时刷新出所有的电厂
				...
				strgrouping.append("decode(grouping(g.mingc)+grouping(dc.fgsmc)+grouping(dc.mingc),3,'',2,'小计',1,dc.fgsmc,'&nbsp;&nbsp;&nbsp;&nbsp;'||dc.mingc) as diancmc,\n");
				strwhere.append("");
				strgroupby.append("group by rollup(g.mingc,dc.fgsmc,dc.mingc)\n");
				strhaving.append("");
				strorderby.append("order by grouping(g.mingc) desc,g.mingc,grouping(dc.fgsmc) desc,dc.fgsmc desc,grouping(dc.mingc) desc,max(dc.xuh)\n");
			}else if (jib==2){//选分公司的时候刷新出分公司下所有的电厂
				String ranlgs = "select id from diancxxb where shangjgsid= "+this.getTreeid();
				...
				try{
					...
					if(rl.next()){//燃料公司
						strgrouping.append("decode(grouping(g.mingc),1,'总计',g.mingc) as meikmc,\n");
						strgrouping.append("decode(grouping(g.mingc)+grouping(dc.rlgsmc)+grouping(dc.mingc),3,'',2,'小计',1,dc.rlgsmc,'&nbsp;&nbsp;&nbsp;&nbsp;'||dc.mingc) as diancmc,\n");
						strwhere.append(" and (dc.fgsid="+this.getTreeid()+" or dc.rlgsid= "+this.getTreeid()+") \n");
						strgroupby.append("group by rollup(g.mingc,dc.rlgsmc,dc.mingc)\n");
						//strhaving.append("having not grouping(dc.rlgsmc)=1\n");
						strorderby.append("order by grouping(g.mingc) desc,g.mingc,grouping(dc.rlgsmc) desc,dc.rlgsmc desc,grouping(dc.mingc) desc,max(dc.xuh)\n");
					}else{//分公司
						strgrouping.append("decode(grouping(g.mingc),1,'总计',g.mingc) as meikmc,\n");
						strgrouping.append("decode(grouping(g.mingc)+grouping(dc.fgsmc)+grouping(dc.mingc),3,'',2,'小计',1,dc.fgsmc,'&nbsp;&nbsp;&nbsp;&nbsp;'||dc.mingc) as diancmc,\n");
						strwhere.append(" and (dc.fgsid="+this.getTreeid()+" or dc.rlgsid= "+this.getTreeid()+") \n");
			...
			if(jib==1){//选集团时刷新出所有的电厂
				...
				try{
					...
					if(rl.next()){//燃料公司
						...
						strgrouping.append("decode(grouping(dc.mingc)+grouping(g.mingc),2,'',1,'小计',g.mingc) as meikmc,\n");
						strwhere.append(" and (dc.fgsid="+this.getTreeid()+" or dc.rlgsid= "+this.getTreeid()+") \n");
						strgroupby.append("group by rollup(dc.rlgsmc,dc.mingc,g.mingc)\n");
						strhaving.append("having not grouping(dc.rlgsmc)=1\n");
			 ...
			 "(select sum(n.hej) as hej from tiedbpfyl n ,diancxxb d where to_char(n.riq,'yyyy')='"+ intyear+"' "+strGongsID+") a");
		...
		if (!getBaoblxValue().getValue().equals("分矿百分比表")){
			...
			strSQL.append("from tiedbpfyl dh,vwdianc dc,gongysb g\n");
