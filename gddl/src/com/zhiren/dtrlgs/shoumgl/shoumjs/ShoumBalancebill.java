 ...
 * 夏峥 
 * 2009-08-24
 * 注销boolean3和boolean7 因为需要在弹出页面时不刷新结算单查询页面
 * 当结算单编号为空时，不可点击验收明细
 */
public class ShoumBalancebill  extends BasePage{
...
//	明晰查询
	...
	private void Yansmx(){
		...
		zhuangt = 2;
//		((Visit) getPage().getVisit()).getSession().setAttribute("bianh", this.getJiesbhSel()+";");
//		((Visit) getPage().getVisit()).getSession().setAttribute("type", "kuangf");
//		((Visit) getPage().getVisit()).setboolean7(true);
		((Visit) getPage().getVisit()).setString10(this.getJiesbhSel()+";");
		((Visit) getPage().getVisit()).setString11("kuangf");
...
//		((Visit) getPage().getVisit()).setboolean3(true);
	...
	public void beginResponse(IMarkupWriter writer, IRequestCycle cycle) {
		...
		if (!visit.getActivePageName().toString().equals(
			...
			(!visit.getActivePageName().toString().equals("BalanceMingx"))) {
//					|| visit.getboolean7())
...
//			} else {
				((Visit) getPage().getVisit()).setboolean3(true);
//			}
		} else if (Jiesbhsel != null && !Jiesbhsel.equals("")) {
				((Visit) getPage().getVisit()).setboolean3(true);
			}
...
//					jsd.setAllPages(bianh.length);
					for (int p = 0; p < bianh.length; p++) {
						sb.append(jsd.getKuangfjsd(bianh[p], p,visit.getString3()));
						k+=jsd.getAllPages();
					}
					setAllPages(k);
					_CurrentPage=1;
