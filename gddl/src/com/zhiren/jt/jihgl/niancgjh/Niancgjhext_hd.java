...
public  class Niancgjhext_hd extends BasePage implements PageValidateListener {
	private String msg = "";
...
//		con.setAutoCommit(false);
		...
		while (rsl.next()){
			...
			"and meikxxb_id=(select id from meikxxb where mingc='"+rsl.getString("meikxxb_id")+"')";
...
//			}
			...
			for (int i=1;i<=12;i++){
				...
				if (htl!=0){
					...
					String bb ="insert into niancgjhb(id,gongysb_id,diancxxb_id,hej,riq,meikxxb_id) values(" +
					"getNewId(getDiancId('"+ rsl.getString("mingc")+"')),"+
					"getGongysId('"+rsl.getString("gongysb_id")+"'),"+
					"getDiancId('"+ rsl.getString("mingc")+"'),"+
					htl+"," +
					"to_date('"+strDate+"','yyyy-mm-dd'),(select id from meikxxb where mingc='"+rsl.getString("meikxxb_id")+"'))";
					con.getInsert(bb);
//					if (flag == -1) {
//						con.rollBack();
//						con.Close();
...
//					}
		...
		while (rsl.next()){
			...
			"and gongysb_id=getGongysId('"+rsl.getString("gongysb_id")+"') " +
			"and diancxxb_id=getDiancId('"+ rsl.getString("mingc")+"')"+
			"and meikxxb_id="+((IDropDownModel) getMeikModel()).getBeanId(rsl
...
//		}
	...
	public void CoypLastYearData(){
		...
		String copyData = "select n.*\n"
				+ "  from niancgjhb n, diancxxb d,gongysb g,meikxxb m\n"
				+ " where n.diancxxb_id = d.id(+) and n.meikxxb_id=m.id(+)\n"
		...
		while(rslcopy.next()){
			...
			con.getInsert("insert into niancgjhb(id,gongysb_id,diancxxb_id,hej,riq,meikxxb_id) values(" +
					_id+","+gongysb_id +","+ diancxxb_id+","+hej+","+"to_date('"+strriq+"','yyyy-mm-dd'),"+meikxxb_id+")");
	...
	public void getSelectData() {
		...
		String chaxun = "select dcmk.diancid as id,dcmk.mingc,dcmk.gongysmc as gongysb_id,m.mingc as meikxxb_id,'' as faz_id,\n"
				...
				+ "      (select id,mingc from gongysb) dq) dcmk,\n"
				+ " (select riq, hej,g.id as diqid,dc.id as diancid,n.meikxxb_id\n"
				...
				+ "        and n.meikxxb_id=m.id\n"
				+ "  group by dcmk.diancid,dcmk.mingc,dcmk.gongysmc ,m.mingc "
		...
		egu.getColumn("quann").setWidth(60);
