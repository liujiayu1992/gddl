...
public class GongysReport extends BasePage {
	...
	private String getGongys() {
		...
		if(visit.getLong1()==0){//无参数--打印供应商列表
			...
			buffer.append("       rownum as xuh,\n");
			buffer.append("       g.bianm as bianm,\n");
			buffer.append("       g.quanc as quanc,\n");
			buffer.append("       g.mingc as mingc,\n");
			buffer.append("       shengfb.quanc as shengf,\n");
			buffer.append("       decode(g.fuid, 0, '大供货单位', '小供货单位') as leib\n");
			buffer.append("  from gonghdwdmsqb g, shengfb\n");
			buffer.append(" where shengfb.id(+) = g.shengfb_id\n");
			buffer.append(" order by xuh, g.mingc");
			...
			rt.setTitle("供　货  单  位　查　询", ArrWidth);
...
//			rt.body.mergeFixedCols();
			...
			try{
				...
				buffer.append("from gonghdwdmsqb g,shengfb,gonghdwdmsqb gzb\n");
