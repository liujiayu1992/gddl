...
public class Rengcymkmc extends BasePage implements PageValidateListener {
	private String msg = "";
	...
	private void Save() {
		...
		while (delrsl.next()) {
			sbsqlDel.append("delete from rengcymkmcb where id = ").append(delrsl.getString("id")).append(";\n");
		}
		sbsqlDel.append("end;");
		...
		while(mdrsl.next()) {
			if("0".equals(mdrsl.getString("id"))) {
				sbsql.append("insert into rengcymkmcb(id, meikxxb_id,meikmc,lursj, lurry, beiz) values(getnewid(")
				.append(visit.getDiancxxb_id()).append("), ")
				.append("(select max(id) from meikxxb where mingc='"+mdrsl.getString("meikmc")+"')")
				.append(",'").append(mdrsl.getString("meikmc")).append("', ")
				.append("sysdate").append(", '").append(""+visit.getRenymc()+"")
				.append("', '").append(mdrsl.getString("beiz")).append("');\n");
			} else {
				sbsql.append("update rengcymkmcb set ")
				.append("meikxxb_id = ").append("(select max(id) from meikxxb where mingc='"+mdrsl.getString("meikmc")+"')")
...
//		Visit visit = (Visit) getPage().getVisit();
		JDBCcon con = new JDBCcon();
		String sql="select rg.id, rg.meikmc,rg.lurry,to_char(rg.lursj,'yyyy-mm-dd hh24:mi:ss') as lursj,rg.beiz\n" +
			"from rengcymkmcb rg,meikxxb mk\n" + 
		...
		egu.setTableName("rengcymkmcb");
		egu.getColumn("id").setHidden(true);
		egu.getColumn("id").setEditor(null);
		
	
		egu.getColumn("meikmc").setHeader("人工采样煤矿名称");
