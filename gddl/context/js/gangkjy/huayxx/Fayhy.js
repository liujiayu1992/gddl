...
{header:'车次/航次',dataIndex:'HANGC',width:100,editor :new Ext.form.TextField({maxLength:50,width:'auto',selectOnFocus:true}),hidden :false},
{header:'采样编码',dataIndex:'CHUANM',width:100,editor :new Ext.form.TextField({maxLength:50,width:'auto',selectOnFocus:true}),hidden :false},
{header:'销售供应商',dataIndex:'BIANM',editor :new Ext.form.TextField({maxLength:12,width:80,selectOnFocus:true}),hidden :false},
{header:'品种',dataIndex:'GONGYS',width:100,editor :new Ext.form.TextField({maxLength:50,width:'auto',selectOnFocus:true}),hidden :false},
{header:'煤量/装船量',dataIndex:'PINZ',width:60,editor :new Ext.form.TextField({maxLength:20,width:'auto',selectOnFocus:true}),hidden :false},
//{header:'车次',dataIndex:'CHEC',width:60,editor :new Ext.form.TextField({maxLength:20,width:'auto',selectOnFocus:true}),hidden :false},
{header:'出库结束时间',dataIndex:'ZHUANGCL',width:100,editor :new Ext.form.NumberField({decimalPrecision:3,selectOnFocus:true}),hidden :false},
//{header:'出库结束时间',dataIndex:'ZHUANGCJSSJ',renderer:function(value){ return (value==null || value=='')?'':('object' != typeof(value)?value:value.dateFormat('Y-m-d'));},width:80,editor :new Ext.form.DateField({allowBlank:true,format:'Y-m-d'}),hidden :false},
//{header:'票重(吨)',dataIndex:'BIAOZ',width:60,editor :new Ext.form.NumberField({decimalPrecision:3,selectOnFocus:true}),hidden :false},
//{header:'净重(吨)',dataIndex:'JINGZ',width:60,editor :new Ext.form.NumberField({decimalPrecision:3,selectOnFocus:true}),hidden :true},
...
{header:'STAR',dataIndex:'STAR',editor :new Ext.form.NumberField({allowBlank:true,decimalPrecision:3,selectOnFocus:true}),hidden :true},
...
var gridDiv_ds = new Ext.data.Store({proxy : new Ext.zr.data.PagingMemoryProxy(gridDiv_data),pruneModifiedRecords:true,reader: new Ext.data.ArrayReader({}, [
{name:'ID'},
{name:'HANGC'},
{name:'CHUANM'},
{name:'BIANM'},
...
{name:'ZHUANGCL'},
//{name:'ZHUANGCJSSJ'},
{name:'MT'},{name:'MAD'},{name:'AAR'},{name:'AAD'},{name:'AD'},{name:'VAR'},{name:'VAD'},{name:'VDAF'},{name:'FCAD'},{name:'QGRAD'},{name:'QGRD'},{name:'QNET_AR'},{name:'STAR'},{name:'STAD'},{name:'STD'},{name:'QBAD'},{name:'HAR'},{name:'HAD'},{name:'HDAF'},{name:'HUAYSJ'},{name:'HUAYBH'},{name:'HUAYY'},{name:'BEIZ'},{name:'SHENHZT'}])});
var gridDiv_plant = Ext.data.Record.create([{name: 'ID', type:'string'},
{name: 'HANGC', type:'string'},
{name: 'CHUANM', type:'string'},
{name: 'BIANM', type:'string'},
{name: 'GONGYS', type:'string'},
{name: 'PINZ', type:'string'},
{name: 'ZHUANGCL', type:'string'},
//{name: 'ZHUANGCJSSJ', type:'date'},
{name: 'MT', type:'float'},{name: 'MAD', type:'float'},{name: 'AAR', type:'float'},{name: 'AAD', type:'float'},{name: 'AD', type:'float'},{name: 'VAR', type:'float'},{name: 'VAD', type:'float'},{name: 'VDAF', type:'float'},{name: 'FCAD', type:'float'},{name: 'QGRAD', type:'float'},{name: 'QGRD', type:'float'},{name: 'QNET_AR', type:'float'},{name: 'STAR', type:'float'},{name: 'STAD', type:'float'},{name: 'STD', type:'float'},{name: 'QBAD', type:'float'},{name: 'HAR', type:'float'},{name: 'HAD', type:'float'},{name: 'HDAF', type:'float'},{name: 'HUAYSJ', type:'date'},{name: 'HUAYBH', type:'string'},{name: 'HUAYY', type:'string'},{name: 'BEIZ', type:'string'},{name:'SHENHZT', tupe:'float'}]);
...
ds:gridDiv_ds,
width:bodyWidth,
clicksToEdit:1,frame:true,tbar: ['出库结束时间',jiexrq=new Ext.form.DateField({
	allowBlank:true,
	format:'Y-m-d',
...
{text:' 刷新',icon:'imgs/btnicon/refurbish.gif',cls:'x-btn-text-icon',minWidth:75,handler:function(){document.getElementById('RefurbishButton').click();}},
{text:'编辑',icon:'imgs/btnicon/insert.gif',cls:'x-btn-text-icon',minWidth:75,handler:EditForm},
...
if(typeof(gridDiv_save)=='function'){ var revalue = gridDiv_save(Mrcd[i]);
 if(revalue=='return'){return;}else if(revalue=='continue'){continue;}}
if(Mrcd[i].get('HANGC') == ''){Ext.MessageBox.alert('提示信息','字段 航次 不能为空');return;
}
if(Mrcd[i].get('CHUANM') == ''){Ext.MessageBox.alert('提示信息','字段 船名 不能为空');return;
}
if(Mrcd[i].get('BIANM') == ''){Ext.MessageBox.alert('提示信息','字段 编码 不能为空');return;
}
//if(Mrcd[i].get('CHUANM') == ''){Ext.MessageBox.alert('提示信息','字段 发货人 不能为空');return;
//}
if(Mrcd[i].get('GONGYS') == ''){Ext.MessageBox.alert('提示信息','字段 发货人 不能为空');return;
}
...
if(Mrcd[i].get('ZHUANGCL') < -100000000){Ext.MessageBox.alert('提示信息',' 第 '+(i+1)+'行,字段车数 低于最小值 -100000000');return;
}
if( Mrcd[i].get('ZHUANGCL') >100000000000){Ext.MessageBox.alert('提示信息',' 第 '+(i+1)+'行,字段 车数 高于最大值 100000000000');return;
}
if(Mrcd[i].get('ZHUANGCL')!=0 && Mrcd[i].get('ZHUANGCL') == ''){Ext.MessageBox.alert('提示信息','字段 车数 不能为空');return;
}
//if(Mrcd[i].get('ZHUANGCJSSJ') == ''){Ext.MessageBox.alert('提示信息','字段 接卸日期 不能为空');return;
//}
/*
...
+ '<HANGC update="true">' + Mrcd[i].get('HANGC').replace(/[<]/g,'&lt;').replace(/[>]/g,'&gt;')+ '</HANGC>'
+ '<CHUANM update="true">' + Mrcd[i].get('CHUANM').replace(/[<]/g,'&lt;').replace(/[>]/g,'&gt;')+ '</CHUANM>'
+ '<BIANM update="true">' + Mrcd[i].get('BIANM').replace(/[<]/g,'&lt;').replace(/[>]/g,'&gt;')+ '</BIANM>'
+ '<GONGYS update="true">' + Mrcd[i].get('GONGYS').replace(/[<]/g,'&lt;').replace(/[>]/g,'&gt;')+ '</GONGYS>'
+ '<PINZ update="true">' + Mrcd[i].get('PINZ').replace(/[<]/g,'&lt;').replace(/[>]/g,'&gt;')+ '</PINZ>'
//+ '<CHEC update="true">' + Mrcd[i].get('CHEC').replace(/[<]/g,'&lt;').replace(/[>]/g,'&gt;')+ '</CHEC>'
+ '<ZHUANGCL update="true">' + Mrcd[i].get('ZHUANGCL')+ '</ZHUANGCL>'
//+ '<ZHUANGCJSSJ update="true">' + ('object' != typeof(Mrcd[i].get('ZHUANGCJSSJ'))?Mrcd[i].get('ZHUANGCJSSJ'):Mrcd[i].get('ZHUANGCJSSJ').dateFormat('Y-m-d'))+ '</ZHUANGCJSSJ>'
...
+ '<STAR update="true">' + Mrcd[i].get('STAR')+ '</STAR>'
...
+ '<SHENHZT update="true">' + Mrcd[i].get('SHENHZT').replace(/[<]/g,'&lt;').replace(/[>]/g,'&gt;')+ '</SHENHZT>'
 + '</result>' ; }
...
var Qbad_e = new Ext.form.NumberField({width:100,height:20,decimalPrecision:3, listeners:{'change':function(){CountValue();},'specialkey':function(){Had_e.focus(true)}}}); 
...
var AutoCountHad=new Ext.form.Checkbox({boxLabel:'化验值自动计算',height:20,checked:false,width:'auto',listeners:{'check':function(){CountValue();}}});
...
function EditForm(){
	...
	Qnetar_e.setValue(rec.get('QNET_AR'));
	...
	Rpt_window.show();
	CountValue();
	}else{Ext.MessageBox.alert('警告','煤量为0不能化验或已审核信息不能编辑!');return;}
...
function shenhztChk(){
	var rec=gridDiv_sm.getSelected();
	//对净重判断
	var IsequalsZero=rec.get('PINZ');
	//alert('bobo');
	//if(IsequalsZero>0){return true;}
	//    else{return false;}
	//对已审核状态的判断
	var chk=rec.get('SHENHZT');
	if(chk==0 && IsequalsZero>0){	return true;}
	else{return false;}
}
