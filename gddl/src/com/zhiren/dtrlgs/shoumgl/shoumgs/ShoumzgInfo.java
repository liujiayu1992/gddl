...
import com.zhiren.dtrlgs.faygl.faygs.Fayzgcl;

public class ShoumzgInfo {
	...
	public static void CountChengb(long diancxxb_id,List fhlist,boolean qiangzcs) {
		...
		if(strzglb.equals("中国大唐")){
			...
			Shoumzgcl.CountCb_PerFah(fhlist, qiangzcs);
		}		
//		因为结算改动造成成本模块出现BUG 2008-11-04
//		Shoumzgcl.CountCb(list, diancxxb_id, 0);
	}
	
	public static void CountChengb(List list,boolean qiangzcs) {//List中存放diancxxb_id 和 fahb_id
		String strzglb="大唐国际";
//		strzglb=MainGlobal.getXitxx_item("成本", "暂估算法分类", String.valueOf(diancxxb_id), strzglb).trim();
		...
		for(int i =0;i<list.size();i++) {
			...
			long dcid = ((ShoumcbBean)list.get(i)).getDiancID();
			String fhid = ""+((ShoumcbBean)list.get(i)).getFhfyID();//fahb_id||fayslb_id
		...
		if(strzglb.equals("中国大唐")){
			...
			Shoumzgcl.CountCb_PerFah(Fahidlist, qiangzcs);
		}		
//		因为结算改动造成成本模块出现BUG 2008-11-04
...
//		return newhtid;
	...
	public static String getHtidSql(int IntModel,String fahid) {
		StringBuffer sb = new StringBuffer();
		sb.append("select h.hetb_id from fahb f, fahhtb h\n")
