 ...
 * 作者：夏峥
 * 时间：2014-03-19
 * 描述：基于src/com/zhiren/dc/huaygl/huaysh/ruchysh/Ruchyejsh_duotsh.java 1.1.1.2 
 * 		为英化热电特殊制作
 */

public class Huayes_xb extends BasePage implements PageValidateListener {
	private String CustomSetKey = "Huayes_xb";
	private String msg = "";
...
//			增加日志
	...
	//	化验二级审核是否只显示化验的原始指标(Mt,Mad,Vdaf,Aad,Stad,Had,Qb,ad,Qgr,ad,Qnet,ar),默认为全部显示
		...
		//二级审核是否用日期过滤条件，默认为是
		sql = "select zhi from xitxxb where mingc = '化验二级审核不用到货日期过滤'  and zhuangt = 1  ";
		rsl = con.getResultSetList(sql);
		while (rsl.next()) {
			if (rsl.getString("zhi").equals("是")) {
				tiaojriq = false;
	...
	public void getSelectData(){
		JDBCcon con = new JDBCcon();
		String sql = "select a.id,a.meikdw,a.faz,a.caiyb_id,a.pinz,a.ches,a.shul,a.huaybh," +
						"zm.bianm huaybianh,a.huaysj,a.qnet_ar,round(a.qnet_ar/0.0041816) qnet_ar1,a.aar,a.ad,a.vdaf,a.mt,a.stad," +
						"a.aad,a.aad_YS,a.mad,a.qbad,a.qbad_YS,a.had,a.had_YS,a.vad,a.fcad,a.std,a.qgrad,a.hdaf,a.qgrad_daf," +
						...
						"       Z.QNET_AR,Z.AAR,Z.AD,Z.VDAF,Z.MT,Z.STAD,Z.AAD,Z.AAD_YS,\n" + 
						"       Z.MAD,Z.QBAD,Z.HAD,Z.QBAD_YS,Z.HAD_YS,Z.VAD,Z.FCAD,Z.STD,\n" + 
						"       Z.QGRAD,Z.QGRD,Z.HDAF,Z.QGRAD_DAF,Z.SDAF,Z.T1,Z.T2,Z.T3,Z.T4,\n" + 
						...
						"               AVG(L.T4) T4\n" + 
						"          FROM ZHILLSB L\n" + 
						"         WHERE ID IN ("+getZhilbids()+")\n" + 
						"         GROUP BY L.ZHILB_ID, L.HUAYSJ) Z,CAIYB C,\n" + 
...
//		ExtGridUtil egu = new ExtGridUtil("gridDiv", rsl);
		...
		egu.getColumn("qnet_ar").setRenderer("renderRZ");
		egu.getColumn("qnet_ar1").setHeader("收到基低位热量<p>Qnet,ar(Kcal/kg)</p>");
		egu.getColumn("qnet_ar1").setEditor(null);
		egu.getColumn("qnet_ar1").setRenderer("renderRZ");
		...
		egu.getColumn("aad").setRenderer("renderAAD");
		...
		egu.getColumn("qbad").setRenderer("renderQB");
		...
		egu.getColumn("had").setRenderer("renderHAD");
		...
		egu.getColumn("qnet_ar1").setWidth(110);
	...
	public void getAvg() {
		...
		mdrsl.close();
		zhilb_ids = zhilb_ids.substring(1);
		String sql = "select avg(z.qnet_ar) qnet_ar,round(avg(z.qnet_ar/0.0041816)) qnet_ar1\n"
				+ "       avg(z.aar) aar,\n"
				+ "       avg(z.ad) ad,\n"
				+ "       avg(z.vdaf) vdaf,\n"
				+ "       avg(z.mt) mt,\n"
		...
		egu.getColumn("qnet_ar").setEditor(null);
		egu.getColumn("qnet_ar1").setHeader("收到基地位热量<p>Qnet,ar(Kcal/kg)</p>");
		egu.getColumn("qnet_ar1").setEditor(null);
		egu.getColumn("aar").setHeader("收到基灰分<p>Aar(%)</p>");
		egu.getColumn("aar").setEditor(null);
		egu.getColumn("ad").setHeader("干燥基灰分<p>Ad(%)</p>");
