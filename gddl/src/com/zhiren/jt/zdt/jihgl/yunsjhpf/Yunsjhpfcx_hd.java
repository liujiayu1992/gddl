...
public class Yunsjhpfcx_hd extends BasePage {
	...
	private String RT_DR3 = "yunsjhpf_hd";// 运输计划批复_邯郸

	private String RT_DR4 = "yunsjhbcpf_hd";// 运输计划补充批复_邯郸
	...
	public String getPrintTable() {
		...
		if (mstrReportName.equals(RT_DR3)) {// 运输计划批复
			leib = 1;
			tablename = "yunsjhb";
			title = "运输计划查询";
	...
	private String getSelectData(String title, String tablename, long leix) {
		...
		if (tablename.equals("yunsjhb")) {
			...
			if(getLeixSelectValue().getValue().equals("分厂")){
				...
				ArrHeader[0] = new String[] { "单位", "发站(港)", "到站(港)", "品种",  "运输方式",
						"计划内", "计划外", "总车数", "总吨数","完成车数","完成吨数","到货率%" };//"批准号",
				...
				ArrHeader[0] = new String[] { "单位","煤矿单位", "发站(港)", "到站(港)", "品种", "运输方式",
						"计划内", "计划外", "总车数", "总吨数","完成车数","完成吨数","到货率%" };//"批准号",
...
//				ArrWidth = new int[] { 100,100, 80, 80, 80, 80, 80, 80,  80, 80 };
				...
				ArrHeader[0] = new String[] { "单位","供货单位","煤矿单位", "发站(港)", "到站(港)", "品种", "运输方式",
						"计划内", "计划外", "总车数", "总吨数","完成车数","完成吨数","到货率%" };//"批准号",
...
//				ArrWidth = new int[] { 100,100,100, 80, 80, 80, 80, 80, 80,  80, 80 };
				...
				ArrHeader = new String[1][11];
				ArrHeader[0] = new String[] { "供货单位","煤矿单位","单位", "发站(港)", "到站(港)", "品种", "运输方式",
						"计划内", "计划外", "总车数", "总吨数","完成车数","完成吨数","到货率%" };//"批准号",
		...
		if (getLeixSelectValue() != null) {
			...
			//**********************分矿*************************//				
				...
				groupingmc = " decode(grouping(g.mingc),1,'总计',g.mingc) as dianwmc,\n"// '&nbsp;&nbsp;&nbsp;&nbsp;'||
					    + " decode(grouping(g.mingc)+grouping(m.mingc),2,'',1,'小计',m.mingc) as meikxxb_id,"
						+ " decode(grouping(m.mingc)+grouping(cz.mingc),2,'',1,'小计',cz.mingc) as faz_id, \n";
				danwmc = "cz.mingc";
		    ...
		    //**********************分厂分矿*************************//				
				...
				if (jib == 3 || jib == -1) {// 按电厂分
					groupingmc = " decode(grouping(diancmc),1,'总计',diancmc) as dianwmc,\n"
							...
							+ " decode(grouping(m.mingc)+grouping(g.mingc),2,'',1,'小计',m.mingc) as meikxxb_id,"
							+ " decode(grouping(m.mingc)+grouping(cz.mingc),2,'',1,'小计',cz.mingc) as faz_id, \n";
					danwmc = "g.mingc";
		    ...
		    //**********************分矿分厂*************************//					
				...
				groupingmc = " decode(grouping(g.mingc),1,'总计',g.mingc) as gongysmc,\n"
						+ " decode(grouping(g.mingc)+grouping(m.mingc),2,'',1,'小计',m.mingc) as meikxxb_id,\n"
						+ " decode(grouping(m.mingc)+grouping(diancmc),2,'',1,'小计',diancmc) as dianwmc,\n"
						+ " decode(grouping(diancmc)+grouping(cz.mingc),2,'',1,'小计',cz.mingc) as faz_id, \n";
				danwmc = "diancmc";
