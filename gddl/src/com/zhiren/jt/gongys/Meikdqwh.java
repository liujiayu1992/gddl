...
public class Meikdqwh extends BasePage implements PageValidateListener {
//	 判断是否是集团用户
...
//	系统日志表中的类别字段
	private static  final String leiBConstant="煤矿地区维护";
...
//	用来判断是否可编辑上级公司编码
	private String isEditShangjgsbm;

	public String getIsEditShangjgsbm() {
		return isEditShangjgsbm;
	}

	public void setIsEditShangjgsbm(String isEditShangjgsbm) {
		this.isEditShangjgsbm = isEditShangjgsbm;
	}
	...
	private void WriteLog(JDBCcon con){
			...
			if(!logMsg.equals("")){//不为空，需要写入日志记录
				...
				String sql=" insert into xitrzb(id,diancxxb_id,yonghm,leib,shij,zhuangt,beiz) values(" +
						...
						+visit.getDiancxxb_id()+",'"+visit.getRenymc()+"','"+leiBConstant+"',to_date('"+date_str+"','YYYY-MM-DD,HH24:mi:ss'),'"+this.zhuangT+"','"+logMsg+"')";
...
//		sql.append("End;");
		...
		while (mdrsl.next()) {
			...
			String SaveMsgLocal="";
			int meikdq_id = Integer.parseInt(MainGlobal.getNewID(visit.getDiancxxb_id()));
//			sql2.append("getnewid(").append(visit.getDiancxxb_id()).append(")");
//			String gys_id = "";
			if ("0".equals(mdrsl.getString("ID"))) {
				String sql_check="select id from gongysb where (1=0 ";
				sql.append("insert into ").append(tableName).append("(id, leix");
				String mingc="";
				String quanc="";
				String bianm="";
...
//						long FUID=(getExtGrid().getColumn(mdrsl.getColumnNames()[i]).combo).getBeanId(mdrsl.getString("fuid"));
//						if(FUID==-1L){
...
//						}
				...
				if(!mdrsl.getString("SHANGJGSBM").equals("")){
					if(this.Shujpd(con, sql_check + ") and leix = 0")!=0){  //在本地库中已经存在,要进行的操作
						...
						sql.append(") values(").append(meikdq_id).append(" ,0 ").append(sql2).append(")\n");
						int flag=con.getInsert(sql.toString());
						//插入时不需要记录日志
						if(flag==-1){  //插入不成功
		//					this.WriteLog();
						}else{        //插入成功 更新gongysdcglb表
							int id = Integer.parseInt(MainGlobal.getNewID(visit.getDiancxxb_id()));
							long diancxxb_id = visit.getDiancxxb_id();
							sql_dc.append("insert into gongysdcglb values(")
								.append(id).append(",").append(diancxxb_id).append(",").append(meikdq_id).append(")");
				...
				for (int i = 1; i < mdrsl.getColumnCount(); i++) {
					...
					if (mdrsl.getColumnNames()[i].equals("FUID")) {
						if (mdrsl.getString("fuid").equals("请选择")
								|| mdrsl.getString("fuid").equals("")) {
							sql.append(-1).append(",");
						} else {
				...
				sql_update.append(") and leix = 0 and id <> ").append(mdrsl.getString("ID"));
...
//	查询编码值（根据id），判断前后是否改变，进而判断是否需要更改下级单位的上级编码字段
	...
	public void getSelectData() {
		...
		String sql = 
			...
			"       g.quanc,\n" + 
			"       g.piny,\n" + 
			"       DECODE((SELECT mingc FROM gongysb WHERE ID = g.fuid),\n" + 
			"              NULL,\n" + 
			"              '请选择',\n" + 
			"              (SELECT mingc FROM gongysb WHERE ID = g.fuid)) AS fuid,\n" + 
			...
			"       g.yunsnl,\n" + 
			"       g.heznx,\n" + 
			"       decode(g.rongqgx, 1, '优', 2, '良', 3, '中', 4, '差', '') rongqgx,\n" + 
			"       decode(g.xiny, 1, '优', 2, '良', 3, '中', 4, '差', '') xiny,\n" + 
			"       g.gongsxz,\n" + 
			"       g.kegywfmz,\n" + 
			"       g.kegywfmzzb,\n" + 
			"       decode(g.shifss, 1, '是', 2, '否', '') shifss,\n" + 
			"       g.shangsdz,\n" + 
			"       g.zicbfb,\n" + 
			"       g.shoumbfb,\n" + 
			"       g.qitbfb,\n" + 
...
//		融洽关系
		...
		egu.getColumn("rongqgx").setComboEditor(gridId,
				new IDropDownModel(rqgx));
		egu.getColumn("rongqgx").editor.allowBlank = true;
...
//		信誉
		...
		egu.getColumn("xiny").editor.allowBlank = true;
...
//		是否上市
		...
		egu.getColumn("shifss").setComboEditor(gridId,
				new IDropDownModel(sfss));
		egu.getColumn("shifss").editor.allowBlank = true;
		...
		if (str.equals("否")) {
			...
			egu.addOtherScript("document.all.isEditShangjgsbm.value='是';");
		}
		String handler = 
			"var disable = true;\n" +
...
//		return sb.toString();
	...
	public void getFuidModels() {
		String sql = "select id, mingc from gongysb where leix = 0 order by xuh,mingc";
...
//	 ////////////////////////////融洽关系///////////////////////////////////////
	public IDropDownBean getRQGXValue() {
		if (((Visit) getPage().getVisit()).getDropDownBean7() == null) {
			((Visit) getPage().getVisit())
					.setDropDownBean7((IDropDownBean) getRQGXModel()
							.getOption(0));
		}
	...
	public void getRQGXModels() {
		...
		((Visit) getPage().getVisit())
				.setProSelectionModel7(new IDropDownModel(ls));
		return;
	}
...
//	 ////////////////////////////信誉///////////////////////////////////////
	public IDropDownBean getXYValue() {
		if (((Visit) getPage().getVisit()).getDropDownBean4() == null) {
			((Visit) getPage().getVisit())
					.setDropDownBean4((IDropDownBean) getXYModel()
							.getOption(0));
		}
	...
	public void getXYModels() {
		...
		ls.add(new IDropDownBean(4, "差"));
		((Visit) getPage().getVisit())
				.setProSelectionModel4(new IDropDownModel(ls));
		return;
	}
...
//	 ////////////////////////////是否上市///////////////////////////////////////
	public IDropDownBean getSFSSValue() {
		if (((Visit) getPage().getVisit()).getDropDownBean5() == null) {
			((Visit) getPage().getVisit())
					.setDropDownBean5((IDropDownBean) getSFSSModel()
							.getOption(0));
		}
	...
	public void getSFSSModels() {
		...
		((Visit) getPage().getVisit())
				.setProSelectionModel5(new IDropDownModel(ls));
		return;
	}
