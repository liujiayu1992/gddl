...
import com.zhiren.main.validate.Login;
	...
	public class Baobpzsz extends BasePage implements PageValidateListener {
//		进行页面提示信息的设置
		private String _msg;
...
//		保存分组的改动
		...
		public void getSelectData() {
			// Visit visit = (Visit) getPage().getVisit();
			JDBCcon con = new JDBCcon();
			ResultSetList rsl = con.getResultSetList("select id,baobpzb_id,zidm,biaot,xuh,kuand,decode(shifxs,0,'否',1,'是')as shifxs,format from Baobpzzb  where baobpzb_id="+((Visit) this.getPage().getVisit()).getString1()+"order by xuh");

			
			ExtGridUtil egu = new ExtGridUtil("gridDiv", rsl);
			egu.setTableName("Baobpzzb");
			egu.getColumn("id").setHeader("ID");
			egu.getColumn("id").setHidden(true);
			egu.getColumn("baobpzb_id").setHeader("报表配置表ID");	
			egu.getColumn("baobpzb_id").setEditor(null);
			egu.getColumn("baobpzb_id").setHidden(true);
			egu.getColumn("zidm").setHeader("字段名");		
			egu.getColumn("biaot").setHeader("表头");
			egu.getColumn("biaot").setWidth(150);
			egu.getColumn("xuh").setHeader("序号");	
			egu.getColumn("xuh").editor.setMaxValue("99999");
			egu.getColumn("kuand").setHeader("表头宽度");
			egu.getColumn("shifxs").setHeader("是否显示");	
			egu.getColumn("format").setHeader("格式化");	
			...
			sb.append("{text:' 添加',icon:'imgs/btnicon/insert.gif',cls:'x-btn-text-icon',minWidth:75,handler:function() {	\n")
				...
				.append("var plant = new gridDiv_plant({ID: '0',BAOBPZB_ID: '").append(((Visit) this.getPage().getVisit()).getString1()).append("',ZIDM: '',BIAOT: '',XUH: '',SHIFXS: '是'});	\n")
