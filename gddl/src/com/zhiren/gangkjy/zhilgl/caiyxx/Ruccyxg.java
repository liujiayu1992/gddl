...
public class Ruccyxg extends BasePage implements PageValidateListener {
	// 界面用户提示
	private String msg = "";
...
//		 卸煤日期的ora字符串格式
		...
		if(visit.isGSUser()){
			strDianc = " and (dc.id="+visit.getDiancxxb_id() +" or dc.fuid="+visit.getDiancxxb_id()+")";
		}
		if(visit.isDCUser()){
			strDianc = " and dc.id="+visit.getDiancxxb_id();
		}
		
		String sql = 
			"select c.id,f.zhilb_id,z.huaybh,to_char(f.daohrq,'yyyy-mm-dd') xiemrq,\n" +
			"vf.mingc fahr,vp.mingc pinz,\n" + 
			"c.bianm,c.yangplb,c.yangpzl,s.mingc sjdw,c.jieyr,l.mingc caiylb,c.beiz\n" + 
			"from caiyb c,\n" + 
			"(select t.id,t.mingc from item t,itemsort s\n" + 
			"where t.itemsortid = s.id and s.mingc = '质量检验单位') s,\n" + 
			"caiylbb l, fahb f, zhillsb z, vwfahr vf, vwpinz vp, diancxxb dc \n" + 
			"where c.caiylbb_id = l.id and c.songjdwb_id = s.id(+)\n" + 
			"and l.mingc = '进厂化验' and f.zhilb_id = z.zhilb_id \n" + 
			"and z.caiyb_id = c.id and f.diancxxb_id = dc.id "+strDianc+"\n" + 
			"and f.daohrq >= "+strxmrqOra+"\n" + 
			"and f.daohrq < "+strxmrqeOra+" +1\n" + 
			"and f.gongysb_id = vf.id and f.pinzb_id = vp.id";
...
//		设置grid隐藏列
		...
		// 设置grid列标题
		egu.getColumn("xiemrq").setHeader(Local.xiemsj);
		...
		egu.getColumn("sjdw").setHeader(Local.songjdw);
...
//		设置列宽度
		egu.getColumn("xiemrq").setWidth(80);
		...
		egu.getColumn("sjdw").setWidth(140);
		egu.getColumn("jieyr").setWidth(60);
		egu.getColumn("caiylb").setWidth(60);
		egu.getColumn("beiz").setWidth(180);
//		设定grid列可否编辑
		egu.getColumn("xiemrq").setEditor(null);
		egu.getColumn("fahr").setEditor(null);
...
//		 返回
		GridButton Return = new GridButton("返回", "ReturnRuccy");
	...
	private void save() {
		...
		while (rs.next()) {
			...
			sql += "update fahb set zhilb_id = 0 where zhilb_id =" + zhilbid + ";\n";
		...
		while (rs.next()) {
			...
			String yangplb = getExtGrid().getColumn("yangplb").combo.getBeanStrId(rs
					.getString("yangplb"));
			String songjdw = getExtGrid().getColumn("sjdw").combo.getBeanStrId(rs
					.getString("sjdw"));
