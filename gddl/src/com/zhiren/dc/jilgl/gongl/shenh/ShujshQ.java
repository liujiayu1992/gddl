...
public class ShujshQ extends BasePage implements PageValidateListener {
//	界面用户提示
	private String msg="";
...
//	发货日期下拉框
	...
	public void setFahrqModels() {
		...
		String sql = "select rownum xuh,fahrq from (select distinct to_char(fahrq,'yyyy-mm-dd') fahrq from fahb where diancxxb_id = "+visit.getDiancxxb_id()+" and hedbz = "+SysConstant.HEDBZ_YJJ+" and yunsfsb_id = "+SysConstant.YUNSFS_QIY+" order by fahrq) ";
	...
	private void Save() {
		...
		while(rsl.next()) {
			...
			if(flag == -1) {
				con.rollBack();
				con.Close();
				WriteLog.writeErrorLog(ErrorMessage.ShujshQ001);
				setMsg(ErrorMessage.ShujshQ001);
				return;
			}
			...
			if(flag == -1) {
				con.rollBack();
				con.Close();
				WriteLog.writeErrorLog(ErrorMessage.ShujshQ002);
				setMsg(ErrorMessage.ShujshQ002);
				return;
			}
			sb.delete(0, sb.length());
		}
	...
	public void getSelectData() {
		...
		sb.append("p.mingc pinzb_id, f.daohrq, \n");
		sb.append("(select fc.mingc from chezxxb fc where fc.id = f.faz_id) faz_id, \n");
		sb.append("(select dc.mingc from chezxxb dc where dc.id = f.daoz_id) daoz_id, \n");
		sb.append("f.jingz, f.biaoz, f.zongkd, f.ches, f.chec, j.mingc jihkjb_id \n");
		sb.append("from fahb f, gongysb g, meikxxb m, pinzb p, jihkjb j \n");
		sb.append("where f.gongysb_id = g.id and f.meikxxb_id = m.id \n");
		sb.append("and f.pinzb_id = p.id and f.jihkjb_id = j.id \n");
		sb.append("and f.hedbz = ").append(SysConstant.HEDBZ_YJJ)
		.append(" and f.yunsfsb_id =").append(SysConstant.YUNSFS_QIY).append(" \n");
