...
public class Rikcjgcx extends BasePage {
 ...
 * 时间：2013-11-28
 * 描述：变更标题
 * 		增加来煤标煤单价列：来煤燃料费/(来煤数量*来煤热值/7000)
 * 		增加耗煤标煤单价列：耗煤燃料费/(耗煤数量*耗煤热值/7000)
 */	
//	界面用户提示
	private String msg="";
...
//	
	...
	private String getKucrb() {
		...
		String strSQL = 
			...
			"       SUM(R.LAIMSL) LAIMSL,\n" + 
			"       DECODE(SUM(R.LAIMSL), 0, 0, SUM(R.LAIMRZ * R.LAIMSL) / SUM(R.LAIMSL)) LAIMRZ,\n" + 
			"       SUM(R.LAIMRLF) LAIMRLF,\n" + 
			"       SUM(R.HAOML) HAOML,\n" + 
			"       DECODE(SUM(R.HAOML), 0, 0, SUM(R.HAOMRZ * R.HAOML) / SUM(R.HAOML)) HAOMRZ,\n" + 
			...
			"           AND K.RIQ BETWEEN DATE '"+beginRiq+"' - 1 AND DATE'"+beginRiq+"') MZ,\n" + 
			"       KUCMRBB R,\n" + 
		...
		ArrHeader[0]=new String[] {"单位","煤种","来煤","来煤","来煤","来煤","耗煤","耗煤","耗煤","耗煤","库存","库存","库存","库存","库存","库存","库存","库存","下周来<br>煤预测","库存<br>调整量<br>(吨)"};
		ArrHeader[1]=new String[] {"单位","煤种","数量","热值","燃料费","标煤单价","数量","热量","燃料费","标煤单价","控制目标","数量","热值","标煤单价","原煤单价","每卡单价","库存费用","库存与控制目标差","下周来<br>煤预测","库存<br>调整量<br>(吨)"};
		ArrHeader[2]=new String[] {"单位","煤种","吨","Kcal/Kg","万元","元/吨","吨","Kcal/Kg","万元","元/吨","万吨","万吨 ","Kcal/Kg","元/吨"," 元/吨","元/Kcal","万元","万吨 ","万吨","库存<br>调整量<br>(吨)"};
		int ArrWidth[]=new int[] {80,80,70,60,60,60,70,60,60,60,60,60,60,60,60,60,70,60,60,40};
		//rs.beforefirst();
		rt.setBody(new Table(rs, 3, 0, 2));
		rt.body.setHeaderData(ArrHeader);
		rt.body.setWidth(ArrWidth);
		rt.body.setColAlign(1, Table.ALIGN_CENTER);
		rt.setTitle(getDiancmc(String.valueOf(visit.getDiancxxb_id()))+"来耗存量质价表",ArrWidth);
