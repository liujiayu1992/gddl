...
//{header:'车次/航次',dataIndex:'CHEC',width:100,editor :new Ext.form.TextField({maxLength:50,width:'auto',selectOnFocus:true}),hidden :false},
...
{header:'销售供应商',dataIndex:'GONGYS',editor :new Ext.form.TextField({maxLength:12,width:80,selectOnFocus:true}),hidden :false},
{header:'车次/航次',dataIndex:'CHEC',width:60,editor :new Ext.form.TextField({maxLength:20,width:'auto',selectOnFocus:true}),hidden :false},
{header:'品种',dataIndex:'PINZ',width:100,editor :new Ext.form.TextField({maxLength:50,width:'auto',selectOnFocus:true}),hidden :false},
{header:'煤量/装船量',dataIndex:'MEIL',width:60,editor :new Ext.form.TextField({maxLength:20,width:'auto',selectOnFocus:true}),hidden :false},
{header:'出库结束时间',dataIndex:'CHUKJSSJ',width:100,editor :new Ext.form.NumberField({decimalPrecision:3,selectOnFocus:true}),hidden :false},
...
{name:'MEIL'},
{name:'CHUKJSSJ'},
{name:'MT'},{name:'MAD'},{name:'AAR'},{name:'AAD'},{name:'AD'},{name:'VAR'},{name:'VAD'},{name:'VDAF'},{name:'FCAD'},{name:'QGRAD'},{name:'QNET_AR'},{name:'STAD'},{name:'STD'},{name:'QBAD'},{name:'HAD'},{name:'HDAF'}])});
var gridDiv_plant = Ext.data.Record.create([{name: 'ID', type:'string'},
{name: 'FID', type:'string'},
{name: 'GONGYS', type:'string'},
{name: 'CHEC', type:'string'},
{name: 'PINZ', type:'string'},
{name: 'MEIL', type:'string'},
{name: 'CHUKJSSJ', type:'date'},
{name: 'MT', type:'float'},{name: 'MAD', type:'float'},{name: 'AAR', type:'float'},{name: 'AAD', type:'float'},{name: 'AD', type:'float'},{name: 'VAR', type:'float'},{name: 'VAD', type:'float'},{name: 'VDAF', type:'float'},{name: 'FCAD', type:'float'},{name: 'QGRAD', type:'float'},{name: 'QGRD', type:'float'},{name: 'QNET_AR', type:'float'},{name: 'STAD', type:'float'},{name: 'STD', type:'float'},{name: 'QBAD', type:'float'},{name: 'HAD', type:'float'},{name: 'HDAF', type:'float'}]);
...
if(Mrcd[i].get('MEIL') < -100000000){Ext.MessageBox.alert('提示信息',' 第 '+(i+1)+'行,字段 煤量 低于最小值 -100000000');return;
}
if( Mrcd[i].get('MEIL') >100000000000){Ext.MessageBox.alert('提示信息',' 第 '+(i+1)+'行,字段 煤量 高于最大值 100000000000');return;
}
if(Mrcd[i].get('MEIL')!=0 && Mrcd[i].get('MEIL') == ''){Ext.MessageBox.alert('提示信息','字段 煤量 不能为空');return;
}
gridDivsave_history += '<result>' + '<sign>U</sign>'
+ '<ID update="true">' + Mrcd[i].get('ID').replace(/[<]/g,'&lt;').replace(/[>]/g,'&gt;')+ '</ID>'
+ '<FID update="true">' + Mrcd[i].get('FID').replace(/[<]/g,'&lt;').replace(/[>]/g,'&gt;')+ '</FID>'
...
+ '<MEIL update="true">' + Mrcd[i].get('MEIL')+ '</MEIL>'
+ '<CHUKJSSJ update="true">' + ('object' != typeof(Mrcd[i].get('CHUKJSSJ'))?Mrcd[i].get('CHUKJSSJ'):Mrcd[i].get('CHUKJSSJ').dateFormat('Y-m-d'))+ '</CHUKJSSJ>'
...
var Had_e = new Ext.form.NumberField({width:100,height:20,listeners:{'change':function(){CountValue();}}});
...
items:[{columnWidth :.25, items:[{html:'',height:24},{html:strdivs + '水分 M (%)</div>',height:21},{html:strdivs + '灰分 A (%)</div>',height:21},{html:strdivs + '挥发分 V (%)</div>',height:21},{html:strdivs + '固定碳 FC (%)</div>',height:21},{html:strdivs + '高位热 Qgr (MJ/kg)</div>',height:21},{html:strdivs + '低位热 Qnet (MJ/kg)</div>',height:21},{html:strdivs + '全硫 St (%)</div>',height:18},{html:'<hr>',height:21},{html:strdivs + '挥发分 Vdaf (%)</div>',height:21}, {html:'',height:21}]},
{columnWidth :.25, items:[{html:'收到基(ar)',height:22}, Mt_e, Aar_e, Var_e, FCar_e, {html:'<div align=center valign=center style="width:100"><hr></div>',height:20}, Qnetar_e, Star_e,{html:'<hr>'}, Vdaf_e, AutoCountHad]},
{columnWidth :.25, items:[{html:'空气干燥基(ad)',height:22}, Mad_e, Aad_e, Vad_e, FCad_e, Qgrad_e, {html:'<div align=center valign=center style="width:100"><hr></div>',height:20}, Stad_e,{html:'<hr>',height:21},{html:strdivs + '弹筒热值(MJ/kg)</div>',height:21},{html:strdivs + '空气干燥基氢(Had)</div>',height:21}]},
{columnWidth :.25, items:[{html:'干燥基(d)',height:22},{html:'<div align=center valign=center style="width:100"><hr></div>',height:20}, Ad_e, Vd_e, FCd_e, {html:'<div align=center valign=center style="width:100"><hr></div>',height:21}, {html:'<div align=center valign=center style="width:100"><hr></div>',height:21}, Std_e,{html:'<hr>'}, Qbad_e, Had_e]}],
...
function EditForm(){
	...
	Rpt_window.show();
	CountValue();
	}else{Ext.MessageBox.alert('警告','煤量为0不能化验!');return;}
