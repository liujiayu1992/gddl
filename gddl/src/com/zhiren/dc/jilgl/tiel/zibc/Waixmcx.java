 ...
 * 		 把报表的格式设定为A4打印形式。
 */
public class Waixmcx extends BasePage {
...
//	煤矿下拉框
	...
	public void setKuangbModels() {
		...
		sb.append("and c.fahb_id=f.id and c.meicb_id=20117418 and f.daohrq<="+DateUtil.FormatOracleDate(getERiq())+" \n"
...
//	显示方式下拉框
	...
	private String REPORT_NAME_WAIXMCCX = "Waixmccx";// zhillsb中的数据
...
//  判断电厂Tree中所选电厂时候还有子电厂   
	...
	private String getWaixmccx() {
		...
		if(getXiansfsValue().getId()==1){
			sql = "select h.daohrq,h.meikdwmc,h.ches as cheph,h.maoz,h.piz,h.jingz,h.biaoz from " 
				...
				+ "sum(c.maoz) maoz,sum(c.piz) piz,sum(c.maoz-c.piz) jingz,sum(c.biaoz) biaoz,count(*) ches \n"
				+ "from chepb c,fahb f,meikxxb mk where c.meicb_id=20117418 \n"
