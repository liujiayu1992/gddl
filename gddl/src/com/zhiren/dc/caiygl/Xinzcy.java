...
public class Xinzcy extends BasePage implements PageValidateListener {
	private String msg = "";
	...
	public void setZhiybm(String zybm){
		zhiybm = zybm;
	}
		  ...
		  if(rs.next()){//是酒泉
			if(rs.getString("zhi").equals("是")){
				sql =
					...
					"       c.bianm as jincph,\n" + 
					"       getFahxx_jq(c.zhilb_id) as fahxx,\n" + //使用酒泉的方法
					"       getCaiybh4zl(c.zhilb_id) as caiybh,\n" + 
					"       getHuaybh4zl(c.zhilb_id) as huaybh\n" + 
					"from caiyb c,(select distinct f.diancxxb_id,zhilb_id from fahb f,diancxxb d\n" + 
			...
			sql = 
				...
				"       getCaiybh4zl(c.zhilb_id) as caiybh,\n" + 
				"       getHuaybh4zl(c.zhilb_id) as huaybh\n" + 
				"from caiyb c,(select distinct f.diancxxb_id,y.caiyb_id from zhuanmb zm,zhuanmlb l,yangpdhb y, fahb f,diancxxb d, caiyb c\n" +
				"where f.diancxxb_id = d.id and zm.zhuanmlb_id = l.id and l.jib = 2 and y.zhilblsb_id = zm.zhillsb_id\n" + 
				"and f.zhilb_id = c.zhilb_id and c.id = y.caiyb_id "+dcsql+"\n" + 
				"and zm.bianm ='"+getZhiybm()+"') f " +
				"where f.caiyb_id = c.id order by c.caiyrq,c.bianm\n";
			}
...
//		 设置树
		...
		String zybmcx = MainGlobal.getXitxx_item("采制化", "新增样按制样编码查询", getTreeid(), "否");
		if("是".equals(zybmcx)){
			egu.addTbarText("制样编码:");
			TextField tf=new TextField();
			tf.setWidth(80);
			tf.setValue(getZhiybm());
			//tf.setId("Zhiybm");
			tf.setListeners("change:function(own,n,o){document.getElementById('Zhiybm').value = n}");
...
//		 刷新按钮
		...
		comb1
				.setListeners("select:function(own,rec,index){ Ext.getDom('Leib').selectedIndex=index;}");
...
//		egu.addTbarText("-");
	...
	private void Update(IRequestCycle cycle) {
		...
		String sql = 
			"select * from leibb where id = " + leibb_id + "\n";
		...
		//得到部门
		String sSql = 
			"select * from bumb order by zhuhybm desc,mingc";
		ResultSetList rsl = con.getResultSetList(sSql);
		if(rsl.next()){
			bumb_id = rsl.getString("ID");
		}
		rsl.close();
		sql = "update zhillsb set shenhzt = 5 where shenhzt = 7 and  zhilb_id=" + zhilb_id;
		con.getDelete(sql);
		sql = "delete from zhilb where id=" + zhilb_id;
		con.getDelete(sql);
//		String sql_b = 
//			"select distinct l.id\n" +
//			"from yangpdhb y,caiyb c,leibb l,\n" + 
//			"     (select l.id from leibb l where beiz = '新增') x\n" + 
//			"where y.caiyb_id = c.id\n" + 
//			"      and c.id = "+caiyb_id+"\n" + 
//			"      and y.leibb_id = x.id\n";
//		ResultSetList rsl_b = con.getResultSetList(sql_b);
//		if(rsl_b.next()){
//			setMsg("新增样已经存在！");
//		}else{
