 ...
 * @author huochaoyuan
 *陕西分公司日报程序用
 *新建“月运输计划录入”(Yueysjh_sx)
 *在原有页面上显示“生效日期”(yueysjh.riq)列，
 *人工填入计划生效日期，实现相同口径的计划一个月内出现两条，
 *但生效日期不同，报表取数时根据该时间取到相应的量；
 */
public class Yueysjh_sx extends BasePage implements PageValidateListener {
	...
	public void getSelectData(ResultSetList rsl) {
		...
		egu.getColumn("riq").setHeader("生效日期");
...
//		egu.getColumn("riq").setEditor(null);
		...
		// 设定合计列不保存
		sb
				.append("function gridDiv_save(record){if(record.get('diancxxb_id')=='合计') return 'continue';}");
