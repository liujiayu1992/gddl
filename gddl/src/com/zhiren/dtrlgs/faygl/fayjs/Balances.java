	       ...
	       "          max(pinzb_id) as pinzb_id,\n"+
            ...
            if (rs.next()) {
                ...
                bsv.setXianshr(rs.getString("yuanshdw"));
	          	...
	          	bsv.setDaibcc(rs.getString("chec"));
...
//		结算数、质量
		...
		try{
			...
			rsl=con.getResultSetList(Fayjsdcz.getJiesszl_Sql(bsv, Jieszbsftz, SelIds, Diancxxb_id, Gongysb_id, Hetb_id, 
...
//				数量(合同中以月为单位，每月存一条，暂定先取一条数)
				sql="select nvl(htsl.hetl,0) as hetl,htb.gongfdwmc,htb.gongfkhyh,htb.gongfzh \n"
					...
					+ " and (htsl.diancxxb_id="+Diancxxb_id+" or htsl.diancxxb_id is null) and (htsl.riq<=to_date('"+Fayjsdcz.FormatDate(Minfahrq)
...
//					SelIds为全部结算lie_id
//					特殊指标数组只能赋值一次
					bsv.setJieszbtscl_Items(bsv.getJieszbtscl_Items()+Fayjsdcz.getJieszbtscl(rsl.getString("zhib"), SelIds));
				}
...
//									获得结算数量、质量
									...
									Dbltmp=Fayjsdcz.getUnit_transform(rsl.getString("zhib"),rsl.getString("danw"),Dbltmp,bsv.getMj_to_kcal_xsclfs());
...
//										获得结算数量、质量
										...
										while(rsl.next()){
											...
											Dbljijzb=Fayjsdcz.getUnit_transform(rsl.getString("zhib"),rsl.getString("danw"),Dbljijzb,bsv.getMj_to_kcal_xsclfs());
										
											if(Dbljijzb>=rsl.getDouble("xiax")&&Dbljijzb<=(rsl.getDouble("shangx")==0?1e308:rsl.getDouble("shangx"))
													&&Fayjsdcz.getHet_condition(bsv.getJiagpzId(), String.valueOf(bsv.getRanlpzb_Id()))
											){
...
//		函数形参：rsl 合同的数据集，bsh 结算自动计算的容器
		...
		bsv.setHetmdjgs(Fayjsdcz.getTransform_Hetjgs(
...
//					得到参照项目的结算标准
					Dblczxm=Fayjsdcz.getZhib_info(bsv,rsl.getString("canzxm"),"js");
					
//					指标的结算指标
					Dbltmp=Fayjsdcz.getUnit_transform(rsl.getString("zhib"),rsl.getString("danw"),Dbltmp,bsv.getMj_to_kcal_xsclfs());
					Dblczxm=Fayjsdcz.getUnit_transform(rsl.getString("canzxm"),rsl.getString("canzxmdw"),Dblczxm,bsv.getMj_to_kcal_xsclfs());
					
					if(Dbltmp>=Fayjsdcz.getZengkkxx_Value(Strimplementedzb, rsl.getString("zhib"), Dblimplementedzbsx, rsl.getDouble("xiax"))&&Dbltmp<=(rsl.getDouble("shangx")==0?1e308:rsl.getDouble("shangx"))
							&&Fayjsdcz.getHet_condition(rsl.getString("canzxm"), Dblczxm, rsl.getDouble("canzxx"), rsl.getDouble("canzsx"))
							&&Fayjsdcz.getJieszbtscl_Zkk(bsv.getJiesxs(), rsl.getString("jiesxs"))	//如果该指标需要特殊（单批次）处理，将不参加统一的增扣款的计算
					){
...
//						给重新计算增扣款的特殊处理的指标数组打标志
						Fayjsdcz.Mark_Tsclzbs_bz(bsv.getTsclzbs(),Tsclzb_item[0]+","+rsl.getString("zhib"));
...
//						解析运费价格明细中的数据
						bsh=Fayjsdcz.CountYfjsfa(bsv.getMeikxxb_Id(),bsv.getDiancxxb_id(),bsv.getFaz_Id(),
			...
			do {
				...
				if(Dbljijzb>=rsl.getDouble("xiax")&&Dbljijzb<=(rsl.getDouble("shangx")==0?1e308:rsl.getDouble("shangx"))){
					...
					if(Fayjsdcz.CheckMljRz(rsl.getString("zhib"))){
						...
						bsh.set(rsl.getString("zhib")+"_上限", Fayjsdcz.getUnit_transform(rsl.getString("zhib"), Locale.qiankmqk_danw, (rsl.getDouble("shangx")==0?1e308:rsl.getDouble("shangx"))));
...
//			总金额：	(hansmj-超出部分的折价)*结算数量+超出部分的折价×超出部分
			_Meikzkktzsj=Fayjsdcz.Zengkktz(bsv);
		}
		//价格金额计算
//		2008-12-9zsj加：
...
//						给指标赋值
					Fayjsdcz.setJieszbzdj_Tszb(zhibbm,bsv,zhibzdj,CustomMaths.Round_new(CustomMaths.div(zhibjsbzjqz, zhibjsbzdysl),bsv.getMeikzkkblxsw()),zhibzje);
...
//						给指标赋值
					Fayjsdcz.setJieszbzdj_Tszb(zhibbm,bsv,zhibzdj,zhibjsbz,zhibzje);
				}
//				将特殊增扣款的折价金额记录下来
				jieszjecj = zhibzje;
			}
...
//		bsv.setBuhsyf(_Buhsyf);
		...
		sql.append(Fayjsdcz.InsertDanpcjsmkb(bsv,this.getZhibModel()));
...
//		_Shulzje=(double)Math.round(_Shul*_Jiessl*100/100);		记录超过合同标准的按吨奖励的算法
	...
	private void computData_Yf(){
		...
		//计算盈亏，折价金额
		bsv.setQnetar_yk(Fayjsdcz.reCoundYk(MainGlobal.Mjkg_to_kcalkg(bsv.getQnetar_ht(),"-",0),
				MainGlobal.Mjkg_to_kcalkg(bsv.getQnetar_js(),0,bsv.getMj_to_kcal_xsclfs()),bsv.getQnetar_yk()));	
		_Qnetarzje=CustomMaths.add((double)CustomMaths.Round_new(_Qnetar*_Jiessl,2),bsv.getQnetar_zje_tscl());
		bsv.setQnetar_zje(_Qnetarzje);
		bsv.setQnetar_zdj(CustomMaths.Round_new(CustomMaths.div(_Qnetarzje, _Jiessl),bsv.getMeikzkkblxsw()));
	
		bsv.setStd_yk(Fayjsdcz.reCoundYk(bsv.getStd_ht(),bsv.getStd_js(),bsv.getStd_yk()));
		_Stdzje=CustomMaths.add((double)CustomMaths.Round_new(_Std*_Jiessl,2),bsv.getStd_zje_tscl());
		bsv.setStd_zje(_Stdzje);
		bsv.setStd_zdj(CustomMaths.Round_new(CustomMaths.div(_Stdzje, _Jiessl),bsv.getMeikzkkblxsw()));
	
		bsv.setAd_yk(Fayjsdcz.reCoundYk(bsv.getAd_ht(),bsv.getAd_js(),bsv.getAd_yk()));
		_Adzje=CustomMaths.add((double)CustomMaths.Round_new(_Ad*_Jiessl,2),bsv.getAd_zje_tscl());
		bsv.setAd_zje(_Adzje);
		bsv.setAd_zdj(CustomMaths.Round_new(CustomMaths.div(_Adzje, _Jiessl),bsv.getMeikzkkblxsw()));
...
//				如果有运费，那么用总煤款减去总运费剩下的就是总煤款
				...
				if(bsv.getYunzfhj()>0){
					...
					Fayjsdcz.UpdateDanpcjsmkb(bsv.getMeikjsb_id(), bsv.getHansmj(), bsv.getJiakhj(), 
...
//						该指标需要增扣款特殊处理
						bsh.eval(bsv.getGongs_Mk());
//						得到增扣款价格信息
						Fayjsdcz.setJieszb_Tszbcl(bsh, bsv, tmp[1]);
//						要找到该增扣款对应的数量
						rsl=con.getResultSetList(Fayjsdcz.getJiesszl_Sql(bsv, Jieszbsftz, SelIds, Diancxxb_id, Gongysb_id, 
