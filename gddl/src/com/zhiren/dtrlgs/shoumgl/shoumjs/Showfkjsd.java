...
public class Showfkjsd extends BasePage {
...
//		
//	
//			return 	cj.getKuangfjsd(_jiesdbh,0,"kuangfjsmkb,kuangfjsyfb");
		...
		try {
			...
			sql = " select js.bianm from kuangfjsmkb js,fuktzb fk where js.fuktzb_id=fk.id and fk.fukdbh='"
					+ _jiesdbh + "'";
			ResultSetList rs = cn.getResultSetList(sql);

			while (rs.next()) {
				if (strbianhs.equals("")) {
					strbianhs = rs.getString("bianm");
				} else {
					strbianhs = strbianhs + "," + rs.getString("bianm");
				}
			}
			if(strbianhs.equals("")){
				return "";
			}else{
				String bianh[] = strbianhs.split(",");
				...
				for (int p = 0; p < bianh.length; p++) {
					sb.append(jsd.getKuangfjsd(bianh[p],p,"kuangfjsmkb,kuangfjsyfb"));
