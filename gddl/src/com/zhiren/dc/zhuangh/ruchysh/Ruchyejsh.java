...
//	保存页面上比率的总和
	...
	public void setJincpcModels() {
		...
		String Dicid = "";
		Dicid = " and f.diancxxb_id=" + getTreeid();
		if (hasDCid(null, getTreeid())){
			Dicid = " and f.diancxxb_id in (select id from diancxxb where fuid=" + getTreeid() + ")";
		}
		...
		.append("and to_char(f.daohrq,'yyyy-mm-dd')>='").append(getBeginRiq())//DateUtil.FormatOracleDate(getBeginRiq())
		.append("' and to_char(f.daohrq,'yyyy-mm-dd') <='").append(getEndRiq()).append("'")
		.append(where)
		.append(Dicid);
...
//		ExtGridUtil egu = new ExtGridUtil("gridDiv", rsl,CustomSetKey);
		...
		egu.getColumn("bil").setEditor(null);
...
//				"    total += rsl[i].get('BIL');\n" + 
//				"}\n" + 
//				"document.all.BilTotal.value = total;";
...
//			Mrcd.length=1;
//			if(Mrcd.length==1){ alert("hhe");Ext.MessageBox.alert('提示信息','只能选择一条');}
			String condition = 
				"var rsl = gridDiv_grid.getSelectionModel().getSelections();\n" +
				"var total = 0;\n" + 
				"var Mrcd = gridDiv_ds.getModifiedRecords();\n" +
				"if(Mrcd.length==1){ for (i = 0; i < rsl.length; i ++) {\n" + 
				"    if (rsl[i].get('BIL') == '' || rsl[i].get('BIL') == null) {\n" +
				"        rsl[i].set('BIL',1);\n" + 
				"    }\n" +
				"    total += rsl[i].get('BIL');\n" + 
				"}}\n if(rsl.length>1){ Ext.MessageBox.alert('提示信息','只能选择一条')}\n" + 
				"document.all.BilTotal.value = total;";
			egu.addToolbarButton("确认", GridButton.ButtonType_SubmitSel_condition, "SelectButton", condition);
		} else {//修改只能选择一条的页面判断
			String condition1 = 
				"var Mrcd = gridDiv_ds.getModifiedRecords();\n" +
				"if(Mrcd.length>1){ Ext.MessageBox.alert('提示信息','只能选择一条');}";
			
			egu.addToolbarButton("确认", GridButton.ButtonType_SubmitSel_condition, "SelectButton",condition1);//GridButton.ButtonType_SubmitSel
		}
		egu.addTbarText("-");
