...
* 时间：2009-12-03
* 作者： ll
* 修改内容：1、在表头增加“其中：”其中：供热耗煤量  其中：其他耗煤量 
* 		   
*/ 
/* 
* 时间：2010-01-4
...
//				this.getTree();
    ...
    private String getSelectData(){
		...
		/*strzt=strFunctionName+"(dqmc,fgsmc,diancmc,to_date('"+intyear+"-"+intMonth+"-01','yyyy-mm-dd'),to_date('"+intyear+"-"+intMonth+"-01','yyyy-mm-dd'),'yueshchjb','本月'"+(strFunctionName.equalsIgnoreCase("getShenhzt_fenkj")?","+getYuebValue().getId()+"":"")+" ) as bqby," +
		      strFunctionName+"(dqmc,fgsmc,diancmc,to_date('"+intyear+"-"+intMonth+"-01','yyyy-mm-dd'),to_date('"+intyear+"-"+intMonth+"-01','yyyy-mm-dd'),'yueshchjb','累计'"+(strFunctionName.equalsIgnoreCase("getShenhzt_fenkj")?","+getYuebValue().getId()+"":"")+" ) as bqlj," +
		      strFunctionName+"(dqmc,fgsmc,diancmc,add_months(to_date('"+intyear+"-"+intMonth+"-01','yyyy-mm-dd'),-12),add_months(to_date('"+intyear+"-"+intMonth+"-01','yyyy-mm-dd'),-12),'yueshchjb','本月'"+(strFunctionName.equalsIgnoreCase("getShenhzt_fenkj")?","+getYuebValue().getId()+"":"")+" ) as tqby," +
		      strFunctionName+"(dqmc,fgsmc,diancmc,add_months(to_date('"+intyear+"-"+intMonth+"-01','yyyy-mm-dd'),-12),add_months(to_date('"+intyear+"-"+intMonth+"-01','yyyy-mm-dd'),-12),'yueshchjb','累计'"+(strFunctionName.equalsIgnoreCase("getShenhzt_fenkj")?","+getYuebValue().getId()+"":"")+" ) as tqlj " ;*/
        if(getYuebValue().getValue().equals("全部")){
            dianckjmx_bm="";
            dianckjmx_tj="";
            koujid="'',";
        ...
        strzt="nvl(getShenhzt("+koujid+strDiancFID+"dqmc,fgsmc,diancmc,to_date('"+intyear+"-"+intMonth+"-01','yyyy-mm-dd'),to_date('"+intyear+"-"+intMonth+"-01','yyyy-mm-dd'),'yueshchjb','本月'"+","+visit.getRenyjb()+"),0)as bqby,\n" +
                "nvl(getShenhzt("+koujid+strDiancFID+"dqmc,fgsmc,diancmc,to_date('"+intyear+"-"+intMonth+"-01','yyyy-mm-dd'),to_date('"+intyear+"-"+intMonth+"-01','yyyy-mm-dd'),'yueshchjb','累计'"+","+visit.getRenyjb()+"),0) as bqlj,\n" +
                "nvl(getShenhzt("+koujid+strDiancFID+"dqmc,fgsmc,diancmc,add_months(to_date('"+intyear+"-"+intMonth+"-01','yyyy-mm-dd'),-12),add_months(to_date('"+intyear+"-"+intMonth+"-01','yyyy-mm-dd'),-12),'yueshchjb','本月'"+","+visit.getRenyjb()+"),0) as tqby,\n" +
                "nvl(getShenhzt("+koujid+strDiancFID+"dqmc,fgsmc,diancmc,add_months(to_date('"+intyear+"-"+intMonth+"-01','yyyy-mm-dd'),-12),add_months(to_date('"+intyear+"-"+intMonth+"-01','yyyy-mm-dd'),-12),'yueshchjb','累计'"+","+visit.getRenyjb()+"),0) as tqlj\n" ;
        ...
        strSQL=
                "select  "+biaot+",\n" +
                        ...
                        "(\n" +
                        "  select decode(1,1,'本月') as fenx,shc.diancxxb_id as diancxxb_id,"+shouhcsl+"\n" +
                        ...
                        "             and zb.fenx(+)=shc.fenx and shc.fenx='本月'"+zhuangt+yueshcmzt+"\n" +
                        "  union\n" +
                        "  select decode(1,1,'累计') as fenx,shc.diancxxb_id as diancxxb_id,"+shouhcsl+"\n" +
                        "       from  "+yb_zb+yb_shc+",diancxxb dc\n" +
                        "       where zb.diancxxb_id =dc.id and shc.diancxxb_id =dc.id "+strGongsID+"\n" +
                        "             and zb.riq=to_date('"+intyear+"-"+intMonth+"-01','yyyy-mm-dd')\n" +
                        "             and shc.riq=to_date('"+intyear+"-"+intMonth+"-01','yyyy-mm-dd')\n" +
...
//----------------------------库存累计取本月---------------------------------------------------------------------------------------
                        "(\n" +
                        " select decode(1,1,'本月') as fenx,shc.diancxxb_id as diancxxb_id,"+shouhckc+"\n" +
                        "      from  "+yb_zb+yb_shc+",diancxxb dc\n" +
                        "      where zb.diancxxb_id =dc.id and shc.diancxxb_id =dc.id  "+strGongsID+"\n" +
                        "            and zb.riq=to_date('"+intyear+"-"+intMonth+"-01','yyyy-mm-dd')\n" +
                        "            and shc.riq=to_date('"+intyear+"-"+intMonth+"-01','yyyy-mm-dd')\n" +
                        "            and zb.fenx(+)=shc.fenx and shc.fenx='本月' "+zhuangt+yueshcmzt+" \n" +
                        " union\n" +
                        " select decode(1,1,'累计') as fenx,shc.diancxxb_id as diancxxb_id,"+shouhckc+"\n" +
                        "      from  "+yb_zb+yb_shc+",diancxxb dc\n" +
                        "      where zb.diancxxb_id =dc.id and shc.diancxxb_id =dc.id  "+strGongsID+"\n" +
                        "            and zb.riq=to_date('"+intyear+"-"+intMonth+"-01','yyyy-mm-dd')\n" +
                        "            and shc.riq=to_date('"+intyear+"-"+intMonth+"-01','yyyy-mm-dd')\n" +
                        "            and zb.fenx(+)=shc.fenx and shc.fenx='本月' "+zhuangt+yueshcmzt+" ) smkc,\n" +
                        "(\n" +
                        " select decode(1,1,'本月') as fenx,shc.diancxxb_id as diancxxb_id,"+shouhckc+"\n" +
                        "      from  "+yb_zb+yb_shc+",diancxxb dc\n" +
                        "      where zb.diancxxb_id =dc.id and shc.diancxxb_id =dc.id  "+strGongsID+"\n" +
                        ...
                        "            and zb.riq=add_months(to_date('"+intyear+"-"+intMonth+"-01','yyyy-mm-dd'),-12)\n" +
                        "            and shc.riq=add_months(to_date('"+intyear+"-"+intMonth+"-01','yyyy-mm-dd'),-12)\n" +
                        "            and zb.fenx(+)=shc.fenx and shc.fenx='本月' "+zhuangt+yueshcmzt+") smkctq,"+
//-------------------------------------------------------------------------------------------------------------------
                        dianc +
...
//				直属分厂汇总	
        ...
        ArrHeader[0]=new String[] {"单位名称","单位名称","发电量","发电量","全部耗用原煤量","全部耗用原煤量","其中：发电耗煤量","其中：发电耗煤量","其中：供热耗煤量","其中：供热耗煤量","其中：其他耗煤量","其中：其他耗煤量","存储损耗量","存储损耗量",
                "水分差调整","水分差调整","实际库存","实际库存","帐面库存","帐面库存","审核状态","审核状态","审核状态","审核状态",};
        ArrHeader[1]=new String[] {"单位名称","单位名称","本期","同期","本期","同期","本期","同期","本期","同期","本期","同期","本期","同期","本期","同期","本期","同期","本期","同期","本期","本期","同期","同期"};
