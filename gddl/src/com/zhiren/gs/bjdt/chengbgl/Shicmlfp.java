...
public  class Shicmlfp extends BasePage implements PageValidateListener {
	private String msg = "";
	...
	public void Save() {
		...
		while(mdrsl.next()) {
			...
			if("0".equals(mdrsl.getString("ID"))) {
				...
				sql.append("insert into ").append(tableName).append("(id,riq,diancxxb_id,gongysb_id,jihkjb_id,hetl,yushtb_id)" );
...
//			return value==null||"".equals(value)?"null":value;
	...
	public void getSelectData() {
		...
		String chaxun = 
			...
			"from caigysmxb c  where c.diancxxb_id="+getDiancValue().getId()+" and c.riq=to_date('"+getNianfValue().getId()+"','yyyy')\n" + 
			"union\n" + 
			"select 1 id,'"+getDiancValue().getValue()+"' diancxxb_id,'合同供煤' as gongysb_id,sum(c.hetl) hetl\n" + 
			"from caigysmxb c  where c.diancxxb_id="+getDiancValue().getId()+" and c.jihkjb_id=1 and c.riq=to_date('"+getNianfValue().getId()+"','yyyy')\n" + 
			"union\n" + 
			"select 1 id,'"+getDiancValue().getValue()+"' diancxxb_id,'市场采购' as gongysb_id,sum(c.hetl) hetl\n" + 
			"from caigysmxb c  where c.diancxxb_id="+getDiancValue().getId()+" and c.jihkjb_id=2 and c.riq=to_date('"+getNianfValue().getId()+"','yyyy')\n" + 
			"union\n" + 
			"select c.id,d.mingc diancxxb_id,g.mingc gongysb_id,c.hetl\n" + 
			"from caigysmxb c,diancxxb d, gongysb g where c.diancxxb_id=d.id and c.gongysb_id=g.id " +
...
//总量
	public String getHetl() {
		...
		String sql_hetl = "select sum(hetl) hetl from caigysmxb where diancxxb_id="
				+ getDiancValue().getId();
