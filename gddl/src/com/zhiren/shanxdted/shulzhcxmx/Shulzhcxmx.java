package com.zhiren.shanxdted.shulzhcxmx;
 ...
 * 内容:数量综合查询明细
 */
public class Shulzhcxmx extends BasePage implements PageValidateListener {
	...
	private static final String BAOBPZB_GUANJZ = "SHUJZHCXMX_GJZ_ED";// baobpzb中对应的关键字
//	界面用户提示
	private String msg="";
...
//	获取相关的SQL
	...
	public StringBuffer getBaseSql() {
		...
		sb.append("   decode(grouping(fx.dc) + grouping(fx.mk), 1, '合计', fx.mk) mk,\n");
		sb.append("  decode(grouping(fx.dc) + grouping(fx.mk) + grouping(cx.ysdw),1,'小计',cx.ysdw) ysdw,\n");
		
		sb.append(" decode(grouping(fx.dc) + grouping(fx.mk) + grouping(cx.ysdw) + grouping(fx.yunsfsb_id),1,'小计',fx.yunsfsb_id) yunsfsb_id,");
		...
		sb.append(" group by grouping sets('1',(fx.dc),(fx.dc, fx.mk),\n");
		sb.append(" (fx.dc, fx.mk, cx.ysdw, fx.yunsfsb_id, cx.maoz, cx.piz, cx.jingz, cx.yuns, cx.yingk, cx.zongkd))\n");

		sb.append(" order by grouping(fx.dc) desc,fx.dc desc,grouping(fx.mk) desc,fx.mk asc,grouping(cx.ysdw) desc,cx.ysdw asc\n");
		return sb;
    }
...
//	刷新衡单列表
	public void getSelectData() {
		...
		if(visit.getboolean6()){
			ToolbarButton retur = new ToolbarButton(null,"返回","function(){document.getElementById('ReturnButton').click();}");
			retur.setIcon(SysConstant.Btn_Icon_Return);
...
//		sb.append("select bp.zidm,bp.biaot,bp.xuh,bp.kuand,bp.format from baobpzzb bp,baobpzb b where bp.baobpzb_id=b.id and bp.shifxs=1 and b.guanjz='"+v.getInt1()+"' order by xuh");
        ...
        if(rsl.getRows()>0){
        	...
        	rs = rstmp;
        	 ArrHeader = new String[][] {{Locale.diancxxb_id_fahb,  Locale.meikxxb_id_fahb,Locale.yunsdw_id_chepb, Locale.yunsfsb_id_fahb,
    		 ...
    		 ArrWidth = new int[] {100,  120, 120,  70, 70,70, 70, 50, 50, 50, 50 };
...
//	-------------------------电厂Tree-----------------------------------------------------------------
	...
	public String getTreeid_dc() {
		String treeid = ((Visit) getPage().getVisit()).getString11();
		if (treeid == null || treeid.equals("")) {
			((Visit) getPage().getVisit()).setString11(String
	...
	public void setTreeid_dc(String treeid) {
		((Visit) getPage().getVisit()).setString11(treeid);
	}
	DefaultTree dc ;
...
//	-------------------------电厂Tree END-------------------------------------------------------------
	...
	private String getMK(){
		Visit visit=(Visit)this.getPage().getVisit();
		return visit.getString12();
	}
	
	private String getYSDW(){
		Visit visit=(Visit)this.getPage().getVisit();
	...
	private String getBRIQ(){
		Visit visit=(Visit)this.getPage().getVisit();
		return visit.getString15();
	}
	
	private String getERIQ(){
		Visit visit=(Visit)this.getPage().getVisit();
...
//	页面初始化
	public void beginResponse(IMarkupWriter writer, IRequestCycle cycle) {
		...
		if(visit.getActivePageName()!=null && visit.getActivePageName().equals("Shulzhcx")){
			this.getMeikModels();
			getDiancmcModels();
			this.getGongysDropDownModels();
			
			String reportType = visit.getInt1()+"";
			if(reportType != null) {
				visit.setInt2(Integer.parseInt(reportType));
			}else{
				visit.setInt2(RPTTYPE_TZ_ALL);
			}
//			
//			String dc=cycle.getRequestContext().getParameter("dc");
//			String ysdw=cycle.getRequestContext().getParameter("ysdw");
//			String briq=cycle.getRequestContext().getParameter("briq");
//			String eriq=cycle.getRequestContext().getParameter("eriq");
//			String mk=cycle.getRequestContext().getParameter("mk");
			
			String dc=this.getDC();
			String ysdw=this.getYSDW();
			String briq=this.getBRIQ();
			String eriq=this.getERIQ();
			String mk=this.getMK();
		
//			System.out.println(dc+"--"+ysdw+"--"+briq+"--"+eriq+mk);
			this.setTreeid_dc(dc);
			this.setMeikid(ysdw);
			this.setBRiq(briq);
			this.setERiq(eriq);
			this.setTreeid(mk);
...
//	按钮的监听事件
	...
	private boolean _ReturnButtonChick=false;
	public void ReturnButton(IRequestCycle cycle){
		_ReturnButtonChick=true;
	}
...
//	表单按钮提交监听
	public void submit(IRequestCycle cycle) {
		...
		if(_ReturnButtonChick){
			_ReturnButtonChick=false;
			cycle.activate("Shulzhcx");
		}
	}
//	页面登陆验证
