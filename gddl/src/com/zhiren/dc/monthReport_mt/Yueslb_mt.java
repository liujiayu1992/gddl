 ...
 * 描述：马头数量月报维护
 */
public class Yueslb_mt extends BasePage implements PageValidateListener {
	// 界面用户提示
	private String msg = "";
	...
	private void Save() {
		...
		while (rsl.next()) {
			...
			if (rs.next()) {
				...
				sql +=	"suopsl = suopsl + " + suopsl + ","
					+ "suopje = suopje + " + suopje;
				
					
				sql+=   " where yuetjkjb_id =" + rsl.getString("yuetjkjb_id")
	...
	public void DelData() {
		...
		String CurrODate = DateUtil.FormatOracleDate(getNianf() + "-"
				+ getYuef() + "-01");
		String strSql = "delete from yueslb where yuetjkjb_id in (select id from yuetjkjb_mt where riq="
				+ CurrODate
				+ " and diancxxb_id="
				+ getTreeid()
				+ ")";
	...
	public void CreateData() {
		...
		sql = "insert into yuetjkjb_mt(id,riq,diancxxb_id,xuh,gongysb_id,meikxxb_id,pinzb_id,jihkjb_id,yunsfsb_id)\n"
				...
				+ "t.dqid as gongysb_id,t.meikxxb_id,t.pinzb_id,t.jihkjb_id,t.yunsfsb_id from\n"
				+ "(select f.diancxxb_id,m.meikdq_id as DQID,m.id as meikxxb_id,f.pinzb_id,f.jihkjb_id,f.yunsfsb_id\n"
				...
				+ "\n"
				+ "union\n"
				+ "select y.diancxxb_id,y.gongysb_id as DQID,y.meikxxb_id,y.pinzb_id,y.jihkjb_id,y.yunsfsb_id\n"
				+ "from yuetjkjb_mt y,yuehcb h where y.id = h.yuetjkjb_id and y.riq = add_months("
//				关于库存是否大于0这个有争议暂时改为 不取大于0
				...
				+ "minus\n"
				+ "select y.diancxxb_id,y.gongysb_id as DQID,y.meikxxb_id,y.pinzb_id,y.jihkjb_id,y.yunsfsb_id\n"
				+ "from yuetjkjb_mt y where y.riq ="
				+ CurrODate
				+ "\n"
...
//			long meikxxb_id = rs.getLong("meikxxb_id");
			...
			sql = "select distinct\n"
					...
					+ "from jiesb j, jieszbsjb s, zhibb z, fahb f, meikxxb m\n"
					+ "where j.id = s.jiesdid(+) and s.zhibb_id = z.id(+)\n"
					+ "and f.jiesb_id = j.id and f.meikxxb_id = m.id\n"
					+ "and f.diancxxb_id =" + diancxxb_id + "\n"
					+ "and m.meikdq_id = " + gongysb_id + " and f.jihkjb_id = "
					+ jihkjb_id + "\n" + "and f.yunsfsb_id = " + yunsfsb_id
					+ " and f.pinzb_id = " + pinzb_id + "\n"
			...
			datars.close();
			sql = "select sl.* from yuetjkjb_mt tj,yueslb sl  where sl.yuetjkjb_id=tj.id and riq="
					+ LastODate
					+ " and diancxxb_id="
					+ diancxxb_id
					+ "\n"
			...
			if (datars.next() && intYuef != 1) {
				...
				suopsl_lj = suopsl_lj + datars.getDouble("SUOPSL");// suopsl
				zhejje_lj = zhejje_lj + datars.getDouble("SUOPJE");// zhejje
		...
		if (intYuef != 1) {// 取上个月有这个月没有的数据，本月值为0，累计值等与上个月累计
			...
			while (rssylj.next()) {
				sql = "select y.id yid,s.* from yuetjkjb_mt y,yueslb s where y.id = s.yuetjkjb_id(+) "
						+ " and s.fenx(+) = '"
				...
				if (recby.next()) {
					...
					sql = getInsertYueslbSql(Long.parseLong(MainGlobal
							...
							recby.getDouble("sanfsl"), recby
									...
									.getDouble("kuidsp"), recby.getDouble("suopsl"), recby.getDouble("zhejje"));
					flag = con.getInsert(sql);
					if (flag == -1) {
						WriteLog.writeErrorLog(this.getClass().getName() + "\n"
								+ ErrorMessage.InsertYueslbFailed + "\nSQL:"
	...
	public void getSelectData() {
		...
		String CurrODate = DateUtil.FormatOracleDate(getNianf() + "-"
				+ getYuef() + "-01");
		String sql = "select s.id, t.id yuetjkjb_id, g.mingc gongysb_id, m.mingc meikxxb_id, j.mingc jihkjb_id, p.mingc pinzb_id,\n"
				...
				+ "from yueslb s, yuetjkjb_mt t, gongysb g, pinzb p, yunsfsb y, jihkjb j, meikxxb m\n"
				+ "where t.id = s.yuetjkjb_id\n" 
