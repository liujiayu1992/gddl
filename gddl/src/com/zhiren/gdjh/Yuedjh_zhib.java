 ...
 * 描述：调整入炉综合标煤单价计算公式
 */
public class Yuedjh_zhib extends BasePage implements PageValidateListener {
	...
	public void DelData() {
		...
		String strSql = "delete from yuedjh_zhib where riq = " + CurrODate
...
//		initData();
		...
		Item1
				.setRenderer("function(value,metadata){metadata.css='tdTextext1'; return value;}");
		egu.addColumn(Item1);
		GridColumn Unit1 = new GridColumn(0, "UNIT1", "单位1", 80);
		Unit1.setDataType(GridColumn.DataType_String);
		Unit1.setUpdate(true);
		Unit1
				.setRenderer("function(value,metadata){metadata.css='tdTextext2'; return value;}");
		egu.addColumn(Unit1);
		...
		if (rows >= 0) {
			...
			varCss = getGridData(con, diancxxb_id, rows, col_odd, data,
					CurrODate);
		...
		if (getZhangt(con)) {
			...
			if(visit.isFencb() && isParentDc(con) && MainGlobal.getXitxx_item("计划模块", "分厂别总厂显示按钮", this.getTreeid(), "否").equals("否")){
				...
				egu.addTbarBtn(gbs);
//					如果界面中没有数据则显示复制按钮
				String sql = "select s.id zhuangt from yuedjh_zhib s\n"
						+ " where  s.diancxxb_id = " + getTreeid() + "\n"
						+ "   and s.riq = " + CurrODate;
				if(!con.getHasIt(sql)){
	...
	private int UpdateDataBase(JDBCcon con, String diancxxb_id, String CurrODate, String upsql) {
		...
		// 更新当月计算值
		flag = countYuejhzbgs(con, diancxxb_id, CurrODate);
		if (flag == -1) {
			setMsg("保存过程发生异常！");
			return -1;
		}
	...
	private void initData() {
		...
		String diancxxb_id = getTreeid();
		String CurrODate = DateUtil.FormatOracleDate(getNianf() + "-"+getYuef()+"-01");
		String sql = "select * from yuedjh_zhib where riq = " + CurrODate
				+ " and diancxxb_id =" + diancxxb_id;
		if (!con.getHasIt(sql)) {
...
//	复制功能
	private void CopyData() {
		...
		String strDate=getNianf() + "-"+getYuef()+"-01";
		
		String CopySql=
			"INSERT INTO YUEDJH_ZHIB\n" +
			"(ID,DIANCXXB_ID,RIQ,FADL,GONGDMH,FADCYDL,FADBML,GONGRL,GONGRMH,\n" + 
			"GONGRBML,BIAOMLHJ,SHANGYMKC,SHANGYMKCDJ,SHANGYMKCRZ,YUEMKCJHZ,\n" + 
			"YUEMKCRZ,HAOYYML,RLZHBMDJ,ZHUANGT)\n" + 
			"(SELECT GETNEWID("+this.getTreeid()+"),DIANCXXB_ID,ADD_MONTHS(RIQ, 1) RIQ,\n" + 
			"FADL,GONGDMH,FADCYDL,FADBML,GONGRL,GONGRMH,GONGRBML,BIAOMLHJ,\n" + 
			"SHANGYMKC,SHANGYMKCDJ,SHANGYMKCRZ,YUEMKCJHZ,YUEMKCRZ,0 HAOYYML,\n" + 
			"0 RLZHBMDJ,0 ZHUANGT\n" + 
			"FROM YUEDJH_ZHIB\n" + 
...
// 		每次保存时重新计算指标中的相关参数
	...
	private int countYuejhzbgs(JDBCcon con, String diancxxb_id,
			String CurrODate) {
		String sql = getYuejhzbdySql(diancxxb_id, "gongs", "leijgs");
	...
	private String getYuejhzbdySql(String diancxxb_id, String gongsm) {
		return getYuejhzbdySql(diancxxb_id, gongsm, null);
	}
	...
	private String getYuejhzbdySql(String diancxxb_id, String gongsm,
		...
		String sql = "select ZIDM,GONGS from JIHZBDYB\n"
				...
				+ " \n" + " and diancxxb_id =" + diancxxb_id
				+ " and mokmc='月计划' order by bianm";
		return sql;
	}

	private String getInsertSql(String diancxxb_id, String CurrODate) {
		String sql = "insert into yuedjh_zhib (id,diancxxb_id,riq) values(getnewid("
	...
	private int getUpdateSql(JDBCcon con, String diancxxb_id, String CurrODate,
		...
		upsql = "update yuedjh_zhib set " + upsql + " where riq = "
	...
	private int getGridDataRows(JDBCcon con, String diancxxb_id) {
		...
		String sql = "select nvl(sum(decode(mod(xuh,2),0,1,0)),0) even\n"
				...
				+ "where zhuangt = 1 and mokmc='月计划' and diancxxb_id = "
				+ diancxxb_id;
		ResultSetList rs = con.getResultSetList(sql);
		if (rs.next()) {
	...
	private String getColumnValue(JDBCcon con, String date, String diancxxb_id,
			String zidm) {
		String value = "";
		String sql = "select " + zidm + " from yuedjh_zhib where diancxxb_id="
	...
	private String[] getGridData(JDBCcon con, String diancxxb_id, int rows,
		...
		String sql = "select *\n" + "from JIHZBDYB\n" + "where mod(xuh,2)= "
				+ col + " and zhuangt = 1 and mokmc='月计划'\n"
				+ "and diancxxb_id = " + diancxxb_id + "\n" + "order by xuh";
		ResultSetList rsl = con.getResultSetList(sql);
		GridVar[0] = "";
	...
	private String getOtherVarScript(JDBCcon con, String diancxxb_id) {
		String script = "";
		String sql = "select * from JIHZBDYB where zhuangt = 0 and mokmc='月计划' and diancxxb_id ="
	...
	private String getCountColSetScript(JDBCcon con, String diancxxb_id) {
		String gongs = "";
		String sql = getYuejhzbdySql(diancxxb_id, "gongs");
		ResultSetList rsl = con.getResultSetList(sql);
		while (rsl.next()) {
