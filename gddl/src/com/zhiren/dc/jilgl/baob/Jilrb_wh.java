 ...
 * 作者：王磊
 * 时间：2009-06-16 14：59
 * 描述：修改过衡日报过日车车数统计不正确的问题
 */
/*
 * 作者：张立东
 * 时间：2009-08-16 21：30
 * 描述：修改过衡日报汽车日报条件改成一致。即：qingcsj改成“shijname”
 */
/*
 * 作者：ww
 * 时间：2009-12-26
 * 描述：添加选择报表统计时间是按到货日期还是按发货日期，默认为到货日期
 * 	   配置参数：
 ...
 * 		 把报表的格式设定为A4打印形式。
 */
public class Jilrb_wh extends BasePage {
	...
	/**
	 * 作者:童忠付 时间:2009-4-3 内容:报表配置时,增加报表关键字
	 */
	...
	public StringBuffer getBaseSql() {
		...
		String huoh_qic = " select * from xitxxb where mingc='汽车过衡日报为轻车时间' and zhi='是' and zhuangt=1 and leib='数量' and diancxxb_id="
				+ visit.getDiancxxb_id();
		...
		String yuns_huoc_all = " sum(round_new(f.yuns," + visit.getShuldec()
				+ ")) yuns,\n";// 火车的 或者 所有运输方式的 运损 计算表达式

		String yuns_qic = " sum(round_new(f.yuns, nvl((" + roundPoint_QIY_sql
				+ ")," + visit.getShuldec() + ")  ))  yuns,\n";// 汽车的运损 计算表达式

		if (rsl.next()) {

			yuns_huoc_all = " sum(round_new(f.yingk," + visit.getShuldec()
					+ ")) + sum(round_new(f.biaoz," + visit.getShuldec()
					+ ")) " + "- sum(round_new(f.jingz," + visit.getShuldec()
					+ ")) yuns,\n ";// 火车的 或者 所有运输方式的 运损 计算表达式

			yuns_qic = " sum(round_new(f.yingk, nvl((" + roundPoint_QIY_sql
					+ ")," + visit.getShuldec() + ")  )) + "
		...
		sb
				.append(
						"select fahdw, meikdw,yunsdw, faz, pinz, laimsl, jingz, biaoz,  \n")
		...
		sb
				.append("decode(grouping(g.mingc)+grouping(m.mingc)-grouping(d.mingc),2,'合计',g.mingc) as fahdw, decode(grouping(m.mingc)-grouping(g.mingc)-grouping(d.mingc),1,'小计',m.mingc) as meikdw,c.yunsdw,z.mingc faz,p.mingc pinz, \n");
		...
		case RPTTYPE_GH_ALL:
			...
			sb
					...
					.append(
							"sum(round_new(f.biaoz," + visit.getShuldec()
									+ ")) biaoz, \n")
					// .append("sum(round_new(f.yingd,"+visit.getShuldec()+"))
					// yingd,
					// \n")
			...
			sb
					.append("from fahb f, (select distinct fahb_id,count(id) ches from chepb where zhongcsj>=to_date('"
							+ this.getRiq()
							+ "','yyyy-mm-dd') and zhongcsj<to_date('"
							+ this.getRiq()
							+ "','yyyy-mm-dd')+1 group by fahb_id) c,gongysb g, vwdianc d, meikxxb m, chezxxb z, pinzb p\n");
		...
		case RPTTYPE_GH_QIY:
			...
			sb
					.append("from fahb f,(select distinct fahb_id,count(id) ches from chepb where "
							+ shijname
							+ ">=to_date('"
							+ this.getRiq()
							+ "','yyyy-mm-dd') and "
			...
			sb
					...
					.append(
							"sum(round_new(f.yingk,nvl((" + roundPoint_HUOY_sql
									+ ")," + visit.getShuldec()
									+ "))) yingk, \n").append(yuns_qic).append(
							"sum(round_new(f.zongkd,nvl(("
									+ roundPoint_HUOY_sql + "),"
									+ visit.getShuldec() + "))) zongkd, \n")
					.append("sum(f.ches) ches \n");
		...
		case RPTTYPE_SB_QIY:
			...
			sb
					.append("from fahb f, (select distinct yunsdwb_id, fahb_id, y.mingc as yunsdw from chepb c, yunsdwb y " +
							"where c.yunsdwb_id = y.id) c,vwdianc d, gongysb g, meikxxb m, chezxxb z, pinzb p \n");
			sb.append("where f.yunsfsb_id = ").append(SysConstant.YUNSFS_QIY);
			sb.append(" and f." + strTongjrq + " = ").append(
					DateUtil.FormatOracleDate(getRiq())).append(" \n");
			break;
		...
		sb.append("and f.gongysb_id = g.id and f.meikxxb_id = m.id and f.id = c.fahb_id \n");
		...
		if (getTreeid() != null && !"".equals(getTreeid())
			...
			if (this.hasFenC(getTreeid())) {
				...
				sb
						.append("group by grouping sets ((g.mingc),(d.mingc),(g.mingc,d.mingc),(d.fgsmc,g.mingc,d.mingc,m.mingc,c.yunsdw,p.mingc,z.mingc)) \n");
			} else {
				sb.append(" and f.diancxxb_id = d.id and d.id =").append(
						this.getTreeid()).append("\n");
				sb.append("  and f.faz_id = z.id and f.pinzb_id = p.id \n");
				sb
						.append("group by grouping sets ((d.mingc),(g.mingc,d.mingc),(d.fgsmc,g.mingc,d.mingc,m.mingc,c.yunsdw,p.mingc,z.mingc)) \n");
			}
		...
		sb
				.append("order by grouping(d.mingc) desc,d.mingc,g.mingc desc,grouping(m.mingc) desc,c.yunsdw desc) \n");
		return sb;
	}
	...
	public String getPrintTable() {
		...
		if (rsl.getRows() > 0) {
			...
			rs = rstmp;
			if (v.getInt1() == Jilrb_wh.RPTTYPE_SB_QIY) {
				ArrHeader = new String[][] { { 
						Locale.gongysb_id_fahb, Locale.meikxxb_id_fahb,
						Locale.yunsdw_id_chepb, Locale.faz_id_fahb,
						Locale.pinzb_id_fahb, Locale.laimsl_fahb,
						Locale.jingz_fahb, Locale.biaoz_fahb,
						Locale.yingk_fahb, Locale.yuns_fahb,
						Locale.zongkd_fahb, Locale.ches_fahb } };
				ArrWidth = new int[] { 100, 120, 70, 70, 50, 50, 50, 50, 50,
						50, 50, 50 };
			} else {
				ArrHeader = new String[][] { { 
						Locale.gongysb_id_fahb, Locale.meikxxb_id_fahb,
						Locale.faz_id_fahb, Locale.pinzb_id_fahb,
						Locale.laimsl_fahb, Locale.jingz_fahb,
						Locale.biaoz_fahb, Locale.yingk_fahb, Locale.yuns_fahb,
						Locale.zongkd_fahb, Locale.ches_fahb } };

				ArrWidth = new int[] { 100, 120, 70, 50, 50, 50, 50, 50, 50,
						50, 50 };
			}
			aw = rt.paperStyle(((Visit) this.getPage().getVisit())
					.getDiancxxb_id(), ((Visit) this.getPage().getVisit())
		...
		if (v.getInt1() == Jilrb_wh.RPTTYPE_SB_QIY) {
			ArrWidth = new int[] { 100, 120, 70, 70, 50, 50, 50, 50, 50, 50,
					50, 50 };
		} else {
		...
		if (v.getInt1() == Jilrb_wh.RPTTYPE_GH_HUOY) {
			String str = MainGlobal.getXitxx_item("数量", "是否显示速度统计栏", String
					.valueOf(((Visit) getPage().getVisit()).getDiancxxb_id()),
					"否");
			if (str.equals("是")) {
				...
				String sql = "select case\n"
						...
						+ "   and cp.zhongcsj >= to_date('" + getRiq()
						+ "', 'yyyy-mm-dd')\n"
						+ "   and cp.zhongcsj < to_date('" + getRiq()
						+ "', 'yyyy-mm-dd') + 1 order by jib) temp\n"
						+ "group by temp.jib";
