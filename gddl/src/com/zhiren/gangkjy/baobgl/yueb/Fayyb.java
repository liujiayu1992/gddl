...
//		DateUtil.FormatDate(new Date());
		...
		// 设置明细行SQL
		String sqltable =
			"select decode(grouping(dc.mingc),1,'总计',dc.mingc) as fahdw,\n" +
			"        decode(grouping(g.mingc)+grouping(dc.mingc),1,'合计',g.mingc) as gongys,\n" + 
			"       decode(grouping(dc2.mingc)+grouping(g.mingc),1,'小计',dc2.mingc) as shouhr,\n" + 
			"       tb2.fenx as leix,\n" + 
			"       ybb.pinz,\n" + 
			...
			"from\n" + 
			"(select f.diancxxb_id as fahd, f.gongysb_id as gys,f.shr_diancxxb_id as shr,pz.mingc pinz,decode(1,1,'本月','本月') as leix,f.chec as chec, lc.mingc as chuanm,fz.mingc as faz,\n" + 
			...
			"group by(f.diancxxb_id, f.gongysb_id,f.shr_diancxxb_id ,pz.mingc,decode(1,1,'本月','本月') ,f.chec, lc.mingc,fz.mingc,dz.mingc,f.fahrq, f.daohrq)\n" + 
			"union\n" + 
			"select f.diancxxb_id as fahd, f.gongysb_id as gys,f.shr_diancxxb_id as shr,pz.mingc pinz,decode(1,1,'累计','累计') as leix,f.chec as chec, lc.mingc as chuanm,fz.mingc as faz,\n" + 
			"dz.mingc as daoz,f.fahrq as fahrq, f.daohrq as daohrq,sum(f.meil) as meil,sum(f.meil*zl.qnet_ar) as rel\n" + 
			"from fayslb f,diancxxb dc, vwpinz pz, luncxxb lc, vwchez fz, vwchez dz, zhilb zl\n" + 
			"where f.diancxxb_id=dc.id and f.pinzb_id=pz.id(+) and lc.id(+)=f.luncxxb_id and fz.id(+)=f.faz_id and dz.id(+)=f.daoz_id and zl.id(+)=f.zhilb_id\n" + 
			...
			"(select decode(1,1,'本月','本月') fenx from dual union select decode(2,2,'累计','累计')from dual) fx\n" + 
		...
		sb.append(sqltable);
			...
			ArrHeader[0] = new String[] {"发货地点","供应商","收货人","当月/累计","品种","车次/航次","船名","发站","到站","煤量","发热量(Qnet_ar)"};
