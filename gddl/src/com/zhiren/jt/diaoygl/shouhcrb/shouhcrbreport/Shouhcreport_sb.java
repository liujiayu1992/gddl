...
public class Shouhcreport_sb extends BasePage {
	...
	private String getShouhcreport() {
		...
		if (jib == 1) {// 选集团时刷新出所有的电厂
			strGongsID = " ";
			mingc = "(select quanc from diancxxb where jib = 1)";
			...
			mingc = "(select quanc from diancxxb where jib = 2)";
		} else if (jib == 3) {// 选电厂只刷新出该电厂
			strGongsID = " and dc.id= " + this.getTreeid();
			notHuiz = " and not  grouping(dc.mingc)=1";// 当电厂树是电厂时,去掉分公司和集团汇总
			mingc = "(select quanc from diancxxb where jib = 3)";
		} else if (jib == -1) {
			strGongsID = " and dc.id = "
					+ ((Visit) getPage().getVisit()).getDiancxxb_id();
		}

		String sbsql = "select decode(grouping(dc.mingc) + grouping(dc.leib),2,"+mingc+",1,'★' || dc.leib || '小计',max(dc.mingc)) as diancmc,\n"
				+ "       sum(dh.hej * 10000) as 本月计划,\n"
				+ "       sum(round(dh.hej * 10000 /daycount(to_date('"
				+ this.getEndriqDate()
				+ "', 'yyyy-mm-dd')),0)) as 日均计划,\n"
				+ "       sum(h.dangrgm) as 当日供煤,\n"
				+ "       sum(lj.dangrgm) as 累计供煤,\n"
				+ "       sum((round(dh.hej * 10000 /daycount(to_date('"
				+ this.getEndriqDate()
				+ "', 'yyyy-mm-dd')),0) * to_char(to_date('"
				+ this.getEndriqDate()
				+ "', 'yyyy-mm-dd'), 'dd'))) as 累计应发,\n"
				+ "       sum(((round(dh.hej / daycount(to_date('"
				+ this.getEndriqDate()
				+ "', 'yyyy-mm-dd')),0) *to_char(to_date('"
				+ this.getEndriqDate()
				+ "', 'yyyy-mm-dd'), 'dd')) - h.dangrgm)) as 超欠,\n"
				...
				+ "          from niancgjhb dh\n"
				+ "         where dh.riq = First_day(to_date('"
				+ this.getEndriqDate()
				+ "', 'yyyy-mm-dd'))\n"
		...
		ArrHeader[0] = new String[] { "单位名称", "合同计划执行情况", "合同计划执行情况",
				"合同计划执行情况", "合同计划执行情况", "合同计划执行情况", "合同计划执行情况", "合同计划执行情况",
				"供煤总量", "供煤总量", "耗用情况", "耗用情况", "库存", "可用天数", "正常储备线" };
		ArrHeader[1] = new String[] { "单位名称", "本月计划", "日均计划", "当日供煤", "累计供煤",
				"累计应发", "超欠 + -", "到货率 %", "当日", "累计", "当日", "累计", "库存",
				"可用天数", "正常储备线" };
		...
		rt.setDefaultTitle(6, 3, FormatDate(DateUtil.getDate(End_riq)),
				Table.ALIGN_CENTER);
	...
	public void ShengcTXTFile() {
		UploadData_dt up = new UploadData_dt();
	...
	private void getToolbars() {
		...
		if (getShangbry(visit.getString1())) {
			ToolbarButton tb3 = new ToolbarButton(null, "上报",
					"function(){document.getElementById('UploadButton').click();}");
			tb1.addItem(tb3);
			tb1.addText(new ToolbarText("-"));
		}
	...
	public boolean getShangbry(String visitUserName) {// 获取有日报上传权限的用户名
		...
		String sql = "select xt.zhi from xitxxb xt where xt.mingc='日报上报人员' and xt.zhi='"
				+ visitUserName + "'";
