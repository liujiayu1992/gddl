...
public class Yuedjshsbmdj_char  extends BasePage implements PageValidateListener {
	...
	public String getPrintChar(){
		...
		"SELECT RIQ,\n" +
		"       FENX,\n" + 
		"       ROUND(DECODE(QNET_AR, 0, 0, RUCMJ * 29.271 / QNET_AR), 2) BIAOMDJM\n" + 
		"  FROM (SELECT TO_CHAR(T.RIQ, 'mm') || '月' RIQ,\n" + 
		"               J.FENX,\n" + 
		"               ROUND(DECODE(SUM(J.JIESL),\n" + 
		"                            0,\n" + 
		"                            0,\n" + 
		"                            SUM(J.JIESL * (J.MEIJ + J.YUNJ + J.DAOZZF + J.ZAF +\n" + 
		"                                J.QIT + J.KUANGQYF- J.MEIJS - J.YUNJS - J.ZAFS )) / SUM(J.JIESL)),\n" + 
		"                     2) RUCMJ,\n" + 
		"               ROUND(DECODE(SUM(J.JIESL),\n" + 
		"                            0,\n" + 
		"                            0,\n" + 
		"                            SUM(J.JIESL * J.QNET_AR) / SUM(J.JIESL)),\n" + 
		"                     2) QNET_AR\n" + 
		...
		CategoryDataset dataset = cd.getRsDataChart(rstmp, "RIQ","FENX","BIAOMDJM");//rs记录集构造生成图片需要的数据
...
//		柱子上字的倾斜度。0:水平
		...
		return ct.ChartBar3D(getPage(), dataset,  getYearValue().getValue()+"年入厂不含税标煤单价",  800,  400);
	}
