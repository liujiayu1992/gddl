 ...
 * 内容:质量管理 装船化验报告
 */
public class Fayhybg extends BasePage implements PageValidateListener {
...
//		}
		...
		buffer.append(
				...
				"       qgrd,\n" + 
				"       beiz\n" + 
				"  from (select nvl(zc.chec,' ') as hangc,\n" + 
				"		nvl(l.mingc,' ') as luncxxb_id,\n"+
				"		nvl(g.mingc,' ') as xiaosgysb_id,\n"+
				"		nvl(to_char(zc.CHUKKSSJ, 'yyyy-MM-dd'), ' ') as CHUKKSSJ,\n" +
				"		nvl(to_char(zc.CHUKJSSJ, 'yyyy-MM-dd'), ' ') as CHUKJSSJ,\n" + 
				"		nvl(to_char(zc.fahrq, 'yyyy-MM-dd'), ' ') as fahrq,\n" + 
				"		nvl(null, ' ') as jihcq,\n"+
//				"		nvl(zc.meil,'') as zhuangcl,"+
				"		nvl(to_char(zc.daohrq,'yyyy-MM-dd'),' ') as daohrq,"+
				"		nvl(ry.mingc,' ') as caozy,"+
				"		nvl(zc.beiz,' ') as beiz,"+
				"               sum(round_new(zc.meil,"+v.getShuldec()+")) zhuangcl,\n" + 
			
				"               decode(sum(round_new(zc.meil,"+v.getShuldec()+")),\n" + 
				"                      0,\n" + 
				"                      0,\n" + 
				"                      round_new(sum(round_new(z.mt,"+v.getMtdec()+") * round_new(zc.meil,"+v.getShuldec()+")) / sum(round_new(zc.meil,"+v.getShuldec()+")), "+v.getMtdec()+")) as mt,\n" + 
				"               decode(sum(round_new(zc.meil,"+v.getShuldec()+")),\n" + 
				"                      0,\n" + 
				"                      0,\n" + 
				"                      round_new(sum(z.mad * round_new(zc.meil,"+v.getShuldec()+")) / sum(round_new(zc.meil,"+v.getShuldec()+")), 2)) as mad,\n" + 
				"               decode(sum(round_new(zc.meil,"+v.getShuldec()+")),\n" + 
				"                      0,\n" + 
				"                      0,\n" + 
				"                      round_new(sum(z.aad * round_new(zc.meil,"+v.getShuldec()+")) / sum(round_new(zc.meil,"+v.getShuldec()+")), 2)) as aad,\n" + 
				"               decode(sum(round_new(zc.meil,"+v.getShuldec()+")),\n" + 
				"                      0,\n" + 
				"                      0,\n" + 
				"                      round_new(sum(z.ad * round_new(zc.meil,"+v.getShuldec()+")) / sum(round_new(zc.meil,"+v.getShuldec()+")), 2)) as ad,\n" + 
				"               decode(sum(round_new(zc.meil,"+v.getShuldec()+")),\n" + 
				"                      0,\n" + 
				"                      0,\n" + 
				"                      round_new(sum(z.aar * round_new(zc.meil,"+v.getShuldec()+")) / sum(round_new(zc.meil,"+v.getShuldec()+")), 2)) as aar,\n" + 
				"               decode(sum(round_new(zc.meil,"+v.getShuldec()+")),\n" + 
				"                      0,\n" + 
				"                      0,\n" + 
				"                      round_new(sum(z.vad * round_new(zc.meil,"+v.getShuldec()+")) / sum(round_new(zc.meil,"+v.getShuldec()+")), 2)) as vad,\n" + 
				"               decode(sum(round_new(zc.meil,"+v.getShuldec()+")),\n" + 
				"                      0,\n" + 
				"                      0,\n" + 
				"                      round_new(sum(z.vdaf * round_new(zc.meil,"+v.getShuldec()+")) / sum(round_new(zc.meil,"+v.getShuldec()+")), 2)) as vdaf,\n" + 
				"               decode(sum(round_new(zc.meil,"+v.getShuldec()+")),\n" + 
				"                      0,\n" + 
				"                      0,\n" + 
				"                      round_new(sum(round_new(z.qbad,"+v.getFarldec()+") * round_new(zc.meil,"+v.getShuldec()+")) / sum(round_new(zc.meil,"+v.getShuldec()+")), "+v.getFarldec()+")) as qbad,\n" + 
				"               decode(sum(round_new(zc.meil,"+v.getShuldec()+")),\n" + 
				"                      0,\n" + 
				"                      0,\n" + 
				"                      round_new(sum(round_new(z.qnet_ar,"+v.getFarldec()+") * round_new(zc.meil,"+v.getShuldec()+")) /\n" + 
				"                                          sum(round_new(zc.meil,"+v.getShuldec()+"))\n" + 
				"                                           * 1000 / 4.1816,\n" + 
				"                                0)) as qbar,\n" + 
				"               decode(sum(round_new(zc.meil,"+v.getShuldec()+")),\n" + 
				"                      0,\n" + 
				"                      0,\n" + 
				"                      round_new(sum(round_new(z.qnet_ar,"+v.getFarldec()+") * round_new(zc.meil,"+v.getShuldec()+")) / sum(round_new(zc.meil,"+v.getShuldec()+")), "+v.getFarldec()+")) as farl,\n" + 
				"               decode(sum(round_new(zc.meil,"+v.getShuldec()+")),\n" + 
				"                      0,\n" + 
				"                      0,\n" + 
				"                      round_new(sum(z.sdaf * round_new(zc.meil,"+v.getShuldec()+")) / sum(round_new(zc.meil,"+v.getShuldec()+")), 2)) as sdaf,\n" + 
				"               decode(sum(round_new(zc.meil,"+v.getShuldec()+")),\n" + 
				"                      0,\n" + 
				"                      0,\n" + 
				"                      round_new(sum(z.stad * round_new(zc.meil,"+v.getShuldec()+")) / sum(round_new(zc.meil,"+v.getShuldec()+")), 2)) as stad,\n" + 
				"               decode(sum(round_new(zc.meil,"+v.getShuldec()+")),\n" + 
				"                      0,\n" + 
				"                      0,\n" + 
				"                      round_new(sum(z.std * round_new(zc.meil,"+v.getShuldec()+")) / sum(round_new(zc.meil,"+v.getShuldec()+")), 2)) as std,\n" + 
				"               decode(sum(round_new(zc.meil,"+v.getShuldec()+")),\n" + 
				"                      0,\n" + 
				"                      0,\n" + 
				"                      round_new(sum(round_new(z.stad*(100-z.mt)/(100-z.mad),2) * round_new(zc.meil,"+v.getShuldec()+")) / sum(round_new(zc.meil,"+v.getShuldec()+")), 2)) as star,\n" + 
				"               decode(sum(round_new(zc.meil,"+v.getShuldec()+")),\n" + 
				"                      0,\n" + 
				"                      0,\n" + 
				"                      round_new(sum(z.hdaf * round_new(zc.meil,"+v.getShuldec()+")) / sum(round_new(zc.meil,"+v.getShuldec()+")), 2)) as hdaf,\n" + 
				"               decode(sum(round_new(zc.meil,"+v.getShuldec()+")),\n" + 
				"                      0,\n" + 
				"                      0,\n" + 
				"                      round_new(sum(z.had * round_new(zc.meil,"+v.getShuldec()+")) / sum(round_new(zc.meil,"+v.getShuldec()+")), 2)) as had,\n" + 
				
				"               decode(sum(round_new(zc.meil,"+v.getShuldec()+")),\n" + 
				"                      0,\n" + 
				"                      0,\n" + 
				"                      round_new(sum(z.fcad * round_new(zc.meil,"+v.getShuldec()+")) / sum(round_new(zc.meil,"+v.getShuldec()+")), 2)) as fcad,\n" + 
				"               decode(sum(round_new(zc.meil,"+v.getShuldec()+")),\n" + 
				"                      0,\n" + 
				"                      0,\n" + 
				"                      round_new(sum(z.qgrd * round_new(zc.meil,"+v.getShuldec()+")) / sum(round_new(zc.meil,"+v.getShuldec()+")), 2)) as qgrd \n" + 
	//			"               decode(grouping(g.mingc), 1, 0, nvl(max(g.xuh), 99999)) dx,\n" + 
	//			"               decode(grouping(m.mingc), 1, 0, nvl(max(m.xuh), 99999)) mx\n" + 
				"         from  zhillsb z,caiyb c,fayslb zc,caiylbb cb, luncxxb l,vwxuqdw g, diancxxb dc,renyxxb ry\n" + 
				"          where z.caiyb_id=c.id and c.caiylbb_id=cb.id  and ry.id=zc.lurry\n" + 
				"          	and zc.diancxxb_id = dc.id and cb.bianm='FY' and z.zhilb_id=zc.zhilb_id \n" + 
				"           and l.id(+)=zc.luncxxb_id   and zc.shr_diancxxb_id=g.id \n 	"+ 
				"			and "+huaybh+
				str+
				"           and z.huaysj >= to_date('"+getRiq()+"', 'yyyy-mm-dd')\n" + 
				"           and z.huaysj <= to_date('"+getRiq2()+"', 'yyyy-mm-dd')\n" + 
				"			         group by zc.chec,l.mingc, g.mingc, zc.chukkssj,zc.CHUKJSSJ,zc.daohrq, zc.fahrq,ry.mingc, zc.meil,zc.beiz)");
			return buffer;
	}
	// 燃料采购部指标完成情况日报
...
//				cheph = buffer.toString().substring(0, buffer.length() - 1);
				...
				ArrHeader[1] = new String[] { Local.zhuangckssj_zhuangcb,
						"" + rs.getString("CHUKKSSJ") + "", Local.zhuangcsj_zhuangcb,
						"" + rs.getString("CHUKJSSJ") + "", Local.ligsj_zhuangcb,
						"" + rs.getString("FAHRQ") + "" };
		...
		if(getTreeid().equals("501")){
			biaot=Local.zhuangchybg_zhuangcb;
		}else{
			biaot="发 运 化 验 报 告";
		}
...
//	得到电厂树下拉框的电厂名称或者分公司,集团的名称
	...
	private void setBianmModels() {
		...
		sb.append("select distinct z.id, lc.mingc||'☆'||z.huaybh mingc,z.huaysj,zc.diancxxb_id "+
					" from fayslb zc, zhillsb z, caiylbb cb, caiyb c,luncxxb lc ,diancxxb dc"+
					" where zc.diancxxb_id=dc.id and z.caiyb_id = c.id  and c.caiylbb_id = cb.id "+
					" and cb.bianm = 'FY'  and zc.zhilb_id = z.zhilb_id"+
