	...
	private void getUploadFtp(String tableName, boolean shangcfs) {
		...
		if (tableName == "") {
			...
			try {
				...
				num = FileName.length;
				fu.connectServer(getServerIp(), getServerUserName(),
						getServerPassWord(), filepath);
	...
	public void ShangcTXTFile(boolean shangcfs) {
		...
		try {
			...
			while (rs.next()) {
				...
				if (shangcfs) {
					String FileName = "hc" + gsbm
							+ FormatDate(riq).substring(8, 10);
					ct.CreatTxt("c://Ribsc/" + FileName + ".txt");
					String datasql = "select to_char(max(h.riq), 'yyyymmdd') as riq,\n"
							+ "       decode(grouping(dc.bianm), 1, max(gs.bianm), dc.bianm) as diancbm,\n"
							+ "       round(round(sum(dh.hej * 10000), 0), 2) as hetjh,\n"
							+ "       round(round(sum(h.dangrgm), 0), 2) as dangrgm,\n"
							+ "       round(round(sum(lj.dangrgm), 0), 2) as leijgm,\n"
							+ "       round(round(sum(h.dangrgm), 0), 2) as dangr,\n"
							+ "       round(round(sum(lj.dangrgm), 0), 2) as leij,\n"
							+ "       round(round(sum(h.haoyqkdr), 0), 2) as dangrhy,\n"
							+ "       round(round(sum(lj.haoyqkdr), 0), 2) as leijhy,\n"
							+ "       round(round(sum(h.kuc), 0), 2) as kuc\n"
							+ "  from shouhcrbb h,\n"
							+ "       diancxxb gs,\n"
							+ "       dianckjpxb px,\n"
							+ "       (select dc.id, dc.mingc, dc.bianm, dc.zhengccb, dc.rijhm\n"
							+ "          from diancxxb dc\n"
							+ "         where dc.jib = 3) dc,\n"
							+ "       (select dh.diancxxb_id, sum(dh.hej) as hej\n"
							+ "          from niancgjhb dh\n"
							+ "         where dh.riq = First_day("
							+ OraDate(riq)
							+ ")\n"
							...
							+ "   and h.diancxxb_id = dh.diancxxb_id(+)\n"
							+ "   and h.diancxxb_id = lj.diancxxb_id\n"
							...
							+ OraDate(riq)
							+ "\n"
							+ " group by rollup(dc.bianm)\n"
							+ "having not(grouping(dc.bianm) = 1)\n"
							+ " order by dc.bianm, max(px.xuh)";
					...
					if (!con.getHasIt(datasql.toString())) {
						...
						while (rsdata.next()) {
							...
							fileline.append(getNum(8, 0, String.valueOf(Math
									.round(hetjh))));
							fileline.append(getNum(8, 0, String.valueOf(Math
									.round(dangrgm))));
							fileline.append(getNum(8, 0, String.valueOf(Math
					...
					for (int i = 0; i < 7; i++) {//7天以内的数据
						...
						if (!con.getHasIt(datasql.toString())) {
							...
							while (rsdata.next()) {
								...
								fileline.append(getStr(8, strdate));
								fileline.append(getStr(6, diancbm));
								fileline.append(getNum(8, 0, String
										.valueOf(Math.round(hetjh))));
								fileline.append(getNum(8, 0, String
										.valueOf(Math.round(dangrgm))));
								fileline.append(getNum(8, 0, String
										.valueOf(Math.round(leijgm))));
								fileline.append(getNum(8, 0, String
										.valueOf(Math.round(dangr))));
								fileline.append(getNum(8, 0, String
										.valueOf(Math.round(leij))));
								fileline.append(getNum(8, 0, String
										.valueOf(Math.round(dangrhy))));
								fileline.append(getNum(8, 0, String
										.valueOf(Math.round(leijhy))));
								fileline.append(getNum(8, 0, String
										.valueOf(Math.round(kuc))));
								filedata = fileline.toString();
