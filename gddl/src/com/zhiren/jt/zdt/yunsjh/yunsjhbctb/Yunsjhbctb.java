package com.zhiren.jt.zdt.yunsjh.yunsjhbctb;
...
public class Yunsjhbctb extends BasePage implements PageValidateListener {
	...
	public void CoypLastYueData() {
		...
		String copyData = "select y.id,d.mingc as diancxxb_id,g.mingc as gongysb_id,y.riq,\n"
				+ "(select c.mingc from chezxxb c where y.faz_id=c.id) as faz_id,\n"
				+ "(select c.mingc from chezxxb c where y.daoz_id=c.id) as daoz_id,\n"
				+ "p.mingc as pinm,y.yuanc,y.yuand,y.beiz,y.leix\n"
				+ "from yunsjhtbb y,vwdianc d,gongysb g,pinzb p\n"
		...
		while (rslcopy.next()) {
			...
			String _id = MainGlobal.getNewID(((Visit) getPage().getVisit())
					.getDiancxxb_id());
			String sql="insert into yunsjhtbb(id,riq,diancxxb_id,gongysb_id,faz_id,daoz_id,pinm,yuanc,yuand,beiz,leix) values("
				+
...
//					+ ")";
		...
		if (rsl == null) {
			String chaxun = "select y.id,d.mingc as diancxxb_id,g.mingc as gongysb_id,y.riq,\n"
					...
					+ " (select c.mingc from chezxxb c where y.daoz_id=c.id) as daoz_id,\n"
					+ " p.mingc as pinm,y.yuanc,y.yuand,y.beiz,y.leix\n"
					+ " from yunsjhtbb y,diancxxb d,gongysb g,pinzb p\n"
					+ " where y.gongysb_id=g.id and y.diancxxb_id=d.id and y.pinm=p.id and leix=0 \n"
					+ " and to_char(y.riq,'yyyy-mm')='"
					+ intyear
					+ "-"
		...
		egu.getColumn("yuanc").setHeader("车数");
		egu.getColumn("yuand").setHeader("吨数");
