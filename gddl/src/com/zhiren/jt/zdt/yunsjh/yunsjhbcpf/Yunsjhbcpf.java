...
public class Yunsjhbcpf extends BasePage implements PageValidateListener {
...
//		sb.append("select y.id,y.riq,d.diancmc as diancxxb_id,g.mingc as gongysb_id,\n");
//		sb.append(" (select cz.mingc from chezxxb cz where y.faz_id=cz.id) as faz_id,\n");
//		sb.append(" (select cz.mingc from chezxxb cz where y.daoz_id=cz.id) as daoz_id,\n");
//		sb.append(" p.mingc as pinm,yt.yuanc,yt.yuand,y.pic,y.pid,y.pizjhh,y.leix\n");
//		sb.append(" from yunsjhb y,gongysb g, pinzb p,yunsjhtbb yt,\n");
//		sb.append(" (select diancid as diancxxb_id,diancmc from \n");
//		sb.append(" (select id as fengsid, mingc as fengsmc, fuid from diancxxb where fuid = 100) gs,\n");
//		sb.append(" (select dc.id as diancid, dc.mingc as diancmc, fuid from diancxxb dc where dc.fuid != 100) dc\n");
//		sb.append(" where dc.fuid = gs.fengsid) d\n");
//		sb.append(" where y.diancxxb_id=d.diancxxb_id and y.gongysb_id=g.id and y.pinm=p.id and y.leix=0\n");
//		sb.append(" and y.diancxxb_id=yt.diancxxb_id(+) and y.gongysb_id=yt.gongysb_id(+)\n");
//		sb.append(" and y.faz_id=yt.faz_id(+) and y.daoz_id=yt.daoz_id(+) and y.pinm=yt.pinm(+)\n");
//		sb.append(" and to_char(y.riq,'yyyy-mm')='" + intyear + "-" + StrMonth + "'"+str
//				+ "\n");
		...
		String chaxun = "select y.id,d.mingc as diancxxb_id,g.mingc as gongysb_id,y.riq,\n"
			...
			+ " from yunsjhb y,vwdianc d,gongysb g,pinzb p\n"
			+ " where y.gongysb_id=g.id and y.diancxxb_id=d.id and y.pinm=p.id and y.leix=0 \n"
...
//		egu.getColumn("gongysb_id").setEditor(new ComboBox());
		...
		String GongysSql = "select id,mingc from gongysb where zhuangt=1 order by mingc";
